export const PROJECT_DEFINITION_MUTATION_AUTHORITY_SCHEMA = 'project-definition-mutation-authority-v1' as const;

export type ProjectDefinitionMutationOperation = 'define' | 'amend';

export type ProjectDefinitionMutationAuthority = Readonly<{
  schema: typeof PROJECT_DEFINITION_MUTATION_AUTHORITY_SCHEMA;
...
type RepositoryDisposition = Readonly<{
  repository: string;
  disposition: string;
}>;

type Dependencies = Readonly<{
  readRepositoryDisposition: (repository: string) => Promise<RepositoryDisposition | null>;
  readSourceRevision: (repository: string) => Promise<string>;
}>;
...
function exactRevision(value: unknown, field: string): string {
  ...
  if (!SHA40.test(revision)) fail('PROJECT_DEFINITION_MUTATION_AUTHORITY_INVALID', `${field} must be an exact 40-character Git revision`, { field });
  return revision;
}

function normalizedRequest(raw: unknown) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) fail('PROJECT_DEFINITION_MUTATION_AUTHORITY_INVALID', 'project definition mutation authority request must be an object');
  const input = raw as Record<string, unknown>;
  const operation = input.operation === 'define' || input.operation === 'amend' ? input.operation : null;
  if (!operation) fail('PROJECT_DEFINITION_MUTATION_AUTHORITY_INVALID', 'operation must be define or amend');
  const repository = typeof input.repository === 'string' ? input.repository.trim() : '';
  if (!REPOSITORY.test(repository)) fail('PROJECT_DEFINITION_MUTATION_AUTHORITY_INVALID', 'repository must be owner/repo');
  const projectRef = typeof input.project_ref === 'string' ? input.project_ref.trim() : '';
  if (projectRef !== `github:${repository}`) fail('PROJECT_DEFINITION_MUTATION_AUTHORITY_SCOPE_MISMATCH', 'project_ref must identify the requested repository', { project_ref:projectRef, repository });
...
export function createProjectDefinitionMutationAuthorityPolicy(dependencies: Dependencies) {
  ...
  return Object.freeze({
    async require(raw: unknown): Promise<ProjectDefinitionMutationAuthority> {
