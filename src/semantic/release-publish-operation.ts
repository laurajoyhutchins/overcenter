...
import type { ReleaseSemverAuthority, ReleaseSemverPlan, ReleaseSemverTransition } from './release-semver-plan.js';

export const RELEASE_PUBLISH_RESULT_SCHEMA = 'release-publish-v1' as const;

type ResolveAuthority = (input: Readonly<{ project_ref: string }>) => Promise<unknown>;
type ReadTransitions = (input: Readonly<{ project_ref: string; authority: ReleaseSemverAuthority }>) => Promise<readonly ReleaseSemverTransition[]>;
type CreateRelease = (input: Readonly<Record<string, unknown>>) => Promise<unknown>;

export type ReleasePublishDependencies = Readonly<{
  resolveAuthority: ResolveAuthority;
  readTransitions: ReadTransitions;
  createRelease: CreateRelease;
}>;

export type ReleasePublishResult = Readonly<{
  ok: true;
  schema: typeof RELEASE_PUBLISH_RESULT_SCHEMA;
  project_ref: string;
  version: string;
  tag_name: string;
  target_sha: string;
...
function fail(code: string, message: string, details: unknown = null, mayHaveMutated = false): never {
...
function text(value: unknown, field: string): string {
  ...
  if (!normalized) fail('RELEASE_PUBLISH_INVALID', `${field} must be a non-empty string`, { field });
  return normalized;
}

function authority(raw: unknown, field: string): ReleaseSemverAuthority {
...
function authorityKey(value: ReleaseSemverAuthority): string {
...
function strictReleaseFailure(result: Record<string, unknown>): never {
  fail(
    typeof result.error === 'string' && result.error ? result.error : 'RELEASE_PUBLISH_PRIMITIVE_FAILED',
    typeof result.message === 'string' && result.message ? result.message : 'release publication primitive failed',
    result,
    result.may_have_mutated === true,
  );
}

export async function publishReleasePlan(raw: unknown, dependencies: ReleasePublishDependencies): Promise<ReleasePublishResult> {
  ...
  if (!Array.isArray(transitions)) fail('RELEASE_PUBLISH_RUNTIME_INVALID', 'exact-revision transition definitions are unavailable');
  ...
  if (result.ok !== true) strictReleaseFailure(result);
