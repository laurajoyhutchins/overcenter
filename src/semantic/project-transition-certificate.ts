...
import type { ProjectExecutionIntent } from './project-graph-types.js';

export const PROJECT_TRANSITION_CERTIFICATE_SCHEMA = 'project-transition-certificate-v1' as const;
export const PROJECT_TRANSITION_OBLIGATION_SCHEMA = 'project-transition-obligation-v1' as const;

export type TransitionCertificateAuthority = Readonly<{
  kind: string;
  repository?: string;
  revision: string;
  derivation: string;
...
export type TransitionCertificateSubject = Readonly<{
  project_ref: string;
  transition_id: string;
  obligation_fingerprint: string;
}>;

export type TransitionCertificateEvidenceRef = Readonly<{
  kind: string;
  ref: string;
  subject: TransitionCertificateSubject;
}>;

export type TransitionCertificatePrerequisiteRef = Readonly<{
  transition_id: string;
  obligation_fingerprint: string;
  certificate_ref: string;
}>;

export type TransitionCertificateTransition = Readonly<{
  transition_id: string;
  kind: string;
...
export type ProjectTransitionCertificate = Readonly<{
  schema: typeof PROJECT_TRANSITION_CERTIFICATE_SCHEMA;
  semantic_identity: string;
  subject: TransitionCertificateSubject;
  authority: TransitionCertificateAuthority;
  prerequisite_evidence: readonly TransitionCertificatePrerequisiteRef[];
  authoritative_effect_evidence: readonly TransitionCertificateEvidenceRef[];
  settlement_evidence: readonly TransitionCertificateEvidenceRef[];
  execution_evidence: readonly TransitionCertificateEvidenceRef[];
...
function canonicalAcceptance(intent: ProjectExecutionIntent | undefined): readonly Readonly<{ kind: string; requirement: string }>[] {
...
export async function computeTransitionObligationFingerprint(transition: TransitionCertificateTransition): Promise<string> {
...
function canonicalPrerequisites(values: readonly TransitionCertificatePrerequisiteRef[]): readonly TransitionCertificatePrerequisiteRef[] {
...
function canonicalEvidence(values: readonly TransitionCertificateEvidenceRef[]): readonly TransitionCertificateEvidenceRef[] {
  return Object.freeze([...values]
    .map((value) => Object.freeze({
...
async function semanticIdentity(subject: TransitionCertificateSubject, claimedOutcome: string): Promise<string> {
...
export async function buildTransitionCertificate(input: Readonly<{
  project_ref: string;
  transition: TransitionCertificateTransition;
  authority: TransitionCertificateAuthority;
  prerequisite_evidence: readonly TransitionCertificatePrerequisiteRef[];
  authoritative_effect_evidence: readonly TransitionCertificateEvidenceRef[];
  settlement_evidence: readonly TransitionCertificateEvidenceRef[];
  execution_evidence: readonly TransitionCertificateEvidenceRef[];
  claimed_outcome: string;
}>): Promise<ProjectTransitionCertificate> {
...
function subjectMatches(evidence: TransitionCertificateEvidenceRef, subject: TransitionCertificateSubject): boolean {
...
export async function verifyTransitionCertificate(
  certificate: ProjectTransitionCertificate,
  transition: TransitionCertificateTransition,
  authority: TransitionCertificateAuthority,
): Promise<Readonly<{ accepted: boolean; reasons: readonly string[] }>> {
  const reasons: string[] = [];
  const expectedFingerprint = await computeTransitionObligationFingerprint(transition);
  if (certificate.schema !== PROJECT_TRANSITION_CERTIFICATE_SCHEMA) reasons.push('CERTIFICATE_SCHEMA_MISMATCH');
  ...
  if (canonicalJson(certificate.authority) !== canonicalJson(authority)) reasons.push('AUTHORITY_PROVENANCE_MISMATCH');
  ...
  if (canonicalJson(expectedDependencies) !== canonicalJson(actualDependencies)) reasons.push('PREREQUISITE_CLOSURE_INCOMPLETE');
  ...
  if (certificate.authoritative_effect_evidence.length === 0) reasons.push('AUTHORITATIVE_EFFECT_EVIDENCE_MISSING');
  if (certificate.settlement_evidence.length === 0) reasons.push('SETTLEMENT_EVIDENCE_MISSING');
  if (certificate.execution_evidence.length === 0) reasons.push('EXECUTION_EVIDENCE_MISSING');
  if (evidence.some((entry) => !subjectMatches(entry, certificate.subject))) reasons.push('EVIDENCE_SUBJECT_MISMATCH');

  const expectedSemanticIdentity = await semanticIdentity(certificate.subject, certificate.claimed_outcome);
  if (certificate.semantic_identity !== expectedSemanticIdentity) reasons.push('CERTIFICATE_IDENTITY_MISMATCH');
