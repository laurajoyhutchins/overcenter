...
function text(value: unknown, field: string, max = 512): string {
  ...
  if (!normalized || normalized.length > max) return fail(`${field} is invalid`, { field });
  return normalized;
}
...
export async function deriveProjectTransitionGithubWorkspace(authority: Readonly<Record<string, unknown>>): Promise<Readonly<{
  ...
  if (!authority || authority.subject !== 'project_transition') return fail('execution authority must be a project transition');
  const repository = text(authority.repository, 'repository', 256);
  if (!REPOSITORY.test(repository)) return fail('repository is invalid', { repository });
  ...
  const gitAuthority = authority.authority && typeof authority.authority === 'object' && !Array.isArray(authority.authority)
    ? authority.authority as Readonly<Record<string, unknown>>
    : null;
  ...
  const generation = {
    schema:'project-transition-github-workspace-generation-v1' as const,
    repository,
