...
function statusFor(result) {
  ...
  if (String(result?.error || '').endsWith('_INDETERMINATE')) return 409;
  if (String(result?.error || '').startsWith('INVALID_') || String(result?.error || '').endsWith('_MISMATCH')) return 422;
  return 502;
}

export default async function (req, res) {
  const response = await executeCorrelatedCommand('github.issue.close', req.body || {}, (input) => closeGithubIssueWithGitHubApp(input), { statusForFailure:statusFor, flattenDetails:true });
