...
function statusFor(result) {
  ...
  if (result.error === 'GITHUB_SETUP_REQUIRED') return 412;
  if (result.error === 'GITHUB_PERMISSION_DENIED') return 403;
...
export default async function (req, res) {
  const result = await applyGithubChangeset(req.body || {}, { db });
