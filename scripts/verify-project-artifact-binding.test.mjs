...
import { bindProjectArtifact, classifyProjectArtifactBinding } from '../lib/project-artifact-lineage.js';
...
test('bound issue can satisfy one exact semantic obligation', async () => {
...
test('unbound lookalike remains ambiguous even when provider state appears satisfied', () => {
  assert.deepEqual(
    classifyProjectArtifactBinding(null, { provider:{ repository:'laurajoyhutchins/overcenter', kind:'issue', id:732, state:'closed' } }),
    { classification:'ambiguous', evidence:{ reason:'durable-binding-absent' } },
  );
});

test('provider identity mismatch remains ambiguous rather than inheriting satisfaction', () => {
  ...
  assert.deepEqual(
    classifyProjectArtifactBinding(binding, { provider:{ repository:'laurajoyhutchins/overcenter', kind:'issue', id:733, state:'closed' } }),
    { classification:'ambiguous', evidence:{ reason:'provider-identity-mismatch' } },
  );
});
