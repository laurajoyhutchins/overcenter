...
function functionLikeHelper(statement) {
  if (ts.isFunctionDeclaration(statement) && statement.name && HELPER_NAMES.has(statement.name.text) && statement.parameters.length >= 2) return statement.name.text;
...
function isNativeTestStatement(statement) {
  return ts.isExpressionStatement(statement)
    && ts.isCallExpression(statement.expression)
    && callIdentifier(statement.expression) === 'test';
}
...
function stripLegacyRunnerImports(file, source) {
  ...
  for (const statement of sf.statements) {
    ...
    if (legacy.length !== elements.length || statement.importClause.name) throw new Error(`${file}: mixed legacy runner import requires explicit migration`);
...
function stripCollectorBookkeeping(file, source, collectors) {
  ...
  const edits = [];
  for (const statement of sf.statements) {
    if (isNativeTestStatement(statement)) continue;
    if (statementReferencesCollector(statement, collectors)) edits.push({ start:statement.getFullStart(), end:statement.end, text:'' });
  }
  return applyEdits(source, edits);
}

function assertNoTopLevelCollectorReferences(file, source, collectors) {
  const sf = parse(file, source);
  for (const statement of sf.statements) {
    if (isNativeTestStatement(statement)) continue;
    if (statementReferencesCollector(statement, collectors)) {
      throw new Error(`${file}: legacy collector bookkeeping survived migration`);
    }
  }
}
  ...
  for (const statement of runner.body.statements) {
    if (ts.isVariableStatement(statement)) {
      for (const declaration of statement.declarationList.declarations) {
        ...
        if (declaration.initializer.elements.length && declaration.initializer.elements.every((element) => ts.isIdentifier(element) && /^test[A-Z]/.test(element.text))) {
...
function migrateSource(file, source) {
  ...
  for (const statement of runner.body.statements) {
    if (localHelpers.has(functionLikeHelper(statement)) || isEmptyArrayDeclaration(statement, collectors) || ts.isReturnStatement(statement) || containsOtherSuiteCall(statement, runnerName)) {
      removeStatement(statement);
      continue;
    }
    if (ts.isVariableStatement(statement)) {
      const text = statement.getText(sf);
      if ([...collectors].some((name) => new RegExp(`\\b${name}\\.(?:filter|every|map|length)\\b`).test(text))) removeStatement(statement);
    }
  }
  ...
  assertNoTopLevelCollectorReferences(file, promoted, collectors);
  ...
  if (reparsed.statements.some((statement) => ts.isFunctionDeclaration(statement) && statement.name && exported(statement) && RUNNER_NAME.test(statement.name.text))) throw new Error(`${file}: exported legacy runner survived migration`);
...
function migrateEmbeddedAuthSuite(source) {
  const file = 'lib/github-app-auth.js';
  const sf = parse(file, source);
  const runner = sf.statements.find((statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === 'runGitHubAppAuthRegressionTests');
  if (!runner?.body || !exported(runner)) throw new Error(`${file}: embedded auth regression runner not found`);
  const bodyStart = runner.body.getStart(sf) + 1;
  const bodyEnd = runner.body.end - 1;
  let body = source.slice(bodyStart, bodyEnd);
  body = body.replace(/^\s*const results = \[\];\s*$/m, '');
  body = body.replace(/^\s*async function test\(name, fn\) \{[\s\S]*?^\s*\}\s*$/m, '');
  body = body.replace(/\bawait\s+test\s*\(/g, 'test(');
  body = body.replace(/^\s*const failed = results\.filter\([\s\S]*?^\s*return \{ ok: failed\.length === 0, passed: results\.length - failed\.length, failed: failed\.length, results \};\s*$/m, '');
  if (/\bresults\b/.test(body)) throw new Error(`${file}: auth suite result collector survived extraction`);

  const internalExport = `\nexport const githubAppAuthVerificationInternals = Object.freeze({\n  permissionProfile,\n  installationTokenRequestBody,\n  withInstallationTokenClient,\n  createInstallationResolver,\n  githubRequest,\n});\n`;
  const production = applyEdits(source, [{ start:runner.getFullStart(), end:runner.end, text:internalExport }]);
  const testSource = `import test from 'node:test';\nimport {\n  githubAppChangesetPermissionProfile,\n  githubAppCapabilityCatalog,\n  githubAppFallbackPolicy,\n  inspectGitHubAppCapabilities,\n  githubAppAuthVerificationInternals,\n} from './github-app-auth.js';\n\nconst {\n  permissionProfile,\n  installationTokenRequestBody,\n  withInstallationTokenClient,\n  createInstallationResolver,\n  githubRequest,\n} = githubAppAuthVerificationInternals;\n\n${body.trim()}\n`;
  parse('lib/github-app-auth.test.js', testSource);
  return { production, testSource };
}
...
console.log(JSON.stringify({ schema:'node-test-migration-v2', migrated_count:migrated.length, failure_count:failures.length, migrated, failures }, null, 2));
