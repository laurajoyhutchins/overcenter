...
function invalid(message) {
  throw Object.assign(new Error(message), { code:'REQUEST_INVALID', statusCode:400 });
}
...
export function createCloudRunAuthorityProofInspector({ db }) {
  ...
  return async function inspect(input = {}) {
    ...
    const quiescent = phase === 'quiescent';

    if (quiescent) {
      if (transitionId || observedAt || requestedRunId) invalid('quiescent proof does not accept transition_id, observed_at, or run_id');
    } else if (!requestedRunId && (!transitionId || !observedAt)) {
      invalid('transition_id and observed_at are required when run_id is absent');
    }
    ...
    try {
      ...
      const executionState = quiescent
        ? []
        : transitionId
          ? await many(client, `SELECT subject_key,subject_kind,project_ref,transition_id,authority_epoch,lease_ref,run_id,authority_repository,authority_revision,expires_at,hard_expires_at,updated_at FROM execution_state WHERE (run_id=$1) OR (project_ref=$2 AND transition_id=$3 AND subject_kind='project_transition') ORDER BY subject_key`, [runId, projectRef, transitionId])
      ...
      return Object.freeze({
        ...
        transition_id:quiescent ? null : (transitionId || null),
        observed_at:quiescent ? null : (observedAt || null),
        run,
        leases,
