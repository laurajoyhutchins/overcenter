...
test('subject-aware project-transition settlement checkpoints supplied continuation before settlement', async () => {
  const leaseRef = '11111111-1111-4111-8111-111111111111';
  ...
  const service = createSubjectAwareLeaseSettlementService({
    ...
    projectTransitions:{
      async checkpoint(input) { calls.push(['checkpoint', input]); return { ok:true }; },
      async settle(input) { calls.push(['settle', input]); return { ok:true, status:'settled' }; },
    },
  });
...
test('subject-aware project-transition settlement does not invent a checkpoint without continuation', async () => {
  const leaseRef = '22222222-2222-4222-8222-222222222222';
  let checkpoints = 0;
  const service = createSubjectAwareLeaseSettlementService({
    readLease:async () => ({
      lease_id:leaseRef,
