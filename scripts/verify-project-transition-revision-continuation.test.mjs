...
test('unchanged transition authority survives an unrelated authoritative graph revision', async () => {
  ...
  const runs = new Map([['run-1',{run_id:'run-1',status:'active',deadline_at:'2026-08-30T08:00:00Z'}]]);
  ...
  currentGraph = graph('2'.repeat(40));
  const verified = await service.require({
