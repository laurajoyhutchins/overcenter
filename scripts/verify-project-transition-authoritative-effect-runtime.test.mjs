...
test('authoritative-effect confirmation rejects merged PR evidence without immutable merge commit identity', async () => {
  const { service, calls } = fixture({
  ...
  assert.equal(calls.some(([kind]) => kind === 'compareCommits'), false);
});
