...
test('queued materialization returns a convergent pending outcome without final success', async () => {
  ...
  state.production = DEV; const result = await reconcileProduction({ repo:'laurajoyhutchins/overcenter' }, ports);
  assert.equal(result.outcome, 'materialization_pending'); assert.equal(result.materialization_run_ref, 'github-actions-run:99'); assert.equal(state.calls.some(([name]) => name === 'final'), false);
});
...
test('indeterminate materialization fails closed and cannot claim convergence', async () => {
  ...
  state.production = DEV; const error = await failure(() => reconcileProduction({ repo:'laurajoyhutchins/overcenter' }, ports));
...
test('immutable runtime mismatch fails closed', async () => {
  const { state, ports } = fixture(() => ({ reconcileRuntime: async () => ({ state:'succeeded', revision:DEV, verification_ref:'immutable-runtime:prod:507:manifest', deployment_version:507 }), verifyFinalState: async () => ({ development_revision:DEV, production_revision:DEV, runtime:runtime(DRIFT, 507) }) }));
  state.production = DEV; const error = await failure(() => reconcileProduction({ repo:'laurajoyhutchins/overcenter' }, ports)); assert.equal(error.code, 'PRODUCTION_RECONCILIATION_RUNTIME_MISMATCH');
});

test('final Git drift prevents convergence success', async () => {
  const { state, ports } = fixture(() => ({ verifyFinalState: async () => ({ development_revision:DRIFT, production_revision:DEV, runtime:runtime(DEV, 507) }) }));
  state.production = DEV; state.runtime = runtime(PROD, 505); const error = await failure(() => reconcileProduction({ repo:'laurajoyhutchins/overcenter' }, ports)); assert.equal(error.code, 'PRODUCTION_RECONCILIATION_FINAL_DRIFT');
});

test('caller mechanical coordinates are rejected', async () => {
  const { ports } = fixture(); const error = await failure(() => reconcileProduction({ repo:'laurajoyhutchins/overcenter', candidate_sha:DEV }, ports)); assert.equal(error.code, 'PRODUCTION_RECONCILIATION_REQUEST_INVALID'); assert.deepEqual(error.details.unsupported_fields, ['candidate_sha']);
});