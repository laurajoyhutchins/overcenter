...
test('pending project.amend persists durable external-verification recovery coordinates', async () => {
  ...
  const runtime = createProjectAuthoringProductionRuntime({
    ...
    beginProjectAuthoringRecovery:async (input) => {
      ...
      return {
        ...
        recovery_ref:'project-authoring:github:example/project:pending-test',
  ...
  await assert.rejects(
    ...
    (error) => error?.code === 'PROJECT_AUTHORING_INTEGRATION_PENDING'
      ...
      && error?.details?.recovery?.recovery_ref === 'project-authoring:github:example/project:pending-test',
  );
