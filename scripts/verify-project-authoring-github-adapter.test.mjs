...
test('GitHub-backed authoring preserves stable provider failure code and certainty', async () => {
  const adapter = createProjectAuthoringGithubAdapter({
    ...
    applyChangeset:async () => ({
      ok:false,
      error:'GITHUB_CHANGESET_UNEXPECTED_ERROR',
      message:'unexpected GitHub changeset provider failure',
      phase:'preflight.resolve_base',
  ...
  await assert.rejects(
    ...
    (error) => error?.code === 'GITHUB_CHANGESET_UNEXPECTED_ERROR'
      && error?.may_have_mutated === false
      && error?.details?.result?.phase === 'preflight.resolve_base',
  );
});
