...
test('production runtime wires typed binding, authoritative graph reader, and project-transition lease service', async () => {
  const calls=[];
  const service=createCompatibilityTransitionConfirmationRuntime({
    ...
    projectTransitions:{async acquire(input){calls.push(['acquire',input]);},async settle(input){calls.push(['settle',input]);}},
  });
  await assert.rejects(()=>service.confirm({run_id:'run-1',work_ref:'unknown'}),error=>error?.code==='COMPATIBILITY_TRANSITION_BINDING_NOT_FOUND');
