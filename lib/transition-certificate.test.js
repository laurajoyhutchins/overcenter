...
test('semantic obligation and dependency changes invalidate identity while provenance/evidence instances do not define it', async () => {
  ...
  const changedSubject = { ...subject, obligation_fingerprint:'d'.repeat(64) };
  const semanticChange = await createTransitionCertificate(input({
    obligation_fingerprint:'d'.repeat(64),
    evidence_refs:[evidence('verification','github-actions:123',changedSubject)],
    authoritative_effect_evidence:[evidence('github_revision','1'.repeat(40),changedSubject)],
  }));
