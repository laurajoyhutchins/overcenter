...
export async function runGithubTextTransportSpec() {
  ...
  results.push(await run('content bounds are enforced before compression and again at reference resolution', async () => {
    await expectError(prepareGithubContentReference('x'.repeat(2_000_001), { storage:new FakeStorage(), now:nowAt(T0), idFactory:()=>id(13) }), 'INVALID_CONTENT_TRANSPORT');
    const storage = new FakeStorage();
    const identifier = id(14);
    const prepared = await prepareGithubContentReference('hello\n', { storage, policy:TEST_POLICY, now:nowAt(T0), idFactory:()=>identifier });
    const manifest = await loadManifest(storage, identifier); manifest.content_bytes = 2_000_001; await saveManifest(storage, identifier, manifest);
