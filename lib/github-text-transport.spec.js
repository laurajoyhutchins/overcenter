...
import { githubChangesetSemanticRequestHash } from './github-apply-changeset.js';
import { expandGithubContentReferences, prepareGithubContentReference, resolveGithubContentReference } from './github-content-transport.js';
import { selectGithubTextTransport } from './github-text-transport.js';
...
// Text transport cases execute directly through node:test when this module is loaded.

  await (test('transport selector deterministically covers raw, gzip-inline, staged identity, and staged gzip', async () => {
  ...
  await (test('opaque references round-trip raw and compressed-inline content without exposing transport choices', async () => {
  ...
  await (test('staged references reconstruct bounded chunks and exact UTF-8', async () => {
    const storage = new FakeStorage();
  ...
  await (test('missing and corrupted staged chunks fail closed', async () => {
  ...
  await (test('reordered or duplicated staged manifest entries fail closed', async () => {
    const content = deterministicNoise(512);
  ...
  await (test('expired references are semantically stale and cleaned up best-effort', async () => {
    const storage = new FakeStorage();
  ...
  await (test('interrupted staged preparation never publishes a usable partial reference', async () => {
  ...
  await (test('raw, legacy gzip, and opaque references produce one semantic request hash', async () => {
  ...
  await (test('opaque staged content references round-trip Kubernetes-sized text while retaining a bounded maximum', async () => {
    const storage = new FakeStorage();
    ...
    await expectError(selectGithubTextTransport('x', { max_content_bytes:10_000_001 }), 'INVALID_CONTENT_TRANSPORT');
  }));

  await (test('oversized stored content metadata still fails closed at reference resolution', async () => {
    const storage = new FakeStorage();
