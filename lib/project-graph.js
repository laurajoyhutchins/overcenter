...
export async function dispatchProjectTransition(input = {}, handlers = {}) {
  ...
  if (selection.idle) return selection.idle;
...
export async function executeProjectTransitionLifecycle(input = {}, handlers = {}) {
  ...
  const selected = selection.selected;
  const requiredHandlers = Object.freeze({
  ...
  for (const [phase, handler] of Object.entries(requiredHandlers)) {
    if (typeof handler !== 'function') {
  ...
  const transition = transitionDescriptor(selected);
  const phases = {};
  phases.ENABLE = await requiredHandlers.ENABLE(transition, Object.freeze({ phases:Object.freeze({}) }));
  phases.ACQUIRE = await requiredHandlers.ACQUIRE(transition, Object.freeze({ phases:Object.freeze({ ...phases }) }));
  phases.EXECUTE = await requiredHandlers.EXECUTE(transition, Object.freeze({ phases:Object.freeze({ ...phases }) }));
  phases.COMMIT = await requiredHandlers.COMMIT(transition, Object.freeze({ phases:Object.freeze({ ...phases }) }));
  phases.CONFIRM = await requiredHandlers.CONFIRM(transition, Object.freeze({ phases:Object.freeze({ ...phases }) }));
