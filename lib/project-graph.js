...
function normalizeLiteral(value, field) {
  ...
  if (Array.isArray(value)) return Object.freeze(value.map((entry, index) => normalizeLiteral(entry, `${field}[${index}]`)));
  if (value && typeof value === 'object') {
    ...
    for (const key of Object.keys(value).sort()) {
      ...
      normalized[key] = normalizeLiteral(value[key], `${field}.${key}`);
    }
    return Object.freeze(normalized);
  }
  fail('INVALID_PROJECT_GRAPH', 'phase binding literal must be JSON-compatible', { field });
}

function normalizePhaseInput(raw, nodeId, phase) {
  ...
  for (const [field, rawSource] of Object.entries(raw)) {
    const semanticField = text(field, `phase_bindings.${phase}.input field`);
    ...
    if (keys[0] === 'literal') {
      normalized[semanticField] = Object.freeze({ literal:normalizeLiteral(rawSource.literal, `phase_bindings.${phase}.input.${semanticField}.literal`) });
      continue;
    }
    const from = text(rawSource.from, `phase_bindings.${phase}.input.${semanticField}.from`);
    const segments = from.split('.');
    const safeSegments = segments.every((segment) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(segment) && !['__proto__','prototype','constructor'].includes(segment));
...
function normalizePhaseBindings(raw, nodeId) {
  if (raw == null) return Object.freeze({});
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    fail('INVALID_PROJECT_GRAPH', 'phase_bindings must be an object', { node_id:nodeId });
  }
  const allowed = new Set(['ACQUIRE', 'COMMIT', 'CONFIRM']);
  const normalized = {};
  for (const [rawPhase, rawBinding] of Object.entries(raw)) {
    ...
    const evidence = rawEvidence.map((value, index) => text(value, `phase_bindings.${phase}.evidence[${index}]`));
    ...
    normalized[phase] = Object.freeze({
      primitive:text(rawBinding.primitive, `phase_bindings.${phase}.primitive`),
      evidence:Object.freeze([...evidence]),
      input:normalizePhaseInput(rawBinding.input, nodeId, phase),
    });
  }
  return Object.freeze(normalized);
}
...
function normalizeNode(raw) {
  ...
  return Object.freeze({
    ...
    phase_bindings:normalizePhaseBindings(raw.phase_bindings, id),
  });
}
...
function transitionDescriptor(selected) {
  return Object.freeze({
    ...
    phase_bindings:selected.phase_bindings,
  });
}
...
function structuralNode(node) {
  return Object.freeze({
    id:node.id,
    priority:node.priority,
    requires:Object.freeze([...node.requires].sort()),
    executor:node.executor,
    phase_bindings:node.phase_bindings,
  });
}

function sameStructuralNode(left, right) {
  return JSON.stringify(structuralNode(left)) === JSON.stringify(structuralNode(right));
}

function assertConfirmedAmendment(pendingReplan, confirmationEvaluation) {
  const expectedById = new Map(pendingReplan.evaluation.nodes.map((node) => [node.id, node]));
  const confirmedById = new Map(confirmationEvaluation.nodes.map((node) => [node.id, node]));

  for (const nodeId of pendingReplan.amendment.remove_node_ids) {
    if (confirmedById.has(nodeId)) {
      fail('PROJECT_REPLAN_CONFIRMATION_MISMATCH', 'confirmed graph still contains a node removed by the committed amendment', {
        node_id:nodeId,
        mutation:'remove',
      });
    }
  ...
  for (const nodeId of pendingReplan.amendment.upsert_node_ids) {
    const expected = expectedById.get(nodeId) || null;
    const confirmed = confirmedById.get(nodeId) || null;
    if (!expected || !confirmed || !sameStructuralNode(expected, confirmed)) {
      fail('PROJECT_REPLAN_CONFIRMATION_MISMATCH', 'confirmed graph does not contain the committed structural node amendment', {
        node_id:nodeId,
        mutation:'upsert',
      });
    }
...
export async function executeProjectTransitionLifecycle(input = {}, handlers = {}) {
  ...
  for (const phase of pendingPhases) {
    ...
    if (outcome.replan !== undefined) {
      if (phase !== 'EXECUTE') {
        fail('PROJECT_REPLAN_PHASE_INVALID', 'project graph replanning is only valid from the execute phase', {
          node_id:selected.id,
          phase,
        });
      }
      const replan = outcome.replan;
      if (!replan || typeof replan !== 'object' || Array.isArray(replan) ||
          !replan.graph || typeof replan.graph !== 'object' || Array.isArray(replan.graph) ||
          !replan.amendment || typeof replan.amendment !== 'object' || Array.isArray(replan.amendment)) {
        fail('INVALID_PROJECT_REPLAN', 'execute replan must provide a refreshed graph and graph amendment', {
          node_id:selected.id,
        });
      }
      const replanned = applyProjectGraphAmendment(replan.graph, replan.amendment);
      const replannedSelected = replanned.evaluation.nodes.find((node) => node.id === selected.id) || null;
      if (!replannedSelected) {
        fail('PROJECT_REPLAN_INVALID', 'replanned graph must retain the selected transition', {
          node_id:selected.id,
        });
      }
      if (!['WAITING', 'OFF_NOMINAL'].includes(replannedSelected.state)) {
        fail('PROJECT_REPLAN_INEFFECTIVE', 'replanned graph must make the selected transition non-executable', {
          node_id:selected.id,
          state:replannedSelected.state,
        });
      }
      pendingReplan = Object.freeze({
        graph:replanned.graph,
        evaluation:replanned.evaluation,
        amendment:replanned.amendment,
      });
    }
  }
  ...
  if (pendingReplan) {
    assertConfirmedAmendment(pendingReplan, confirmationEvaluation);
    if (!['WAITING', 'OFF_NOMINAL'].includes(confirmedSelected.state)) {
      fail('PROJECT_REPLAN_INEFFECTIVE', 'confirmed replanned graph must make the selected transition non-executable', {
        node_id:selected.id,
    ...
    return Object.freeze({
      dispatched:true,
      replanned:true,
      reason:'PROJECT_GRAPH_REPLANNED',
      transition,
      phases:Object.freeze({ ...phases }),
      graph:confirmedGraph,
      evaluation:confirmationEvaluation,
      amendment:pendingReplan.amendment,
