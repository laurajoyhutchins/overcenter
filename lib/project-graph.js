...
import { assertPredecessorClosedObligationSet } from './project-obligation-contract.js';
...
// Executor and phase-binding normalization are generated from the typed semantic contracts.
...
function normalizeNode(raw) {
  ...
  return Object.freeze({
    ...
    execution_intent:normalizeProjectExecutionIntent(raw.execution_intent, id, fail),
    phase_bindings:normalizeProjectPhaseBindings(raw.phase_bindings, id, fail),
  });
}
...
export function evaluateProjectGraph(input = {}) {
  ...
  assertPredecessorClosedObligationSet(nodes, [...done]);
  ...
  if (!complete && frontier.length === 0 && !evaluated.some((node) => node.state === 'OFF_NOMINAL')) {
    fail('PROJECT_GRAPH_EMPTY_FRONTIER_INVARIANT', 'incomplete project graph has no READY work and no explicit off-nominal blocker', {
