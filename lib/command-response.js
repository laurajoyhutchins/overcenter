...
const COMMAND_REJECTION_OVERRIDES = new Map([
  ...
  ['github.repository.rename:GITHUB_APP_PERMISSION_DENIED', true],
  ['github.repository.rename:GITHUB_PERMISSION_DENIED', true],
  ['github.repository.rename:GITHUB_NOT_FOUND', true],
...
register(['PROJECT_BOOTSTRAP_CONFIRMATION_INVALID'], 'validation', false, 400);
...
register(['TEXT_PRECONDITION_FAILED'], 'precondition', false, 422, true);
...
register([
  ...
  'PROJECT_BOOTSTRAP_CONFIRMATION_NOT_READY',
  'PROJECT_BOOTSTRAP_CONFIRMATION_NOT_CONFIRMED',
  'PROJECT_BOOTSTRAP_CONFIRMATION_AUTHORITY_CHANGED',
  'PROJECT_TRANSITION_RUN_NOT_ACTIVE',
  'PROJECT_TRANSITION_NOT_READY',
  'PROJECT_TRANSITION_AUTHORITY_STALE',
  'PROJECT_TRANSITION_LEASE_STALE',
  'PROJECT_TRANSITION_LEASE_SCOPE_MISMATCH',
  'PROJECT_AUTHORING_AUTHORITY_STALE',
  'PROJECT_DEFINITION_MUTATION_AUTHORITY_STALE',
...
register([
  ...
  'GITHUB_REPOSITORY_RENAME_REJECTED',
  ...
  'PROJECT_TRANSITION_IDEMPOTENCY_CONFLICT',
  'PROJECT_TRANSITION_LEASE_ALREADY_SETTLED',
...
register([
  ...
  'PROJECT_BOOTSTRAP_CONFIRMATION_UNAVAILABLE',
], 'setup');
...
register(['PROJECT_AUTHORING_INTEGRATION_PENDING'], 'precondition', true, 409, true);
register(['PRODUCTION_RECONCILIATION_SOURCE_NOT_VERIFIED','PRODUCTION_PROMOTION_SOURCE_NOT_VERIFIED'], 'precondition', true, 409, true);
...
register(['GITHUB_REPOSITORY_RENAME_INDETERMINATE'], 'upstream', true, 502);
...
const REFRESH_AUTHORITY_CODES = new Set([
  ...
  'GITHUB_AUTO_MERGE_STATE_CHANGED', 'GITHUB_REPOSITORY_METADATA_STATE_CHANGED', 'GITHUB_REPOSITORY_IDENTITY_CHANGED', 'GITHUB_REPOSITORY_TEMPLATE_STATE_CHANGED', 'GITHUB_REPOSITORY_NOT_TEMPLATE', 'GITHUB_MILESTONE_STATE_CHANGED', 'GITHUB_RELEASE_STATE_CHANGED', 'GITHUB_DEFAULT_BRANCH_CHANGED', 'GITHUB_PRODUCTION_PROMOTION_STATE_CHANGED', 'GITHUB_PRODUCTION_PROMOTION_CANDIDATE_CHANGED', 'GITHUB_INTEGRATION_RECOMPUTE_REQUIRED',
]);
...
function mechanicalRecommendedAction(code, { retryable, mayHaveMutated, errorClass } = {}) {
  if (code === 'PROJECT_AUTHORING_INTEGRATION_PENDING') return 'retry_same_request';
  if (mayHaveMutated || String(code).includes('INDETERMINATE')) return 'reconcile_external_effect';
  if (code === 'TEXT_PRECONDITION_FAILED') return 'execute_recovery_operation';
