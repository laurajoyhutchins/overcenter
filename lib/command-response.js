...
register([
  ...
  'GITHUB_REPOSITORY_APPROVAL_REQUIRED',
  'GITHUB_REPOSITORY_APPROVAL_REJECTED',
  'GITHUB_USER_IDENTITY_MISMATCH',
...
register([
  ...
  'GITHUB_USER_AUTH_REQUIRED',
  'GITHUB_USER_AUTH_DEVICE_FLOW_DISABLED',
  'GITHUB_USER_AUTH_CONFIGURATION_ERROR',
], 'setup');
...
register(['GITHUB_REPOSITORY_CREATE_INDETERMINATE'], 'upstream', true, 502);
...
const RESERVED_FAILURE_KEYS = new Set([
  ...
  'error_class', 'retryable', 'rejection', 'may_have_mutated', 'recommended_action', 'details', 'status', 'httpStatus', 'name', 'stack',
]);
...
const REFRESH_AUTHORITY_CODES = new Set([
  ...
  'GITHUB_DEFAULT_BRANCH_CHANGED', 'GITHUB_INTEGRATION_RECOMPUTE_REQUIRED',
]);
...
const STOP_AND_REQUEUE_CODES = new Set([
  'LEASE_EXPIRED', 'LEASE_INVALID', 'HEARTBEAT_LIMIT_REACHED', 'RUN_BUDGET_EXHAUSTED',
  'NO_PROGRESS_HEARTBEAT', 'CHECKPOINT_REQUIRED',
]);
const RESUME_RUN_CODES = new Set(['RUN_NOT_FOUND', 'RUN_NOT_ACTIVE', 'RUN_NOT_REGISTERED', 'RUN_HAS_ACTIVE_LEASE']);

function mechanicalRecommendedAction(code, { retryable, mayHaveMutated, errorClass } = {}) {
  if (code === 'GITHUB_REPOSITORY_APPROVAL_REQUIRED') return 'await_owner_approval';
  if (code === 'GITHUB_REPOSITORY_APPROVAL_REJECTED') return 'stop';
