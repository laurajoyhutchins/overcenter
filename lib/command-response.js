...
function register(codes, errorClass, retryable = false, httpStatus = DEFAULT_STATUS[errorClass]) {
  for (const code of codes) EXACT.set(code, { error_class: errorClass, retryable, http_status: httpStatus });
}
...
register(['IDEMPOTENCY_IN_PROGRESS'], 'conflict', true);
...
function fallbackClassification(code) {
  if (code.startsWith('INVALID_')) return { error_class: 'validation', retryable: false, http_status: 422 };
  if (code.endsWith('_PERMISSION_DENIED')) return { error_class: 'permission', retryable: false, http_status: 403 };
  if (code.endsWith('_SETUP_REQUIRED') || code.endsWith('_CONFIGURATION_ERROR')) return { error_class: 'setup', retryable: false, http_status: 412 };
  if (code.endsWith('_NOT_FOUND')) return { error_class: 'not_found', retryable: false, http_status: 404 };
  if (code.startsWith('GITHUB_') || code.startsWith('LINEAR_') || code.startsWith('GOOGLE_DRIVE_')) {
    return { error_class: 'upstream', retryable: false, http_status: 502 };
  }
  return { error_class: 'internal', retryable: false, http_status: 500 };
}
...
export function classifyCommandError(code, overrides = {}) {
  ...
  return {
    ...
    retryable: overrides.retryable === undefined ? base.retryable : Boolean(overrides.retryable),
...
const RESERVED_FAILURE_KEYS = new Set([
  'ok', 'command', 'schema_version', 'observed_at', 'error', 'code', 'message',
  'error_class', 'retryable', 'details', 'status', 'httpStatus', 'name', 'stack',
]);
