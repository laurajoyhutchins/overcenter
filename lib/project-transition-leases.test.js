...
export async function runProjectTransitionLeaseTests(){
  ...
  await test('non-READY transition is rejected before lease creation',async()=>{ const f=fixture(); f.setGraph({...f.graph,nodes:[{...f.graph.nodes[0],lifecycle:{state:'Todo',lane:'lane:repo-implementation',operating_condition:'HOLD'}}]}); await expectCode(()=>f.service.acquire({run_id:'run-1',project_ref:'github:laurajoyhutchins/overcenter',transition_id:'transition-a',lease_seconds:600,idempotency_key:'a'}),'PROJECT_TRANSITION_NOT_READY'); assert(f.store.getLease('anything') instanceof Promise,'store shape'); });
