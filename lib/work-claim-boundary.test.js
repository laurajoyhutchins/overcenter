...
// Tests execute directly through native node:test discovery.
  ...
  test('contract-valid semantic claim derives a lease-valid internal retry key', async () => {
    const captured = [];
    const service = {
      async claim(request) {
  ...
  test('materially different claim intent does not collide', async () => {
    const captured = [];
  ...
  test('canonicalization failure cannot reach lease mutation', async () => {
    let calls = 0;
