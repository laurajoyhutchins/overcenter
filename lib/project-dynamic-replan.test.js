...
export async function runProjectDynamicReplanTests() {
  ...
  await test('rejects confirmation that does not contain the committed graph amendment', async()=>{
    const selected = node('implement-runtime', 10);
    const prerequisite = node('establish-schema', 20);
    const replannedSelected = node('implement-runtime', 10, ['establish-schema']);
    ...
    try {
      await executeProjectTransitionLifecycle({ nodes:[selected] }, {
        operator: async () => ({
          ok:true,
          replan:{
            graph:{ nodes:[selected] },
    ...
    assert(code === 'PROJECT_REPLAN_CONFIRMATION_MISMATCH', 'confirmation accepted a graph that did not contain the committed amendment');
  });
