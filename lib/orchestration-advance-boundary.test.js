...
export async function runOrchestrationAdvanceBoundaryTests() {
  ...
  await test('exact WAITING suspension exposes durable recovery identity and release conditions', async()=>{
    const exactGraph = graph();
    ...
    const service = createPostgresOrchestrationAdvanceService({
      ...
      store:store(),
      projectGraphReader:async()=>exactGraph,
      projectTransitions:{
        ...transitions(),
        async suspensionFor(input) {
          ...
          return Object.freeze({
            ...
            conditions:Object.freeze({
              transition_revision_fingerprint:'r'.repeat(64),
              transition_dependency_fingerprint:'q'.repeat(64),
    ...
    assert(result.suspension?.reason === 'waiting for authoritative prerequisite evidence', 'WAITING lost durable recovery reason');
  ...
  await test('finish settlement routes project-transition subjects without consulting legacy work authority', async()=>{
    ...
    assert(calls.graph[0].reason === 'refresh graph frontier', 'graph finish settlement lost reason provenance');
    ...
    assert(!('promotion_condition' in calls.graph[0]) && !('requeue_class' in calls.graph[0]) && !('continuation' in calls.graph[0]) && !('next_state' in calls.graph[0]) && !('next_lane' in calls.graph[0]), 'legacy work routing fields leaked into graph settlement');
  });
