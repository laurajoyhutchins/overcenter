...
import { createSubjectAwareOrchestrationDiagnosisService } from './orchestration-diagnose-runtime.js';
...
const EXECUTION_INTENT = Object.freeze({
  ...
  desired_outcome:'Verify the graph-native agent handoff boundary.',
  acceptance_evidence:Object.freeze([
    Object.freeze({ kind:'verification', requirement:'Evidence must prove the agent handoff boundary is reachable.' }),
  ]),
...
// Native node:test owns registration, execution, and failure reporting.

  test('orchestration.advance is a canonical semantic command', async()=>{
  ...
  test('semantic worker boundary accepts only run_id for orchestration.advance', async()=>{
  ...
  test('production advance factory composes target store graph reader and project transition leases', async()=>{
  ...
  test('project-only advance skips durably suspended blocked work and selects independent READY frontier work', async()=>{
    const acquisitions = [];
    const twoNodeGraph = {
      ...graph(),
      nodes:[
        { ...graph().nodes[0], id:'blocked-work', priority:20 },
        { ...graph().nodes[0], id:'independent-work', priority:10 },
      ],
    };
    const service = createPostgresOrchestrationAdvanceService({
      db:{ async query() { throw new Error('suspension fixture should use injected dependencies'); } },
      store:{
        async getRun(runId) {
          return runId === 'suspension-run'
            ? { run_id:runId, status:'active', target:{ project_ref:PROJECT_REF, horizon:{ kind:'project', ref:PROJECT_REF } } }
            : null;
        },
      },
      projectGraphReader:async()=>twoNodeGraph,
      projectTransitions:{
        async isSuspended(input) { return input.transition_id === 'blocked-work'; },
        async acquire(input) {
        ...
        async settle() { throw new Error('agent handoff must not settle'); },
      },
    });
    const result = await service.advance({ run_id:'suspension-run' });
    assert(result.outcome === 'AGENT_EXECUTION_REQUIRED', 'independent READY transition was not selected');
    assert(result.transition?.id === 'independent-work', `blocked transition was reacquired: ${result.transition?.id || 'none'}`);
    assert(acquisitions.join(',') === 'independent-work', `suspended transition reached lease acquisition: ${acquisitions.join(',')}`);
  });

  test('exact WAITING suspension exposes durable recovery identity and promotion conditions', async()=>{
    const recoveryRef = 'project-transition-suspension:99999999-9999-4999-8999-999999999999';
    const service = createPostgresOrchestrationAdvanceService({
      db:{ async query() { throw new Error('durable suspension fixture should use injected dependencies'); } },
      store:store(),
      projectGraphReader:async()=>graph(),
      projectTransitions:{
        ...transitions(),
        async suspensionFor(input) {
          assert(input.project_ref === PROJECT_REF && input.transition_id === 'agent-work', 'suspension lookup lost exact project transition identity');
          return Object.freeze({
            schema:'project-transition-suspension-v1', recovery_ref:recoveryRef, project_ref:PROJECT_REF, transition_id:'agent-work',
            reason:'waiting for authoritative prerequisite evidence',
            conditions:Object.freeze({ promotion_condition:'Exact verification becomes authoritative', transition_revision_fingerprint:'r'.repeat(64), transition_dependency_fingerprint:'q'.repeat(64), release_when:Object.freeze(['promotion_condition_satisfied','transition_revision_changes','transition_dependencies_change']) }),
          });
        },
      },
    });
    const result = await service.advance({ run_id:'advance-boundary-run' });
    assert(result.outcome === 'WAITING', `durably suspended exact transition returned ${result.outcome}`);
    assert(result.suspension?.recovery_ref === recoveryRef, 'WAITING lost durable recovery identity');
    assert(result.suspension?.conditions?.promotion_condition === 'Exact verification becomes authoritative', 'WAITING lost durable promotion condition');
    assert(result.suspension?.conditions?.release_when?.includes('transition_dependencies_change'), 'WAITING lost mechanical release condition');
  });

  test('agent handoff includes revision-bound predecessor context from project transition authority', async()=>{
    const contextCalls = [];
    const service = createPostgresOrchestrationAdvanceService({
      db:{ async query() { throw new Error('context fixture should use injected dependencies'); } },
      store:store(),
      projectGraphReader:async()=>graph(),
      projectTransitions:{
        ...transitions(),
        async contextFor(input) {
          ...
          return Object.freeze({
            ...
            predecessors:Object.freeze([{ transition_id:'predecessor', source_authority_revision:REVISION, evidence:Object.freeze([{ kind:'candidate_revision', ref:'abc123' }]) }]),
          });
        },
      },
    });
    const result = await service.advance({ run_id:'advance-boundary-run' });
    assert(result.outcome === 'AGENT_EXECUTION_REQUIRED', 'context regression did not reach agent handoff');
    assert(result.predecessor_context?.schema === 'project-transition-context-v1', 'agent packet omitted predecessor context');
    assert(result.predecessor_context?.authority?.revision === REVISION, 'predecessor context lost exact graph authority');
    assert(result.predecessor_context?.predecessors?.[0]?.evidence?.[0]?.ref === 'abc123', 'predecessor context lost bounded settlement evidence');
    assert(contextCalls.length === 1 && contextCalls[0].transition_id === 'agent-work', 'agent handoff did not derive context for the selected transition');
    assert(contextCalls[0].authority?.revision === REVISION, 'context derivation was not fenced to selected lease authority');
  });

  test('production semantic path reaches graph read exact authority operator settlement and fresh confirmation', async()=>{
    const events = [];
    let settled = false;
    let reads = 0;
    const service = createPostgresOrchestrationAdvanceService({
  ...
  test('semantic worker execution delegates to the production advance service', async()=>{
    const calls = [];
  ...
  test('advance journal projection is bounded and preserves handoff identity without capability material', async()=>{
  ...
  test('finish settlement routes project-transition subjects without consulting legacy work authority', async()=>{
    ...
    const result = await service.settleByRef({ lease_ref:'graph-lease', disposition:'blocked', reason:'wait for an authoritative promotion', promotion_condition:'authoritative promotion evidence changes', evidence:[{kind:'graph',ref:'stale'}], idempotency_key:'finish-graph' });
    ...
    assert(calls.graph[0].run_id === 'graph-run' && calls.graph[0].lease_ref === 'graph-lease' && calls.graph[0].disposition === 'blocked', 'graph finish settlement lost durable lease identity');
    assert(calls.graph[0].reason === 'wait for an authoritative promotion', 'graph finish settlement lost reason provenance');
    assert(calls.graph[0].promotion_condition === 'authoritative promotion evidence changes', 'graph finish settlement lost promotion-condition provenance');
    assert(JSON.stringify(calls.graph[0].evidence) === JSON.stringify([{kind:'graph',ref:'stale'}]), 'graph finish settlement lost evidence provenance');
    assert(!('requeue_class' in calls.graph[0]) && !('continuation' in calls.graph[0]) && !('next_state' in calls.graph[0]) && !('next_lane' in calls.graph[0]), 'legacy work routing fields leaked into graph settlement');
  });

  test('finish settles current graph authority and leaves superseded project-transition history non-blocking', async()=>{
  ...
  test('diagnosis reports superseded graph lease as historical without a Linear authority read', async()=>{
    const revisionA = 'a'.repeat(40);
    ...
    const diagnosis = await createSubjectAwareOrchestrationDiagnosisService({
      store,
  ...
  test('finish settlement preserves the legacy work path and fails closed on ambiguous subject state', async()=>{
  ...
  // Execution is owned by native node:test discovery.