...
export async function runOrchestrationAdvanceBoundaryTests() {
  ...
  await test('project-only advance skips durably suspended blocked work and selects independent READY frontier work', async()=>{
    const acquisitions = [];
    const twoNodeGraph = {
      ...graph(),
      nodes:[
  ...
  await test('exact WAITING suspension exposes durable recovery identity and promotion conditions', async()=>{
  ...
  await test('agent handoff includes revision-bound predecessor context from project transition authority', async()=>{
    const contextCalls = [];
    const service = createPostgresOrchestrationAdvanceService({
