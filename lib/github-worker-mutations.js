...
export async function coalesceGithubLeaseScopedChangeset(input,options={}){
  ...
  if(!executionAuthority||typeof executionAuthority.require!=='function'||typeof readBranch!=='function'||typeof withGithub!=='function'||!db||typeof db.query!=='function') return fail('EXECUTION_AUTHORITY_UNAVAILABLE','lease-scoped mechanical coalescing runtime is incomplete',null,503);
  ...
  const row=(await db.query(`SELECT idempotency_key, receipt FROM github_changeset_receipts
       WHERE repo = $1 AND branch = $2 AND commit_sha = $3 AND state = 'succeeded'
       ORDER BY updated_at DESC LIMIT 1`,[prepared.workspace.repository,prepared.workspace.branch,head])).rows?.[0]||null;
  ...
  if(!parentReceipt||!currentLeaseRef||parentLeaseRef!==currentLeaseRef) return fail('MECHANICAL_COALESCE_PARENT_AUTHORITY_MISMATCH','only the lease that created the immediately preceding mechanical workspace head may replace it',{expected_head:head,expected_lease_ref:currentLeaseRef||null,parent_lease_ref:parentLeaseRef||null,phase:'coalesce.preflight',may_have_mutated:false},409);
  const coalesceRequestSha256=await sha256Text(canonicalJson({lease_ref:currentLeaseRef,changes:input.changes,commit_message:commitMessage}));
  if(parentReceipt.coalesce_request_sha256===coalesceRequestSha256) return {...parentReceipt,idempotent_replay:true};
  ...
  if(revalidated.workspace.workspace_digest!==prepared.workspace.workspace_digest||revalidated.workspace.observed_head!==head) return fail('EXECUTION_AUTHORITY_STALE','managed workspace authority changed before mechanical coalescing',{expected_workspace_digest:prepared.workspace.workspace_digest,actual_workspace_digest:revalidated.workspace.workspace_digest,expected_head:head,actual_head:revalidated.workspace.observed_head,phase:'coalesce.preflight',may_have_mutated:false},409);
  const result=await withGithub({repo:prepared.workspace.repository,branch:prepared.workspace.branch,changes:input.changes},(github)=>coalesceGithubMechanicalChangeset({repo:prepared.workspace.repository,branch:prepared.workspace.branch,expected_head:head,changes:input.changes,commit_message:commitMessage},{github}));
  const replacementReceipt={...result,execution_authority:prepared.execution_authority,idempotency_key:row.idempotency_key,coalesced_from:head,coalesce_request_sha256:coalesceRequestSha256};
  let updated;
  try {
    updated=await db.query(`UPDATE github_changeset_receipts
       ...
       RETURNING idempotency_key`,[prepared.workspace.repository,row.idempotency_key,prepared.workspace.branch,result.commit_sha,result.tree_sha,canonicalJson(replacementReceipt),head]);
  } catch(error) {
    return fail('GITHUB_COALESCE_RECEIPT_WRITE_FAILED','mechanical coalescing changed GitHub state but durable replay evidence could not be written',{expected_head:head,replacement_head:result.commit_sha,phase:'coalesce.receipt',may_have_mutated:true,cause:String(error?.message||error)},503);
  }
  if(Number(updated?.rowCount||0)!==1) return fail('GITHUB_COALESCE_RECEIPT_FENCE_LOST','mechanical coalescing changed GitHub state but lost the exact predecessor receipt fence',{expected_head:head,replacement_head:result.commit_sha,phase:'coalesce.receipt',may_have_mutated:true},409);
