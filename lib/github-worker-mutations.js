...
function jsonObject(value){
  ...
  try{const parsed=JSON.parse(String(value||''));return parsed&&typeof parsed==='object'&&!Array.isArray(parsed)?parsed:null;}catch{return null;}
}

async function defaultAuthorizeMechanicalCoalesce({db,repository,branch,parent_head,lease_ref}){
  if(!db||typeof db.query!=='function'||!repository||!branch||!parent_head||!lease_ref)return false;
  const effectRef=`${repository}#${branch}@${parent_head}`;
  const result=await db.query(
    `SELECT resolution FROM operation_state
      WHERE command='github.apply_changeset'
        AND idempotency_scope=$1
        AND state='succeeded'
        AND effect_ref=$2
      ORDER BY resolved_at DESC NULLS LAST, updated_at DESC
      LIMIT 1`,
    [`repository:${repository}`,effectRef],
  );
  const resolution=jsonObject(result?.rows?.[0]?.resolution);
  return resolution?.receipt?.execution_authority?.lease_ref===lease_ref;
}
...
export function createGithubWorkerMutationRuntime(options={}){
  ...
  const authorizeMechanicalCoalesce=options.authorizeMechanicalCoalesce||((request)=>defaultAuthorizeMechanicalCoalesce({...request,db}));
  const runId=options.run_id||null;
  const shared={executionAuthority,readBranch,withGithub,applyChangeset,authorizeMechanicalCoalesce,db,run_id:runId};
