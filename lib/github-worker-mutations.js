...
import { coalesceGithubLeaseScopedMechanicalChangeset as coalesceMechanicalChangeset } from 'lib/github-mechanical-changeset-coalescing.js';
import {
...
export async function coalesceGithubLeaseScopedMechanicalChangeset(input,options={}){return coalesceMechanicalChangeset(input,options);}
...
export async function coalesceGithubLeaseScopedMechanicalChangesetWithReceipts(input,options={}){
  const db=options.db;const store=options.operations||createCompactProviderOperationPostgresStore(db);const now=options.now||(()=>new Date().toISOString());const attempt=options.attemptToken||crypto.randomUUID();
  const digest=await sha256Text(canonicalJson({schema:'github-mechanical-coalescing-v1',lease_ref:input?.lease_ref,changes:input?.changes,commit_message:input?.commit_message}));const scope=`project-transition:${String(input?.lease_ref||'')}`;const key=`github-mechanical-coalescing-v1:${digest}`;const observedAt=now();
  const claim=await store.claim({command:'github.coalesce_mechanical_changeset',scope,idempotency_key:key,request_sha256:digest,attempt_token:attempt,created_at:observedAt,stale_before:new Date(Date.parse(observedAt)-30000).toISOString(),recovery_payload:{phase:'claim'}});
  if(claim.outcome==='terminal'){const receipt=claim.operation?.resolution?.receipt||null;if(!receipt)return fail('IDEMPOTENCY_RECEIPT_INVALID','stored coalescing receipt is missing',null,500);return{...receipt,idempotent_replay:true};}
  if(claim.outcome==='conflict')return fail('IDEMPOTENCY_CONFLICT','mechanical coalescing identity conflicts with durable state',null,409);
  if(claim.outcome==='in_progress')return fail('IDEMPOTENCY_IN_PROGRESS','mechanical coalescing is already in progress',null,409);
  if(claim.outcome==='indeterminate')return fail('MECHANICAL_COALESCE_RECOVERY_REQUIRED','prior mechanical coalescing attempt has unresolved provider uncertainty',{idempotency_key:key},409,true);
  try{
    const result=await coalesceMechanicalChangeset(input,options);
    const receipt={...result,idempotency_key:key,idempotent_replay:false};
    await store.succeed({command:'github.coalesce_mechanical_changeset',scope,idempotency_key:key,attempt_token:attempt,updated_at:now(),effect_kind:'github_commit_branch_replacement',effect_ref:`${receipt.repo}#${receipt.branch}@${receipt.commit_sha}`,resolution:{receipt}});
    return receipt;
  }catch(error){
    if(error?.may_have_mutated===true)await store.markIndeterminate({command:'github.coalesce_mechanical_changeset',scope,idempotency_key:key,attempt_token:attempt,updated_at:now(),recovery_payload:{phase:'provider_uncertain',error_code:error?.code||null}});
    else await store.abandon({command:'github.coalesce_mechanical_changeset',scope,idempotency_key:key,attempt_token:attempt});
    throw error;
  }
}

async function readPriorChangesetFromDb(db,{repo,branch,commit_sha}){
  const result=await db.query(`SELECT resolution FROM operation_state WHERE command='github.apply_changeset' AND idempotency_scope=$1 AND state='succeeded' AND effect_ref=$2 ORDER BY resolved_at DESC LIMIT 1`,[`repository:${repo}`,`${repo}#${branch}@${commit_sha}`]);
  const raw=result?.rows?.[0]?.resolution;
  if(!raw)return null;
  if(typeof raw==='object')return raw.receipt||null;
  try{return JSON.parse(raw)?.receipt||null;}catch{return null;}
}
...
export function createGithubWorkerMutationRuntime(options={}){
  ...
  return Object.freeze({
    ...
    coalesceMechanicalChangeset(request){
      return coalesceGithubLeaseScopedMechanicalChangesetWithReceipts(request,{...shared,readPriorChangeset:(input)=>readPriorChangesetFromDb(db,input)});
    },
  });
}
