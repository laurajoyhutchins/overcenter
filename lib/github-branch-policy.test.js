...
// Test cases register directly with the native Node test runner.
  ...
  test('branch vocabulary is exactly the approved semantic set', async () => {
    assert(JSON.stringify(WORK_BRANCH_TYPES) === JSON.stringify(['feat','fix','refactor','test','docs','chore','research','work']), 'branch types drifted');
    assert(BRANCH_POLICY_VERSION === 'branch-policy-v1', 'policy version drifted');
  });

  test('new work branch grammar accepts semantic kebab names only', async () => {
    for (const branch of ['feat/add-stack-support', 'fix/check-race', 'docs/branch-policy-v1', 'research/source-audit-2', 'work/ignore-stale-historical-project-transition-leases-0123456789abcdef01234567']) {
  ...
  test('repository merge policy is squash-only and cleans merged heads', async () => {
  ...
  test('default branch rules are strict, PR-only, linear, and no-force/no-delete', async () => {
  ...
  test('branch policy request is exact and head-bound', async () => {
  ...
  test('fresh repository branch policy permits an empty required-check set without weakening structural protections', async () => {
    const normalized = normalizeGithubBranchPolicyRequest({
      repo: 'laurajoyhutchins/example',
      expected_head: 'a'.repeat(40),
      required_checks: [],
    });
    assert(Array.isArray(normalized.required_checks) && normalized.required_checks.length === 0, 'empty required checks were rejected');
    const rules = desiredDefaultBranchRules([]);
    const types = rules.map((rule) => rule.type).sort();
    assert(types.includes('deletion'), 'deletion protection missing');
    assert(types.includes('non_fast_forward'), 'non-fast-forward protection missing');
    assert(types.includes('required_linear_history'), 'linear-history protection missing');
    assert(types.includes('pull_request'), 'pull-request protection missing');
    assert(!types.includes('required_status_checks'), 'empty check set emitted an invalid required-status-check rule');
  });

  test('production policy protects explicit main without requiring pull requests', async () => {
  ...
  test('production policy reconcile is idempotent and head-fenced', async () => {
