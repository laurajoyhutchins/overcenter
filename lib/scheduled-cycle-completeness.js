...
// Compatibility-only scheduler policy. Lifecycle membership and commands come from work-lifecycle.js.
export const scheduledCycleSchedulePolicy = Object.freeze([
  Object.freeze({ id:'portfolio-dispatcher', title:'Enable', minute:0, automation_id:'6a74054183c88191a47278f43c61a4dd' }),
  Object.freeze({ id:'source-data-implementation', title:'Acquire', minute:12, automation_id:'6a74053648d88191bdcf9e6ad4ed1d8c' }),
  Object.freeze({ id:'repository-implementation', title:'Execute', minute:24, automation_id:'6a74051febd08191a86e737908a3e322' }),
  Object.freeze({ id:'portfolio-integration', title:'Commit', minute:36, automation_id:'6a740515088481919dd97d3be5d89b64' }),
  Object.freeze({ id:'exact-head-verification', title:'Confirm', minute:48, automation_id:'6a74052aeeb48191a22b828fc8ecb715' }),
]);

if (scheduledCycleSchedulePolicy.length !== PRODUCTIVE_STAGES.length) {
  throw new Error('scheduled cycle compatibility policy must cover every productive lifecycle stage');
}

export const scheduledCycleParticipants = Object.freeze(PRODUCTIVE_STAGES.map((stage, index) => Object.freeze({
  ...scheduledCycleSchedulePolicy[index],
  stage,
...
export function scheduledCycleIdForParticipant(participantId, anchorIso) {
  const participant = participantFor(participantId);
  return nearestCycle(participant, iso(anchorIso, 'anchor_at'));
}

export function scheduledRunId(cycleId, participantId) {
  const participant = participantFor(participantId);
  const normalizedCycle = cycleIdForStart(cycleStart(cycleId));
  return `scheduled:${normalizedCycle}:${participant.id}`;
}
...
async function recordForParticipant({ participant, cycleId, events, store }) {
  ...
  const explicitFailedClosed = eventCount(ordered, 'failed_closed') > 0;
  ...
  const explicitRunId = latestValue(ordered, 'run_id');
  let runId = explicitRunId;
  let run = runId && typeof store.runEvidence === 'function' ? await store.runEvidence(runId) : null;
  if (!explicitRunId && typeof store.runEvidence === 'function') {
    const derivedRunId = scheduledRunId(cycleId, participant.id);
    const derivedRun = await store.runEvidence(derivedRunId);
    if (derivedRun) { runId = derivedRunId; run = derivedRun; }
  }
  const startedAt = earliestValue(ordered, 'reported_started_at') || run?.started_at || null;
  const firstExternalEvidenceAt = externalEvents.length ? externalEvents.map((event)=>event.observed_at).sort()[0] : (run?.started_at || null);
  ...
  const failedClosed = explicitFailedClosed || (run?.status === 'finished' && ['blocked','failed'].includes(run?.disposition));
  const missing = eventCount(ordered, 'missing') > 0 && !acknowledged && !run;
  let classification = 'expected';
  if (accepted) classification = 'accepted';
  if (startedAt) classification = 'started';
  ...
  return {
    ...
    first_external_evidence_at:firstExternalEvidenceAt,
    acknowledged,
    claimed,
    idle,
    completed,
...
export function createScheduledCycleService({ store, now = () => new Date().toISOString() } = {}) {
  ...
  async function appendMissing(participant, cycleId, observedAt) {
    ...
    if (typeof store.runEvidence === 'function' && await store.runEvidence(scheduledRunId(cycleId, participant.id))) return null;
...
export function createPostgresScheduledCycleStore(dbBinding) {
  ...
  return {
    ...
    async cycleIdsSince(since) { const result=await dbBinding.query('SELECT DISTINCT cycle_id FROM scheduled_cycle_events WHERE cycle_id >= $1 ORDER BY cycle_id ASC', [since]); return (result.rows || []).map(row => row.cycle_id); },
    async runEvidence(runId) {
      const run = await row(`SELECT r.run_id,r.status,r.disposition,r.last_work_ref,r.started_at,r.finished_at,
...
export function createPostgresScheduledCycleService(options = {}) { return createScheduledCycleService({ store:options.store || createPostgresScheduledCycleStore(options.db), now:options.now }); }
