...
export async function runGithubIntegrationTests() {
  ...
  await test('internal direct exact-head mode returns merged instead of an asynchronous submission', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
      reviewPullRequest: async () => packet(),
      integrationApi: api,
      preferDirectMerge: true,
    });
    check(result.ok && result.outcome === 'merged', `unexpected outcome ${JSON.stringify(result)}`);
    check(result.integration_transport === 'direct_exact_head', `unexpected transport ${JSON.stringify(result)}`);
    check(api.calls.length === 1 && api.calls[0].op === 'merge_direct', `direct merge was not used ${JSON.stringify(api.calls)}`);
  });

  await test('exact-head direct merge uses the synchronous GitHub merge endpoint with SHA fencing', async () => {
    const calls = [];
    const api = createGithubIntegrationApiAdapter({
      async call(_name, request = {}) {
        calls.push(request);
        if (request.method === 'PUT' && request.path === '/repos/owner/repo/pulls/7/merge') {
          return { status: 200, body: { merged: true, sha: 'cccccccccccccccccccccccccccccccccccccccc', message: 'merged' } };
        }
        throw new Error(`unexpected GitHub call ${request.method} ${request.path}`);
      },
    });
    const result = await api.mergeDirect({ repo: 'owner/repo', pull_request: 7, expected_head: HEAD, merge_method: 'squash' });
    check(result.ok && result.status === 'merged', `unexpected result ${JSON.stringify(result)}`);
    check(calls.length === 1, `unexpected calls ${JSON.stringify(calls)}`);
    check(calls[0].body?.sha === HEAD && calls[0].body?.merge_method === 'squash', `direct merge was not exact-head fenced ${JSON.stringify(calls[0])}`);
  });

  await test('lost direct-merge acknowledgement reconciles authoritative merged state without replay', async () => {
    const calls = [];
    const api = createGithubIntegrationApiAdapter({
      async call(_name, request = {}) {
        calls.push(request);
        if (request.method === 'PUT' && request.path === '/repos/owner/repo/pulls/7/merge') throw new Error('socket vanished after merge');
        if (request.method === 'GET' && request.path === '/repos/owner/repo/pulls/7') {
          return { status: 200, body: { merged: true, merge_commit_sha: 'cccccccccccccccccccccccccccccccccccccccc', head: { sha: HEAD } } };
        }
        throw new Error(`unexpected GitHub call ${request.method} ${request.path}`);
      },
    });
    const result = await api.mergeDirect({ repo: 'owner/repo', pull_request: 7, expected_head: HEAD, merge_method: 'squash' });
    check(result.ok && result.status === 'merged' && result.reconciled_after_indeterminate === true, `unexpected result ${JSON.stringify(result)}`);
    check(JSON.stringify(calls.map(call => `${call.method} ${call.path}`)) === JSON.stringify(['PUT /repos/owner/repo/pulls/7/merge', 'GET /repos/owner/repo/pulls/7']), `mutation was replayed ${JSON.stringify(calls)}`);
  });
