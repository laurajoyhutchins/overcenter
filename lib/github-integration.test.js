import { reconcileGithubIntegration } from 'lib/github-integration.js';
...
export async function runGithubIntegrationTests() {
  ...
  await test('inspect reports a protected exact-head PR ready without mutating', async () => {
    const api = fakeApi();
    ...
    check(api.calls.length === 0, 'inspect mutated GitHub');
  });
