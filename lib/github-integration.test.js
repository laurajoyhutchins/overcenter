...
// Native node:test owns execution and result reporting.
  ...
  test('inspect reports a protected exact-head PR ready without any write transport', async () => {
  ...
  test('apply updates a stale standalone branch using exact expected head', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
  ...
  test('stale standalone PR can be identified before integration policy is configured', async () => {
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: false,
    }, {
      reviewPullRequest: async () => packet({
  ...
  test('authoritative current-base freshness permits only update before integration policy gating', async () => {
    const calls = [];
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('up-to-date PR still fails closed when integration policy is unavailable', async () => {
    const calls = [];
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('stale stack remains non-mutating before integration policy gating', async () => {
    const calls = [];
    const result = await reconcileGithubIntegration({
  ...
  test('stale stack is never merge-updated and requires cascading rebase', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 9, expected_head: HEAD, apply: true,
    }, {
  ...
  test('unprotected repositories are refused before mutation', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
  ...
  test('plan-limited private repository can integrate a standalone PR under explicit exact-head fallback', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('plan-limited exact-head integration falls back to synchronous merge when async transport is forbidden', async () => {
    const calls = [];
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('ordinary protected integration does not reinterpret async permission denial as a transport fallback', async () => {
    const calls = [];
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('pending required checks wait without mutation', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('ready standalone PR submits exact-head asynchronous squash merge', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
  ...
  test('ready stacked PR uses the same atomic asynchronous merge primitive', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 9, expected_head: HEAD, apply: true,
    }, {
  ...
  test('polling an asynchronous merge result is read-only and attributable', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
  ...
  test('cross-repository PRs are not automatically integrated', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
  ...
  test('already merged PRs are idempotent', async () => {
    const api = fakeApi();
    const result = await reconcileGithubIntegration({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
  ...
  test('direct merge adapter sends an exact-head squash request to the standard GitHub endpoint', async () => {
    const calls = [];
    const api = createGithubIntegrationApiAdapter({
      async call(_name, request = {}) {
        calls.push(request);
        return { status: 200, body: { merged: true, sha: 'cccccccccccccccccccccccccccccccccccccccc', message: 'Pull Request successfully merged' } };
      },
    });
    const result = await api.mergeDirect({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, merge_method: 'squash',
    });
    check(result.ok && result.status === 'merged', `unexpected direct merge result ${JSON.stringify(result)}`);
    check(calls.length === 1 && calls[0].method === 'PUT' && calls[0].path === '/repos/owner/repo/pulls/7/merge', `unexpected endpoint ${JSON.stringify(calls)}`);
    check(calls[0].body?.sha === HEAD && calls[0].body?.merge_method === 'squash', 'direct merge request lost exact-head squash semantics');
  });

  test('lost merge transport certainty is explicit and requires reconciliation', async () => {
    const api = createGithubIntegrationApiAdapter({
  ...
  test('lost update-branch transport certainty is explicit and requires reconciliation', async () => {
    const api = createGithubIntegrationApiAdapter({
      async call() { throw new Error('socket disappeared after write'); },
    });
  ...
  test('GitHub App wrapper read-only inspection never requests a write token', async () => {
    let withAppCalls = 0;
  ...
  test('GitHub App wrapper requests contents-write only for exact-head async merge', async () => {
    const profiles = [];
    const result = await reconcileGithubIntegrationWithGitHubApp({
      repo: 'owner/repo', pull_request: 7, expected_head: HEAD, apply: true,
    }, {
  ...
  test('missing optional pull-request write permission degrades stale standalone update to worktree recovery', async () => {
    const profiles = [];
