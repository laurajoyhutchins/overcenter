...
import { runCodexExecutionCapacityTests } from './codex-execution-capacity.test.js';
...
export async function runProjectDispatchTests() {
  ...
  await test('keeps Codex execution capacity environment-agnostic', async()=>{
    const result = await runCodexExecutionCapacityTests();
    assert(result.ok === true, `Codex execution capacity regressions failed: ${JSON.stringify(result.tests.filter((entry) => !entry.ok))}`);
  });

  await test('does not expose an executor-only dispatch bypass', async()=>{
    // Static namespace inspection is required because Hatchable V8 does not support dynamic import().
    assert(!('dispatchProjectTransition' in projectGraph), 'executor-only project dispatch remains publicly callable');
  });
