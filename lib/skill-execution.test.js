...
// Tests execute directly through native node:test discovery.
  ...
  test('implementation worker policy is server-owned and pins required and available skill revisions', async()=>{
  ...
  test('non-implementation worker gets an explicit empty policy', async()=>{
  ...
  test('permitted skill activation is durable and idempotent', async()=>{
  ...
  test('unknown skill activation fails closed', async()=>{
  ...
  test('skill completion is idempotent and conflicting replay fails closed', async()=>{
    const store=memoryStore();const service=createSkillExecutionService({store});
  ...
  test('run skill state exposes remaining required before completion and clears it after completion', async()=>{
    const store=memoryStore();const service=createSkillExecutionService({store});
  ...
  test('historical run without stored policy never acquires a retroactive requirement', async()=>{
