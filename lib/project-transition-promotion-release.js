...
export function createPromotionAwareProjectTransitionLeaseService({store,operationStore,readProjectGraph,now=()=>new Date().toISOString(),uuid=()=>crypto.randomUUID()}={}){
  ...
  async function releaseSuspension(input={}){
    ...
    if(claim.outcome==='terminal'){
      ...
      return Object.freeze({ok:true,schema:'project-transition-suspension-release-v1',released:true,recovery_ref:recoveryRef,authority_revision:release.authority_revision,evidence:proof,idempotent_replay:true});
    }
    ...
    return Object.freeze({ok:true,schema:'project-transition-suspension-release-v1',released:true,recovery_ref:recoveryRef,authority_revision:release.authority_revision,evidence:proof,idempotent_replay:false});
  }
