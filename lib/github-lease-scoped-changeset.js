...
const ALLOWED_FIELDS=new Set(['lease_ref','changes','commit_message','coalesce_mechanical']);
...
function validateInput(input){
  ...
  if(input.coalesce_mechanical!==undefined&&typeof input.coalesce_mechanical!=='boolean') return fail('INVALID_REQUEST','coalesce_mechanical must be a boolean',{field:'coalesce_mechanical'});
  return {lease_ref:validateLeaseRef(input.lease_ref),changes:input.changes,commit_message:input.commit_message,coalesce_mechanical:input.coalesce_mechanical===true};
}
...
export async function resolveGithubLeaseScopedChangeset(input,{executionAuthority,readBranch}={}){
  ...
  const explicit={
    ...
    ...(request.coalesce_mechanical ? {coalesce_mechanical:true} : {}),
  };
