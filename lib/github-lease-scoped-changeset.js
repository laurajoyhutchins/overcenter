...
const ALLOWED_FIELDS=new Set(['lease_ref','changes','commit_message','coalesce_previous_mechanical']);
...
function validateInput(input){
  ...
  const coalescePreviousMechanical=input.coalesce_previous_mechanical===undefined?false:input.coalesce_previous_mechanical;
  if(typeof coalescePreviousMechanical!=='boolean') return fail('INVALID_REQUEST','coalesce_previous_mechanical must be a boolean',{field:'coalesce_previous_mechanical'});
  return {lease_ref:validateLeaseRef(input.lease_ref),changes:input.changes,commit_message:input.commit_message,...(coalescePreviousMechanical?{coalesce_previous_mechanical:true}:{})};
}
...
export async function resolveGithubLeaseScopedChangeset(input,{executionAuthority,readBranch}={}){
  ...
  const explicit={
    ...
    ...(request.coalesce_previous_mechanical===true?{coalesce_previous_mechanical:true}:{}),
  };
...
export async function applyGithubLeaseScopedChangeset(input,options={}){
  const {executionAuthority,readBranch,withGithub,applyChangeset,expectedWorkspace=null,authorizeMechanicalCoalesce=null,...applyOptions}=options;
  ...
  const result=await withGithub({repo:resolved.request.repo,branch:resolved.request.branch,changes:resolved.request.changes},async(github)=>{
    ...
    return applyChangeset(resolved.request,{
      ...applyOptions,
      executionAuthority:revalidatingAuthority,
      github:guardedGithub,
      ...(request.coalesce_previous_mechanical===true?{coalesceAuthorization:{expectedHead:workspaceEvidence.observed_head}}:{}),
    });
  });
  if(result?.error==='MECHANICAL_CHANGESET_MUST_COALESCE'
      && request.coalesce_previous_mechanical!==true
      && typeof authorizeMechanicalCoalesce==='function'){
    const authorized=await authorizeMechanicalCoalesce({
      lease_ref:request.lease_ref,
      repository:resolved.request.repo,
      branch:resolved.request.branch,
      parent_head:result.parent_head||workspaceEvidence.observed_head,
    });
    if(authorized===true){
      return applyGithubLeaseScopedChangeset({...request,coalesce_previous_mechanical:true},{
        ...options,
        expectedWorkspace:workspaceEvidence,
        authorizeMechanicalCoalesce:null,
      });
    }
  }
  return result;
