...
const ALLOWED_FIELDS=new Set(['lease_ref','changes','commit_message','coalesce_mechanical_head']);
...
function validateInput(input){
  ...
  if(input.coalesce_mechanical_head!==undefined&&typeof input.coalesce_mechanical_head!=='boolean') return fail('INVALID_REQUEST','coalesce_mechanical_head must be a boolean',{field:'coalesce_mechanical_head'});
  return {lease_ref:validateLeaseRef(input.lease_ref),changes:input.changes,commit_message:input.commit_message,coalesce_mechanical_head:input.coalesce_mechanical_head===true};
}
...
export async function resolveGithubLeaseScopedChangeset(input,{executionAuthority,readBranch}={}){
  ...
  const explicit={
    ...
    ...(request.coalesce_mechanical_head?{coalesce_mechanical_head:true}:{}),
  };
