...
} from './project-transition-github-recovery.js';
import {
...
function observationFromSettlement(row) {
  const projectionReceipt = object(row?.settle_receipt);
  const canonicalReceipt = object(row?.settlement_receipt)
    || object(projectionReceipt?.canonical_receipt)
    || projectionReceipt;
  const subject = object(canonicalReceipt?.project_transition)
    || object(projectionReceipt?.project_transition);
  const disposition = canonicalReceipt?.project_transition_disposition
    || projectionReceipt?.disposition
    || null;
  const canonicalSchema = canonicalReceipt?.schema === 'settlement-receipt-v1';
  const projectionSchema = projectionReceipt?.schema === 'project-transition-lease-settlement-v1';
  if ((!canonicalSchema && !projectionSchema)
  ...
  return Object.freeze({
    ...
    provenance:Object.freeze({
      ...
      lease_ref:String(row?.lease_ref || row?.lease_id || projectionReceipt?.lease_ref || canonicalReceipt?.lease_ref || ''),
      run_id:String(row?.run_id || ''),
      settled_at:String(row?.settled_at || projectionReceipt?.settled_at || canonicalReceipt?.settled_at || ''),
    }),
  });
}
...
export function createGitHubProjectGraphRuntime(options = {}) {
  ...
  const projectTransitionLeases = options.projectTransitionLeases || createProjectTransitionLeasePostgresStore(dbBinding);
  const recoverySeedReader = options.readProjectTransitionRecoverySeedWithGitHubApp || readProjectTransitionRecoverySeedWithGitHubApp;
  const now = options.now || (() => new Date().toISOString());
  if (typeof withApp !== 'function' || typeof definitionFactsReader !== 'function' || typeof branchRoleResolver !== 'function'
      || typeof recoverySeedReader !== 'function'
      || !projectTransitionLeases || typeof projectTransitionLeases.getActiveLeasesForProject !== 'function'
      || typeof now !== 'function' || !dbBinding || typeof dbBinding.query !== 'function') {
  ...
  return Object.freeze({
    ...
    async readProjectExecutionAuthorities({ project_ref, repository:repositoryInput, revision:revisionInput, derivation:derivationInput } = {}) {
      const { projectRef, repository } = repositoryFromProjectRef(project_ref);
      ...
      if (repository !== observedRepository || !SHA40.test(revision) || !derivation) {
        fail('PROJECT_GRAPH_GITHUB_AUTHORITY_INVALID', 'exact project execution authority coordinate is required', {
          project_ref:projectRef,
      ...
      return Object.freeze(await projectTransitionLeases.getActiveLeasesForProject(projectRef, String(now())));
    },
    async readProjectObservations({ project_ref, repository:repositoryInput, revision:revisionInput, derivation:derivationInput } = {}) {
      ...
      const recoveryObservations = await recoverySeedReader(
        { project_ref:projectRef, repository, revision },
        { withGitHubAppApiClient:withApp },
      );
      const result = await dbBinding.query(
        `SELECT lease_ref, run_id, settled_at, NULL::jsonb AS settle_receipt,
                settlement_receipt
           FROM execution_state
          WHERE subject_kind='project_transition'
            AND lifecycle='settled'
            AND settled=true
            AND settlement_receipt->>'project_transition_disposition'='completed'
            AND settlement_receipt->'project_transition'->>'project_ref'=$1
         UNION ALL
         SELECT l.lease_id::text AS lease_ref, l.run_id, l.settled_at,
                l.settle_receipt, NULL::jsonb AS settlement_receipt
           FROM work_leases l
          WHERE l.status='settled'
            AND l.claim_receipt->>'subject'='project_transition'
            AND l.settle_receipt->>'subject'='project_transition'
            AND l.settle_receipt->>'disposition'='completed'
            AND l.settle_receipt->'project_transition'->>'project_ref'=$1
            AND NOT EXISTS (
              SELECT 1
          ...
          ORDER BY settled_at ASC, lease_ref ASC`,
        [projectRef],
      );
      return mergeProjectTransitionObservations(
        recoveryObservations,
        (result?.rows || []).map(observationFromSettlement),
      );
    },
  });
}
