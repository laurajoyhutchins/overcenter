...
export function normalizeRepositoryBranchRoleBinding(input = {}) {
  ...
  const developmentBranch = safeBranch(input.development_branch ?? 'dev', 'development_branch');
...
export function createPostgresRepositoryBranchRoleStore(dbBinding) {
  return {
...
export function createPostgresRepositoryBranchRoleService(options = {}) {
  return createRepositoryBranchRoleService({ store: options.store || createPostgresRepositoryBranchRoleStore(options.db) });
}
