...
function normalizeDueOn(value) {
  if (typeof value !== 'string' || !value.trim()) fail('INVALID_REQUEST', 'due_on must be an RFC3339 timestamp', { field: 'due_on' }, 422);
  const timestamp = Date.parse(value);
  if (!Number.isFinite(timestamp)) fail('INVALID_REQUEST', 'due_on must be an RFC3339 timestamp', { field: 'due_on' }, 422);
...
function transportFailure(response, phase, path, mayHaveMutated = false) {
  ...
  if (status === 401 || status === 403) {
    return { ok: false, error: 'GITHUB_APP_PERMISSION_DENIED', message, upstream_status: status, required_permissions: { pull_requests: 'write', metadata: 'read' }, ...evidence };
  }
