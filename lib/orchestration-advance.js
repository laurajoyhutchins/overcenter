...
import { deriveProjectTransitionProofState } from './project-transition-proof-state.js';

const LEASE_AUTHORIZED_MUTATIONS = Object.freeze([
...
async function staleReconciliationKey(runId, projectRef, leaseRef, definition) {
  const digest = await sha256Text(canonicalJson({
    kind:'reconcile-stale-authority',
    run_id:runId,
    project_ref:projectRef,
    lease_ref:leaseRef,
    authority:definition,
  }));
  return `orchestration.advance:reconcile-stale-authority:${digest}`;
}
...
function publicTransition(transition) {
  return Object.freeze({
    ...
    unmet_requirements:Object.freeze([...(transition.unmet_requirements || [])]),
    state:transition.state || null,
    executor:Object.freeze({ ...(transition.executor || {}) }),
    execution_intent:transition.execution_intent || null,
    lifecycle:Object.freeze({ ...(transition.lifecycle || {}) }),
  });
}

async function predecessorContext(projectTransitions, input) {
  if (typeof projectTransitions?.contextFor !== 'function') return null;
  const context = await projectTransitions.contextFor(input);
  if (!context) return null;
  if (context.schema !== 'project-transition-context-v1'
      || context.project_ref !== input.project_ref
      || context.transition_id !== input.transition_id
      || context.authority?.revision !== input.authority?.revision) {
    fail('ORCHESTRATION_ADVANCE_CONTEXT_AUTHORITY_INVALID', 'cross-transition context is not bound to the selected project authority', {
      project_ref:input.project_ref,
      transition_id:input.transition_id,
      expected_revision:input.authority?.revision || null,
      actual_revision:context.authority?.revision || null,
    });
  }
  return context;
}

function requireAgentExecutionIntent(transition) {
  if (transition?.executor?.kind === 'agent' && !transition.execution_intent) {
    fail('ORCHESTRATION_ADVANCE_EXECUTION_INTENT_REQUIRED', 'agent transition cannot enter execution without canonical repository-owned execution intent', {
      transition_id:transition.id,
    });
  }
}
...
function exactTarget(evaluation) {
  if (evaluation?.horizon?.kind !== 'transition') return null;
  const target = evaluation.nodes.find((node) => node.id === evaluation.horizon.ref) || null;
  if (!target) {
    fail('ORCHESTRATION_ADVANCE_TARGET_INVALID', 'transition horizon evaluation lost its exact target', {
      transition_id:evaluation.horizon.ref,
    });
  }
  return target;
}
...
function isStaleAuthority(error) {
  return error?.code === 'PROJECT_TRANSITION_AUTHORITY_STALE';
}
...
export function createOrchestrationAdvanceService({ store, readProjectGraph, projectTransitions, executeOperator } = {}) {
  ...
  async function advance(input = {}) {
    ...
    const exact = exactTarget(evaluation);

    if (exact) {
      if (exact.state === 'DONE') {
        return Object.freeze({
          ...baseResult('PROJECT_COMPLETE', runId, projectRef, evaluation),
          transition:publicTransition(exact),
        });
      }
      if (exact.state === 'OFF_NOMINAL') {
        return Object.freeze({
          ...baseResult('OFF_NOMINAL', runId, projectRef, evaluation),
          transition:publicTransition(exact),
          off_nominal:Object.freeze([exact.id]),
        });
      }
      if (exact.state === 'WAITING') {
        return Object.freeze({
          ...baseResult('WAITING', runId, projectRef, evaluation),
          transition:publicTransition(exact),
          waiting_on:Object.freeze([...(exact.unmet_requirements || [])]),
        });
      }
    } else {
    ...
    if (typeof store.activeLeaseForRun === 'function') {
      const existingAuthority = await store.activeLeaseForRun(runId, new Date().toISOString());
      if (existingAuthority) {
        try {
          if (typeof projectTransitions.require === 'function') {
            const lease = await projectTransitions.require({
              lease_ref:existingAuthority.lease_id,
              run_id:runId,
              project_ref:projectRef,
            });
            const candidates = exact ? [exact] : evaluation.frontier;
            const transition = candidates.find((candidate) => candidate.id === lease.transition_id) || null;
            if (transition?.executor?.kind === 'agent') {
              requireAgentExecutionIntent(transition);
              return Object.freeze({
                ...baseResult('AGENT_EXECUTION_REQUIRED', runId, projectRef, evaluation),
                transition:publicTransition(transition),
                lease_ref:lease.lease_ref,
                lease_authorized_mutations:LEASE_AUTHORIZED_MUTATIONS,
                transition_definition_fingerprint:lease.transition_definition_fingerprint || null,
                authority:Object.freeze({ ...(lease.authority || authority(graph)) }),
                proof_state:await deriveProjectTransitionProofState({ transition, authority:Object.freeze({ ...(lease.authority || authority(graph)) }), occupancy:{ occupied:true, expires_at:existingAuthority.expires_at }, evidence:[] }),
                predecessor_context:await predecessorContext(projectTransitions, Object.freeze({
                  project_ref:projectRef,
                  transition_id:transition.id,
                  authority:Object.freeze({ ...(lease.authority || authority(graph)) }),
                })),
                expires_at:existingAuthority.expires_at || null,
              });
            }
          }
          ...
          if (typeof projectTransitions.reconcileStale !== 'function') throw error;
          const definition = authority(graph);
          await projectTransitions.reconcileStale({
            lease_ref:existingAuthority.lease_id,
            run_id:runId,
            idempotency_key:await staleReconciliationKey(runId, projectRef, existingAuthority.lease_id, definition),
            stale_error:Object.freeze({ code:error.code, details:error.details || null }),
          });
        }
      }
    }
    ...
    for (const transition of candidates) {
      requireAgentExecutionIntent(transition);
      let suspension = null;
      if (typeof projectTransitions.suspensionFor === 'function') {
        suspension = await projectTransitions.suspensionFor({ project_ref:projectRef, transition_id:transition.id });
      } else if (typeof projectTransitions.isSuspended === 'function' && await projectTransitions.isSuspended({ project_ref:projectRef, transition_id:transition.id })) {
        suspension = Object.freeze({ schema:'project-transition-suspension-v1', recovery_ref:null, project_ref:projectRef, transition_id:transition.id, reason:'blocked_settlement_promotion', conditions:null });
      }
      if (suspension) {
        if (exact) {
          return Object.freeze({
            ...baseResult('WAITING', runId, projectRef, evaluation),
            transition:publicTransition(exact),
            waiting_on:Object.freeze(['blocked_settlement_promotion']),
            suspension,
          });
        }
        continue;
      ...
      try {
        ...
        if (isOccupied(error)) {
          if (exact) {
            return Object.freeze({
              ...baseResult('TRANSITION_OCCUPIED', runId, projectRef, evaluation),
              transition:publicTransition(exact),
              occupied:true,
            });
          }
      ...
      if (transition.executor?.kind === 'agent') {
        return Object.freeze({
          ...
          proof_state:await deriveProjectTransitionProofState({ transition, authority:Object.freeze({ ...(lease.authority || definition) }), occupancy:null, evidence:[] }),
          predecessor_context:await predecessorContext(projectTransitions, Object.freeze({
            project_ref:projectRef,
            transition_id:transition.id,
            authority:Object.freeze({ ...(lease.authority || definition) }),
