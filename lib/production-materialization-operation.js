...
function postEffectFailure(code, message = code) {
    throw Object.assign(new Error(message), { code });
}
function postMutation(error) {
    if (error instanceof ProductionMaterializationFailure)
        return error;
...
function normalizeRepo(value) {
    ...
    if (!REPO.test(repo))
        reject('PRODUCTION_MATERIALIZATION_REPOSITORY_INVALID', 'repo must be in owner/name form');
    return repo;
}
function normalizeRevision(value, field) {
    const revision = String(value || '').trim().toLowerCase();
    if (!SHA40.test(revision))
        reject('PRODUCTION_MATERIALIZATION_REVISION_INVALID', `${field} must be an exact 40-character Git SHA`);
    return revision;
}
function normalizeVersion(value, field) {
    const version = Number(value);
    if (!Number.isSafeInteger(version) || version < 1)
        reject('PRODUCTION_MATERIALIZATION_RUNTIME_INVALID', `${field} must be a positive integer`);
    return version;
}
...
function runtimePath(pathInput) {
    const path = String(pathInput || '');
    if (path === SOURCE_MATERIALIZATION_RECEIPT_PATH)
        return false;
    if (!path || path.startsWith('/') || path.includes('\\') || path.split('/').includes('..'))
        return false;
    if (['hatchable.toml', 'package.json', 'seed.sql'].includes(path))
        return true;
    if (/^migrations\/[^/]+\.sql$/.test(path))
        return true;
...
async function desiredRecords(files, normalizeContent) {
    const seen = new Set();
    const records = [];
    for (const file of files) {
        const path = String(file?.path || '');
        if (!HATCHABLE_TRANSPORT_SOURCE_PATHS.has(path))
            continue;
        if (seen.has(path))
            reject('PRODUCTION_MATERIALIZATION_DUPLICATE_SOURCE_PATH', `duplicate source path: ${path}`);
        if (typeof file?.content !== 'string' || file.content.includes('\u0000'))
            reject('PRODUCTION_MATERIALIZATION_SOURCE_INVALID', `source must be UTF-8 text: ${path}`);
        seen.add(path);
        const content = normalizeContent ? await normalizeContent(file) : file.content;
        if (typeof content !== 'string' || content.includes('\u0000'))
            reject('PRODUCTION_MATERIALIZATION_SOURCE_INVALID', `normalized source must be UTF-8 text: ${path}`);
...
async function manifestHash(records) {
...
function runtimeSourceMap(files) {
    ...
    for (const file of files) {
        ...
        if (!runtimePath(path))
            continue;
        if (result.has(path))
            reject('PRODUCTION_MATERIALIZATION_RUNTIME_INVALID', `duplicate runtime path: ${path}`);
        result.set(path, file);
    }
    return result;
}
export async function productionRuntimeSourceManifest(files) {
...
function sourceMatches(files, records, requireSize) {
    const observed = runtimeSourceMap(files);
    if (observed.size !== records.length)
        return false;
    for (const record of records) {
        const file = observed.get(record.path);
        if (!file || String(file.hash || '').toLowerCase() !== record.hash)
            return false;
        if (requireSize && Number(file.size) !== record.size)
            return false;
    }
    return true;
}
function exactVerifiedRevision(observation, revision) {
...
export async function materializeProduction(intent, ports) {
    ...
    if (!String(runtime.runtime_ref || '').trim())
        reject('PRODUCTION_MATERIALIZATION_RUNTIME_INVALID', 'runtime_ref is required');
