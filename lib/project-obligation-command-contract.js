...
export function normalizeProjectAddObligationRequest(raw) {
  ...
  const executor = obligation.executor === undefined
    ? Object.freeze({ kind:'agent', role:'implementation' })
    : record(obligation.executor, 'obligation.executor');
  const acceptanceEvidence = obligation.acceptance_evidence === undefined ? [] : obligation.acceptance_evidence;
  if (!Array.isArray(acceptanceEvidence)) fail('obligation.acceptance_evidence must be an array', { field:'obligation.acceptance_evidence' });
