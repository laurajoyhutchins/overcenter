...
export function createInvocationObservationService({ store } = {}) {
  ...
  async function attach(input) {
    ...
    return Object.freeze({
      ...
      attachment:Object.freeze({ schema:'invocation-ref-v1', invocation_id:current.invocation_id, run_id:current.run_id, sequence:current.sequence }),
      observation:current,
    });
  }
