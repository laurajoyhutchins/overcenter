...
export async function runOrchestrationAdvanceSingleAuthorityTests() {
  ...
  await test('stale run authority is mechanically requeued before reacquiring the fresh frontier', async()=>{
    ...
    assert(reconciliations.length === 1, 'stale authority was not reconciled exactly once');
    assert(reconciliations[0].lease_ref === LEASE_REF && reconciliations[0].run_id === 'stale-authority-run', 'stale reconciliation lost run-owned lease identity');
    assert(acquisitions.length === 1 && acquisitions[0].transition_id === 'first-ready', 'fresh frontier authority was not reacquired after reconciliation');
    assert(result.outcome === 'AGENT_EXECUTION_REQUIRED' && result.lease_ref === freshLeaseRef, 'advance did not hand off the fresh authority');
  });
