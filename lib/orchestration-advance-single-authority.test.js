...
export async function runOrchestrationAdvanceSingleAuthorityTests() {
  ...
  await test('sequential advance does not mint a second authority while its run already owns one', async()=>{
    const acquisitions = [];
    const service = createOrchestrationAdvanceService({
      store:{
        ...
        async activeLeaseForRun(runId) {
          ...
          return { lease_id:'11111111-1111-4111-8111-111111111111', run_id:runId, status:'active' };
        },
      },
    ...
    assert(result.outcome === 'WAITING', `active run authority should stop boundedly, got ${result.outcome}`);
    assert(acquisitions.length === 0, `sequential advance attempted a second authority: ${acquisitions.join(',')}`);
    assert(result.frontier.join(',') === 'first-ready,second-ready', 'bounded wait lost authoritative frontier evidence');
  });
