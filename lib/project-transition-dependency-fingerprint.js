...
export async function projectTransitionDependencyFingerprint(definition) {
    const transitionId = requireText(definition.transition_id, 'transition_id');
    if (!Array.isArray(definition.requires))
        throw new TypeError('requires must be an array');
    ...
    if (unique.length !== requires.length)
        throw new TypeError('requires must not contain duplicates');
