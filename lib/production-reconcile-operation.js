...
function verifiedRuntime(raw, selectedRevision, { allowStale = false } = {}) {
  ...
  const versionValid = version == null || (Number.isSafeInteger(version) && version >= 1);
...
export async function reconcileProduction(rawIntent, ports = {}) {
  ...
  const developmentVerification = normalizeDevelopmentVerification(
    await verifyDevelopmentRevision(intent.repo, selectedRevision, roles),
    selectedRevision,
  );
  ...
  if (materialization?.deployment_version != null) positiveInteger(materialization.deployment_version, 'materialization.deployment_version');
  ...
  const finalRuntime = verifiedRuntime(finalState?.runtime, selectedRevision);
  const outcome = mayHaveMutated || materialization?.mutation_attempted === true
    ? 'converged'
    : 'already_converged';
  return success(outcome, intent, selectedRevision, developmentVerification, finalRuntime);
}