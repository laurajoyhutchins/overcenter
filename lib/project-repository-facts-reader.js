...
export async function readProjectRepositoryFactsWithGitHubApp(input = {}, options = {}) {
  const { repository, revision } = requireCoordinate(input.repository, input.revision);
  const withApp = options.withGitHubAppApiClient || withGitHubAppApiClient;
  if (typeof withApp !== 'function') {
    fail('PROJECT_REPOSITORY_FACTS_READER_UNAVAILABLE', 'GitHub App project facts transport is unavailable', { repository });
  }
  return withApp(
    repository,
    async (apiClient) => createProjectRepositoryFactsReader(apiClient)({ repository, revision }),
