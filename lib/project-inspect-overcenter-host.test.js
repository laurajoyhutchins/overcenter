...
async function testWorkerOwnsProviderGraphRuntimeBinding() {
  ...
  if (!source.includes("projectInspectFor({ readProjectGraph: createAuthoritativeProjectGraphReader(createGitHubProjectGraphRuntime({ db:runtime.db || hatchableDb })) })")) {
    throw new Error('project.inspect worker transport must own provider graph-runtime binding outside the semantic host');
  }
}
