...
export async function runGithubRepositoryTemplateTests() {
  ...
  results.push(await run('selected-repository installation rejects creation before token use or mutation', async () => {
    let withAppCalls = 0;
  ...
  results.push(await run('all-repositories installation retains verified native template generation', async () => {
    const client = queuedClient([
      { method: 'GET', path: templatePath, result: templateRepo() },
      { method: 'GET', path: destinationPath, result: response(404, { message: 'Not Found' }) },
