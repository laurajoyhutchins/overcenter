...
import {
  ...
  createGithubRepositoryFromTemplateWithGitHubApp,
...
// Native node:test owns per-case execution and failure reporting.
...
// Cases register directly with node:test.
  ...
  test('template ensure schema is exact and requires explicit desired state', async () => {
  ...
  test('template creation schema is narrow and defaults to default-branch generation only', async () => {
  ...
  test('repository template permissions are command-owned and narrow', async () => {
  ...
  test('already-template state is a verified no-op', async () => {
  ...
  test('template ensure enforces expected state and verifies the write', async () => {
  ...
  test('stale template expected state fails closed before mutation', async () => {
    const client = queuedClient([{ method: 'GET', path: templatePath, result: templateRepo({ is_template: true }) }]);
    const result = await ensureGithubRepositoryTemplate({
      repo: TEMPLATE,
  ...
  test('ambiguous template write reconciles without replaying mutation', async () => {
    const client = queuedClient([
      { method: 'GET', path: templatePath, result: templateRepo({ is_template: false }) },
  ...
  test('repository creation requires an authoritative template flag', async () => {
  ...
  test('exact existing destination is an idempotent verified no-op', async () => {
    const client = queuedClient([
  ...
  test('pre-existing nonmatching destination fails closed', async () => {
    const client = queuedClient([
      { method: 'GET', path: templatePath, result: templateRepo() },
  ...
  test('create sends the narrow template generation request and verifies exact destination identity', async () => {
    const client = queuedClient([
      { method: 'GET', path: templatePath, result: templateRepo() },
  ...
  test('ambiguous create reconciles by exact destination and never replays POST', async () => {
    const client = queuedClient([
      { method: 'GET', path: templatePath, result: templateRepo() },
      { method: 'GET', path: destinationPath, result: response(404, { message: 'Not Found' }) },
  ...
  test('selected-repository installation rejects creation before token use or mutation', async () => {
    let withAppCalls = 0;
    const result = await createGithubRepositoryFromTemplateWithGitHubApp({
      template_repo: TEMPLATE,
      destination_repo: DESTINATION,
      description: 'Generated for verification',
      private: true,
    }, {
      getGitHubAppInstallationScope: async () => ({
        repository: TEMPLATE,
        installation_id: 42,
      ...
      withGitHubAppApiClient: async () => {
        withAppCalls += 1;
        throw new Error('selected installation must fail before token use');
      },
    });
    assert(result.ok === false && result.error === 'GITHUB_REPOSITORY_TEMPLATE_INSTALLATION_SCOPE_UNSUPPORTED', 'selected installation did not fail with the bounded scope error');
    assert(result.phase === 'precondition' && result.may_have_mutated === false, 'selected installation failure did not prove no mutation');
    assert(result.observed_repository_selection === 'selected' && result.required_repository_selection === 'all', 'installation scope evidence is incomplete');
    assert(withAppCalls === 0, 'selected installation minted or used a repository token');
  });

  test('all-repositories installation retains verified native template generation', async () => {
    const client = queuedClient([
      { method: 'GET', path: templatePath, result: templateRepo() },
      { method: 'GET', path: destinationPath, result: response(404, { message: 'Not Found' }) },
      { method: 'POST', path: generatePath, result: response(201, { id: 2 }) },
      { method: 'GET', path: destinationPath, result: generatedRepo() },
    ]);
    ...
    assert(result.ok === true && result.outcome === 'created' && result.verified === true, result.message || 'all-repositories generation failed');
    assert(permissionProfile === 'repository_from_template', 'all-repositories generation used the wrong permission profile');
    assert(repositoryScope === 'installation', 'all-repositories generation stayed narrowed to the source repository token');
    assert(client.calls.filter((call) => call.method === 'POST' && call.path === generatePath).length === 1, 'native template generation POST was not executed exactly once');
  });

  test('command response and journal projections register template semantics without storing description text', async () => {
  ...
  // No legacy aggregate result collector.
