...
const GITHUB_APP_CAPABILITIES = Object.freeze({
  ...
  project_facts: Object.freeze({ permissions: Object.freeze({ metadata: "read", pull_requests: "read", checks: "read" }), fallback: Object.freeze({ class: "fail_closed", mechanism: null }) }),
  ...
  integration_merge: Object.freeze({ permissions: Object.freeze({ contents: "write" }), fallback: Object.freeze({ class: "fail_closed", mechanism: null }) }),
...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("project facts profile is the narrow read-only authority union", async () => {
    const profile = permissionProfile("project_facts");
    check(JSON.stringify(profile) === JSON.stringify({ metadata: "read", pull_requests: "read", checks: "read" }), "project facts scope is not the expected read-only authority profile");
    check(!Object.values(profile).includes("write"), "project facts profile requests write access");
    const fallback = githubAppFallbackPolicy("project_facts");
    check(fallback.class === "fail_closed" && fallback.mechanism === null, "project facts fallback must fail closed");
  });
  ...
  await test("branch policy, stack, and default-branch mutation permissions remain command-owned", async () => {
    ...
    check(JSON.stringify(permissionProfile("integration_merge")) === JSON.stringify({ contents: "write" }), "integration merge scope changed");
