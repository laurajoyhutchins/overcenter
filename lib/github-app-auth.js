...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("review packet identity profile stays minimal", async () => {
    check(JSON.stringify(permissionProfile("review_packet")) === JSON.stringify({ metadata: "read", pull_requests: "read" }), "review packet identity scope broadened");
  });
