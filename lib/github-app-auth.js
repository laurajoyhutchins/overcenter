...
const PERMISSION_PROFILES = Object.freeze({
  ...
  integration: Object.freeze({ contents: "write", pull_requests: "write" }),
...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("branch policy, stack, and default-branch mutation permissions remain command-owned", async () => {
    ...
    check(JSON.stringify(permissionProfile("integration")) === JSON.stringify({ contents: "write", pull_requests: "write" }), "integration scope changed");
