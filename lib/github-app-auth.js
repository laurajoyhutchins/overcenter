// Runtime secrets are injected by the composition root.
import { boundedSafeRead, githubTransportEvidence } from "lib/github-transport.js";

const API_VERSION = "2026-03-10";
const USER_AGENT = "Overcenter/1.0";

function requireSecretProvider(options = {}) {
  const secrets = options.secrets;
  if (!secrets || typeof secrets.get !== 'function') {
    throw Object.assign(new Error('GitHub App secret provider is required.'), {
...
async function githubAppCredentials(options = {}) {
  const secrets = requireSecretProvider(options);
  return {
    appId:await secrets.get('GITHUB_APP_ID'),
    privateKeyValue:await secrets.get('GITHUB_APP_PRIVATE_KEY'),
  };
}
...
async function githubRawRequest(path, { token, method = "GET", body, query, headers = {}, responseEncoding = "text" } = {}) {
  ...
  if (responseEncoding !== "text") throw new Error(`Unsupported GitHub response encoding: ${responseEncoding}`);
...
export async function getGitHubAppIdentity(options = {}) {
  const { appId, privateKeyValue } = await githubAppCredentials(options);
...
function installationTokenMintOptions(repo, profileName, repositoryScope, jwt) {
  return {
    token: jwt,
    method: "POST",
...
async function createInstallationToken(repo, profileName, options = {}) {
  ...
  async function mintToken(installationId) {
    return githubRequest(
      ...
      installationTokenMintOptions(repo, profileName, repositoryScope, jwt),
    );
  }
...
export async function createEncryptedGitHubInstallationLease(repoInput, profileName, encryptToken, options = {}) {
  const repo = validateRepo(repoInput);
  if (typeof encryptToken !== "function") throw new Error("encryptToken callback is required");
  const auth = await createInstallationToken(repo, profileName, options);
  ...
  const apiClient = {
    async call(name, callOptions = {}) {
      ...
      return rawRequest(callOptions.path, {
        ...
        responseEncoding: callOptions.responseEncoding || "text",
      });
    },
...
export async function withGitHubAppApiClient(repoInput, callback, options = {}) {
  ...
  return withInstallationTokenClient(auth, callback, options);
}

export function createGitHubAppAuth(options = {}) {
  const secrets = requireSecretProvider(options);
  const withApiClient = (repo, callback, callOptions = {}) => withGitHubAppApiClient(repo, callback, { ...callOptions, secrets });
  return Object.freeze({
    withApiClient,
    getIdentity:(callOptions = {}) => getGitHubAppIdentity({ ...callOptions, secrets }),
    getInstallationScope:(repo, callOptions = {}) => getGitHubAppInstallationScope(repo, { ...callOptions, secrets }),
    inspectCapabilities:(repo, callOptions = {}) => inspectGitHubAppCapabilities(repo, { ...callOptions, withGitHubAppApiClient:withApiClient }),
    createEncryptedInstallationLease:(repo, profileName, encryptToken, callOptions = {}) => createEncryptedGitHubInstallationLease(repo, profileName, encryptToken, { ...callOptions, secrets }),
  });
}
...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("stateless installation token mint is bounded-retry safe", async () => {
    const options = installationTokenMintOptions("owner/repo", "project_facts", "repository", "jwt");
    check(options.mayHaveMutated === false, "token mint lost effect-free classification");
    check(options.retrySafeRead === true, "token mint did not opt into bounded transient retry");
    check(options.headers?.["X-GitHub-Stateless-S2S-Token"] === "enabled", "token mint did not force stateless issuance");
    let calls = 0;
    const originalFetch = globalThis.fetch;
    globalThis.fetch = async (_url, request) => {
      calls += 1;
      check(request.headers["X-GitHub-Stateless-S2S-Token"] === "enabled", "stateless mint header was not sent");
      const status = calls < 3 ? 500 : 201;
      return {
        status,
        headers: new Headers({ "x-github-request-id": `MINT-${calls}` }),
        async text() { return JSON.stringify(status === 201 ? { token: "ghs_stateless", expires_at: "2099-01-01T00:00:00Z" } : { message: "temporary mint failure" }); },
      };
    };
    try {
      const response = await githubRequest("/app/installations/42/access_tokens", {
        ...options,
        sleep: async () => {},
        random: () => 0,
      });
      check(response.status === 201 && response.body?.token === "ghs_stateless", "stateless mint did not recover from transient failures");
      check(calls === 3, `stateless mint expected 3 attempts, observed ${calls}`);
    } finally {
