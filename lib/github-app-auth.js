...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("workflow changesets require the GitHub workflow permission", async () => {
    check(JSON.stringify(permissionProfile("workflow_changeset")) === JSON.stringify({ contents: "write", workflows: "write" }), "workflow changeset scope is missing");
  });
