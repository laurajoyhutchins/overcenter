...
const GITHUB_APP_CAPABILITIES = Object.freeze({
  ...
  archive_repository: Object.freeze({ permissions: Object.freeze({ administration: "write" }), fallback: Object.freeze({ class: "fail_closed", mechanism: null }) }),
...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("review permissions cannot leak into changeset profile", async () => {
    ...
    check(!("pull_requests" in changeset) && !("checks" in changeset) && !("statuses" in changeset) && !("administration" in changeset), "review permission leaked into changeset profile");
  });
