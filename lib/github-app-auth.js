let hostConfigPromise = null;

async function getHostConfigValue(name) {
  if (!hostConfigPromise) {
    hostConfigPromise = import("hatchable").then(({ config }) => config);
  }
...
export async function getGitHubAppIdentity() {
  const appId = await getHostConfigValue("GITHUB_APP_ID");
  const privateKeyValue = await getHostConfigValue("GITHUB_APP_PRIVATE_KEY");
