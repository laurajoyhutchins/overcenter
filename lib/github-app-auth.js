...
export async function runGitHubAppAuthRegressionTests() {
  ...
  await test("branch deletion permission profile remains contents write only", async () => {
    check(JSON.stringify(permissionProfile("delete_branch")) === JSON.stringify({ contents: "write" }), "branch deletion scope changed");
  });
