...
async function readRepositoryVisibility(apiClient, repo, options, phase, allowMissing = false) {
  const read = await safeCall(apiClient, `/repos/${repo}`, { method: 'GET' }, options, phase);
  if (!read.ok) { if (allowMissing && read.error === 'GITHUB_NOT_FOUND') return { ok: true, public: false }; return read; }
  const body = read.response?.body || {}; return { ok: true, public: body.private === false && body.visibility !== 'private' };
}
async function enforcePublicMetadataPolicy(apiClient, normalized, options = {}) {
  const target = await readRepositoryVisibility(apiClient, normalized.repo, options, 'public_metadata_target_visibility');
  if (!target.ok || target.public === false) return target.ok ? { ok: true, applied: false } : target;
  const fields = [['title', normalized.title], ['body', normalized.body]], findings = [];
  for (const [field,text] of fields) for (const {rule} of detectMetadataTextViolations(text)) findings.push({field,rule});
  const [owner] = normalized.repo.split('/'); const coordinates = new Set(fields.flatMap(([,text])=>extractOwnerRepositoryCoordinates(text, owner)));
  for (const coordinate of [...coordinates].sort()) { if (coordinate.toLowerCase()===normalized.repo.toLowerCase()) continue; const observed=await readRepositoryVisibility(apiClient,coordinate,options,'public_metadata_reference_visibility',true); if(!observed.ok)return observed; if(!observed.public)findings.push({field:'metadata',rule:'non_public_repository_coordinate'}); }
  findings.sort((a,b)=>a.field.localeCompare(b.field)||a.rule.localeCompare(b.rule));
  if(findings.length)return fail('PUBLIC_METADATA_POLICY_VIOLATION','Pull-request metadata violates the public repository policy.',{phase:'public_metadata_preflight',may_have_mutated:false,findings});
...
export async function createGithubPullRequest(input, options = {}) {
  ...
  const metadataPolicy = await enforcePublicMetadataPolicy(apiClient, normalized, options);
  if (!metadataPolicy.ok) return metadataPolicy;
