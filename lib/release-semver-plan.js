...
export const RELEASE_SEMVER_PLAN_SCHEMA = 'release-semver-plan-v1';
const IMPACT_RANK = Object.freeze({ none: 0, patch: 1, minor: 2, major: 3 });
...
function text(value, field) {
    ...
    if (!normalized)
        fail('RELEASE_SEMVER_PLAN_INVALID', `${field} must be a non-empty string`, { field });
    return normalized;
}
...
function normalizeAuthority(raw, field) {
    const input = record(raw, field);
    const kind = text(input.kind, `${field}.kind`).toLowerCase();
    if (kind !== 'github')
        fail('RELEASE_SEMVER_PLAN_INVALID', `${field}.kind must be github`, { kind });
...
function normalizeStringArray(raw, field) {
    if (!Array.isArray(raw))
        fail('RELEASE_SEMVER_PLAN_INVALID', `${field} must be an array`, { field });
...
function stableVersion(value, field) {
...
export function bumpStableSemver(baseVersion, impact) {
    const base = stableVersion(baseVersion, 'base_version');
    if (!(impact in IMPACT_RANK))
        fail('RELEASE_SEMVER_PLAN_INVALID', 'impact level is unsupported', { impact });
    if (impact === 'none')
        return base.raw;
    if (impact === 'patch')
    ...
    if (base.major === 0)
        return `0.${base.minor + 1}.0`;
    return `${base.major + 1}.0.0`;
}
function aggregateImpact(impacts) {
    let selected = 'none';
    for (const impact of impacts) {
        if (IMPACT_RANK[impact.level] > IMPACT_RANK[selected])
...
export async function deriveReleaseSemverPlan(raw) {
    ...
    if (authorityKey(horizonAuthority) !== authorityKey(authority))
        fail('RELEASE_SEMVER_AUTHORITY_STALE', 'release horizon authority does not match the exact project authority', { expected: authority, actual: horizonAuthority });
    ...
    if (scopeNodeIds.length === 0)
        fail('RELEASE_SEMVER_PLAN_INVALID', 'release horizon scope must not be empty');
    ...
    if (targetsOutsideScope.length)
        fail('RELEASE_SEMVER_PLAN_INVALID', 'release horizon targets must be included in dependency-closed scope', { transition_ids: targetsOutsideScope });
    ...
    if (!Array.isArray(input.transitions))
        fail('RELEASE_SEMVER_PLAN_INVALID', 'transitions must be an array');
    const transitions = input.transitions.map((rawTransition, index) => {
        ...
        const versionImpact = normalizeProjectVersionImpact(transition.version_impact, id, (_code, message, details) => fail('RELEASE_SEMVER_PLAN_INVALID', message, details));
        return Object.freeze({ id, version_impact: versionImpact });
    });
    const byId = new Map();
    for (const transition of transitions) {
        if (byId.has(transition.id))
            fail('RELEASE_SEMVER_PLAN_INVALID', 'transition ids must be unique', { transition_id: transition.id });
        byId.set(transition.id, transition);
    }
    const missingDefinitions = scopeNodeIds.filter((id) => !byId.has(id));
    if (missingDefinitions.length)
        fail('RELEASE_SEMVER_PLAN_INVALID', 'release horizon scope is missing exact-revision transition definitions', { transition_ids: missingDefinitions });
    ...
    if (missingImpact.length)
        fail('RELEASE_SEMVER_IMPACT_REQUIRED', 'every newly included release transition must declare version_impact', { transition_ids: missingImpact });
    ...
    const horizon = Object.freeze({ kind: 'release', ref: horizonRef, target_node_ids: targetNodeIds, scope_node_ids: scopeNodeIds });
