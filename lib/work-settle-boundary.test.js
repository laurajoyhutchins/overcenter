...
export async function runWorkSettleBoundaryTests() {
  ...
  results.push(await run('primary work.settle transport dispatches by durable lease subject', async () => {
    const source = await readFile(new URL('../mcp/work.settle.js', import.meta.url), 'utf8');
    check(source.includes('createPostgresSubjectAwareLeaseSettlementService'), 'primary work.settle does not use subject-aware settlement');
    check(!source.includes('createPostgresWorkLeaseService'), 'primary work.settle still binds directly to the legacy work lease service');
  }));
