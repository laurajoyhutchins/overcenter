...
export async function runWorkSettleBoundaryTests() {
  ...
  results.push(await run('work.settle discovery encodes disposition-dependent runtime requirements', async () => {
    const rules = WORK_SETTLE_INPUT_SCHEMA?.allOf;
    check(Array.isArray(rules) && rules.length >= 6, 'work.settle discovery does not expose conditional settlement rules');
    ...
    for (const requiredFragment of [
      ...
      check(serialized.includes(requiredFragment), `work.settle discovery is missing conditional rule evidence for ${requiredFragment}`);
    }
    const descriptions = JSON.stringify(WORK_SETTLE_INPUT_SCHEMA?.properties || {});
    check(descriptions.includes('effective continuation'), 'work.settle discovery does not describe persisted effective-continuation requirements');
  }));
