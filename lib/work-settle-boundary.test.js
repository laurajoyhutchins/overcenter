import workSettleMcp from 'mcp/work.settle.js';
...
export async function runWorkSettleBoundaryTests() {
  ...
  results.push(await run('MCP and semantic worker transport expose the same non-secret lease_ref selector', async () => {
    const required = new Set(workSettleMcp.inputSchema?.required || []);
    const properties = workSettleMcp.inputSchema?.properties || {};
    check(required.has('lease_ref'), 'MCP work.settle does not require lease_ref');
    check(!required.has('lease_token'), 'MCP work.settle still requires lease_token');
    check(Boolean(properties.lease_ref), 'MCP work.settle does not expose lease_ref');
    check(!properties.lease_token, 'MCP work.settle still exposes lease_token');
