...
const feedbackResponsibilities = Object.freeze({
  ENABLE: { applicable: true, satisfied: true },
  ACQUIRE: { applicable: true, satisfied: true },
...
export async function runWorkSettleBoundaryTests() {
  ...
  results.push(await run('shared work.settle contract exposes non-secret settlement semantics and no caller-selected successor', async () => {
    ...
    check(!properties.next_state && !properties.next_lane, 'shared work.settle contract advertises caller-selected successor state or lane');
  ...
  results.push(await run('shared boundary canonicalizes lease_ref and invokes settleByRef without capability material', async () => {
    let settled = null;
    const response = await executeWorkSettleBoundary(
      { lease_ref: LEASE_REF, disposition: 'requeue', requeue_class: 'retry_runtime_failure' },
      {
        db: boundaryDb(),
        service: { async settleByRef(request) { settled = request; return { ok: true, lease_ref: request.lease_ref }; } },
        executeCommand: async (_command, input, handler) => ({ status: 200, body: await handler(input) }),
      },
    );
    check(response.status === 200 && response.body?.ok === true, 'shared settlement boundary did not return the service response');
    check(settled?.lease_ref === LEASE_REF, 'shared settlement boundary changed lease_ref');
    check(!('lease_token' in settled), 'shared settlement boundary exposed lease capability material');
  }));

  results.push(await run('semantic lifecycle_facts can deterministically route failed CONFIRM back to EXECUTE', async () => {
    const semantic = validateSemanticWorkerCommand('work.settle', {
      lease_ref: LEASE_REF,
      disposition: 'completed',
      lifecycle_facts: { condition: 'NOMINAL', responsibilities: feedbackResponsibilities },
    });
    const resolution = resolveCompletedStage({ current_stage: 'CONFIRM', lifecycle_facts: semantic.lifecycle_facts });
    check(resolution.next_stage === 'EXECUTE', 'CONFIRM feedback did not resolve to EXECUTE');
    check(resolution.transition_kind === 'feedback', 'CONFIRM feedback was not classified as feedback');
    check(resolution.complete === false, 'CONFIRM feedback incorrectly completed the work');
  }));
