...
// Tests execute directly through native node:test discovery.
  ...
  test('shared work.settle contract and semantic worker transport expose one semantic shape', async () => {
  ...
  test('work.settle discovery encodes disposition-dependent request rules', async () => {
    const allOf = WORK_SETTLE_INPUT_SCHEMA?.allOf;
    check(Array.isArray(allOf) && allOf.length >= 6, 'shared work.settle contract is missing disposition-dependent conditional rules');
    const encoded = JSON.stringify(allOf);
    for (const expected of ['blocked', 'completed', 'requeue', 'wait_for_observable_change', 'reason', 'promotion_condition', 'requeue_class', 'lifecycle_facts', 'operating_condition']) {
      check(encoded.includes(expected), `shared work.settle conditional rules do not mention ${expected}`);
    }
    const continuationDescription = String(WORK_SETTLE_INPUT_SCHEMA?.properties?.continuation?.description || '');
    check(continuationDescription.includes('effective continuation'), 'work.settle discovery does not explain effective continuation requirements');
    const operatingConditionDescription = String(WORK_SETTLE_INPUT_SCHEMA?.properties?.operating_condition?.description || '');
    check(operatingConditionDescription.includes('HOLD') && operatingConditionDescription.includes('NOMINAL'), 'work.settle discovery does not explain operating_condition defaults');
  });

  test('subject-aware settlement sends graph-native leases to project transition authority', async () => {
    let legacyCalls = 0;
  ...
  test('semantic worker work.settle uses subject-aware settlement runtime', async () => {
    const calls = [];
  ...
  test('lease_ref settlement canonicalization derives stable retry identity without capability material', async () => {
    const db = boundaryDb();
  ...
  test('completed Enable lifecycle facts skip non-applicable Acquire and select Execute without caller routing', async () => {
