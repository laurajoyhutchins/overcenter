...
import { githubTextContentMaxBytes, selectGithubTextTransport } from 'lib/github-text-transport.js';
...
const MAX_CONTENT_BYTES = githubTextContentMaxBytes;
const MAX_PAYLOAD_BYTES = githubTextContentMaxBytes;
...
function validateManifest(raw, expectedId) {
  ...
  if (Number.isInteger(observedContentBytes) && observedContentBytes > MAX_CONTENT_BYTES) {
    fail('CONTENT_REF_CONTENT_TOO_LARGE', 'content reference exceeds the bounded UTF-8 content limit', { content_bytes:observedContentBytes });
  }
...
async function gunzipBounded(bytes) {
  ...
  try {
    ...
    while (true) {
      ...
      total += chunk.length;
      if (total > MAX_CONTENT_BYTES) fail('CONTENT_REF_CONTENT_TOO_LARGE', 'content reference expands beyond the bounded UTF-8 content limit', { content_bytes:total });
      chunks.push(chunk);
    }
...
export async function resolveGithubContentReference(contentRef, options = {}) {
  ...
  if (manifest.contentBytes > MAX_CONTENT_BYTES) fail('CONTENT_REF_CONTENT_TOO_LARGE', 'content reference exceeds the bounded UTF-8 content limit');
  ...
  if (canonicalBytes.length > MAX_CONTENT_BYTES) fail('CONTENT_REF_CONTENT_TOO_LARGE', 'content reference expands beyond the bounded UTF-8 content limit', { content_bytes:canonicalBytes.length });
