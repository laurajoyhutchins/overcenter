...
export function productionReconciliationFor(options = {}) {
  ...
  async function observeRuntime(_repo, revision, _roles) {
    if (freshRuntimeEvidence?.revision === revision) return freshRuntimeEvidence;
  ...
  async function pollTrackedRun(repo, runId, revision, roles, mutationAttempted) {
    for (let attempt = 0; attempt < pollAttempts; attempt += 1) {
      ...
      if (state === 'completed') {
        ...
        freshRuntimeEvidence = Object.freeze({
          revision,
          verified:true,
        ...
        return {
          state:'succeeded',
          revision,
          verification_ref:freshRuntimeEvidence.verification_ref,
          deployment_version:null,
  ...
  async function dispatchObservation(repo, revision, roles) {
    ...
    try {
      const response = await github(value, 'production_materialization_dispatch', client => client.call('github', {
        ...
        method:'POST',
        body:{ ref:roles.production, inputs:{ exact_revision:revision, observe_only:true } },
      }), withApp);
      const body = bodyOf(response);
      const runId = Number(body?.workflow_run_id || body?.workflow_run?.id || body?.id || 0);
      if (!Number.isSafeInteger(runId) || runId < 1) {
        fail(
          'PRODUCTION_RECONCILIATION_MATERIALIZATION_DISPATCH_INDETERMINATE',
          'GitHub accepted the runtime observation dispatch without returning a trackable workflow run id',
          null,
          true,
        );
      }
      return pollTrackedRun(value, runId, revision, roles, false);
    } catch (error) {
