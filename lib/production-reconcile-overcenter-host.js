...
async function github(repo, capability, callback, withApp = withGitHubAppApiClient) {
  return withApp(repo, callback, { permissionProfile:capability });
}
...
export function productionReconciliationFor(options = {}) {
  ...
  const promotion = options.productionPromotion || productionPromotionFor({ db });
  ...
  async function successfulMaterialization(repo, revision, roles) {
    const body = await listWorkflowRuns(repo, MATERIALIZATION_WORKFLOW, roles.production);
    const run = exactRuns(body, revision).find(candidate => String(candidate?.status || '') === 'completed'
      && String(candidate?.conclusion || '') === 'success');
    return run || null;
  }

  async function observeRuntime(repo, revision, roles) {
    const run = await successfulMaterialization(repo, revision, roles);
    if (!run) return { revision:null, verified:false, verification_ref:null, deployment_version:null };
    return { revision, verified:true, verification_ref:`github-actions-run:${run.id}`, deployment_version:null };
  }
  ...
  async function reconcileRuntime(repo, revision, roles) {
    ...
    const success = runs.find(run => String(run?.status || '') === 'completed' && String(run?.conclusion || '') === 'success');
    if (success) return { state:'succeeded', revision, verification_ref:`github-actions-run:${success.id}`, deployment_version:null, mutation_attempted:false };
  ...
  const ports = {
    ...
    observeRuntime,
    promote:(input) => promotion.promote(input),
    reconcileRuntime,
    verifyFinalState,
  };
