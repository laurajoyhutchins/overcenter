...
export function createGithubProductionPromotionReceiptStore(dbBinding) {
  return {
    ...
    async succeed(normalized, attemptToken, receipt) {
      const settled = await dbBinding.query(
        `UPDATE github_production_promotion_receipts
          ...
          WHERE repo=$1 AND idempotency_key=$2 AND attempt_token=$3::uuid AND state='processing'
          RETURNING repo,idempotency_key,state`,
        [normalized.repo, normalized.idempotency_key, attemptToken, receipt.new_production_head, canonicalJson(receipt)],
      );
      if (!settled.rows?.[0]) {
        throw Object.assign(new Error('production promotion receipt settlement lost attempt-token authority'), {
