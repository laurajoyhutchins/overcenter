...
export async function runOvercenterMetricsContractTests() {
  ...
  results.push(await run('execution origin and recovery correlation reuse durable command invocation truth', async () => {
    assert(OVERCENTER_METRICS_CONTRACT.dimensions.execution_origin.values.join(',') === 'system,scheduler,agent,operator,recovery,unknown');
    const correlation = OVERCENTER_METRICS_CONTRACT.durable_correlation_facts;
    assert(correlation.source === 'command_invocations.request_projection.metrics_correlation');
    assert(correlation.fields.includes('reasoning_boundary_id') && correlation.fields.includes('recovery_decision_id') && correlation.fields.includes('recovery_attempt_id'));
    assert(correlation.historical_missing === 'unknown');
  }));
