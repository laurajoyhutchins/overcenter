...
function metricWindow(snapshot, start, end, exemplarLimit) {
  ...
  const boundaryRuns = runs.filter((row) => row.agent_execution_boundary === true || row.transition_id || invocations.some((inv) => inv.run_id === row.run_id && inv.command === 'project.advance'));
  ...
  for (const row of [...runs, ...invocations]) {
    ...
    const key = row.run_id || `${row.command || 'unknown'}:${row.started_at || ''}`;
