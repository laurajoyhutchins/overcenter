...
export function projectArtifactBindingFor(options = {}) {
  ...
  const readProjectGraph = options.readProjectGraph || createAuthoritativeProjectGraphReader(createGitHubProjectGraphRuntime({ db, withGitHubAppApiClient }));
  const readProviderArtifact = options.readProviderArtifact || (async ({ repository, kind, number }) => {
    if (typeof withGitHubAppApiClient !== 'function') fail('PROJECT_ARTIFACT_BINDING_RUNTIME_INVALID', 'GitHub App provider is required');
  ...
  const appendBindingObservation = async (binding) => {
    ...
    const sql = "INSERT INTO portfolio_observations (idempotency_key,source_system,entity_type,entity_key,fact_type,observed_at,source_revision,payload,payload_canonical,payload_sha256,ingestion_source) VALUES ($1,'overcenter','project.artifact.binding',$2,'bound',now(),$3,$4::jsonb,$5,$6,'project.artifact.bind') ON CONFLICT (idempotency_key) DO NOTHING";
    await db.query(sql, [idempotencyKey, `${binding.project_ref}#${binding.transition_id}`, binding.authority_revision, payloadCanonical, payloadCanonical, payloadSha]);
    ...
    return Object.freeze({ observation_ref:`portfolio_observation:${row.observation_id}` });
  };
