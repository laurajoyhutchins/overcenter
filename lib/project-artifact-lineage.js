...
export function bindProjectArtifact(input = {}) {
  ...
  if (relationship !== 'full_coverage_equivalence') fail('artifact binding relationship is unsupported', { relationship });
  const condition = requiredText(input.satisfaction?.condition, 'satisfaction.condition', 64).toLowerCase();
  const supported = provider.kind === 'issue' ? condition === 'closed' : condition === 'merged';
  if (!supported) fail('artifact satisfaction condition is unsupported for provider kind', { provider_kind: provider.kind, condition });
  return Object.freeze({ schema:'project-artifact-binding-v1', provenance:'explicit-binding', project_ref:input.project_ref, transition_id:transitionId, authority_revision:authorityRevision, provider, relationship, satisfaction:Object.freeze({ condition }) });
}

export function classifyProjectArtifactBinding(binding, context = {}) {
  if (!binding) return Object.freeze({ classification:'ambiguous', evidence:Object.freeze({ reason:'durable-binding-absent' }) });
  ...
  if (!observed || observed.repository !== binding.provider.repository || observed.kind !== binding.provider.kind || Number(observed.id) !== binding.provider.id) {
  ...
  const satisfied = (binding.provider.kind === 'issue' && binding.satisfaction.condition === 'closed' && state === 'closed') || (binding.provider.kind === 'pull_request' && binding.satisfaction.condition === 'merged' && state === 'merged');
  if (satisfied) return Object.freeze({ classification:'satisfied', evidence:Object.freeze({ repository:binding.provider.repository, provider_kind:binding.provider.kind, provider_id:binding.provider.id, provider_state:state, relationship:binding.relationship, condition:binding.satisfaction.condition }) });
  return Object.freeze({ classification:'active', evidence:Object.freeze({ repository:binding.provider.repository, provider_kind:binding.provider.kind, provider_id:binding.provider.id, provider_state:state, condition:binding.satisfaction.condition }) });
}