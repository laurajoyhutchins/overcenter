...
import { createPostgresRequiredCheckObservationStore, stabilizeRequiredCheckEvaluation } from 'lib/github-required-check-observation.js';
...
function optionalEvidenceUnavailable(surface, error, { required_permission = null } = {}) {
  ...
  if (code === 'GITHUB_PERMISSION_DENIED'
      || (code === 'GITHUB_APP_UPSTREAM_ERROR'
          && [401, 403, 422].includes(status)
          && /permission|not granted|resource not accessible/i.test(message))) {
    reason = 'permission_denied';
    if (code === 'GITHUB_APP_UPSTREAM_ERROR') stableError = 'GITHUB_APP_PERMISSION_DENIED';
...
export function createGithubReviewApiAdapter(apiClient) {
  ...
  async function getPullRequest(repo, pullNumber) {
    ...
    return {
      ...
      stack: body?.stack ? {
        number: Number(body.stack.number || 0) || null,
        size: Number(body.stack.size || 0) || null,
        position: Number(body.stack.position || 0) || null,
        base: body.stack?.base?.ref ? { ref: String(body.stack.base.ref) } : null,
      } : null,
      merge: {
...
// Repository rulesets are the sole branch-policy authority.
...
async function evaluatePolicy({ rules, review, checks, merge, requiredCheckObservation = null }) {
  const ruleReview = rules.pull_request_rules || [];
  const ruleApprovals = ruleReview.map(item => item.required_approvals || 0);
  const policyAvailable = rules.available;
  const policyComplete = policyAvailable && rules.complete;
  const knownRequiredApprovals = Math.max(0, ...ruleApprovals);
  const extraReviewRequirements = [];
  if (ruleReview.some(item => item.require_code_owner_review)) extraReviewRequirements.push('code_owner_review');
  if (ruleReview.some(item => item.require_last_push_approval)) extraReviewRequirements.push('last_push_approval');
  ...
  const reviewAvailable = review.available !== false;
  const requiredApprovals = policyComplete ? knownRequiredApprovals : null;
  const approvalSatisfied = !reviewAvailable
    ? null
    : (hasReviewRequirement
        ? (review.decision === 'APPROVED' ? true : false)
        : (policyComplete ? true : null));

  const requiredChecks = dedupeRequiredChecks(rules.required_checks);
  const checkEvaluation = evaluateRequiredChecks(
    checks,
    requiredChecks,
    policyComplete && checks.enumeration_complete,
  );
  const stabilizedCheckEvaluation = requiredCheckObservation
    ? await stabilizeRequiredCheckEvaluation(checkEvaluation, requiredCheckObservation)
    : checkEvaluation;

  const knownThreadRequirement = ruleReview.some(item => item.require_thread_resolution);
  const threadRequired = knownThreadRequirement ? true : (policyComplete ? false : null);
  const threadSatisfied = threadRequired === true
    ? (reviewAvailable && review.threads_complete ? review.unresolved_thread_count === 0 : null)
    : (threadRequired === false ? true : null);

  const knownBranchUpToDateRequirement = rules.strict_required_status_checks === true;
  const branchUpToDateRequired = knownBranchUpToDateRequirement ? true : (policyComplete ? false : null);
  ...
  if (stabilizedCheckEvaluation.required_satisfied === false) unsatisfied.push('required_status_checks');
  ...
  const configured = policyAvailable ? rules.configured : null;
  const requiredCheckObservationComplete = stabilizedCheckEvaluation.required_satisfied !== null
    || stabilizedCheckEvaluation.required_contexts.length === 0;
  const evaluationComplete = policyComplete && checks.enumeration_complete && reviewAvailable && review.threads_complete
    && requiredCheckObservationComplete
    && (branchUpToDateRequired !== true || branchUpToDateSatisfied !== null);
  let evaluation;
  if (unsatisfied.length) evaluation = 'unsatisfied';
  else if (!policyAvailable) evaluation = 'unavailable';
  else if (configured === false) evaluation = 'not_configured';
  else if (!evaluationComplete || rules.unassessed_rule_types.length) evaluation = 'unknown';
  else evaluation = 'satisfied';

  return {
    review: {
      available: reviewAvailable,
      ...(review.unavailable ? { unavailable: review.unavailable } : {}),
      decision: reviewAvailable ? review.decision : null,
      unresolved_thread_count: reviewAvailable ? review.unresolved_thread_count : null,
      ...(reviewAvailable && review.observed_unresolved_thread_count !== undefined
        ? { observed_unresolved_thread_count: review.observed_unresolved_thread_count } : {}),
      threads_complete: reviewAvailable && review.threads_complete,
      unresolved_threads: reviewAvailable ? review.unresolved_threads : [],
      unresolved_threads_complete: reviewAvailable && review.unresolved_threads_complete,
      changes_requested: reviewAvailable ? review.decision === 'CHANGES_REQUESTED' : null,
    ...
    checks: {
      ...checks,
      ...stabilizedCheckEvaluation,
    },
    protection: {
      available: policyAvailable,
      evaluation,
      source: rules.configured ? 'rulesets' : 'none',
      configured,
...
export async function reviewGithubPullRequest(input, options = {}) {
  try {
    ...
    const [reviewCollection, checkRunsCollection, statusesCollection, changedFilesCollection, rulesCollection] = await Promise.all([
      collectOptionalEvidence(
        'review',
        () => typeof options.reviewProvider === 'function'
      ...
      collectOptionalEvidence(
        'changed_files',
        () => typeof options.changedFilesProvider === 'function'
          ? options.changedFilesProvider(initial.changed_file_count)
    ...
    const reviewRaw = reviewCollection.available
      ? { available: true, ...reviewCollection.value }
      : {
          available: false,
          unavailable: reviewCollection.unavailable,
          decision: null,
    ...
    const changedFiles = changedFilesCollection.available
      ? { available: true, ...changedFilesCollection.value }
      : {
          available: false,
          unavailable: changedFilesCollection.unavailable,
          count: initial.changed_file_count,
          paths: [],
          complete: false,
          limit: MAX_CHANGED_PATHS,
        };
    ...
    const observedAt = options.now ? options.now() : new Date();
    ...
    const policy = await evaluatePolicy({
      ...
      merge: initial.merge,
      requiredCheckObservation: options.requiredCheckObservationStore ? {
        store: options.requiredCheckObservationStore,
...
export async function reviewGithubPullRequestWithGitHubApp(input, options = {}) {
  ...
  if (typeof withApp !== 'function') {
    return { ok:false, error:'RUNTIME_PROVIDER_MISSING', message:'GitHub App auth provider is required', details:{ provider:'githubAppAuth' } };
  }
  const requiredCheckObservationStore = options.requiredCheckObservationStore
    || (options.db ? createPostgresRequiredCheckObservationStore(options.db) : null);
  ...
  try {
    return await withApp(normalized.repo, async (apiClient) => {
      const reviewProvider = async () => {
        return withApp(normalized.repo, async (pullRequestClient) => {
          const adapter = createGithubReviewApiAdapter(pullRequestClient);
          return adapter.getReviewState(normalized.repo, normalized.pull_request);
        }, { permissionProfile: 'review_pull_requests' });
      };
      const changedFilesProvider = async (declaredCount) => {
        return withApp(normalized.repo, async (pullRequestClient) => {
          const adapter = createGithubReviewApiAdapter(pullRequestClient);
          return adapter.listChangedPaths(normalized.repo, normalized.pull_request, declaredCount);
        }, { permissionProfile: 'review_pull_requests' });
      };
      const checkRunsProvider = async (headSha) => {
        return withApp(normalized.repo, async (checksClient) => {
      ...
      const statusesProvider = async (headSha) => {
        return withApp(normalized.repo, async (statusesClient) => {
