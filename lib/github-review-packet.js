...
function normalizeChecks(checkRuns, statuses) {
  const items = [];
  for (const run of checkRuns.items) {
    items.push({
      kind: 'check_run',
  ...
  const rollup = failing.length ? 'FAILURE' : (pending.length ? 'PENDING' : (items.length ? 'SUCCESS' : 'NONE'));
  return {
    rollup_state: rollup,
    passing,
    pending,
    failing,
    items,
    enumeration_complete: Boolean(checkRuns.complete && statuses.complete),
  };
}
...
function evaluateRequiredChecks(checks, required, requiredSetComplete) {
  ...
  for (const requirement of required) {
    ...
    if (!candidates.length) { missing.push(label); continue; }
...
function evaluatePolicy({ rules, classic, review, checks, merge }) {
  ...
  const exactReviewPolicy = rules.complete && classic.available;
  ...
  const checkEvaluation = evaluateRequiredChecks(checks, requiredChecks, rules.complete && classic.available && checks.enumeration_complete);

  const threadRequired = ruleReview.some(item => item.require_thread_resolution) || classic.require_thread_resolution === true;
  const threadSatisfied = threadRequired
    ? (review.threads_complete ? review.unresolved_thread_count === 0 : null)
    : (exactReviewPolicy ? true : null);

  const branchUpToDateRequired = rules.strict_required_status_checks || classic.strict_required_status_checks === true;
  const branchUpToDateSatisfied = branchUpToDateRequired
  ...
  const configured = rules.configured || classic.configured === true;
  const policyComplete = rules.complete && classic.available && checks.enumeration_complete && review.threads_complete
    && (!branchUpToDateRequired || branchUpToDateSatisfied !== null);
  let evaluation;
  if (unsatisfied.length) evaluation = 'unsatisfied';
  else if (!classic.available) evaluation = 'unavailable';
  else if (!configured && rules.complete && classic.configured === false) evaluation = 'not_configured';
  ...
  return {
    ...
    protection: {
      ...
      configured: configured ? true : (rules.complete && classic.configured === false ? false : null),
...
export async function reviewGithubPullRequest(input, options = {}) {
  try {
    ...
    const [reviewRaw, checkRuns, statuses, changedFiles, rulesRaw] = await Promise.all([
    ...
    if (typeof options.protectionProvider === 'function') {
      classicRaw = await options.protectionProvider(initial.base.ref);
    } else if (typeof github.getClassicBranchProtection === 'function' && options.classicProtection === true) {
      classicRaw = await github.getClassicBranchProtection(normalized.repo, initial.base.ref);
    } else {
      classicRaw = {
        available: false,
        unavailable: {
          reason: 'github_app_permission_not_requested',
          required_permission: { administration: 'read' },
        },
      };
    }
    ...
    return await withGitHubAppApiClient(normalized.repo, async (apiClient) => {
      const protectionProvider = async (baseRef) => {
        try {
          ...
          const message = String(error?.message || 'Classic branch protection metadata unavailable.');
          return {
            available: false,
            unavailable: {
              reason: 'github_app_permission_unavailable',
              required_permission: { administration: 'read' },
              upstream_status: error?.status ? Number(error.status) : null,
              message,
            },
          };
        }
      };
      return reviewGithubPullRequest(normalized, { ...options, apiClient, protectionProvider });
