...
import { appendProjectGraphRevisionResumeEvidence, projectGraphRevisionResumeEvidence } from './project-graph-revision-resume-evidence.js';
...
function projectTransitionsFor(options, dbBinding, readProjectGraph) {
  if (options.projectTransitions && typeof options.projectTransitions.acquire === 'function' && typeof options.projectTransitions.settle === 'function') return options.projectTransitions;
  const store = options.projectTransitionStore || createProjectTransitionLeasePostgresStore(dbBinding);
  const operationStore = options.projectTransitionOperationStore || createCompactProviderOperationPostgresStore(dbBinding);
  const service = createPromotionAwareProjectTransitionLeaseService({ store, operationStore, readProjectGraph, now:options.now });
...
export function createPostgresProjectTransitionLeaseService(options = {}) {
  const dbBinding = options.db;
  const readProjectGraph = projectGraphReaderFor({ ...options, db:dbBinding });
  return projectTransitionsFor(options, dbBinding, readProjectGraph);
}
...
async function projectGraphRevisionEvidenceForRun(dbBinding, runId) {
  ...
  return projectGraphRevisionResumeEvidence(result?.rows?.[0]?.settle_receipt);
}

async function executeRegisteredProjectOperator(input, options = {}) {
  if (input.command !== 'orchestration.maintain') {
    return Object.freeze({ ok:false, error:'ORCHESTRATION_ADVANCE_OPERATOR_UNAVAILABLE', message:'declared deterministic project operator is not registered', command:input.command });
  }
  const response = await executeCorrelatedCommand(
    'orchestration.maintain',
    { run_id:input.run_id },
    () => createPostgresSubjectAwareOrchestrationMaintenanceService({ db:options.db }).maintain(),
    { statusForFailure:statusForOrchestrationRunError, defaultError:'ORCHESTRATION_MAINTENANCE_ERROR', defaultMessage:'orchestration.maintain failed', flattenDetails:true, db:options.db },
  );
...
export function createPostgresTargetAwareOrchestrationRunService(options = {}) {
  ...
  return createTargetAwareOrchestrationRunService({ store, createBaseService, projectGraphReader:projectGraphReaderFor(options) });
}
...
export function createPostgresOrchestrationAdvanceService(options = {}) {
  ...
  const executeOperator = typeof options.executeOperator === 'function' ? options.executeOperator : (input) => executeRegisteredProjectOperator(input, { ...options, db:dbBinding });
...
export function createPostgresOrchestrationDriveService(options = {}) {
  const advance = options.orchestrationAdvance && typeof options.orchestrationAdvance.advance === 'function' ? options.orchestrationAdvance : createPostgresOrchestrationAdvanceService(options);
  return createOrchestrationDriveService({ advance:(input) => advance.advance(input), max_advances:options.maxAdvances ?? 8 });
}
...
export async function orchestrationTargetResumePacket(input, options = {}) {
  ...
  const packet = appendProjectGraphRevisionResumeEvidence(basePacket, graphRevisionEvidence);
  ...
  try {
    const targetService = createTargetAwareOrchestrationRunService({ store, createBaseService:(facade) => createPostgresOrchestrationRunService({ ...options, db:dbBinding, store:facade }), projectGraphReader:projectGraphReaderFor(options) });
    ...
    return { ...packet, target, target_evaluation:null, target_evaluation_error:{ code:String(error?.code || 'TARGET_EVALUATION_FAILED'), message:String(error?.message || error) } };
  }
}
...
export function statusForOrchestrationAdvanceRuntimeError(error) { return statusForOrchestrationAdvanceError(error) || statusForTargetAwareOrchestrationError(error); }
export function statusForOrchestrationDriveRuntimeError(error) { return statusForOrchestrationDriveError(error) || statusForOrchestrationAdvanceRuntimeError(error); }
