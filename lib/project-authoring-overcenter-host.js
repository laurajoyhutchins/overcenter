...
function mappedRecoveryObservation(prior, observed, authority) {
  ...
  if (observed?.ok && ['merged','already_merged'].includes(outcome)) return Object.freeze({ outcome:'ready', integration:observed, already_merged:true });
  if (String(authority?.revision || '').toLowerCase() !== String(prior.expected_revision || '').toLowerCase()) return Object.freeze({ outcome:'base_moved', observed_authority_revision:authority?.revision || null });
  if (!observed?.ok) {
  ...
  if (observed?.expected_head && String(observed.expected_head).toLowerCase() !== String(prior.staged_revision).toLowerCase()) return Object.freeze({ outcome:'head_moved', observed_head:observed.expected_head });
  if (outcome === 'waiting') {
    const checks = observed?.evidence?.checks || {};
    if (checks.enumeration_complete === true && Array.isArray(checks.failing_required) && checks.failing_required.length > 0) return Object.freeze({ outcome:'failed_verification', waiting_on:checks.failing_required, integration:observed });
...
export function createProjectAuthoringRecoveryForRuntime(runtime = {}, overrides = {}) {
  ...
  const inspectIntegration = overrides.inspectIntegration || (async (prior) => reconcileGithubIntegrationRoleAware({ repo:String(prior.project_ref).slice('github:'.length), pull_request:prior.pull_request, expected_head:prior.staged_revision, apply:false }, providerOptions));
  const integrateCandidate = overrides.integrateCandidate || (async (prior, observed) => {
    ...
    const result = await mergeProjectAuthoringCandidate({ repository:String(prior.project_ref).slice('github:'.length), expected_head:prior.staged_revision }, { pull_request:prior.pull_request }, providerOptions);
  ...
  return createProjectAuthoringRecoveryService({
    operations,
    ...
    async readAuthoritativeProject(prior) {
      ...
      const authority = await graphRuntime.resolveProjectAuthority({ project_ref:prior.project_ref });
      const [stagedFacts, currentFacts] = await Promise.all([graphRuntime.readProjectFacts({ repository, revision:prior.staged_revision }), graphRuntime.readProjectFacts({ repository, revision:authority.revision })]);
      ...
      if (!stagedDefinition || !currentDefinition || JSON.stringify(stagedDefinition) !== JSON.stringify(currentDefinition)) return Object.freeze({ confirmed:false, authority_revision:authority.revision });
...
const legacyProjectAuthoringFor = createProjectAuthoringWorkerBinding({
  ...
  applyGithubChangeset(request, writerOptions, runtime) { return applyGithubChangesetRoleAware(request, { ...writerOptions, db:runtime.db, withGitHubAppApiClient:runtime.withGitHubAppApiClient }); },
