...
import { createPostgresProjectAuthoringRecoveryService } from './project-authoring-recovery.js';
...
const rawProjectAuthoringFor = createProjectAuthoringWorkerBinding({
...
export function projectAuthoringRecoveryFor(runtime = {}) {
  const raw = rawProjectAuthoringFor(runtime);
  return createPostgresProjectAuthoringRecoveryService({
    db:runtime.db || hatchableDb,
    executeAuthoring(command, input) {
      return command === 'project.define' ? raw.define(input) : raw.amend(input);
    },
  });
}

export function projectAuthoringFor(runtime = {}) {
  if (runtime.projectAuthoring && typeof runtime.projectAuthoring.define === 'function' && typeof runtime.projectAuthoring.amend === 'function') return runtime.projectAuthoring;
  const recovery = projectAuthoringRecoveryFor(runtime);
  return Object.freeze({
    define(input) { return recovery.execute('project.define', input); },
    amend(input) { return recovery.execute('project.amend', input); },
  });
}