...
function exactInput(input) {
  ...
  return Object.freeze({
    ...
    lease_ref: input.lease_ref == null ? null : text(input.lease_ref, 'lease_ref', 128),
  });
}
  ...
  async function confirm(rawInput = {}) {
    ...
    if (input.lease_ref) {
      if (typeof projectTransitions.require !== 'function') {
        fail('COMPATIBILITY_TRANSITION_LEASE_UNAVAILABLE', 'existing project-transition authority cannot be verified');
      }
      lease = await projectTransitions.require({
        lease_ref: input.lease_ref,
        run_id: input.run_id,
      ...
      if (typeof projectTransitions.acquire !== 'function') {
        fail('COMPATIBILITY_TRANSITION_LEASE_UNAVAILABLE', 'project-transition authority cannot be acquired');
      }
