...
function exactInput(input) {
  ...
  const unknown = Object.keys(input).filter((key) => !['run_id', 'work_ref'].includes(key)).sort();
  ...
  return Object.freeze({ run_id: text(input.run_id, 'run_id'), work_ref: text(input.work_ref, 'work_ref', 128) });
}
...
export function createCompatibilityTransitionConfirmationService({ bindings, compatibilityWork, readProjectGraph, projectTransitions } = {}) {
  ...
  if (!projectTransitions || typeof projectTransitions.acquire !== 'function' || typeof projectTransitions.settle !== 'function') throw new TypeError('projectTransitions acquire/settle are required');
  ...
  async function confirm(rawInput = {}) {
    ...
    const lease = await projectTransitions.acquire({
      ...
      lease_seconds: 600,
      idempotency_key: `compatibility-transition-acquire:${identity}`,
    });
    if (lease.transition_definition_fingerprint !== fingerprint) {
      fail('COMPATIBILITY_TRANSITION_AUTHORITY_CHANGED', 'transition definition changed during compatibility confirmation acquisition', { transition_id: binding.transition_id });
    }
