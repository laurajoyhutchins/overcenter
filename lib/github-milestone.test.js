...
export async function runGithubMilestoneTests() {
  ...
  results.push(await run('milestone permission is command-owned and narrow', async () => {
    ...
    assert(JSON.stringify(permissions) === JSON.stringify({ issues: 'write', metadata: 'read' }), 'milestone permission profile is not narrow');
    assert(!('issues' in githubAppPermissionProfile('changeset')), 'issues permission leaked into ordinary changesets');
  }));
  ...
  results.push(await run('permission failure is typed and non-mutating', async () => {
    ...
    assert(result.required_permissions?.issues === 'write', 'required issue permission is missing');
