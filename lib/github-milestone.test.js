...
import { commandFailure, commandSuccess } from './command-response.js';
import { githubAppPermissionProfile } from './github-app-auth.js';
...
// Legacy suite wrapper removed during node:test migration.

  test('request schema is exact and milestone title is stable identity', async () => {
    const normalized = normalizeGithubMilestoneRequest({
      repo: REPO,
      desired_state: { title: ' Provider Contract ', description: 'Contract layer', state: 'open' },
    ...
    assert(!Object.prototype.hasOwnProperty.call(normalized.desired_state, 'due_on'), 'omitted due date was materialized');
    let nullRejected = false;
    try { normalizeGithubMilestoneRequest({ repo: REPO, desired_state: { title: 'Provider Contract', due_on: null } }); } catch { nullRejected = true; }
    assert(nullRejected, 'nullable due date was accepted');
    let rejected = false;
  ...
  test('milestone permission is command-owned and narrow', async () => {
  ...
  test('already compliant milestone is a verified no-op', async () => {
    ...
    const result = await ensureGithubMilestone({ repo: REPO, desired_state: { title: 'Provider Contract', description: 'Contract layer', state: 'open' } }, { apiClient: client, sleep: async () => {} });
  ...
  test('missing milestone is created once and verified by number', async () => {
    const created = milestone({ description: 'Contract layer' });
    const client = queuedClient([
      { method: 'GET', path: `${BASE}?state=all&per_page=100&page=1`, result: response(200, []) },
      { method: 'POST', path: BASE, inspect(request) { assert(JSON.stringify(request.body) === JSON.stringify({ title: 'Provider Contract', description: 'Contract layer', state: 'open' }), 'create body changed'); }, result: response(201, created) },
  ...
  test('existing milestone patches only changed declared fields and verifies', async () => {
  ...
  test('stale expected milestone state fails closed before mutation', async () => {
  ...
  test('duplicate exact milestone titles fail closed as ambiguous', async () => {
  ...
  test('ambiguous create reconciles by re-observing exact desired milestone', async () => {
    const current = milestone({ description: 'Contract layer' });
    const client = queuedClient([
      { method: 'GET', path: `${BASE}?state=all&per_page=100&page=1`, result: response(200, []) },
  ...
  test('permission failure is typed and non-mutating', async () => {
  ...
  test('command response and journal projections preserve milestone semantics without descriptions', async () => {
  ...
  // Native node:test supplies execution and result reporting.
