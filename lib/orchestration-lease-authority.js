...
export function durableLeaseSubject(lease) {
  ...
  const canonicalSubject = typeof lease.subject_kind === 'string'
    ? lease.subject_kind.trim().toLowerCase()
    : '';
  if (canonicalSubject
      && !['project_transition', 'legacy_work', 'provider_operation'].includes(canonicalSubject)) {
    fail('ORCHESTRATION_LEASE_SUBJECT_INVALID', 'canonical execution subject kind is unsupported', {
      lease_ref:lease.lease_id || null,
      subject_kind:canonicalSubject,
    });
  }
  const projectedSubject = gateIsProjectTransition ? 'project_transition' : 'legacy_work';
  if (canonicalSubject && canonicalSubject !== projectedSubject) {
    fail('ORCHESTRATION_LEASE_SUBJECT_INVALID', 'canonical execution subject disagrees with lease projection', {
      lease_ref:lease.lease_id || null,
      canonical_subject:canonicalSubject,
      projected_subject:projectedSubject,
    });
  }
  return canonicalSubject || projectedSubject;
}
