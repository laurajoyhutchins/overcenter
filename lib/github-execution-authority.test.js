...
class MemoryReceipts {
  ...
  async succeed(normalized, attemptToken, receipt) {
    if (this.row?.attempt_token !== attemptToken) return false;
    this.row.receipt = receipt;
    this.row.state = 'succeeded';
    return true;
  }
...
// Tests register directly with node:test; helpers below retain domain-specific assertions.

  test('missing authority is rejected before GitHub mutation', async () => {
  ...
  test('unknown authority token is rejected', async () => {
    const { service } = await fixture();
  ...
  test('non-owning slot is rejected', async () => {
    const { service } = await fixture({ slot: { lease_id: '22222222-2222-4222-8222-222222222222' } });
    await expectCode(service.require({ lease_token: TOKEN, repository: REPO, allowed_gates: [GATE] }), 'EXECUTION_AUTHORITY_STALE');
  }));

  test('inactive orchestration run is rejected', async () => {
    const { service } = await fixture({ run: { status: 'finished' } });
    await expectCode(service.require({ lease_token: TOKEN, repository: REPO, allowed_gates: [GATE] }), 'EXECUTION_AUTHORITY_STALE');
  }));

  test('wrong repository is rejected', async () => {
    const { service } = await fixture();
  ...
  test('wrong lifecycle gate is rejected', async () => {
    const { service } = await fixture();
  ...
  test('authoritative work projection change invalidates authority', async () => {
    const { service } = await fixture({ current_issue: issue({ priority: 4 }) });
    await expectCode(service.require({ lease_token: TOKEN, repository: REPO, allowed_gates: [GATE] }), 'EXECUTION_AUTHORITY_STALE');
  }));

  test('valid authority permits mutation and returns non-secret evidence', async () => {
    const { service } = await fixture();
    const github = new FakeGithub();
  ...
  test('succeeded idempotent replay is non-mutating and does not re-require live authority', async () => {
    const { service } = await fixture();
    const github = new FakeGithub();
  ...
  test('lease token is excluded from changeset idempotency and orchestration semantic identity', async () => {
  ...
  test('durable changeset request excludes lease token', async () => {
    const calls = [];
    const db = {
  ...
  test('execution authority errors are expected precondition rejections', async () => {
