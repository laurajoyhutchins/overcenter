...
export function createPostgresOrchestrationRunStore(dbBinding = db) {
  ...
  return {
    ...
    async activeLeaseForRun(runId, observedAt) { return row("SELECT lease_id,work_ref,gate,status,expires_at FROM work_leases WHERE run_id=$1 AND status IN ('claiming','active','settling') AND expires_at > $2 ORDER BY created_at DESC LIMIT 1", [runId, observedAt]); },
  ...
  return {
    ...
    async reconcileInvocation(invocation) {
      ...
      if (invocation.command === 'orchestration.start' && invocation.run_id) {
        const run = (await dbBinding.query('SELECT run_id,status,deadline_at,predecessor_run_id FROM orchestration_runs WHERE run_id=$1', [invocation.run_id])).rows?.[0] || null;
        if (run) receipt = { run_id: run.run_id, status: run.status, deadline_at: run.deadline_at, predecessor_run_id: run.predecessor_run_id || null };
        else {
          await dbBinding.query("UPDATE orchestration_command_invocations SET outcome='failed', completed_at=now(), error_code='ORCHESTRATION_START_NOT_APPLIED', error_class='internal', retryable=true, rejection=false, may_have_mutated=false, result_projection=$2::jsonb WHERE invocation_id=$1 AND outcome='running'", [invocation.invocation_id, JSON.stringify({ run_id: invocation.run_id, run_record_present: false })]);
          return { reconciled: true, command: invocation.command, outcome: 'failed', reason: 'RUN_RECORD_ABSENT' };
        }
      }
      ...
      return { reconciled: true, command: invocation.command, outcome: 'succeeded' };
    },
  };
}
...
export function statusForOrchestrationRunError(error) {
  ...
  if (['RUN_NOT_ACTIVE','IDEMPOTENCY_CONFLICT','HORIZON_PRECONDITION_CHANGED','RUN_HAS_ACTIVE_LEASE'].includes(code)) return 409;
