...
export function createPostgresOrchestrationRunStore(dbBinding) {
  ...
  return {
    ...
    async leasesForRun(runId) { const result=await dbBinding.query('SELECT lease_id,work_ref,gate,status,created_at,settled_at,settle_plan,settle_receipt,reconciliation FROM work_leases WHERE run_id=$1 ORDER BY created_at ASC, lease_id ASC', [runId]); return result.rows || []; },
