...
import { semanticIdempotencyKey } from 'lib/operator-commands.js';
import { LIVE_LEASE_STATUSES as SEMANTIC_LIVE_LEASE_STATUSES, RUN_FINISH_DISPOSITIONS, RUN_MODES, WORK_SETTLEMENT_DISPOSITIONS } from 'lib/execution-lifecycle-contracts.js';
import { createPostgresRepositoryLifecycleService, canonicalRepository } from 'lib/repository-disposition.js';
import { normalizedStoredSkillPolicy, resolveWorkerSkillPolicy } from 'lib/skill-execution.js';
import { createPostgresSemanticJournalInvocationResolver } from 'lib/orchestration-semantic-journal-resolution.js';

const RUN_SCHEMA = 'orchestration-run-v1';
const HORIZON_SCHEMA = 'orchestration-horizon-v1';
const RUN_RECEIPT_SCHEMA = 'orchestration-run-receipt-v1';
...
const FINISH_DISPOSITIONS = new Set(RUN_FINISH_DISPOSITIONS);
const LIVE_LEASE_STATUSES = Object.freeze([...SEMANTIC_LIVE_LEASE_STATUSES]);
const LIVE_LEASE_STATUS_SQL = LIVE_LEASE_STATUSES.map((status) => `'${status}'`).join(',');
const JOURNALED_EXTERNAL_EFFECT_COMMANDS = new Set(['github.apply_changeset','github.delete_branch','github.required_checks.ensure','github.pages.ensure','github.integration.reconcile','github.repository_template.ensure','github.repository_from_template.create','linear.archive','portfolio.reconcile_work_surface']);
const JOURNALED_AUTHORITY_OBSERVATION_COMMANDS = new Set(['github.review_packet','github.capabilities']);
const ABANDONED_DISPOSITION = 'abandoned';
const ABANDONED_STOP_REASON = 'UNOBSERVABLE_SESSION_TERMINATION: orchestration run deadline elapsed without orchestration.finish; available control-plane telemetry contains no session-layer termination reason; no live lease remained; terminalized by orchestration maintenance.';
const WORKER_TRANSPORT_REVISION = 'worker-transport-v2';
const ORCHESTRATION_KERNEL_REVISION = 'orchestration-kernel-v1';
const ORCHESTRATION_RECOVERY_PROTOCOL = 'orchestration-recovery-v1';
...
function currentOrchestrationInterpreterProvenance() {
  return {
    worker_transport_revision: WORKER_TRANSPORT_REVISION,
    orchestration_kernel_revision: ORCHESTRATION_KERNEL_REVISION,
    orchestration_recovery_protocol: ORCHESTRATION_RECOVERY_PROTOCOL,
  };
}
function normalizeContractProvenance(value) {
  if (value == null) return { status: 'not_supplied', ...currentOrchestrationInterpreterProvenance() };
  ...
  return {
    ...
    ...currentOrchestrationInterpreterProvenance(),
  };
}
...
function boundedReceiptProjection(value, depth = 0) {
  ...
  if (Array.isArray(value)) return value.slice(0, 25).map((item)=>boundedReceiptProjection(item, depth + 1));
  ...
  for (const key of Object.keys(value).slice(0, 30)) {
    ...
    result[String(key).slice(0, 128)] = boundedReceiptProjection(value[key], depth + 1);
  }
  return result;
}
function journalEvidenceProjection(invocation) {
  return {
    sequence: invocation?.sequence == null ? null : Number(invocation.sequence),
    command: boundedReceiptText(invocation?.command, 128),
    target_kind: boundedReceiptText(invocation?.target_kind, 128),
    target_ref: boundedReceiptText(invocation?.target_ref, 512),
    outcome: boundedReceiptText(invocation?.outcome, 64),
    request: boundedReceiptProjection(invocation?.request_projection || {}),
    result: boundedReceiptProjection(invocation?.result_projection || {}),
  };
}
function boundedEvidenceRefs(plan) {
  const source = Array.isArray(plan?.evidence) ? plan.evidence : (Array.isArray(plan?.replay_request?.evidence) ? plan.replay_request.evidence : []);
  return source.slice(0, 25).map((item) => {
    if (!item || typeof item !== 'object' || Array.isArray(item)) return null;
    const kind = boundedReceiptText(item.kind, 128);
    const ref = boundedReceiptText(item.ref, 1000);
    return kind && ref ? { kind, ref } : null;
  }).filter(Boolean);
}
function settlementProjection(lease) {
  ...
  const canonicalReceipt = receipt?.canonical_receipt && typeof receipt.canonical_receipt === 'object' && !Array.isArray(receipt.canonical_receipt)
    ? receipt.canonical_receipt
    : (lease?.settlement_receipt && typeof lease.settlement_receipt === 'object' && !Array.isArray(lease.settlement_receipt) ? lease.settlement_receipt : null);
  const authorityAfter = receipt ? {
    state: boundedReceiptText(receipt.current_state, 128),
    lane: boundedReceiptText(receipt.current_lane, 128),
    revision: boundedReceiptText(receipt.settlement_authoritative_revision, 512),
    execution_fingerprint: boundedReceiptText(receipt.successor_execution_fingerprint, 128),
  } : null;
  return {
    lease_id: boundedReceiptText(lease?.lease_id, 128),
    work_ref: boundedReceiptText(lease?.work_ref, 128),
    gate: boundedReceiptText(lease?.gate, 128),
    lease_status: boundedReceiptText(lease?.status, 64),
    claimed_at: lease?.created_at || null,
    settled_at: lease?.settled_at || receipt?.settled_at || null,
    settlement_disposition: boundedReceiptText(receipt?.disposition, 64),
    evidence_refs: boundedEvidenceRefs(lease?.settle_plan),
...
function runScopeAllows(scope, issue) {
  return workLeaseInternals.scopeAllowsProjection(scope, workLeaseInternals.executionProjection(issue));
}
function normalizeActiveLeaseSettlement(value) {
  if (value == null) return null;
  const input = object(value, 'active_lease_settlement');
  const allowed = new Set(['disposition','evidence','reason','promotion_condition','requeue_class','continuation','next_state','next_lane']);
  const unknown = Object.keys(input).filter((key)=>!allowed.has(key));
  if (unknown.length) throw err('REQUEST_INVALID', 'active_lease_settlement contains unsupported fields', { fields: unknown.sort() });
  const disposition = requiredString(input.disposition, 'active_lease_settlement.disposition', 64);
  if (!WORK_SETTLEMENT_DISPOSITIONS.includes(disposition)) throw err('REQUEST_INVALID', 'active_lease_settlement.disposition is unsupported');
  return {
    disposition,
    evidence: Array.isArray(input.evidence) ? input.evidence : [],
    reason: input.reason ?? null,
    promotion_condition: input.promotion_condition ?? null,
    requeue_class: input.requeue_class ?? null,
    continuation: input.continuation ?? null,
    next_state: input.next_state ?? null,
    next_lane: input.next_lane ?? null,
  };
}
...
function normalizeFinish(input) {
  ...
  return {
    ...
    active_lease_settlement: normalizeActiveLeaseSettlement(input?.active_lease_settlement),
  };
}

function normalizeScope(value) {
  const input = object(value, 'scope');
  const legacyProject = optionalString(input.project, 'scope.project', 256);
  const team = optionalString(input.team, 'scope.team', 256);
  const projects = Array.isArray(input.projects) ? input.projects.map((x,i)=>requiredString(x, `scope.projects[${i}]`, 256)) : [];
  if (projects.length > 25) throw err('REQUEST_INVALID', 'scope.projects may contain at most 25 entries');
  ...
  const normalizedLanes = [...new Set(lanes)].sort();
  const normalizedRepositories = [...new Set(repositories)].sort();
  const direction = optionalString(input.direction, 'scope.direction', 1000);
  if (legacyProject) {
    if (team || projects.length) throw err('REQUEST_INVALID', 'legacy scope.project cannot be combined with scope.team or scope.projects');
    return { project: legacyProject, lanes: normalizedLanes, repositories: normalizedRepositories, direction };
  }
  if (!team) throw err('REQUEST_INVALID', 'scope.team is required when scope.project is omitted', { field: 'scope.team' });
  return { team, projects: [...new Set(projects)].sort(), lanes: normalizedLanes, repositories: normalizedRepositories, direction };
}

function normalizeStart(input) {
  const mode = requiredString(input?.mode, 'mode', 32).toLowerCase();
  if (!RUN_MODES.includes(mode)) throw err('REQUEST_INVALID', 'mode must be scheduled or interactive');
...
export function createOrchestrationRunService({ store, authoritative = null, leases = null, repositoryLifecycle = null, now = () => new Date().toISOString() } = {}) {
  if (!store) throw new TypeError('store is required');

  async function repositoryLifecycleForIssue(issue) {
    if (!repositoryLifecycle || typeof repositoryLifecycle.observe !== 'function') return null;
    const projection = workLeaseInternals.executionProjection(issue);
    const repository = canonicalRepository(projection.repository);
    return repositoryLifecycle.observe(repository);
  }

  async function assertRepositoryExecutable(issue) {
    const lifecycle = await repositoryLifecycleForIssue(issue);
    if (lifecycle?.ordinary_work_enabled === false) {
      throw err('REPOSITORY_DISPOSED', 'repository disposition prohibits ordinary horizon work', {
  ...
  async function resolveStoredHorizon(run, horizon) {
    ...
    for (const candidate of horizon.candidates || []) {
      try {
        ...
        let status = 'valid';
        if (lifecycle?.ordinary_work_enabled === false) status = 'no_longer_executable';
        else if (issue.state?.name !== candidate.expected_state || currentLane !== candidate.expected_lane) status = 'no_longer_executable';
        else if (evidence.execution_fingerprint !== candidate.execution_fingerprint) status = 'materially_changed';
        resolved.push({ ...candidate, repository: evidence.execution_projection.repository || null, status, current_execution_fingerprint: evidence.execution_fingerprint, authoritative_revision: issue.updatedAt || null, repository_disposition: lifecycle?.disposition || null, successor_repository: lifecycle?.successor_repository || null });
      } catch (error) {
  ...
  async function start(input) {
    ...
    const skillPolicy = resolveWorkerSkillPolicy(request.worker);
    const row = await store.insertRun({
      ...
      latest_horizon_id: null, stop_reason: null, finished_at: null, contract_provenance: request.contract_provenance, skill_policy: skillPolicy,
    });
  ...
  async function checkpointHorizon(input) {
    ...
    for (const candidate of normalizeHorizonCandidates(input?.candidates)) {
      ...
      const evidence = await fingerprintIssue(issue);
      candidates.push({ ...candidate, repository: evidence.execution_projection.repository || null, execution_projection: evidence.execution_projection, execution_fingerprint: evidence.execution_fingerprint, authoritative_revision: issue.updatedAt || null });
    }
  ...
  async function receipt(input) {
    ...
    const run = await store.getRun(runId);
    if (!run) throw err('RUN_NOT_FOUND', `orchestration run ${runId} was not found`);
    const leases = typeof store.leasesForRun === 'function' ? (await store.leasesForRun(runId)) || [] : [];
    const invocations = typeof store.invocationsForRun === 'function' ? (await store.invocationsForRun(runId)) || [] : [];
    const settlements = leases.slice(0, 50).map(settlementProjection);
    const unproven = [];
    if (run.status !== 'finished') unproven.push({ code:'RUN_NOT_FINISHED', ref:runId });
    for (const lease of leases.slice(0, 50)) {
      if (LIVE_LEASE_STATUSES.includes(lease.status)) unproven.push({ code:'LEASE_NOT_TERMINAL', ref:String(lease.lease_id || lease.work_ref || 'unknown') });
      if (lease.status === 'settled' && !lease.settle_receipt) unproven.push({ code:'SETTLEMENT_RECEIPT_MISSING', ref:String(lease.lease_id || lease.work_ref || 'unknown') });
    }
    const journal = { total: 0, succeeded: 0, rejected: 0, failed: 0, indeterminate: 0, running: 0, unresolved: 0 };
    const externalEffects = [];
    const authorityObservations = [];
    for (const invocation of invocations.slice(0, 250)) {
      journal.total += 1;
      if (Object.prototype.hasOwnProperty.call(journal, invocation.outcome)) journal[invocation.outcome] += 1;
      if (JOURNALED_EXTERNAL_EFFECT_COMMANDS.has(invocation.command)) externalEffects.push(journalEvidenceProjection(invocation));
      if (JOURNALED_AUTHORITY_OBSERVATION_COMMANDS.has(invocation.command)) authorityObservations.push(journalEvidenceProjection(invocation));
      const unresolved = ['running','indeterminate'].includes(invocation.outcome) && invocation.resolved !== true && invocation.command !== 'orchestration.finish';
      if (unresolved) {
        journal.unresolved += 1;
        unproven.push({ code:'UNRESOLVED_INVOCATION', ref:`${invocation.sequence || '?'}:${boundedReceiptText(invocation.command, 128) || 'unknown'}` });
      }
    }
    const counts = {
      leases: leases.length,
      settled: leases.filter((lease)=>lease.status === 'settled').length,
      completed: leases.filter((lease)=>lease.settle_receipt?.disposition === 'completed').length,
      requeued: leases.filter((lease)=>lease.settle_receipt?.disposition === 'requeue').length,
      blocked: leases.filter((lease)=>lease.settle_receipt?.disposition === 'blocked').length,
      terminal_without_settlement: leases.filter((lease)=>['expired','invalidated','rejected'].includes(lease.status)).length,
    };
    const body = {
      ...
      evidence_status: unproven.length === 0 ? 'complete' : 'incomplete',
      counts,
      settlements,
      external_effects: externalEffects.slice(0, 50),
      authority_observations: authorityObservations.slice(0, 50),
      journal,
      unproven: unproven.slice(0, 50),
    };
    return { ...body, receipt_sha256: await sha256Text(canonicalJson(body)) };
  }

  async function finish(input) {
    const request = normalizeFinish(input);
    const runId = request.run_id;
    const hashRequest = request.active_lease_settlement ? request : (() => { const { active_lease_settlement: _none, ...legacyShape } = request; return legacyShape; })();
    const requestSha = await sha256Text(canonicalJson(hashRequest));
    const run = await store.getRun(runId);
    if (!run) throw err('RUN_NOT_FOUND', `orchestration run ${runId} was not found`);
    if (run.status === 'finished') {
      ...
      return { ok: true, ...publicRun(run), idempotent_replay: true, run_receipt: await receipt({ run_id: runId }), work_authority_changed: false };
    }
    let workAuthorityChanged = false;
    ...
    if (typeof store.activeLeaseForRun === 'function') {
      ...
      if (activeLease && !request.active_lease_settlement) throw err('RUN_HAS_ACTIVE_LEASE', 'orchestration run cannot finish while it still owns an active lease without explicit settlement semantics', {
        ...
        required_transition: 'retry_finish_with_active_lease_settlement',
        required_command: 'orchestration.finish',
        required_field: 'active_lease_settlement.disposition',
      });
      if (activeLease && request.active_lease_settlement) {
        if (!leases || typeof leases.settleByRef !== 'function') throw err('RECOVERY_FAILED', 'settlement-aware finish requires the canonical lease settlement service', { lease_ref: activeLease.lease_id });
        const settlement = {
          lease_ref: activeLease.lease_id,
          ...request.active_lease_settlement,
          idempotency_key: await semanticIdempotencyKey('work.settle', { lease_id: activeLease.lease_id }),
        };
        const settlementResult = await leases.settleByRef(settlement);
        settlementReceipt = settlementResult?.settlement_receipt || null;
        workAuthorityChanged = true;
        const remaining = await store.activeLeaseForRun(runId, now());
        if (remaining) throw err('RECOVERY_FAILED', 'active lease remained after settlement-aware finish attempted canonical settlement', {
          lease_ref: remaining.lease_id,
          work_ref: remaining.work_ref,
          gate: remaining.gate,
          recovery_operation: 'active_lease_settlement',
        });
      }
    }
    ...
    return { ok: true, ...publicRun(updated), idempotent_replay: false, settlement_receipt: settlementReceipt, run_receipt: await receipt({ run_id: runId }), work_authority_changed: workAuthorityChanged };
  }

  return { start, checkpointHorizon, resolveHorizon, receipt, finish };
}

export function createPostgresOrchestrationRunStore(dbBinding) {
  if (!dbBinding || typeof dbBinding.query !== 'function') throw err('RUNTIME_PROVIDER_MISSING', 'database provider is required', { provider:'db' });
  ...
  return {
    ...
    async activeLeaseForRun(runId, observedAt) {
      const canonical = await row(
        `SELECT e.lease_ref::text AS lease_id,
                e.subject_key AS work_ref,
                'project_transition' AS gate,
                e.run_id,
                'active' AS status,
                ...
                e.lease_epoch,
                e.authority_epoch,
                e.lifecycle AS execution_lifecycle,
                e.settled AS execution_settled,
                e.settlement_receipt
           FROM execution_state e
          WHERE e.run_id=$1
            AND e.subject_kind='project_transition'
            AND e.lifecycle IN ('prepared','executing','effect_uncertain','effect_confirmed')
            AND e.settled=false
      ...
      return row(
        `SELECT lease_id,work_ref,gate,status,expires_at,hard_expires_at,
                created_at,claim_receipt
           FROM work_leases
          WHERE run_id=$1
            AND status IN (${LIVE_LEASE_STATUS_SQL})
            AND expires_at > $2
            AND COALESCE(claim_receipt->>'subject','') <> 'project_transition'
          ORDER BY created_at DESC, lease_id DESC
          LIMIT 1`,
        [runId, observedAt],
      );
    },
    async leasesForRun(runId) {
      const result = await dbBinding.query(
        `SELECT lease_id,work_ref,gate,run_id,status,created_at,expires_at,
                settled_at,settle_plan,settle_receipt,reconciliation,
                subject_kind,execution_id,operation_id,lease_epoch,authority_epoch,
                execution_lifecycle,execution_settled,settlement_receipt
           FROM (
             SELECT e.lease_ref::text AS lease_id,
                    e.subject_key AS work_ref,
                    'project_transition' AS gate,
                    e.run_id,
                    CASE WHEN e.settled THEN 'settled' ELSE 'active' END AS status,
                    e.updated_at AS created_at,
                    e.expires_at,
                    e.settled_at,
                    COALESCE(e.settlement_receipt->'project_transition'->'settlement_plan','{}'::jsonb) AS settle_plan,
                    e.settlement_receipt AS settle_receipt,
                    NULL::jsonb AS reconciliation,
                    e.subject_kind,
                    e.execution_id,
                    e.operation_id,
                    e.lease_epoch,
              ...
              WHERE e.run_id=$1
                AND e.subject_kind='project_transition'
             UNION ALL
             SELECT l.lease_id::text,l.work_ref,l.gate,l.run_id,l.status,l.created_at,l.expires_at,
                    l.settled_at,l.settle_plan,l.settle_receipt,l.reconciliation,
                    NULL::text,NULL::text,NULL::uuid,NULL::bigint,NULL::bigint,
                    NULL::text,NULL::boolean,NULL::jsonb
               FROM work_leases l
              WHERE l.run_id=$1
                AND COALESCE(l.claim_receipt->>'subject','') <> 'project_transition'
           ) leases
    ...
    async invocationsForRun(runId) { const result=await dbBinding.query("SELECT i.sequence,i.command,i.target_kind,i.target_ref,i.outcome,i.error_code,i.may_have_mutated,i.request_projection,i.result_projection,i.started_at,i.completed_at,EXISTS(SELECT 1 FROM orchestration_invocation_resolutions r WHERE r.invocation_id=i.invocation_id) AS resolved FROM orchestration_command_invocations i WHERE i.run_id=$1 ORDER BY i.sequence ASC", [runId]); return result.rows || []; },
...
export function createPostgresOrchestrationRunService(options = {}) {
  ...
  if ((!options.store || !options.repositoryLifecycle || !options.authoritative || !options.leases) && (!dbBinding || typeof dbBinding.query !== 'function')) throw err('RUNTIME_PROVIDER_MISSING', 'database provider is required', { provider:'db' });
  if ((!options.repositoryLifecycle || !options.authoritative || !options.leases) && (!apiBinding || typeof apiBinding.call !== 'function')) throw err('RUNTIME_PROVIDER_MISSING', 'API provider is required', { provider:'api' });
  const repositoryLifecycle = options.repositoryLifecycle || createPostgresRepositoryLifecycleService({ db: dbBinding, api: apiBinding, now: options.now });
  return createOrchestrationRunService({
    store: options.store || createPostgresOrchestrationRunStore(dbBinding),
    authoritative: options.authoritative || createLinearAuthority(apiBinding),
    leases: options.leases || createPostgresWorkLeaseService({ db: dbBinding, api: apiBinding, repositoryLifecycle }),
    repositoryLifecycle,
    now: options.now,
  });
}

export function createOrchestrationMaintenanceService({ store, leases, executionRecoveries = [], limit = 20, now = () => new Date().toISOString() } = {}) {
  if (!store || !leases) throw new TypeError('store and leases are required');
  const recoveries = Array.isArray(executionRecoveries) ? executionRecoveries : [];
  return {
    async maintain() {
      const actions = [];
      ...
      for (const recovery of recoveries) {
        ...
        if (!recovery || typeof recovery.reconcilePending !== 'function') continue;
        const recoveryKind = typeof recovery.kind === 'string' && recovery.kind.trim() ? recovery.kind.trim() : 'execution_operation';
        try {
          const results = (await recovery.reconcilePending({ limit:remaining })) || [];
          for (const result of (Array.isArray(results) ? results : []).slice(0, Math.max(0, limit - actions.length))) {
            actions.push({ kind:'execution_operation_reconciliation', recovery_kind:recoveryKind, operation_id:result?.operation_id || null, result });
          }
        } catch (error) {
          actions.push({ kind:'execution_operation_reconciliation', recovery_kind:recoveryKind, operation_id:null, error:String(error?.code || error?.message || 'failed') });
        }
      }
...
export function createPostgresOrchestrationMaintenanceStore(dbBinding) {
  if (!dbBinding || typeof dbBinding.query !== 'function') throw err('RUNTIME_PROVIDER_MISSING', 'database provider is required', { provider:'db' });
  const semanticResolver = createPostgresSemanticJournalInvocationResolver(dbBinding);
  return {
    async expiredSlots(limit = 20) { const boundedLimit = Math.min(100, Math.max(1, Number(limit) || 20)); const r=await dbBinding.query(`SELECT work_ref,gate,lease_id,expires_at FROM ( SELECT e.subject_key AS work_ref,'project_transition' AS gate,e.lease_ref::text AS lease_id,e.expires_at,e.updated_at FROM execution_state e WHERE e.subject_kind='project_transition' AND e.lifecycle IN ('executing','effect_uncertain','effect_confirmed') AND e.settled=false AND e.lease_ref IS NOT NULL AND e.expires_at <= now() UNION ALL SELECT s.work_ref,s.gate,s.lease_id::text AS lease_id,s.expires_at,s.updated_at FROM work_lease_slots s WHERE s.expires_at <= now() AND NOT EXISTS (SELECT 1 FROM execution_state e WHERE e.subject_kind='project_transition' AND e.subject_key=s.work_ref AND e.lease_ref=s.lease_id AND e.settled=false) ) expired ORDER BY expires_at ASC, work_ref ASC, gate ASC LIMIT ${boundedLimit}`); return r.rows || []; },
                ...
                OR EXISTS (
                  SELECT 1
                   ...
                   WHERE l.run_id=r.run_id
                     AND l.status IN (${LIVE_LEASE_STATUS_SQL})
                     AND l.expires_at > $2
                     AND COALESCE(l.claim_receipt->>'subject','') <> 'project_transition'
                )
              )
    ...
    async reconcileInvocation(invocation) {
      const semanticResolution = await semanticResolver.reconcile(invocation);
      ...
      if (invocation.command === 'work.settle' && invocation.idempotency_key && invocation.request_sha256) {
        const canonical = (await dbBinding.query(
          `SELECT e.settlement_receipt AS receipt
             FROM execution_state e
            WHERE e.subject_kind='project_transition'
              AND e.lifecycle='settled'
              AND e.settled=true
              AND e.settlement_receipt->>'settlement_idempotency_key'=$1
              AND e.settlement_receipt->>'settlement_request_sha256'=$2
            ORDER BY e.updated_at DESC
            LIMIT 1`,
          [invocation.idempotency_key, invocation.request_sha256],
        )).rows?.[0] || null;
        const candidate = object(canonical?.receipt);
        if (candidate.schema === 'settlement-receipt-v1'
            && candidate.settlement_idempotency_key === invocation.idempotency_key
            && candidate.settlement_request_sha256 === invocation.request_sha256) {
          receipt = candidate;
          storedHash = candidate.settlement_request_sha256;
        }
      }
      if (!receipt && invocation.command === 'work.settle' && invocation.idempotency_key) { const x=(await dbBinding.query('SELECT settle_receipt AS receipt, settle_request_hash AS request_sha256 FROM work_leases WHERE settle_idempotency_key=$1 AND settle_receipt IS NOT NULL AND COALESCE(claim_receipt->>'subject','') <> 'project_transition'', [invocation.idempotency_key])).rows?.[0] || null; receipt=x?.receipt||null; storedHash=x?.request_sha256||null; }
...
export function createPostgresOrchestrationMaintenanceService(options = {}) {
  const dbBinding = options.db;
  const apiBinding = options.api;
  if (!options.store && (!dbBinding || typeof dbBinding.query !== 'function')) throw err('RUNTIME_PROVIDER_MISSING', 'database provider is required', { provider:'db' });
  if (!options.leases && (!apiBinding || typeof apiBinding.call !== 'function')) throw err('RUNTIME_PROVIDER_MISSING', 'API provider is required', { provider:'api' });
  return createOrchestrationMaintenanceService({ store: options.store || createPostgresOrchestrationMaintenanceStore(dbBinding), leases: options.leases || createPostgresWorkLeaseService({ db: dbBinding, api: apiBinding }), limit: options.limit || 20, now: options.now });
}
...
export function statusForOrchestrationRunError(error) {
  ...
  if (['RUN_NOT_ACTIVE','IDEMPOTENCY_CONFLICT','HORIZON_PRECONDITION_CHANGED','RUN_HAS_ACTIVE_LEASE','RUN_SCOPE_VIOLATION','RECOVERY_FAILED'].includes(code)) return 409;
