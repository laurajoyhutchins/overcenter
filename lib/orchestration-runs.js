...
export function createPostgresOrchestrationMaintenanceStore(dbBinding) {
  ...
  return {
    ...
    async reconcileInvocation(invocation) {
      ...
      if (!receipt && invocation.command === 'work.settle' && invocation.idempotency_key) { const x=(await dbBinding.query('SELECT settle_receipt AS receipt, settle_request_hash AS request_sha256 FROM work_leases WHERE settle_idempotency_key=$1 AND settle_receipt IS NOT NULL', [invocation.idempotency_key])).rows?.[0] || null; receipt=x?.receipt||null; storedHash=x?.request_sha256||null; }
