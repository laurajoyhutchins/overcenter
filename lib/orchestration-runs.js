...
export function createOrchestrationMaintenanceService({ store, leases, compactRecoveries = [], limit = 20, now = () => new Date().toISOString() } = {}) {
  if (!store || !leases) throw new TypeError('store and leases are required');
  const recoveries = Array.isArray(compactRecoveries) ? compactRecoveries : [];
  return {
    async maintain() {
      const actions = [];
      ...
      for (const recovery of recoveries) {
        ...
        const recoveryKind = typeof recovery.kind === 'string' && recovery.kind.trim() ? recovery.kind.trim() : 'compact_operation';
        try {
          ...
          for (const result of (Array.isArray(results) ? results : []).slice(0, Math.max(0, limit - actions.length))) {
            actions.push({ kind:'compact_operation_reconciliation', recovery_kind:recoveryKind, operation_id:result?.operation_id || null, result });
          }
        } catch (error) {
          actions.push({ kind:'compact_operation_reconciliation', recovery_kind:recoveryKind, operation_id:null, error:String(error?.code || error?.message || 'failed') });
        }
      }
