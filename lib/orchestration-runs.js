  ...
  async function resolveStoredHorizon(run, horizon) {
    ...
    for (const candidate of horizon.candidates || []) {
      try {
        ...
        resolved.push({ ...candidate, status, current_execution_fingerprint: evidence.execution_fingerprint, authoritative_revision: issue.updatedAt || null, repository_disposition: lifecycle?.disposition || null, successor_repository: lifecycle?.successor_repository || null });
      } catch (error) {
