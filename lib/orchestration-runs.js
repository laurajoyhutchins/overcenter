...
import { createOrchestrationMaintenanceService as createPortableOrchestrationMaintenanceService } from 'lib/orchestration-maintenance.js';
...
export function createPostgresOrchestrationRunService(options = {}) {
  ...
  const repositoryLifecycle = options.repositoryLifecycle || createPostgresRepositoryLifecycleService({ db: dbBinding, api: options.api || api, now: options.now });
  return createOrchestrationRunService({
    ...
    leases: options.leases || createPostgresWorkLeaseService({ db: dbBinding, api: options.api || api, repositoryLifecycle }),
    repositoryLifecycle,
    now: options.now,
  });
}

export function createOrchestrationMaintenanceService(options = {}) {
  return createPortableOrchestrationMaintenanceService(options);
}
...
export function createPostgresOrchestrationMaintenanceService(options = {}) {
  return createOrchestrationMaintenanceService({ store: options.store || createPostgresOrchestrationMaintenanceStore(options.db || db), leases: options.leases || createPostgresWorkLeaseService({ db: options.db || db, api: options.api || api }), projectAuthoringRecovery:options.projectAuthoringRecovery || null, limit: options.limit || 20, now: options.now });
}
