  ...
  return {
    ...
    async mergeDirect({ repo, pull_request, expected_head, merge_method = 'squash' }) {
      ...
      try {
        ...
        return fail('GITHUB_INTEGRATION_INDETERMINATE', 'The direct merge request lost transport certainty; reconcile GitHub state before retrying.', {
          phase: 'merge_direct',
          may_have_mutated: true,
          github_message: String(error?.message || error),
        });
      ...
      const status = Number(response?.status || 0);
      if (status === 200) {
        if (response?.body?.merged === true) {
          return {
            ok: true,
            http_status: status,
            status: 'merged',
            sha: String(response?.body?.sha || '').toLowerCase() || null,
            message: upstreamMessage(response?.body, 'Pull request merged.'),
          };
        }
        return fail('GITHUB_INTEGRATION_MERGE_FAILED', upstreamMessage(response?.body, 'GitHub did not merge the pull request.'), { upstream_status: status });
      }
      if (status === 409 || status === 422) {
        return fail('GITHUB_INTEGRATION_RECOMPUTE_REQUIRED', 'GitHub rejected the exact-head direct merge request because repository state changed.', {
          upstream_status: status,
          github_message: upstreamMessage(response?.body, 'GitHub rejected the direct merge.'),
        });
      }
      if (status === 400 || status === 405) {
        return fail('GITHUB_INTEGRATION_NOT_READY', upstreamMessage(response?.body, 'GitHub reports the pull request is not ready to merge.'), { upstream_status: status });
      }
      return fail(status === 403 ? 'GITHUB_APP_PERMISSION_DENIED' : 'GITHUB_UPSTREAM_ERROR',
        upstreamMessage(response?.body, `GitHub returned HTTP ${status || 'unknown'}`), { upstream_status: status || null });
    },
...
function exactHeadPlanFallbackAllowed(review) {
  const unavailable = review.protection?.rulesets_unavailable;
  return review.stack == null
    && review.protection?.available !== true
    && review.protection?.evaluation === 'unavailable'
    && review.protection?.rulesets_available === false
    && review.protection?.rulesets_complete === false
    && unavailable?.reason === 'permission_denied'
    && Number(unavailable?.upstream_status || 0) === 403
    && review.checks?.enumeration_complete === true
    && Array.isArray(review.checks?.required_contexts)
    && review.checks.required_contexts.length === 0
    && review.review?.threads_complete === true
    && review.review?.changes_requested !== true
    && review.review?.approval_requirement_satisfied !== false;
}

function waitingReasons(review, { exactHeadPlanFallback = false } = {}) {
  const reasons = new Set();
  for (const reason of review.protection?.unsatisfied_requirements || []) {
    if (reason !== 'branch_up_to_date') reasons.add(String(reason));
  }
  const zeroCheckFallback = exactHeadPlanFallback
    && review.checks?.enumeration_complete === true
    && Array.isArray(review.checks?.required_contexts)
    && review.checks.required_contexts.length === 0;
  if (review.checks?.required_satisfied !== true && !zeroCheckFallback) reasons.add('required_status_checks');
...
export async function reconcileGithubIntegration(input, options = {}) {
  ...
  const exactHeadPlanFallback = exactHeadPlanFallbackAllowed(review);
  if ((review.protection?.available !== true || review.protection?.configured !== true) && !exactHeadPlanFallback) {
  ...
  if (review.protection?.rulesets_complete === false && !exactHeadPlanFallback) {
  ...
  const waiting = waitingReasons(review, { exactHeadPlanFallback });
  if (waiting.length) {
    return {
      ok: true,
      outcome: 'waiting',
  ...
  if (!normalized.apply) {
    return {
      ...
      integration_policy: exactHeadPlanFallback ? 'exact_head_plan_fallback' : 'github_branch_policy',
  ...
  let integrationTransport = 'async';
  let merge = await integrationApi.mergeAsync({
    repo: normalized.repo,
    pull_request: normalized.pull_request,
    expected_head: normalized.expected_head,
    merge_method: 'squash',
  ...
  const asyncTransportForbidden = !merge.ok
    && merge.error === 'GITHUB_APP_PERMISSION_DENIED'
    && Number(merge.upstream_status || 0) === 403;
  if (exactHeadPlanFallback && asyncTransportForbidden && typeof integrationApi.mergeDirect === 'function') {
    integrationTransport = 'direct_exact_head';
    merge = await integrationApi.mergeDirect({
      repo: normalized.repo,
      pull_request: normalized.pull_request,
      expected_head: normalized.expected_head,
      merge_method: 'squash',
  ...
  if (String(merge.status || '').toLowerCase() === 'merged') {
    return {
      ...
      stack_atomic: Boolean(review.stack),
      integration_policy: exactHeadPlanFallback ? 'exact_head_plan_fallback' : 'github_branch_policy',
      integration_transport: integrationTransport,
      evidence: publicReviewEvidence(review),
    };
  }
  return {
