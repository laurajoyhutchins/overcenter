...
export function createGithubIntegrationApiAdapter(apiClient) {
  ...
  return {
    async updateBranch({ repo, pull_request, expected_head }) {
      ...
      const response = await call('PUT', path, { expected_head_sha: expected_head });
      const status = Number(response?.status || 0);
      if (status === 202) {
        return { ok: true, status, message: upstreamMessage(response.body, 'Updating pull request branch.') };
      }
      if (status === 422 || status === 409) {
        return fail('RECOMPUTE_REQUIRED', 'The pull request head or base changed before GitHub could update the branch.', {
    ...
    async mergeAsync({ repo, pull_request, expected_head, merge_method = 'squash', merge_action = 'direct_merge' }) {
      ...
      const response = await call('PUT', path, {
        sha: expected_head,
        merge_method,
        merge_action,
      });
      ...
      if (status === 409 || status === 422) {
        return fail('RECOMPUTE_REQUIRED', 'GitHub rejected the exact-head asynchronous merge request because repository state changed.', {
          upstream_status: status,
...
export async function reconcileGithubIntegration(input, options = {}) {
  ...
  if (!integrationApi) return fail('GITHUB_TRANSPORT_UNAVAILABLE', 'A GitHub integration transport is required.');
...
export async function reconcileGithubIntegrationWithGitHubApp(input, options = {}) {
  ...
  try {
    if (normalized.merge_request_uuid) {
      ...
      }, { permissionProfile: 'integration' });
    }

    const review = await reviewGithubPullRequestWithGitHubApp({
      repo: normalized.repo,
      pull_request: normalized.pull_request,
      expected_head: normalized.expected_head,
    });
