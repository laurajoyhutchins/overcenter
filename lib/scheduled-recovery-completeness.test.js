...
export async function runScheduledRecoveryCompletenessTests() {
  ...
  await test('scheduled run refuses a second claim while live ownership exists', async () => {
    const runs = new FakeRuns();
    const owned = { lease_ref:'lease-owned', work_ref:'LJH-381', gate:'lane:repo-implementation', status:'active', expires_at:'2026-08-24T15:00:00.000Z' };
    const runtimeStore = {
      async activeLeaseForRun() { return owned; },
      async claimRecoveryStateForRun() { return null; },
      async leaseStateByRef() { return null; },
    };
    let transportCalls = 0;
    const transport = async () => { transportCalls += 1; return { status:200, body:{ ok:true } }; };
    ...
    check(transportCalls === 0, 'second claim reached work.claim despite existing live ownership');
    check(result.ok === false && result.terminal === false, 'existing live ownership must preserve the run and reject a second claim');
    check(result.failure?.error === 'ACTIVE_LEASE_PRESENT', 'second claim did not return the live-ownership precondition');
  });

  await test('claim recovery is scoped to the requested work item', async () => {
    const runs = new FakeRuns();
    const recoveryLookups = [];
    const unrelatedLease = { lease_ref:'lease-other', work_ref:'LJH-999', gate:'lane:repo-implementation', status:'claiming', expires_at:'2026-08-24T15:00:00.000Z' };
    const runtimeStore = {
    ...
    check(runs.finishRequests.length === 1, 'lease-free failed claim did not reach deterministic terminalization');
    check(result.ok === false && result.terminal === true, 'unrelated claiming state incorrectly preserved or replayed the failed claim');
  });
  ...
  await test('active lease replays settlement once because commit phase was never entered', async () => {
    const runs = new FakeRuns();
    const lease = { lease_ref:'lease-active', work_ref:'LJH-384', gate:'lane:repo-implementation', status:'active', expires_at:'2026-08-24T15:00:00.000Z', settle_receipt:null };
    const runtimeStore = {
      async activeLeaseForRun() { return lease.status === 'settled' ? null : lease; },
      async claimRecoveryStateForRun() { return null; },
      async leaseStateByRef() { return { ...lease }; },
    ...
    const transport = async (command, input) => {
      calls.push({ command, input:JSON.parse(JSON.stringify(input)) });
      if (calls.length === 1) return { status:502, body:{ ok:false, error:'LINEAR_UPSTREAM_GRAPHQL', error_class:'upstream', retryable:false, may_have_mutated:true, recommended_action:'reconcile_external_effect' } };
      lease.status = 'settled';
      return { status:200, body:{ ok:true, work_ref:'LJH-384', lease_id:'lease-active', disposition:'completed', previous_lane:'lane:repo-implementation', current_lane:'lane:verification', settled_at:'2026-08-24T14:12:05.000Z', idempotent_replay:false } };
    };
    const service = createScheduledExecutionContextService({ runs, runtimeStore, transport, now:()=>'2026-08-24T14:12:03.000Z' });
    const result = await service.execute({ participant:'repository-implementation', operation:'settle', input:{ disposition:'completed', evidence:[] } });
    check(calls.length === 2, `active pre-commit settlement was attempted ${calls.length} time(s)`);
    check(JSON.stringify(calls[0].input) === JSON.stringify(calls[1].input), 'active pre-commit recovery changed the semantic settlement request');
    check(result.ok === true && result.terminal === true, 'active pre-commit retry did not converge to terminal success');
  });
