...
function issue(ref = 'LJH-T1', state = 'Todo', laneName = 'lane:repo-implementation') {
  return {
    ...
    updatedAt: 'rev-1', archivedAt: null, project: { id: 'p', name: 'Portfolio Orchestration' },
...
class FakeAuthority {
  constructor(items) { this.items = new Map(items.map(x => [x.identifier, clone(x)])); this.rev = 1; this.failNext = false; this.commentWrites = 0; this.history = []; }
  ...
  async transition({ issue: base, expectedRevision, expectedState, expectedLane, targetState, targetLane, description = null }) {
    if (this.failNext) { this.failNext = false; const e = new Error('forced'); e.code = 'LINEAR_UPSTREAM_HTTP'; throw e; }
    const current = this.items.get(base.identifier);
    if (current.updatedAt !== expectedRevision || current.state.name !== expectedState || lane(current)?.name !== expectedLane) {
      const e = new Error('changed'); e.code = 'WORK_STATE_CHANGED'; e.details = { actual_state: current.state.name, actual_lane: lane(current)?.name }; throw e;
    }
    current.state = clone(STATES.find(x => x.name === targetState));
    current.labels = [clone(LABELS.find(x => x.name === targetLane))];
...
export async function runWorkLeaseTests() {
  ...
  await test('4 expired lease can be reclaimed safely', async()=>{const h=harness();await h.service.claim(claimReq());h.advance(1801);const r=await h.service.claim(claimReq(h.ref,'claim-2'));assert(r.ok&&r.lease_id,'reclaim failed');});
  await test('5 stale lease cannot settle after successor claim', async()=>{const h=harness();const a=await h.service.claim(claimReq());h.advance(1801);await h.service.claim(claimReq(h.ref,'claim-2'));await expectCode(()=>h.service.settle(settleReq(a.lease_token)), 'LEASE_EXPIRED');});
  ...
  await test('18 stale In Progress recovery after crashed worker', async()=>{const h=harness();await h.service.claim(claimReq());h.advance(1801);await h.service.claim(claimReq(h.ref,'claim-2'));const seq=h.authority.history.map(x=>x.state).join('>');assert(seq.includes('In Progress>Todo>In Progress'),`bad recovery ${seq}`);});
