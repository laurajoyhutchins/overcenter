...
class FakeAuthority {
  constructor(items) { this.items = new Map(items.map(x => [x.identifier, clone(x)])); this.rev = 1; this.failNext = false; this.ambiguousAfterNext = false; this.commentBeforeNextTransition = false; this.commentWrites = 0; this.history = []; }
...
export async function runWorkLeaseTests() {
  ...
  await test('33 pre-upgrade claim snapshots remain settle-compatible', async()=>{const h=harness();const a=await h.service.claim(claimReq());const lease=await h.store.getLeaseById(a.lease_id);for(const key of ['priority','archived_at','project_id','state','state_id','lane','lane_id','labels']) delete lease.claim_receipt.snapshot[key];h.authority.addComment(h.ref);const r=await h.service.settle(settleReq(a.lease_token,'completed','legacy-snapshot-settle'));assert(r.current_lane==='lane:verification','legacy snapshot was invalidated by comment revision');});
