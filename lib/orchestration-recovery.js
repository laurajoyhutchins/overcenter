  ...
  async function diagnose(input) {
    ...
    if (staleLease) {
      ...
      const base = classifyOrchestrationFailure({
        ...
        error_class: classifyCommandError(activeFailure.error_code, { command:activeFailure.command }).error_class,
      ...
      recoveryFailureCount = failures.filter((row) => {
        ...
        const classified = classifyOrchestrationFailure({
          ...
          error_class: classifyCommandError(row.error_code, { command:row.command }).error_class,
    ...
    const latestTypedFailure = latestFailure ? classifyOrchestrationFailure({
      ...
      error_class: classifyCommandError(latestFailure.error_code, { command:latestFailure.command }).error_class,
