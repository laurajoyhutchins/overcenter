...
function publicCanonicalExecution(execution, includeToken = false) {
...
export function createPostgresOrchestrationRecoveryStore(dbBinding) {
  ...
  return {
    ...
    async currentExecution(runId) {
      return oneCurrent(
        await rows('SELECT * FROM execution_state WHERE run_id = $1 AND lease_ref IS NOT NULL AND lifecycle IN ('prepared', 'executing', 'effect_uncertain', 'effect_confirmed', 'effect_absent') AND settled = false LIMIT 2', [runId]),
        'EXECUTION_STATE_AMBIGUOUS',
        'run has more than one active canonical execution authority',
        { run_id:runId },
      );
    },
    async unresolvedOperation(runId) {
      return oneCurrent(
        ...
        'EXECUTION_OPERATION_AMBIGUOUS',
        'run has more than one unresolved execution operation',
        { run_id:runId },
      );
    },
    async currentLegacyLease(runId) {
      return oneCurrent(
        await rows("SELECT * FROM work_leases WHERE run_id = $1 AND status IN ('claiming','active','settling','invalidated') AND COALESCE(claim_receipt->>'subject','') <> 'project_transition' LIMIT 2", [runId]),
...
async function currentRecoveryState(store, runId) {
  ...
  const sameLeaseProjection = execution && legacyLease
    && String(execution.lease_ref || '') === String(legacyLease.lease_id || '');
  if (execution && legacyLease && !sameLeaseProjection) {
    throw err('EXECUTION_AUTHORITY_AMBIGUOUS', 'run has both canonical and legacy current execution authority', { run_id:runId });
  }
  return { run, execution, operation, legacyLease:sameLeaseProjection ? null : legacyLease };
}
...
export function createOrchestrationDiagnosisService({ store, authoritative, now = () => new Date().toISOString() } = {}) {
  ...
  async function diagnose(input) {
    ...
    if (execution) {
      ...
      if (unexpired) activeExecution = publicCanonicalExecution(execution, false);
    ...
    return {
      ...
      worker_state_source:'derived_from_canonical_current_state',
...
export function createPostgresOrchestrationDiagnosisService(options = {}) {
  return createOrchestrationDiagnosisService({
    store:options.store || createPostgresOrchestrationRecoveryStore(options.db),
    authoritative:options.authoritative || createLinearAuthority(options.api),
    now:options.now,
  });
}
...
export function createOrchestrationResumeService({ store, authoritative, now = () => new Date().toISOString() } = {}) {
  ...
  async function resume(input) {
    ...
    if (execution) {
      const unexpired = Date.parse(execution.expires_at) > Date.parse(observedAt);
      const evidence = [{
        kind:'canonical_execution_state',
        subject_key:execution.subject_key,
        lease_ref:execution.lease_ref,
        authority_epoch:Number(execution.authority_epoch || 0),
        authority_revision:execution.authority_revision || null,
      ...
      if (!unexpired) {
        const requiresCanonicalResolution = ['effect_uncertain', 'effect_confirmed', 'effect_absent'].includes(String(execution.lifecycle))
          || execution.mutation_certainty !== 'definitely_not_mutated';
        const confirmOnly = execution.mutation_certainty === 'may_have_mutated'
          || execution.lifecycle === 'effect_uncertain';
        const expiredEvidence = requiresCanonicalResolution
          ? [{
            ...evidence[0],
            recovery_required:true,
            confirm_only:confirmOnly,
            mutation_certainty:execution.mutation_certainty || null,
            lifecycle:execution.lifecycle || null,
            effect_ref:execution.effect_ref || null,
          }]
          : evidence;
        return baseResume(runId, run, operation, {
          continuation:requiresCanonicalResolution ? 'reconcile_authority' : 'recompute_frontier',
          evidence:expiredEvidence,
        });
      }
      return baseResume(runId, run, operation, {
        continuation:'recover_active_lease',
        active_execution:publicCanonicalExecution(execution, true),
        evidence,
      });
    }
    ...
    if (operation) {
      ...
      if (operation.state === 'prepared' && operation.may_have_mutated !== true && request) {
        return baseResume(runId, run, operation, {
          ...
          evidence:[{ kind:'canonical_operation_state', operation_id:operation.operation_id, state:operation.state, may_have_mutated:false }],
        });
      }
      ...
      return baseResume(runId, run, operation, {
        continuation:'reconcile_authority',
        evidence:[{ kind:'canonical_operation_state', operation_id:operation.operation_id, state:operation.state, may_have_mutated:Boolean(operation.may_have_mutated), effect_kind:operation.effect_kind || null, effect_ref:operation.effect_ref || null }],
      });
    }
...
export async function orchestrationResumePacket(input, options = {}) {
  ...
  const skillService = options.skillService || createPostgresSkillExecutionService({ db:options.db });
