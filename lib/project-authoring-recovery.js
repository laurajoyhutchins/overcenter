...
function exactCommand(commandInput){const normalized=String(commandInput||'').trim();if(!COMMANDS.has(normalized))throw fail('PROJECT_AUTHORING_RECOVERY_REQUEST_INVALID','project authoring recovery command must be project.define or project.amend');return normalized;}
...
export function createProjectAuthoringRecoveryService(options={}){
  ...
  async function persistFailure(identity,input,attemptToken,error,priorMayHaveMutated){
    ...
    if((priorMayHaveMutated||error?.may_have_mutated===true)&&isRecompute(error)){
...
// Recovery phase constants remain private implementation details.