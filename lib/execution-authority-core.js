...
export function createExecutionAuthorityService({ store, authoritative, executionProjection, now = () => new Date().toISOString() } = {}) {
  ...
  return {
    async require(input = {}) {
      ...
      if (!leaseToken && !leaseRef) {
        fail('EXECUTION_AUTHORITY_REQUIRED', 'an active Overcenter work lease is required for this mutation', {
          repository: repositoryIdentity(input.repository) || null,
        });
      }
      if (leaseToken && leaseRef) {
        fail('EXECUTION_AUTHORITY_INVALID', 'execution authority locator is ambiguous');
      }
      if (leaseToken.length > 256) {
        fail('EXECUTION_AUTHORITY_INVALID', 'execution authority token is malformed');
      }
      if (leaseRef.length > 128) {
        fail('EXECUTION_AUTHORITY_INVALID', 'execution authority lease reference is malformed');
      }
