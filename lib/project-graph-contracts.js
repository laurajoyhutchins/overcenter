...
const CANONICAL_COMMAND_SET = new Set(CANONICAL_COMMANDS);
...
function requiredText(value, field, fail) {
    ...
    if (!normalized)
        fail('INVALID_PROJECT_GRAPH', `${field} must be a non-empty string`, { field, value: value ?? null });
    return normalized;
}
function boundedText(value, field, max, fail) {
    const normalized = typeof value === 'string' ? value.trim() : '';
...
function canonicalCommand(value, field, fail) {
...
export function normalizeProjectExecutor(raw, nodeId, fail) {
    if (!isRecord(raw))
...
export function normalizeProjectExecutionIntent(raw, nodeId, fail) {
    if (raw == null)
        return undefined;
    if (!isRecord(raw))
        fail('INVALID_PROJECT_GRAPH', 'execution_intent must be an object', { node_id: nodeId });
    const unknown = Object.keys(raw).filter((key) => !['schema', 'desired_outcome', 'acceptance_evidence', 'source_ref'].includes(key)).sort();
    if (unknown.length)
        fail('INVALID_PROJECT_GRAPH', 'execution_intent contains unsupported fields', { node_id: nodeId, unknown });
    ...
    const acceptanceEvidence = raw.acceptance_evidence.map((entry, index) => {
        if (!isRecord(entry))
            fail('INVALID_PROJECT_GRAPH', 'execution_intent acceptance evidence requirement must be an object', { node_id: nodeId, index });
        const entryUnknown = Object.keys(entry).filter((key) => !['kind', 'requirement'].includes(key)).sort();
        if (entryUnknown.length)
            fail('INVALID_PROJECT_GRAPH', 'execution_intent acceptance evidence requirement contains unsupported fields', { node_id: nodeId, index, unknown: entryUnknown });
        return Object.freeze({
...
function normalizeProjectLiteral(value, field, fail) {
...
export function normalizeProjectPhaseInput(raw, nodeId, phase, fail) {
    if (raw == null)
        return Object.freeze({});
    if (!isRecord(raw)) {
    ...
    for (const [field, rawSource] of Object.entries(raw)) {
        ...
        const safeSegments = segments.every((segment) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(segment) &&
...
function isProjectBindingPhase(value) {
    return value === 'ACQUIRE' || value === 'COMMIT' || value === 'CONFIRM';
}
export function normalizeProjectPhaseBindings(raw, nodeId, fail) {
    if (raw == null)
        return Object.freeze({});
    if (!isRecord(raw)) {
    ...
    for (const [rawPhase, rawBinding] of Object.entries(raw)) {
        ...
        const evidence = rawEvidence.map((value, index) => requiredText(value, `phase_bindings.${phase}.evidence[${index}]`, fail));
