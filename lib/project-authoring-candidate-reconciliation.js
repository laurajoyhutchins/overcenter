...
export function reconcileProjectAuthoringCandidate(input) {
    ...
    const hasBaseReconciliation = input?.staged_base_revision !== undefined || input?.current_base_revision !== undefined;
    if (!hasBaseReconciliation) {
        return Object.freeze({ candidate_revision: current, staged_revision: staged, advanced });
    }
    const stagedBase = exactRevision(input?.staged_base_revision, 'staged_base_revision');
    const currentBase = exactRevision(input?.current_base_revision, 'current_base_revision');
    const baseAdvanced = currentBase !== stagedBase;
    if (baseAdvanced && input?.base_descendant_of_staged !== true) {
        fail('PROJECT_AUTHORING_BASE_RECONCILIATION_REQUIRED', 'current base is not a descendant of the staged base', { staged_base_revision: stagedBase, current_base_revision: currentBase });
    }
    if (baseAdvanced && input?.base_semantics_compatible !== true) {
        fail('PROJECT_AUTHORING_BASE_RECONCILIATION_REQUIRED', 'base movement is not mechanically proven semantic-compatible', { staged_base_revision: stagedBase, current_base_revision: currentBase });
    }
    return Object.freeze({
        candidate_revision: current,
        staged_revision: staged,
        advanced,
        base_revision: currentBase,
