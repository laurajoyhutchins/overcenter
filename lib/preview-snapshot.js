...
export function derivePreviewSnapshot(input = {}) {
  ...
  return Object.freeze({
    ...
    overcenter: Object.freeze({ runs, leases, recovery_pending: recovery }),
...
SELECT
  ...
  (SELECT count(*)::int FROM execution_state
    WHERE operation_kind='github.changeset.apply'
      AND lifecycle IN ('executing','effect_uncertain')
      AND updated_at < now() - interval '${ORCHESTRATION_STUCK_MINUTES} minutes') AS github_changesets_processing,
  (SELECT count(*)::int FROM execution_state
    WHERE operation_kind='github.changeset.apply'
      AND lifecycle='prepared'
      AND updated_at < now() - interval '${ORCHESTRATION_STUCK_MINUTES} minutes') AS github_changesets_prepared,
  (SELECT count(*)::int FROM execution_state
    WHERE operation_kind='portfolio.reconcile'
      AND lifecycle IN ('executing','effect_uncertain')
      AND updated_at < now() - interval '${ORCHESTRATION_STUCK_MINUTES} minutes') AS portfolio_reconcile_processing,
  (SELECT count(*)::int FROM execution_state
    WHERE operation_kind='portfolio.reconcile'
      AND lifecycle='effect_uncertain') AS portfolio_reconcile_indeterminate
...
export function renderPreviewPage(snapshot) {
  ...
  const runs = displayCount(snapshot?.overcenter?.runs);
  const leases = displayCount(snapshot?.overcenter?.leases);
  const recovery = displayCount(snapshot?.overcenter?.recovery_pending);
...
<head>
  ...
  <title>Overcenter</title>
  <meta name="description" content="Overcenter is the portfolio orchestration GitHub App, deployed on Hatchable.">
  <style>
...
<body>
  <main class="control-sheet">
    <header>
      <div class="identity"><h1>OVERCENTER</h1><span class="version">${previewVersionLabel(snapshot)}</span></div>
    ...
    <svg class="diagram" viewBox="0 0 420 326" role="img" aria-labelledby="snapshot-title snapshot-desc">
      <title id="snapshot-title">Overcenter controls diagram</title>
      <desc id="snapshot-desc">Scheduled and interactive execution feed Overcenter run control. GitHub is the authoritative repository state. Linear is the executable-work projection. Each transition follows Enable, Acquire, Execute, Commit, Confirm.</desc>
