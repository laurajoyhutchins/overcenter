...
function definitionFacts() {
  ...
  const content = JSON.stringify({
    ...
    transitions:[
      { id:'second', priority:10, requires:['first'], executor:{ kind:'agent', role:'implementation', skill:'test-driven-development' }, execution_intent:{ schema:'project-execution-intent-v1', desired_outcome:'Prove repository definitions preserve self-contained agent execution intent.', acceptance_evidence:[{ kind:'verification', requirement:'Derived agent transition must preserve the canonical execution intent.' }] } },
...
// Tests execute directly through native node:test.

  test('declaration path is fixed under the current Overcenter namespace', async()=>{
    assert(PROJECT_GRAPH_DERIVATION_DECLARATION_PATH === '.overcenter/project-graph.json', 'declaration path changed');
  });

  test('declaration yields only one registered derivation identity', async()=>{
  ...
  test('JSON declaration parsing is deterministic', async()=>{
  ...
  test('declaration rejects graph state and authority coordinates', async()=>{
  ...
  test('declaration rejects unsupported schema or empty derivation', async()=>{
  ...
  test('declaration rejects malformed JSON and non-object payloads', async()=>{
  ...
  test('Overcenter derivation maps exact repository definition facts to deterministic graph nodes', async()=>{
    ...
    assert(result.nodes[1].execution_intent?.schema === 'project-execution-intent-v1', 'deriver dropped agent execution intent');
    assert(result.nodes[1].execution_intent?.acceptance_evidence?.[0]?.kind === 'verification', 'deriver dropped execution-intent acceptance evidence');
  });

  test('authoritative reader resolves built-in Overcenter derivation without caller registry bookkeeping', async()=>{
    const fixture = definitionFacts();
  ...
  test('compatible completed transition observation marks predecessor DONE across unrelated repository revisions', async()=>{
    const fixture = definitionFacts();
    const derived = deriveOvercenterProjectGraph({
      project_ref:'github:laurajoyhutchins/overcenter',
      authority:{ kind:'github', repository:fixture.repository, revision:fixture.revision, derivation:OVERCENTER_PROJECT_GRAPH_DERIVATION },
  ...
  test('materially different transition fingerprint cannot inherit prior confirmation', async()=>{
    const fixture = definitionFacts();
    const reader = createAuthoritativeProjectGraphReader({
      resolveProjectAuthority:async()=>({ kind:'github', repository:fixture.repository, revision:fixture.revision, derivation:OVERCENTER_PROJECT_GRAPH_DERIVATION }),
      readProjectFacts:async()=>({ schema:'project-authority-facts-v1', repository:fixture.repository, revision:fixture.revision, facts:{ definition_facts:fixture.facts } }),
  ...
  test('production observation adapter projects completed project-transition settlement receipts', async()=>{
    const fixture = definitionFacts();
    const derived = deriveOvercenterProjectGraph({
      project_ref:'github:laurajoyhutchins/overcenter',
      authority:{ kind:'github', repository:fixture.repository, revision:fixture.revision, derivation:OVERCENTER_PROJECT_GRAPH_DERIVATION },
  ...
  test('project-transition durable claim receipt preserves transition-definition fingerprint', async()=>{
    ...
    const prepared = await prepareProjectTransitionLeasePersistence({
      ...
      transition_definition_fingerprint:fingerprint,
      transition_revision_fingerprint:'b'.repeat(64),
      transition_dependency_fingerprint:'a'.repeat(64),
      acquire_idempotency_key:'fingerprint-test',
      acquire_request_hash:'c'.repeat(64),
      status:'active',
      created_at:'2026-08-28T20:00:00.000Z',
      expires_at:'2026-08-28T20:30:00.000Z',
      hard_expires_at:'2026-08-28T21:00:00.000Z',
    }, { capabilityToken:'private-capability' });
    assert(prepared.claim_receipt.project_transition.transition_definition_fingerprint === fingerprint, 'durable claim receipt lost transition-definition fingerprint');
  });

  // Native node:test owns result collection and failure reporting.
