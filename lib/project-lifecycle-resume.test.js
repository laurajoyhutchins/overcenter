...
export async function runProjectLifecycleResumeTests() {
  ...
  await test('resumes a partially completed transition at its first unresolved lifecycle phase', async()=>{
    ...
    const result = await executeProjectTransitionLifecycle({ nodes:[{
      ...
      enable: async () => { calls.push('ENABLE'); return { enabled:true }; },
      acquire: async () => { calls.push('ACQUIRE'); return { lease_id:'lease-1' }; },
      operator: async () => { calls.push('EXECUTE'); return { changed:true }; },
      commit: async () => { calls.push('COMMIT'); return { commit_sha:'abc123' }; },
      confirm: async () => { calls.push('CONFIRM'); return { confirmed:true }; },
    });
