...
function usesInvocationRunIdentity(command) {
  return command !== 'orchestration.diagnose';
}
...
export function createWorkerCommandHandler(options = {}) {
  ...
  return async function workerCommandHandler(req, res) {
    ...
    const runtime = {
      ...
      ...(req.body?.invocation_context === undefined || !usesInvocationRunIdentity(command) ? {} : { invocationContext:req.body.invocation_context }),
    };
