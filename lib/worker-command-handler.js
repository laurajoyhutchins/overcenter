...
function invocationContextFor(command, input, invocationContext) {
  if (command === 'orchestration.diagnose' && typeof input?.run_id === 'string' && input.run_id.length > 0) {
    return {
      ...(invocationContext && typeof invocationContext === 'object' && !Array.isArray(invocationContext) ? invocationContext : {}),
      run_id:input.run_id,
    };
  }
...
export function createWorkerCommandHandler(options = {}) {
  ...
  const providers = options.providers && typeof options.providers === 'object' ? options.providers : null;
  const db = options.db || providers?.db;
  ...
  return async function workerCommandHandler(req, res) {
    ...
    const invocationContext = invocationContextFor(command, input, req.body?.invocation_context);
    const runtime = {
      ...(providers || {}),
      db,
      ...(providers?.githubAppAuth?.withApiClient ? { withGitHubAppApiClient:providers.githubAppAuth.withApiClient } : {}),
      ...(invocationContext === undefined ? {} : { invocationContext }),
    };
