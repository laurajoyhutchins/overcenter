...
function settlementPlan(lease, request, issue) {
  ...
  if (request.disposition === 'requeue') {
    return { state: lease.previous_state, lane: lease.previous_lane, description: null, evidence: request.evidence, replay_request };
  }
  ...
  return { ...target, description: null, evidence: request.evidence, replay_request };
}
...
export function createWorkLeaseService({ store, authoritative, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function claim(input) {
    ...
    const lease = {
      ...
      claim_request_hash: requestHash, claim_request: request, status: 'claiming', created_at: createdAt, expires_at: expiresAt,
  ...
  return {
    ...
    async insertLease(lease) {
      const inserted = await row(`INSERT INTO work_leases (
        lease_id, work_ref, gate, run_id, lease_token, token_hash, claim_idempotency_key, claim_request_hash, claim_request,
      ...
      ON CONFLICT (claim_idempotency_key) DO NOTHING RETURNING *`, [
        ...
        lease.claim_idempotency_key, lease.claim_request_hash, JSON.stringify(lease.claim_request), lease.status, lease.created_at, lease.expires_at,
