...
function parseManagedExecutionDescription(description) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    const key = sectionNames.get(lines[i].trim().toLowerCase());
    if (!key) continue;
    const values = [];
    for (let j = i + 1; j < lines.length; j += 1) {
      const trimmed = lines[j].trim();
      if (!trimmed) break;
      if (sectionNames.has(trimmed.toLowerCase())) break;
      if (/^(repository|authority|github authority|exact coordinate|owner impact|promotion condition)\s*:/i.test(trimmed)) break;
...
function legacyExecutionProjection(snapshotValue) {
  ...
  for (const key of ['project','project_id','archived_at','state','state_id','lane','lane_id','priority','repository','authority','exact_coordinate','acceptance','owner_impact']) copy(key);
