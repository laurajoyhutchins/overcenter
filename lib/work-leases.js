...
function settlementPlan(lease, request, issue) {
  if (request.disposition === 'requeue') {
    return { state: lease.previous_state, lane: lease.previous_lane, description: null };
  }
  ...
  return { ...successor, description: null };
}
...
export function createWorkLeaseService({ store, authoritative, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function recoverClaimReplay(existing, requestHash) {
    ...
    if (existing.status === 'rejected') {
      throw err(existing.rejection_code || 'ALREADY_CLAIMED', 'the original claim request was rejected', existing.rejection_details || null);
    }
  ...
  async function claim(input) {
    ...
    try {
      ...
      await store.rejectLease(lease.lease_id, 'LINEAR_TRANSITION_FAILED', { upstream_code: error?.code || null });
...
export function statusForWorkLeaseError(error) {
  ...
  if (['ALREADY_CLAIMED','STATE_MISMATCH','LANE_MISMATCH','NON_EXECUTABLE_WORK','LEASE_EXPIRED','LEASE_ALREADY_SETTLED','WORK_STATE_CHANGED','IDEMPOTENCY_CONFLICT','CLAIM_INDETERMINATE'].includes(code)) return 409;
