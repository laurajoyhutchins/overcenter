...
import { LEGACY_WORK_EXECUTION_GATES } from './legacy-work-execution-authority-contracts.js';
import { WORK_REQUEUE_CLASSES, WORK_SETTLEMENT_DISPOSITIONS } from './execution-lifecycle-contracts.js';
...
import { normalizeOperatingCondition, resolveCompletedStage } from './work-lifecycle.js';
import { STAGE_BY_LEGACY_LANE, legacyProjectionForStage } from './legacy-lane-compatibility.js';

// Linear lifecycle is not used for lease ownership; the slot is authoritative.
...
const EXECUTION_LANES = new Set(LEGACY_WORK_EXECUTION_GATES);
const PHASE_NEXT_ACTION = Object.freeze({
  'lane:enable': 'Establish that the bounded work identity is executable and determine the next required productive responsibility.',
  'lane:source-implementation': 'Acquire the authoritative inputs required by the bounded work.',
  'lane:repo-implementation': 'Execute the bounded change and produce an exact candidate.',
  'lane:integration': 'Commit the exact candidate into its authoritative durable state.',
  'lane:verification': 'Confirm the exact durable result against the intended outcome.',
});
const DISPOSITIONS = new Set(WORK_SETTLEMENT_DISPOSITIONS);
const REQUEUE_CLASSES = new Set(WORK_REQUEUE_CLASSES);
...
export function executionProjection(issue) {
  const lane = laneOf(issue);
...
function assertHandoffReady(issue) {
  const projection = executionProjection(issue);
  const missing = ['outcome', 'next_action'].filter(field => !normalizeProse(projection[field]));
  if (missing.length) {
    throw err('NON_EXECUTABLE_WORK', 'work item does not contain a bounded executable handoff', {
      work_ref: issue?.identifier || null,
      missing_handoff_fields: missing,
      required_handoff_fields: ['outcome', 'next_action'],
    });
  }
  return projection;
}

// Claims are fenced by the execution projection persisted by the current lease protocol.
...
function leaseExecutionProjection(lease) {
  return lease?.claim_receipt?.execution_projection || null;
}
...
function normalizeSettleRequest(input) {
  ...
  let operatingCondition;
  try {
    operatingCondition = normalizeOperatingCondition(input?.operating_condition ?? (disposition === 'blocked' ? 'HOLD' : 'NOMINAL'));
  } catch (error) {
    throw err('REQUEST_INVALID', 'operating_condition is invalid', { operating_condition: input?.operating_condition ?? null });
  }
  if (disposition === 'blocked' && operatingCondition === 'NOMINAL') throw err('REQUEST_INVALID', 'blocked settlement requires an off-nominal operating_condition');
  if (disposition !== 'blocked' && operatingCondition !== 'NOMINAL') throw err('REQUEST_INVALID', 'off-nominal operating_condition requires blocked settlement');
  if (input?.next_state != null || input?.next_lane != null) {
    throw err('REQUEST_INVALID', 'workers must report lifecycle_facts; successor state and lane are selected by Overcenter');
  }
  const lifecycleFacts = input?.lifecycle_facts == null ? null : input.lifecycle_facts;
  if (lifecycleFacts !== null && (!lifecycleFacts || typeof lifecycleFacts !== 'object' || Array.isArray(lifecycleFacts))) {
    throw err('REQUEST_INVALID', 'lifecycle_facts must be an object');
  }
  if (lifecycleFacts !== null && disposition !== 'completed') {
    throw err('REQUEST_INVALID', 'lifecycle_facts are allowed only for completed settlement');
  }
  return {
    ...normalizeLeaseSelector(input),
    disposition,
...
function blockedDescription(description, reason, promotion, operatingCondition = 'HOLD') {
  const marker = '\n\n## Execution blocker\n';
...
function settlementPlan(lease, request, issue, continuation = null) {
  const replay_request = {
    ...
    continuation,
    lifecycle_facts: request.lifecycle_facts,
  };
  if (request.disposition === 'requeue') {
    return { state: lease.previous_state, lane: lease.previous_lane, description: null, evidence: request.evidence, requeue_class: request.requeue_class, operating_condition: request.operating_condition, continuation, replay_request };
  }
  if (request.disposition === 'blocked') {
    return {
      state: 'Backlog',
      lane: lease.previous_lane,
      description: blockedDescription(issue.description, request.reason, request.promotion_condition, request.operating_condition),
      evidence: request.evidence,
      operating_condition: request.operating_condition,
      continuation,
      replay_request,
  ...
  const currentStage = STAGE_BY_LEGACY_LANE[lease.gate] || null;
  if (!currentStage) throw err('NON_EXECUTABLE_WORK', `No productive lifecycle stage exists for ${lease.gate}`);
  let resolution;
  try {
    resolution = resolveCompletedStage({ current_stage: currentStage, lifecycle_facts: request.lifecycle_facts });
  } catch (error) {
    throw err('REQUEST_INVALID', 'completed settlement lifecycle facts are invalid', {
      lifecycle_error: error?.code || 'INVALID_LIFECYCLE_FACTS',
      lifecycle_details: error?.details || null,
    });
  }
  if (resolution.condition !== 'NOMINAL' || resolution.transition_kind === 'off_nominal') {
    throw err('REQUEST_INVALID', 'completed settlement cannot enter an off-nominal operating condition; use blocked or requeue settlement');
  }
  const target = legacyProjectionForStage(resolution.next_stage, lease.gate);
  return { ...target, description: successorDescription(issue.description, target), evidence: request.evidence, operating_condition: 'NOMINAL', continuation, replay_request, lifecycle_resolution: resolution };
}
...
async function continuationFromLease(store, lease) {
  ...
  if (lease.status === 'expired' && lease.reconciliation?.released_without_linear_mutation === true) {
...
export function createWorkLeaseService({ store, authoritative, repositoryLifecycle = null, skillRequirements = null, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function reconcileExpired(workRef, gate) {
    ...
    const continuationReady = releasedWithoutLinearMutation;
    ...
    return { released_without_linear_mutation: releasedWithoutLinearMutation, reason };
  }
  ...
  async function recoverClaimReplay(existing, requestHash) {
    ...
    if (existing.status === 'claiming') {
      ...
      if (slot?.lease_id === existing.lease_id && issueMatches(issue, { state: existing.previous_state, lane: existing.gate })) {
  ...
  async function claim(input) {
    ...
    // Check them before expired-slot reconciliation releases stale ownership.
    ...
    await reconcileExpired(request.work_ref, lane.name);
    ...
    // Slot reconciliation never mutates the Linear lifecycle. The caller's observed
    // state remains a strict optimistic precondition after reconciliation.
    if (request.expected_state && issue.state?.name !== request.expected_state) {
  ...
  async function settle(input) {
    ...
    if (lease.status === 'settling' && issueMatches(current, plan)) {
      ...
      const receipt = {
        ...
        operating_condition: plan.operating_condition || 'NOMINAL',
        continuation_sha256: plan.continuation_sha256 || null,
        lifecycle_resolution: plan.lifecycle_resolution || null,
...
export function createLinearAuthority(apiBinding) {
  if (!apiBinding || typeof apiBinding.call !== 'function') throw err('RUNTIME_PROVIDER_MISSING', 'API provider is required', { provider:'api' });
...
export function createPostgresLeaseStore(dbBinding) {
...
export function createPostgresWorkLeaseService(options = {}) {
  ...
  if (!options.authoritative && (!apiBinding || typeof apiBinding.call !== 'function')) throw err('RUNTIME_PROVIDER_MISSING', 'API provider is required', { provider:'api' });
  return createWorkLeaseService({
    store: options.store || createPostgresLeaseStore(dbBinding),
    ...
    skillRequirements: options.skillRequirements || createPostgresSkillExecutionService({ db: dbBinding }),
    now: options.now,
...
export const workLeaseConfig = Object.freeze({
  ...
  lifecycle_routing: 'work-lifecycle-v1',
});
