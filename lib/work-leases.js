...
function parseManagedExecutionDescription(description) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    for (let j = i + 1; j < lines.length; j += 1) {
      ...
      if (/^(repository|authority|github authority|exact coordinate|owner impact|promotion condition)\s*:/i.test(structural)) break;
...
function normalizeCheckpointPayload(value) {
  ...
  const nextAction = requiredString(value.next_action_kind, 'checkpoint.next_action_kind', 64);
  if (!NEXT_ACTION_KINDS.has(nextAction)) throw err('REQUEST_INVALID', 'checkpoint.next_action_kind is unsupported', { next_action_kind: nextAction });
  return {
...
function settlementPlan(lease, request, issue, continuation = null) {
  ...
  return { ...target, description: null, evidence: request.evidence, continuation, replay_request };
}
...
async function continuationFromLease(store, lease) {
  ...
  if (lease.status === 'expired' && lease.reconciliation?.restored === true) {
...
export function createWorkLeaseService({ store, authoritative, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function reconcileExpired(workRef, gate) {
    ...
    const restoredEvidence = restored ? await executionEvidence(current) : null;
  ...
  async function claim(input) {
    ...
    if (typeof store.getRunBudget === 'function') {
      ...
      const repository = initialProjection.repository || null;
      const project = initialProjection.project || null;
      if ((scope?.project && scope.project !== project)
        || (Array.isArray(scope?.lanes) && scope.lanes.length && !scope.lanes.includes(lane.name))
        || (Array.isArray(scope?.repositories) && scope.repositories.length && !scope.repositories.includes(repository))) {
        throw err('RUN_SCOPE_VIOLATION', 'work item is outside the registered orchestration run scope', { run_id: request.run_id, project, lane: lane.name, repository });
      }
    }
    ...
    const evidence = await executionEvidence(activeIssue);
    const receipt = {
      ok: true,
  ...
  async function checkpoint(input) {
    ...
    const lease = await store.getLeaseByTokenHash(tokenHash);
    ...
    if (!projectionMatchesExpected(current, claimProjection) || !issueMatches(current, { state: ACTIVE_STATE, lane: lease.gate })) {
  ...
  async function heartbeat(input) {
    ...
    if (runBudget?.status && runBudget.status !== 'active') throw err('RUN_BUDGET_EXHAUSTED', 'orchestration run is not active', { run_id: lease.run_id, status: runBudget.status });
    ...
    if (!Number.isFinite(newExpiryMs) || newExpiryMs <= Date.parse(lease.expires_at)) {
      throw err(runBudget && capMs <= Date.parse(lease.expires_at) ? 'RUN_BUDGET_EXHAUSTED' : 'HEARTBEAT_LIMIT_REACHED', 'heartbeat cannot extend lease beyond its bounded execution horizon', { hard_expires_at: new Date(hardMs).toISOString(), run_deadline_at: runBudget?.deadline_at || null });
    }
  ...
  async function settle(input) {
    ...
    if (!projectionMatches || !issueMatches(current, { state: ACTIVE_STATE, lane: lease.gate })) {
      const changedFields = projectionDiff(claimProjection || {}, currentProjection);
  ...
  return { claim, checkpoint, heartbeat, settle, reconcileExpired };
}
...
export function createPostgresLeaseStore(dbBinding = db) {
  ...
  return {
    ...
    async listContinuationCandidates(workRef, gate) {
      const result = await dbBinding.query("SELECT * FROM work_leases WHERE work_ref = $1 AND ((status = 'settled' AND settle_plan->>'lane' = $2 AND settle_plan->'continuation' IS NOT NULL) OR (status = 'expired' AND gate = $2 AND reconciliation->>'restored' = 'true')) ORDER BY COALESCE(settled_at, updated_at) DESC LIMIT 20", [workRef, gate]);
      return result.rows || [];
    },
    ...
    async extendLeaseWithHeartbeat(input) {
      const saved = await row(`WITH heartbeat_reservation AS (
        INSERT INTO work_lease_heartbeats (lease_id,idempotency_key,request_sha256,progress_sha256,previous_expires_at,new_expires_at,created_at)
        VALUES ($1,$4,$5,$6,$9,$7,$8)
        ON CONFLICT (lease_id,idempotency_key) DO NOTHING RETURNING *
      ), eligible AS (
        SELECT l.lease_id
        FROM work_leases l
        JOIN work_lease_slots s ON s.lease_id=l.lease_id AND s.work_ref=$2 AND s.gate=$3
        JOIN heartbeat_reservation h ON h.lease_id=l.lease_id
        WHERE l.lease_id=$1 AND l.status='active' AND l.expires_at > $8 AND s.expires_at > $8
        FOR UPDATE OF l,s
      ), slot_update AS (
        UPDATE work_lease_slots s SET expires_at=$7, updated_at=now()
        FROM eligible e WHERE s.lease_id=e.lease_id AND s.work_ref=$2 AND s.gate=$3 RETURNING s.lease_id
      ), lease_update AS (
        UPDATE work_leases l SET expires_at=$7, last_heartbeat_at=$8, heartbeat_count=heartbeat_count+1, updated_at=now()
        FROM eligible e, slot_update s WHERE l.lease_id=e.lease_id AND l.lease_id=s.lease_id RETURNING l.heartbeat_count
      ), cleanup AS (
        DELETE FROM work_lease_heartbeats h USING heartbeat_reservation r
        WHERE h.heartbeat_id=r.heartbeat_id AND NOT EXISTS (SELECT 1 FROM lease_update) RETURNING h.heartbeat_id
      ) SELECT r.*, l.heartbeat_count FROM heartbeat_reservation r, lease_update l, slot_update s`, [input.lease_id,input.work_ref,input.gate,input.idempotency_key,input.request_sha256,input.progress_sha256,input.new_expires_at,input.created_at,input.previous_expires_at]);
...
export const workLeaseConfig = Object.freeze({
  ...
  next_action_kinds: [...NEXT_ACTION_KINDS],
...
export const workLeaseInternals = Object.freeze({ normalizeClaimRequest, normalizeCheckpointRequest, normalizeHeartbeatRequest, normalizeSettleRequest, settlementPlan, isExecutable, laneOf, snapshot, executionProjection, executionEvidence, publicLease });