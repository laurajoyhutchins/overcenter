...
export function createWorkLeaseService({ store, authoritative, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function claim(input) {
    ...
    let runBudget = null;
    if (typeof store.getRunBudget === 'function') runBudget = await store.getRunBudget(request.run_id);
    if (runBudget) {
...
export function createPostgresLeaseStore(dbBinding = db) {
  ...
  return {
    ...
    async getRunBudget(runId) { return row('SELECT status, deadline_at, settlement_reserve_seconds, minimum_new_gate_seconds FROM orchestration_runs WHERE run_id = $1', [runId]); },
    ...
    async extendLeaseWithHeartbeat(input) {
      const saved = await row(`WITH lease_update AS (
        UPDATE work_leases SET expires_at=$7, last_heartbeat_at=$8, heartbeat_count=heartbeat_count+1, updated_at=now()
        WHERE lease_id=$1 AND status='active' AND expires_at > $8 RETURNING heartbeat_count
      ), slot_update AS (
        UPDATE work_lease_slots SET expires_at=$7, updated_at=now()
        WHERE lease_id=$1 AND work_ref=$2 AND gate=$3 AND expires_at > $8 AND EXISTS (SELECT 1 FROM lease_update) RETURNING lease_id
      ), receipt AS (
        ...
        SELECT $1,$4,$5,$6,$9,$7,$8 FROM lease_update, slot_update
        ON CONFLICT (lease_id,idempotency_key) DO NOTHING RETURNING *
      ) SELECT receipt.*, lease_update.heartbeat_count FROM receipt, lease_update`, [input.lease_id,input.work_ref,input.gate,input.idempotency_key,input.request_sha256,input.progress_sha256,input.new_expires_at,input.created_at,input.previous_expires_at]);
...
export function statusForWorkLeaseError(error) {
  ...
  if (['ALREADY_CLAIMED','STATE_MISMATCH','LANE_MISMATCH','NON_EXECUTABLE_WORK','LEASE_INVALID','LEASE_EXPIRED','LEASE_ALREADY_SETTLED','WORK_STATE_CHANGED','IDEMPOTENCY_CONFLICT','CLAIM_INDETERMINATE','INVALID_SUCCESSOR','CHECKPOINT_REQUIRED','RUN_BUDGET_EXHAUSTED','HEARTBEAT_LIMIT_REACHED','NO_PROGRESS_HEARTBEAT'].includes(code)) return 409;
