...
import { STAGE_BY_LEGACY_LANE, legacyProjectionForStage, normalizeOperatingCondition, resolveCompletedStage } from './work-lifecycle.js';
...
function parseManagedExecutionDescription(description) {
  ...
  // Legacy imperative fields are input aliases only. The execution contract is declarative.
  const fullyLegacyPacket = Boolean(managed.outcome && managed.next_action);
  if (!managed.objective) managed.objective = managed.outcome;
  if (!managed.gate) managed.gate = managed.next_action;
  if (managed.acceptance.length === 0 && fullyLegacyPacket) managed.acceptance = [managed.outcome];
  return managed;
}
...
function assertHandoffReady(issue) {
  ...
  const missing = [
    !normalizeProse(projection.objective) ? 'objective' : null,
    !normalizeProse(projection.gate) ? 'gate' : null,
    !Array.isArray(projection.acceptance) || projection.acceptance.length === 0 ? 'acceptance' : null,
  ].filter(Boolean);
  if (missing.length) {
    throw err('NON_EXECUTABLE_WORK', 'work item does not contain a bounded declarative execution packet', {
      work_ref: issue?.identifier || null,
      missing_handoff_fields: missing,
      required_handoff_fields: ['objective', 'gate', 'acceptance'],
    });
  }
  return projection;
}
...
function normalizeSettleRequest(input) {
  ...
  if (input?.next_state != null || input?.next_lane != null) {
    throw err('REQUEST_INVALID', 'workers must report lifecycle_facts; successor state and lane are selected by Busbar');
  }
