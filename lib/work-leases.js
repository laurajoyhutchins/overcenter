...
function leaseSnapshotMatches(issue, lease) {
  return snapshotMatchesExpected(issue, lease?.claim_receipt?.snapshot || null);
}
...
export function createWorkLeaseService({ store, authoritative, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function settle(input) {
    ...
    if (!leaseSnapshotMatches(current, lease) || !issueMatches(current, { state: ACTIVE_STATE, lane: lease.gate })) {
      await store.invalidateLease(lease.lease_id, {
        observed_revision: current.updatedAt,
        claimed_revision: lease.active_revision,
        actual_state: current.state?.name || null,
        actual_lane: laneOf(current)?.name || null,
        semantic_snapshot_changed: !leaseSnapshotMatches(current, lease),
        invalidated_at: now(),
      });
      await store.releaseSlot(lease.work_ref, lease.gate, lease.lease_id);
      throw err('WORK_STATE_CHANGED', 'authoritative work state changed after claim', {
        actual_state: current.state?.name || null,
        actual_lane: laneOf(current)?.name || null,
...
export function createLinearAuthority(apiBinding = api) {
  ...
  async function transition({ issue, expectedRevision, expectedState, expectedLane, targetState, targetLane, description = null }) {
    ...
    if (!snapshotMatchesExpected(fresh, snapshot(issue)) || fresh.state?.name !== expectedState || freshLane?.name !== expectedLane) {
