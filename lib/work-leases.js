...
export function createWorkLeaseService({ store, authoritative, now = () => new Date().toISOString(), tokenFactory = defaultTokenFactory } = {}) {
  ...
  async function claim(input) {
    ...
    const lease = {
      ...
      status: 'claiming', created_at: createdAt, expires_at: expiresAt,
  ...
  return { claim, checkpoint, settle, reconcileExpired };
}
  ...
  return {
    ...
    async insertLease(lease) {
      ...
      ON CONFLICT (claim_idempotency_key) DO NOTHING RETURNING *`, [
        ...
        lease.claim_idempotency_key, lease.claim_request_hash, JSON.stringify(lease.claim_request), JSON.stringify(lease.predecessor_continuation), lease.status, lease.created_at, lease.expires_at,
...
export function statusForWorkLeaseError(error) {
  ...
  if (['ALREADY_CLAIMED','STATE_MISMATCH','LANE_MISMATCH','NON_EXECUTABLE_WORK','LEASE_INVALID','LEASE_EXPIRED','LEASE_ALREADY_SETTLED','WORK_STATE_CHANGED','IDEMPOTENCY_CONFLICT','CLAIM_INDETERMINATE','INVALID_SUCCESSOR','CHECKPOINT_REQUIRED'].includes(code)) return 409;
...
export const workLeaseInternals = Object.freeze({ normalizeClaimRequest, normalizeCheckpointRequest, normalizeSettleRequest, settlementPlan, isExecutable, laneOf, snapshot, publicLease });