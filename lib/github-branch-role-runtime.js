...
import { applyGithubChangeset, applyGithubChangesetWithGitHubApp } from 'lib/github-apply-changeset.js';
...
export async function applyGithubChangesetRoleAware(input, options = {}) {
  const delegate = options.applyChangeset || (options.github ? applyGithubChangeset : applyGithubChangesetWithGitHubApp);
  try {
    const roles = await rolesFor(input?.repo, options);
    assertOrdinaryWorkTarget(input?.branch, roles);
    const runtimeDb = options.db;
    const runtimeOptions = delegate === applyGithubChangesetWithGitHubApp && !options.receipts && !options.kernelManaged && runtimeDb
