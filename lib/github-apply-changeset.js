...
async function finalizePrepared({ normalized, github, plan, commitSha, treeSha, replay, executionAuthority = null }) {
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  try {
    ...
    if (!options.kernelManaged && normalized.idempotency_key) {
      fail('EXECUTION_TRANSACTION_REQUIRED', 'idempotency and settlement are owned by the canonical execution transaction kernel', {
        repo: normalized.repo,
    ...
    return await finalizePrepared({ normalized, github, plan, commitSha, treeSha, replay: false, executionAuthority });
  } catch (error) {
