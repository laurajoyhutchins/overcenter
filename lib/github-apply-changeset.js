...
export async function coalesceGithubMechanicalChangeset(input, options = {}) {
  ...
  try { await github.replaceBranch(repo, branch, expectedHead, commitSha); }
    ...
    if (error instanceof GitHubChangesetError) error.details = { ...(error.details || {}), phase: 'coalesce.ref_replace', may_have_mutated: true, expected_head: expectedHead, replacement_head: commitSha, actual_head: after?.sha || null };
    throw error;
  }
