...
export function createGithubApiAdapter(apiClient, options = {}) {
  ...
  async function replaceBranch(repo, branch, expectedHead, sha) {
    const before = await getBranch(repo, branch, { phase: 'coalesce.ref_fence' });
    if (!before || before.sha !== expectedHead) fail('HEAD_MISMATCH', 'managed workspace changed before mechanical replacement', { branch, expected_head: expectedHead, actual_head: before?.sha || null, phase: 'coalesce.ref_fence', may_have_mutated: false }, 409);
    await call('PATCH', `${repoPath(repo)}/git/refs/heads/${encodeBranchPath(branch)}`, {
      body: { sha, force: true },
  ...
  try {
    ...
    if (branch
      ...
      fail('MECHANICAL_CHANGESET_MUST_COALESCE', 'Consecutive mechanical cleanup changesets must be coalesced into one repair commit.', {
        ...
        recovery: githubMechanicalCoalescingRecovery({ parent_head: parentSha }),
        phase: 'preflight',
      }, 409);
    }
