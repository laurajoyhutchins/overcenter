...
import { ExecutionAuthorityError, createPostgresExecutionAuthorityService } from 'lib/execution-authority.js';
...
async function decodeGzipBase64Utf8(value, field, expectedCompressedSha256 = null) {
  ...
  if (expectedCompressedSha256 !== null) {
    const expected = requiredString(expectedCompressedSha256, `${field}_sha256`, 64).toLowerCase();
    if (!/^[0-9a-f]{64}$/.test(expected)) {
      fail('INVALID_SHA', `${field}_sha256 must be a 64-character hexadecimal SHA-256`, { field: `${field}_sha256` }, 422);
    }
    if (compressedSha256 !== expected) {
      fail('CONTENT_CHECKSUM_MISMATCH', 'compressed changeset content failed its SHA-256 precondition', { field, expected_sha256: expected, actual_sha256: compressedSha256, encoded_length: encoded.length, compressed_length: compressed.length }, 422);
    }
  }
  let decoded;
  try {
...
async function expandEncodedChangeContent(input) {
  ...
  for (let index = 0; index < input.changes.length; index += 1) {
    ...
    const hasSingle = raw && typeof raw === 'object' && !Array.isArray(raw) && raw.content_gzip_base64 !== undefined;
    const hasChunks = raw && typeof raw === 'object' && !Array.isArray(raw) && raw.content_gzip_base64_chunks !== undefined;
    if (!hasSingle && !hasChunks) {
      changes.push(raw);
      continue;
    }
    if (raw.content !== undefined || (hasSingle && hasChunks)) {
      fail('INVALID_OPERATION', 'create/update changes must provide exactly one content transport', { path: raw.path || null, index }, 422);
    }
    let encoded = raw.content_gzip_base64;
    if (hasChunks) {
      if (!Array.isArray(raw.content_gzip_base64_chunks) || raw.content_gzip_base64_chunks.length === 0 || raw.content_gzip_base64_chunks.length > 256) {
        fail('INVALID_REQUEST', `changes[${index}].content_gzip_base64_chunks must be a non-empty array with at most 256 items`, { index }, 422);
      }
      encoded = raw.content_gzip_base64_chunks.map((chunk, chunkIndex) => requiredString(chunk, `changes[${index}].content_gzip_base64_chunks[${chunkIndex}]`, 16_384)).join('');
    }
    const content = await decodeGzipBase64Utf8(encoded, `changes[${index}].content_gzip_base64`, raw.content_gzip_base64_sha256 ?? null);
    const { content_gzip_base64: ignoredSingle, content_gzip_base64_chunks: ignoredChunks, content_gzip_base64_sha256: ignoredSha256, ...rest } = raw;
...
export function normalizeGithubChangesetRequest(input) {
  ...
  exactFields(body, new Set([
    ...
    'commit_message', 'idempotency_key', 'lease_token',
  ]), 'request');
  ...
  const leaseToken = body.lease_token === undefined || body.lease_token === null
    ? null
    : requiredString(body.lease_token, 'lease_token', 256);
...
function durableGithubChangesetRequest(normalized) {
  const { lease_token: ignoredLeaseToken, ...durable } = normalized;
  return durable;
}

async function requestHash(normalized) {
  const { idempotency_key: ignoredIdempotencyKey, lease_token: ignoredLeaseToken, ...semantic } = normalized;
  return sha256Text(canonicalJson(semantic));
}

export async function githubChangesetSemanticRequestHash(input) {
  const normalized = normalizeGithubChangesetRequest(await expandEncodedChangeContent(input));
  return requestHash(normalized);
}
...
export function createGithubApiAdapter(apiClient, options = {}) {
  ...
  async function replaceBranch(repo, branch, expectedHead, sha) {
    const repository = await call('GET', repoPath(repo), { phase: 'coalesce.preflight.repository' });
    const repositoryId = String(repository?.node_id || '');
    if (!repositoryId) {
      fail('GITHUB_INVALID_RESPONSE', 'GitHub repository response did not contain a GraphQL node id', { repo, phase: 'coalesce.preflight.repository', may_have_mutated: false }, 502);
    }
    const body = await call('POST', '/graphql', {
      body: { query: 'mutation($repositoryId:ID!,$refUpdates:[RefUpdate!]!){updateRefs(input:{repositoryId:$repositoryId,refUpdates:$refUpdates}){clientMutationId}}', variables: { repositoryId, refUpdates: [{ name: `refs/heads/${branch}`, beforeOid: expectedHead, afterOid: sha, force: true }] } },
      phase: 'mutation.ref_replace', retrySafeRead: false, mayHaveMutated: true,
    });
    if (Array.isArray(body?.errors) && body.errors.length) fail('GITHUB_REF_REJECTED', String(body.errors[0]?.message || 'GitHub rejected atomic ref replacement'), { branch, expected_head: expectedHead, replacement_head: sha, phase: 'mutation.ref_replace', may_have_mutated: false }, 409);
  }

  return { resolveCommit, getBranch, getCommit, getPathEntries, createTree, createCommit, createBranch, updateBranch, replaceBranch };
}
...
export function createGithubChangesetReceiptStore(db) {
  return {
    async claim(normalized, digest, attemptToken) {
      const inserted = await db.query(
        `INSERT INTO github_changeset_receipts (
         ...
         RETURNING *`,
        [normalized.repo, normalized.idempotency_key, digest, canonicalJson(durableGithubChangesetRequest(normalized)), attemptToken, normalized.branch],
      );
    ...
    async succeed(normalized, attemptToken, receipt) {
      const updated = await db.query(
        `UPDATE github_changeset_receipts
            SET state = 'succeeded', receipt = $4::jsonb, updated_at = now()
          WHERE repo = $1 AND idempotency_key = $2
            AND attempt_token = $3::uuid
         RETURNING repo`,
        [normalized.repo, normalized.idempotency_key, attemptToken, canonicalJson(receipt)],
      );
...
function receiptFromPlan(normalized, plan, commitSha, treeSha, idempotentReplay, executionAuthority = null) {
  return {
    ok: true,
    ...
    ...(executionAuthority ? { execution_authority: executionAuthority } : {}),
...
function normalizeUnexpectedGithubChangesetError(error, phase, mutated = false) {
  if (error instanceof GitHubChangesetError) return error;
  const boundedPhase = typeof phase === 'string' && phase.trim()
    ? phase.trim().slice(0, 128)
    : 'changeset';
  return new GitHubChangesetError(
    'GITHUB_CHANGESET_UNEXPECTED_ERROR',
    'unexpected GitHub changeset provider failure',
    {
      phase: boundedPhase,
      error_name: typeof error?.name === 'string' ? error.name.slice(0, 128) : null,
      may_have_mutated: Boolean(mutated),
    },
    502,
  );
}

async function githubChangesetPhase(phase, mutated, operation) {
  try {
    return await operation();
  } catch (error) {
    throw normalizeUnexpectedGithubChangesetError(error, phase, mutated);
  }
}
...
async function settleChangesetReceipt(receipts, normalized, attemptToken, receipt) {
  if (!receipts || !normalized.idempotency_key) return;
  const settled = await receipts.succeed(normalized, attemptToken, receipt);
  if (settled === false) {
    fail('GITHUB_CHANGESET_RECEIPT_FENCE_LOST', 'changeset completion lost attempt-token authority', {
      repo: normalized.repo,
...
async function finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay, executionAuthority = null }) {
  const current = await githubChangesetPhase('finalize.branch_read', true, () =>
    github.getBranch(normalized.repo, normalized.branch, { phase: 'finalize.branch_read' }));
  if (current?.sha === commitSha) {
    const recovered = receiptFromPlan(normalized, plan, commitSha, treeSha, true, executionAuthority);
    await settleChangesetReceipt(receipts, normalized, attemptToken, recovered);
    return recovered;
  }
  ...
  try {
    ...
    const mutationError = normalizeUnexpectedGithubChangesetError(error, 'mutation.ref_update', true);
    let after;
    try {
      after = await githubChangesetPhase('reconcile.ref_readback', true, () =>
        github.getBranch(normalized.repo, normalized.branch, { phase: 'reconcile.ref_readback' }));
    } catch (reconciliationError) {
      const normalizedReconciliationError = normalizeUnexpectedGithubChangesetError(
        reconciliationError,
        'reconcile.ref_readback',
        true,
      );
      const certainty = mutationCertaintyFromEvidence(mutationError, 'possible');
      const mutationDetails = mutationError.details && typeof mutationError.details === 'object' ? mutationError.details : {};
      const reconciliationDetails = normalizedReconciliationError.details && typeof normalizedReconciliationError.details === 'object'
        ? normalizedReconciliationError.details
        : {};
      mutationError.details = Object.freeze({
        ...mutationDetails,
        may_have_mutated:mayHaveMutated(certainty),
        mutation_certainty:certainty,
        reconciliation_error:Object.freeze({
          code:normalizedReconciliationError.code,
          message:normalizedReconciliationError.message,
  ...
  const receipt = receiptFromPlan(normalized, plan, commitSha, treeSha, replay, executionAuthority);
  await settleChangesetReceipt(receipts, normalized, attemptToken, receipt);
  return receipt;
}
...
export async function coalesceGithubMechanicalChangeset(input, options = {}) {
  const body = object(input, 'request');
  exactFields(body, new Set(['repo', 'branch', 'expected_head', 'changes', 'commit_message']), 'request');
  const repo = validateRepo(body.repo);
  const branch = validateBranch(body.branch);
  const expectedHead = validateSha(body.expected_head, 'expected_head');
  const commitMessage = requiredString(body.commit_message, 'commit_message', 10000);
  if (!isMechanicalCleanupMessage(commitMessage)) fail('MECHANICAL_COALESCE_REQUIRES_MECHANICAL_CHANGESET', 'coalescing only accepts a mechanical cleanup changeset', { phase: 'preflight' }, 422);
  const normalized = normalizeGithubChangesetRequest({ repo, base_sha: expectedHead, branch, expected_head: expectedHead, changes: body.changes, commit_message: commitMessage });
  const github = options.github || createGithubApiAdapter(options.apiClient);
  if (typeof github.replaceBranch !== 'function') fail('GITHUB_TRANSPORT_UNAVAILABLE', 'mechanical coalescing requires exact branch replacement support', null, 500);
  const current = await github.getBranch(repo, branch, { phase: 'coalesce.preflight.branch' });
  const actualHead = current?.sha ? String(current.sha).toLowerCase() : null;
  if (actualHead !== expectedHead) fail('HEAD_MISMATCH', 'managed workspace head changed before mechanical coalescing', { expected_head: expectedHead, actual_head: actualHead, branch, phase: 'coalesce.preflight', may_have_mutated: false }, 409);
  const parent = await github.getCommit(repo, expectedHead);
  if (!isMechanicalCleanupMessage(parent.message)) fail('MECHANICAL_COALESCE_PARENT_NOT_MECHANICAL', 'the exact workspace head is not a mechanical cleanup commit', { expected_head: expectedHead, parent_message: parent.message, phase: 'coalesce.preflight', may_have_mutated: false }, 409);
  if (!Array.isArray(parent.parents) || parent.parents.length !== 1 || !SHA40.test(String(parent.parents[0] || '').toLowerCase())) fail('MECHANICAL_COALESCE_PARENT_UNSUPPORTED', 'mechanical coalescing requires one exact linear parent', { expected_head: expectedHead, parent_count: Array.isArray(parent.parents) ? parent.parents.length : null, phase: 'coalesce.preflight', may_have_mutated: false }, 409);
  const grandparentSha = String(parent.parents[0]).toLowerCase();
  const paths = normalized.changes.map(change => change.path);
  const existing = await github.getPathEntries(repo, parent.tree_sha, paths);
  validateExistingTargets(normalized.changes, existing);
  const treeEntries = normalized.changes.map(change => { const entry = existing.get(change.path) || null; if (change.operation === 'delete') return { path: change.path, mode: entry.mode, type: 'blob', sha: null }; return { path: change.path, mode: entry?.mode || '100644', type: 'blob', content: change.content }; });
  const treeSha = await github.createTree(repo, parent.tree_sha, treeEntries);
  const commitSha = await github.createCommit(repo, { message: commitMessage, treeSha, parentSha: grandparentSha });
  try { await github.replaceBranch(repo, branch, expectedHead, commitSha); }
  catch (error) {
    const mutationError = normalizeUnexpectedGithubChangesetError(error, 'coalesce.ref_replace', true);
    let after = null;
    try { after = await github.getBranch(repo, branch, { phase: 'coalesce.reconcile.branch' }); } catch { }
    if (after?.sha === commitSha) return { ok: true, repo, branch, old_head: expectedHead, new_head: commitSha, commit_sha: commitSha, tree_sha: treeSha, replaced_parent: expectedHead, parent_sha: grandparentSha, idempotent_replay: true };
    mutationError.details = { ...(mutationError.details || {}), phase: 'coalesce.ref_replace', may_have_mutated: true, expected_head: expectedHead, replacement_head: commitSha, actual_head: after?.sha || null };
    throw mutationError;
  }
  return { ok: true, repo, branch, old_head: expectedHead, new_head: commitSha, commit_sha: commitSha, tree_sha: treeSha, replaced_parent: expectedHead, parent_sha: grandparentSha, idempotent_replay: false };
}
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  try {
    ...
    if (normalized.idempotency_key) {
      ...
      if (claim.kind === 'claimed' && claim.row?.attempt_token) attemptToken = claim.row.attempt_token;
      if (claim.kind === 'existing' && claim.row.state === 'prepared') {
        preparedClaim = claim.row;
        attemptToken = claim.row.attempt_token;
      }
    }

    if (options.executionAuthority) {
      try {
        executionAuthority = await options.executionAuthority.require({
          lease_token: normalized.lease_token,
          repository: normalized.repo,
          allowed_gates: ['lane:repo-implementation'],
        });
      } catch (error) {
        if (error instanceof ExecutionAuthorityError || String(error?.code || '').startsWith('EXECUTION_AUTHORITY_')) {
          fail(error.code || 'EXECUTION_AUTHORITY_INVALID', String(error?.message || 'execution authority is invalid'), error?.details || null, error?.httpStatus || 409);
        }
        throw error;
      }
    }
    ...
    if (preparedClaim) {
      prepared = true;
      const plan = planFromRow(preparedClaim);
      return await finalizePrepared({
        ...
        attemptToken,
        plan,
        commitSha: preparedClaim.commit_sha,
        treeSha: preparedClaim.tree_sha,
        replay: true,
        executionAuthority,
      });
    }
    ...
    const requestedBase = await githubChangesetPhase('preflight.resolve_base', false, () =>
      github.resolveCommit(normalized.repo, selector));
    const branch = await githubChangesetPhase('preflight.branch_read', false, () =>
      github.getBranch(normalized.repo, normalized.branch));
    ...
    const parent = parentSha === requestedBase.sha
      ? requestedBase
      : await githubChangesetPhase('preflight.parent_commit', false, () =>
        github.getCommit(normalized.repo, parentSha));
    if (branch
    ...
    const paths = normalized.changes.map(change => change.path);
    const existing = await githubChangesetPhase('preflight.tree_read', false, () =>
      github.getPathEntries(normalized.repo, parent.tree_sha, paths));
    validateExistingTargets(normalized.changes, existing);
    if (receipts && normalized.idempotency_key) {
      const fenceHeld = await receipts.heartbeat(normalized, attemptToken, 'preflight_complete');
      if (fenceHeld !== true) {
        fail('GITHUB_CHANGESET_ATTEMPT_FENCE_LOST', 'changeset provider effect requires a durably recorded owning attempt', {
          repo: normalized.repo,
          idempotency_key: normalized.idempotency_key,
    ...
    const treeSha = await githubChangesetPhase('mutation.tree', true, () =>
      github.createTree(normalized.repo, parent.tree_sha, treeEntries));
    if (receipts && normalized.idempotency_key) await receipts.saveTree(normalized, attemptToken, treeSha);
    const commitSha = await githubChangesetPhase('mutation.commit', true, () =>
      github.createCommit(normalized.repo, {
    ...
    return await finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay: false, executionAuthority });
  } catch (error) {
...
export function classifyGithubAppChangesetAuthError(error, permissionProfile = 'changeset') {
  ...
  if (permissionProfile === 'workflow_changeset'
      && status === 422
      && /permissions requested are not granted|workflow/i.test(message)) {
    return {
      ok: false,
      error: 'GITHUB_WORKFLOWS_PERMISSION_REQUIRED',
      message: 'The GitHub App installation must grant Workflows write before mutating .github/workflows/**.',
      upstream_status: 422,
      ...transportEvidence,
    };
  }
...
export async function applyGithubChangesetWithGitHubApp(input, options = {}) {
  const permissionProfile = githubAppChangesetPermissionProfile(
    (Array.isArray(input?.changes) ? input.changes : []).map(change => change?.path),
  );
  const withApp = options.withGitHubAppApiClient;
  if (typeof withApp !== 'function') {
    return classifyGithubAppChangesetAuthError(
      Object.assign(new Error('GitHub auth provider is unavailable'), { code:'RUNTIME_PROVIDER_MISSING' }),
      permissionProfile,
    );
  }
  try {
    ...
    return await withApp(input?.repo, async (apiClient) => {
      return applyGithubChangeset(input, { ...options, apiClient, executionAuthority });
    }, { permissionProfile });
  } catch (error) {
    return classifyGithubAppChangesetAuthError(error, permissionProfile);
  }
}