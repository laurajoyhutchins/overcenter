...
export function normalizeGithubChangesetRequest(input) {
  ...
  exactFields(body, new Set([
    ...
    'commit_message', 'idempotency_key', 'lease_token', 'coalesce_mechanical_head',
  ]), 'request');
  ...
  const coalesceMechanicalHead = body.coalesce_mechanical_head === undefined ? false : body.coalesce_mechanical_head;
  if (typeof coalesceMechanicalHead !== 'boolean') fail('INVALID_REQUEST', 'coalesce_mechanical_head must be a boolean', { field: 'coalesce_mechanical_head' }, 422);
  ...
  return {
    ...
    ...(coalesceMechanicalHead ? { coalesce_mechanical_head: true } : {}),
  };
}
...
export function createGithubApiAdapter(apiClient, options = {}) {
  ...
  async function updateBranch(repo, branch, sha, options = {}) {
    await call('PATCH', `${repoPath(repo)}/git/refs/heads/${encodeBranchPath(branch)}`, {
      body: { sha, force: options.force === true },
      phase: 'mutation.ref_update',
      retrySafeRead: false,
      mayHaveMutated: true,
    });
  ...
  return {
    ...
    async findSucceededByCommit(repo, branch, commitSha) {
      return (await db.query(
        `SELECT * FROM github_changeset_receipts
...
async function finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay, executionAuthority = null }) {
  ...
  try {
    ...
    else await github.updateBranch(normalized.repo, normalized.branch, commitSha, { force: normalized.coalesce_mechanical_head === true });
  } catch (error) {
    if (!(error instanceof GitHubChangesetError)) throw error;
    let after;
    try {
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  try {
    ...
    const consecutiveMechanical = branch
      && isMechanicalCleanupMessage(normalized.commit_message)
      && isMechanicalCleanupMessage(parent.message);
    let commitParentSha = parentSha;
    if (consecutiveMechanical && !normalized.coalesce_mechanical_head) {
      fail('MECHANICAL_CHANGESET_MUST_COALESCE', 'Consecutive mechanical cleanup changesets must be coalesced into one repair commit.', {
        ...
        phase: 'preflight',
        recovery_operation: { command: 'github.apply_changeset', set: { coalesce_mechanical_head: true } },
      }, 409);
    }
    if (normalized.coalesce_mechanical_head) {
      if (!normalized.expected_head || normalized.expected_head !== parentSha) fail('MECHANICAL_CHANGESET_COALESCE_HEAD_FENCE_REQUIRED', 'coalescing requires an exact expected_head matching the current mechanical head', { expected_head: normalized.expected_head, actual_head: parentSha, may_have_mutated: false }, 409);
      if (!consecutiveMechanical || !branch) fail('MECHANICAL_CHANGESET_COALESCE_PRECONDITION_FAILED', 'coalescing requires the exact current head to be a mechanical cleanup commit', { branch: normalized.branch, parent_head: parentSha, may_have_mutated: false }, 409);
      if (!receipts || !normalized.idempotency_key || typeof receipts.findSucceededByCommit !== 'function') fail('MECHANICAL_CHANGESET_COALESCE_AUTHORITY_UNAVAILABLE', 'coalescing requires durable changeset receipt authority', { may_have_mutated: false }, 409);
      const priorRow = await receipts.findSucceededByCommit(normalized.repo, normalized.branch, parentSha);
      const priorReceipt = parseJson(priorRow?.receipt);
      if (!priorRow || priorReceipt?.execution_authority?.lease_id !== executionAuthority?.lease_id) {
        fail('MECHANICAL_CHANGESET_COALESCE_AUTHORITY_MISMATCH', 'current mechanical head was not produced by the same execution lease', { branch: normalized.branch, parent_head: parentSha, may_have_mutated: false }, 409);
      }
      if (!Array.isArray(parent.parents) || parent.parents.length !== 1 || !SHA40.test(String(parent.parents[0] || '').toLowerCase())) fail('MECHANICAL_CHANGESET_COALESCE_PARENT_INVALID', 'current mechanical head does not have one exact parent', { parent_head: parentSha, may_have_mutated: false }, 409);
      commitParentSha = String(parent.parents[0]).toLowerCase();
    }
    const changedPaths = normalized.changes.map(change => ({ path: change.path, operation: change.operation }));
    const plan = {
      baseSha: commitParentSha,
