...
export function normalizeGithubChangesetRequest(input) {
  ...
  exactFields(body, new Set([
    ...
    'commit_message', 'idempotency_key', 'lease_token', 'coalesce_previous_mechanical',
  ]), 'request');
  ...
  const coalescePreviousMechanical = body.coalesce_previous_mechanical === undefined
    ? false
    : body.coalesce_previous_mechanical;
  if (typeof coalescePreviousMechanical !== 'boolean') {
    fail('INVALID_REQUEST', 'coalesce_previous_mechanical must be a boolean', { field: 'coalesce_previous_mechanical' }, 422);
  }
  ...
  return {
    ...
    coalesce_previous_mechanical: coalescePreviousMechanical,
  };
}
...
async function requestHash(normalized) {
  ...
  if (semantic.coalesce_previous_mechanical !== true) delete semantic.coalesce_previous_mechanical;
...
async function finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay, executionAuthority = null }) {
  ...
  try {
    ...
    else if (normalized.coalesce_previous_mechanical) {
      if (typeof github.replaceBranch !== 'function') {
        fail('MECHANICAL_CHANGESET_COALESCE_UNAVAILABLE', 'GitHub transaction adapter does not support bounded mechanical-head replacement.', { phase: 'finalize', may_have_mutated: false }, 503);
      }
      await github.replaceBranch(normalized.repo, normalized.branch, commitSha);
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  try {
    ...
    const consecutiveMechanical = Boolean(branch
    ...
    if (consecutiveMechanical) {
      if (!normalized.coalesce_previous_mechanical) {
        fail('MECHANICAL_CHANGESET_MUST_COALESCE', 'Consecutive mechanical cleanup changesets must be coalesced into one repair commit.', {
          ...
          recovery_command: 'github.apply_changeset',
          recovery_fields: { coalesce_previous_mechanical: true },
          phase: 'preflight',
          may_have_mutated: false,
        }, 409);
      }
      if (!Array.isArray(parent.parents) || parent.parents.length !== 1 || !SHA40.test(parent.parents[0])) {
        fail('MECHANICAL_CHANGESET_COALESCE_PARENT_INVALID', 'Mechanical coalescing requires the current mechanical head to have exactly one parent.', {
          branch: normalized.branch,
          parent_head: parentSha,
          parent_count: Array.isArray(parent.parents) ? parent.parents.length : null,
          phase: 'preflight',
          may_have_mutated: false,
        }, 409);
      }
      commitParentSha = parent.parents[0];
    } else if (normalized.coalesce_previous_mechanical) {
      fail('MECHANICAL_CHANGESET_COALESCE_NOT_APPLICABLE', 'Mechanical coalescing is only valid when the exact current workspace head is itself a mechanical cleanup commit.', {
        branch: normalized.branch,
        parent_head: parentSha,
        parent_message: parent.message || '',
        phase: 'preflight',
