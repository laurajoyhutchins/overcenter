...
async function finalizePrepared({ normalized, github, receipts, plan, commitSha, treeSha, replay, executionAuthority = null }) {
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  try {
    ...
    if (normalized.idempotency_key) {
      ...
      if (claim.kind === 'existing' && claim.row.state === 'prepared') preparedClaim = claim.row;
    }
    ...
    return await finalizePrepared({ normalized, github, receipts, plan, commitSha, treeSha, replay: false, executionAuthority });
  } catch (error) {
