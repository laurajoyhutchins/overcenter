...
async function finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay, executionAuthority = null }) {
  ...
  try {
    ...
    let after;
    try {
      after = await github.getBranch(normalized.repo, normalized.branch, { phase: 'reconcile.ref_readback' });
    } catch (reconciliationError) {
      if (!(reconciliationError instanceof GitHubChangesetError)) throw reconciliationError;
      const certainty = mutationCertaintyFromEvidence(error, 'possible');
      const mutationDetails = error.details && typeof error.details === 'object' ? error.details : {};
      const reconciliationDetails = reconciliationError.details && typeof reconciliationError.details === 'object'
