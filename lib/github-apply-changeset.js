...
export function createGithubApiAdapter(apiClient, options = {}) {
  ...
  async function replaceBranch(repo, branch, expectedHead, sha) {
    ...
    if (!repositoryId) {
      fail('GITHUB_INVALID_RESPONSE', 'GitHub repository response did not contain a GraphQL node id', {
        repo,
        phase: 'coalesce.preflight.repository',
    ...
    const body = await call('POST', '/graphql', {
      ...
      phase: 'mutation.ref_replace',
      retrySafeRead: false,
      mayHaveMutated: true,
    });
    if (Array.isArray(body?.errors) && body.errors.length) {
      fail('GITHUB_REF_REJECTED', String(body.errors[0]?.message || 'GitHub rejected atomic ref replacement'), {
        branch,
        expected_head: expectedHead,
        replacement_head: sha,
        phase: 'mutation.ref_replace',
        may_have_mutated: false,
      }, 409);
    }
...
export async function coalesceGithubMechanicalChangeset(input, options = {}) {
  ...
  const commitMessage = requiredString(body.commit_message, 'commit_message', 10000);
  if (!isMechanicalCleanupMessage(commitMessage)) {
    fail('MECHANICAL_COALESCE_REQUIRES_MECHANICAL_CHANGESET', 'coalescing only accepts a mechanical cleanup changeset', { phase: 'preflight' }, 422);
  }
  ...
  try { await github.replaceBranch(repo, branch, expectedHead, commitSha); }
    ...
    let after = null;
    try { after = await github.getBranch(repo, branch, { phase: 'coalesce.reconcile.branch' }); } catch { /* preserve mutation uncertainty */ }
