...
async function expandEncodedChangeContent(input) {
  ...
  for (let index = 0; index < input.changes.length; index += 1) {
    ...
    const content = await decodeGzipBase64Utf8(encoded, `changes[${index}].content_gzip_base64`);
    const { content_gzip_base64: ignoredSingle, content_gzip_base64_chunks: ignoredChunks, ...rest } = raw;
