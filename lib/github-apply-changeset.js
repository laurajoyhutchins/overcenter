...
export function normalizeGithubChangesetRequest(input) {
  ...
  return {
    ...
    ...(coalescePreviousMechanical ? { coalesce_previous_mechanical: true } : {}),
  };
}
...
export function createGithubApiAdapter(apiClient, options = {}) {
  ...
  async function updateBranch(repo, branch, sha, updateOptions = {}) {
    await call('PATCH', `${repoPath(repo)}/git/refs/heads/${encodeBranchPath(branch)}`, {
      body: { sha, force: updateOptions.force === true },
      phase: 'mutation.ref_update',
      retrySafeRead: false,
      mayHaveMutated: true,
    });
...
async function finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay, executionAuthority = null }) {
  ...
  try {
    if (plan.createdBranch) await github.createBranch(normalized.repo, normalized.branch, commitSha);
      ...
      force: normalized.coalesce_previous_mechanical === true,
    });
  } catch (error) {
    if (!(error instanceof GitHubChangesetError)) throw error;
    let after;
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  try {
    ...
    if (consecutiveMechanical) {
      if (normalized.coalesce_previous_mechanical !== true) {
        fail('MECHANICAL_CHANGESET_MUST_COALESCE', 'Consecutive mechanical cleanup changesets must be coalesced into one repair commit.', {
          ...
          phase: 'preflight',
          recovery_operation: 'github.apply_changeset',
          recovery_requires_same_lease: true,
        }, 409);
      }
      const authorization = options.coalesceAuthorization;
      if (!authorization || authorization.expectedHead !== parentSha) {
        fail('MECHANICAL_CHANGESET_COALESCE_AUTHORITY_REQUIRED', 'Mechanical changeset coalescing requires proof that the current head was produced by the same lease.', {
          branch: normalized.branch,
          parent_head: parentSha,
          phase: 'preflight',
      ...
      if (!Array.isArray(parent.parents) || parent.parents.length !== 1 || !SHA40.test(String(parent.parents[0] || ''))) {
        fail('MECHANICAL_CHANGESET_COALESCE_PARENT_INVALID', 'Mechanical changeset coalescing requires one exact parent commit.', {
          branch: normalized.branch,
          parent_head: parentSha,
          phase: 'preflight',
          may_have_mutated: false,
    ...
    } else if (normalized.coalesce_previous_mechanical === true) {
      fail('MECHANICAL_CHANGESET_COALESCE_NOT_APPLICABLE', 'Mechanical coalescing is allowed only for two consecutive mechanical cleanup commits.', {
        branch: normalized.branch,
        parent_head: parentSha,
        phase: 'preflight',
        may_have_mutated: false,
