...
export function normalizeGithubChangesetRequest(input) {
  ...
  exactFields(body, new Set([
    ...
    'commit_message', 'idempotency_key', 'lease_token', 'coalesce_mechanical',
  ]), 'request');
  ...
  const coalesceMechanical = body.coalesce_mechanical === undefined ? false : body.coalesce_mechanical;
  if (typeof coalesceMechanical !== 'boolean') fail('INVALID_REQUEST', 'coalesce_mechanical must be a boolean', { field:'coalesce_mechanical' }, 422);
  if (coalesceMechanical && !expectedHead) fail('MECHANICAL_CHANGESET_COALESCE_PRECONDITION_FAILED', 'mechanical coalescing requires expected_head to bind the replacement to the exact current workspace head', { phase:'preflight', may_have_mutated:false }, 409);
  ...
  return {
    ...
    coalesce_mechanical: coalesceMechanical,
  };
}
...
async function finalizePrepared({ normalized, github, receipts, attemptToken, plan, commitSha, treeSha, replay, executionAuthority = null }) {
  ...
  try {
    ...
    else await github.updateBranch(normalized.repo, normalized.branch, commitSha, { force: normalized.coalesce_mechanical === true });
  } catch (error) {
    if (!(error instanceof GitHubChangesetError)) throw error;
    let after;
    try {
  ...
  try {
    ...
    if (mechanicalPair && !normalized.coalesce_mechanical) {
      fail('MECHANICAL_CHANGESET_MUST_COALESCE', 'Consecutive mechanical cleanup changesets must be coalesced into one repair commit.', {
        ...
        recovery: { command:'github.apply_changeset', input:{ coalesce_mechanical:true } },
      }, 409);
    }
    if (normalized.coalesce_mechanical && !mechanicalPair) {
      fail('MECHANICAL_CHANGESET_COALESCE_PRECONDITION_FAILED', 'mechanical coalescing is only valid for an exact mechanical head followed by a mechanical cleanup', {
        branch: normalized.branch,
        parent_head: parentSha,
        parent_message: parent.message,
        commit_message: normalized.commit_message,
    ...
    const commitParentSha = normalized.coalesce_mechanical ? parent.parents?.[0] : parentSha;
    if (normalized.coalesce_mechanical && !SHA40.test(String(commitParentSha || ''))) {
      fail('MECHANICAL_CHANGESET_COALESCE_PRECONDITION_FAILED', 'mechanical head must have exactly one valid parent to coalesce safely', { branch:normalized.branch, parent_head:parentSha, phase:'preflight', may_have_mutated:false }, 409);
    }
