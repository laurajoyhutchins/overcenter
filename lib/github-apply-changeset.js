...
function validateBaseRef(value) {
  ...
  if (/[/\\\u0000-\u001f\u007f]/.test(ref) && !ref.includes('/')) {
    fail('INVALID_BASE_REF', 'base_ref contains invalid control characters', { base_ref: ref }, 422);
  }
  if (/\u0000|[\u0001-\u001f\u007f]/.test(ref)) {
    fail('INVALID_BASE_REF', 'base_ref contains invalid control characters', { base_ref: ref }, 422);
  }
...
export function normalizeGithubChangesetRequest(input) {
  ...
  const changes = body.changes.map((raw, index) => {
    ...
    exactFields(change, new Set(['path', 'operation', 'content']), `changes[${index}]`);
    ...
    if (operation === 'delete') {
      if (change.content !== undefined) {
        fail('INVALID_OPERATION', 'delete changes must not include content', { path, operation }, 422);
      }
    ...
    return { path, operation, content: validateUnicodeText(change.content, `changes[${index}].content`) };
...
function setupRequired(error) {
  ...
  return code === 'SetupRequired'
    ...
    || (message.includes('412') && message.includes('API "github" is not connected'));
}
...
export function createGithubApiAdapter(apiClient = api) {
  async function call(method, path, { body = undefined, query = undefined, allow404 = false } = {}) {
    let response;
    try {
      ...
      if (setupRequired(error)) {
        fail('GITHUB_SETUP_REQUIRED', 'Connect GitHub in the Hatchable project Setup page before using this command.', null, 412);
      }
      throw error;
    }
  ...
  async function resolveCommit(repo, selector) {
    ...
    const body = await call('GET', `${base}/commits/${encodePathComponent(selector)}`);
    ...
    if (!SHA40.test(sha) || !SHA40.test(treeSha)) {
      fail('GITHUB_INVALID_RESPONSE', 'GitHub commit response did not contain a full commit and tree SHA', null, 502);
    }
    return { sha, tree_sha: treeSha };
  }

  async function getBranch(repo, branch) {
    const body = await call('GET', `${repoPath(repo)}/git/ref/heads/${encodeBranchPath(branch)}`, { allow404: true });
    if (!body) return null;
    const sha = String(body?.object?.sha || '').toLowerCase();
    if (!SHA40.test(sha)) fail('GITHUB_INVALID_RESPONSE', 'GitHub branch response did not contain a full commit SHA', { branch }, 502);
    return { sha };
  }

  async function getCommit(repo, sha) {
    const body = await call('GET', `${repoPath(repo)}/git/commits/${encodePathComponent(sha)}`);
    ...
    if (!SHA40.test(commitSha) || !SHA40.test(treeSha)) {
      fail('GITHUB_INVALID_RESPONSE', 'GitHub Git commit response was incomplete', { sha }, 502);
    }
    return {
      sha: commitSha,
      tree_sha: treeSha,
  ...
  async function getPathEntries(repo, treeSha, paths) {
    const base = repoPath(repo);
    const recursive = await call('GET', `${base}/git/trees/${encodePathComponent(treeSha)}`, { query: { recursive: '1' } });
    const map = new Map();
    ...
    async function treeEntries(sha) {
      if (!treeCache.has(sha)) {
        const body = await call('GET', `${base}/git/trees/${encodePathComponent(sha)}`);
  ...
  async function createBlob(repo, content) {
    const body = await call('POST', `${repoPath(repo)}/git/blobs`, {
      body: { content, encoding: 'utf-8' },
    });
    const sha = String(body?.sha || '').toLowerCase();
    if (!SHA40.test(sha)) fail('GITHUB_INVALID_RESPONSE', 'GitHub blob creation returned an invalid SHA', null, 502);
    return sha;
  }
  ...
  async function createTree(repo, baseTreeSha, entries) {
    ...
    const sha = String(body?.sha || '').toLowerCase();
    if (!SHA40.test(sha)) fail('GITHUB_INVALID_RESPONSE', 'GitHub tree creation returned an invalid SHA', null, 502);
    return sha;
  }
  ...
  async function createCommit(repo, { message, treeSha, parentSha }) {
    ...
    const sha = String(body?.sha || '').toLowerCase();
    if (!SHA40.test(sha)) fail('GITHUB_INVALID_RESPONSE', 'GitHub commit creation returned an invalid SHA', null, 502);
    return sha;
  }
  ...
  return { resolveCommit, getBranch, getCommit, getPathEntries, createBlob, createTree, createCommit, createBranch, updateBranch };
}
...
async function finalizePrepared({ normalized, github, receipts, plan, commitSha, treeSha, replay }) {
  const current = await github.getBranch(normalized.repo, normalized.branch);
  ...
  try {
    ...
    if (!(error instanceof GitHubChangesetError)) throw error;
    const after = await github.getBranch(normalized.repo, normalized.branch);
...
export async function applyGithubChangeset(input, options = {}) {
  ...
  let prepared = false;
  try {
    normalized = normalizeGithubChangesetRequest(input);
    const github = options.github || createGithubApiAdapter(options.apiClient || api);
    ...
    for (const change of normalized.changes) {
      ...
      const blobSha = await github.createBlob(normalized.repo, change.content);
      treeEntries.push({
        path: change.path,
