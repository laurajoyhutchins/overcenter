...
async function appendProofAndSettle(store, intent, identity, attempt_epoch, facts, disposition) {
    ...
    const evidence = evidenceFor(facts);
...
async function confirmAndSettle(store, provider, intent, identity, attempt_epoch) {
    let confirmation;
    try {
...
export async function recoverExecutionTransaction(input) {
    ...
    if (decision === 'confirm_only' ||
        ...
        return confirmAndSettle(input.store, input.provider, input.intent, claimedIdentity, Math.max(1, claim.snapshot.attempt_epoch));
    }
