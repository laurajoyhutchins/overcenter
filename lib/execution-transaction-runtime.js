...
import { classifyRecovery, mutationCertaintyFromFacts, } from './execution-transaction.js';
...
async function identityFor(intent, context) {
    ...
    return {
        ...
        transition_fingerprint: requiredText(intent.authority.transition_fingerprint, 'authority.transition_fingerprint'),
        operation_kind: requiredText(intent.operation.kind, 'operation.kind'),
        idempotency_scope: requiredText(intent.operation.idempotency_scope, 'operation.idempotency_scope'),
        idempotency_key: requiredText(intent.operation.idempotency_key, 'operation.idempotency_key'),
        intent_sha256,
    };
}
function responseSha(facts) {
...
function predicateFor(facts) {
    return 'status' in facts ? facts.predicate : 'provider-invocation';
}
function unknownInvocation(error) {
    return {
        transport: 'unknown',
        committed: null,
        effect_ref: null,
...
async function requestHash(intent, identity, attempt_epoch) {
...
async function appendProofAndSettle(store, intent, identity, attempt_epoch, facts, disposition) {
...
async function confirmAndSettle(store, provider, intent, identity, attempt_epoch, effect_ref) {
    let confirmation;
    try {
    ...
    return appendProofAndSettle(store, intent, identity, attempt_epoch, confirmation, confirmation.status === 'confirmed' ? 'completed' : 'no_effect');
}
async function executeClaimed(store, provider, intent, identity, context) {
    ...
    return appendProofAndSettle(store, intent, identity, attempt_epoch, invocation, invocationCertainty === 'confirmed_mutated' ? 'completed' : 'no_effect');
}
export async function executeExecutionTransaction(input) {
...
export async function recoverExecutionTransaction(input) {
    ...
    if (!snapshot)
        return fail('EXECUTION_NOT_FOUND', 'execution cannot be recovered');
    ...
    const identity = { ...snapshot.identity, lease_ref, lease_epoch };
    const claim = await input.store.claimExecution({
    ...
    if (decision === 'confirm_only' ||
        ...
        return confirmAndSettle(input.store, input.provider, input.intent, claimedIdentity, Math.max(1, claim.snapshot.attempt_epoch), claim.snapshot.effect_ref);
    }
