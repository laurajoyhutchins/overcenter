...
function extractPkcs1(privateKeyPem) {
  ...
  // Hatchable's secret editor may preserve the PEM body while omitting the
  // BEGIN/END wrapper. Accept bare DER-as-base64 and identify PKCS#1 vs PKCS#8
  // from the first child after the version INTEGER.
  if (!pkcs8 && normalized && !normalized.includes("-----BEGIN")) {
    ...
    const version = readDer(bare, offset, 0x02); offset = version.next;
    if (bare[offset] === 0x02) return bare; // PKCS#1: modulus INTEGER follows version.
    if (bare[offset] === 0x30) pkcs8 = bare; // PKCS#8: AlgorithmIdentifier SEQUENCE follows.
  }
...
async function githubRawRequest(path, { token, method = "GET", body, query, headers = {} } = {}) {
  ...
  const response = await fetch(url.toString(), {
    ...
    headers: {
      ...
      ...headers,
      ...(body === undefined ? {} : { "Content-Type": headers["Content-Type"] || "application/json" })
    },
...
async function createInstallationToken(repo) {
...
export async function withGitHubAppApiClient(repoInput, callback) {
  const repo = validateRepo(repoInput);
  const auth = await createInstallationToken(repo);
  const apiClient = {
    async call(name, options = {}) {
      if (name !== "github") throw new Error(`GitHub App POC client cannot call API ${name}.`);
      return githubRawRequest(options.path, {
        token: auth.token,
  ...
  try {
    return await callback(apiClient, {
      ...
      installation_account: auth.installation.body?.account?.login || null,
      granted_permissions: auth.tokenResponse.body?.permissions || null,
      token_expires_at: auth.tokenResponse.body?.expires_at || null
    });
  } finally {
    try {
      await githubRequest("/installation/token", { token: auth.token, method: "DELETE" });
    } catch {
      // Token is short-lived even if explicit revocation fails. Never persist or return it.
    }
  }
}
