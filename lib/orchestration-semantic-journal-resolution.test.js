...
import { createSemanticJournalInvocationResolver } from 'lib/orchestration-semantic-journal-resolution.js';
...
export async function runOrchestrationSemanticJournalResolutionTests() {
  ...
  await test('production release probes resolve as definitively not applied without another GitHub mutation', async () => {
    // Resolver imported statically for Hatchable module compatibility.
    const resolutions = [];
