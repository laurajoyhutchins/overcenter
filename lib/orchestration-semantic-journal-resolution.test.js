...
async function target() {
  try { return await import('lib/orchestration-semantic-journal-resolution.js'); }
  catch (error) { throw new Error(`semantic journal resolver is missing: ${error?.code || error?.message || error}`); }
}
...
export async function runOrchestrationSemanticJournalResolutionTests() {
  ...
  await test('production release probes resolve as definitively not applied without another GitHub mutation', async () => {
    const { createSemanticJournalInvocationResolver } = await target();
    const resolutions = [];
