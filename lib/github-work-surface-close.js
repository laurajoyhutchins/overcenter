...
async function readSurface(apiClient, normalized, options = {}, phase = 'preflight') {
  ...
  let retried;
  try { retried = await boundedSafeRead(() => apiClient.graphql(normalized.kind === 'pull_request' ? PR_QUERY : ISSUE_QUERY, { owner,name,number:normalized.number }), { sleep:options.sleep, random:options.random, maxAttempts:options.maxAttempts || 3 }); }
  catch (error) { return fail('GITHUB_UPSTREAM_ERROR',String(error?.message || error),{ phase, may_have_mutated:false }); }
  const response = retried.response;
  if (!response || Number(response.status) < 200 || Number(response.status) >= 300 || errors(response).length) {
    const status = Number(response?.status || 0); const detail = summaries(response); const notFound = status === 404 || detail.some((e) => /not found|could not resolve/i.test(e.message));
...
function success(normalized, observed, outcome, extra = {}) { return { ok:true, outcome, repo:normalized.repo, artifact_ref:normalized.artifact_ref, [normalized.kind]:normalized.number, state:observed.state, url:observed.url, ...(normalized.expected_head ? { expected_head:normalized.expected_head } : {}), ...extra }; }
...
export async function closeGithubWorkSurface(kind, input, options = {}) {
  const normalized = normalizeGithubWorkSurfaceCloseRequest(kind,input); if (!normalized.ok) return normalized;
  const apiClient = options.apiClient; if (!apiClient || typeof apiClient.graphql !== 'function') return fail('GITHUB_TRANSPORT_UNAVAILABLE','A GitHub GraphQL transport is required.',{ may_have_mutated:false });
  const before = await readSurface(apiClient,normalized,options,'preflight'); if (!before.ok) return before;
  ...
  const mutation = normalized.kind === 'pull_request' ? CLOSE_PR : CLOSE_ISSUE; const variable = normalized.kind === 'pull_request' ? 'pullRequestId' : 'issueId'; let response;
  try { response = await apiClient.graphql(mutation,{ input:{ [variable]:before.id, clientMutationId:`close:${normalized.kind}:${normalized.number}:${normalized.artifact_ref}` } }); }
  catch (error) { return reconcile(apiClient,normalized,options,{ phase:'close', transport_error:String(error?.message || error), may_have_mutated:true }); }
...
function mapAuthError(error) { const status = Number(error?.status || 0); if ([401,403,422].includes(status)) return fail('GITHUB_APP_PERMISSION_DENIED',String(error?.message || error),{ upstream_status:status, may_have_mutated:false }); if (status === 404) return fail('GITHUB_APP_INSTALLATION_NOT_FOUND','The GitHub App is not installed for this repository.',{ upstream_status:404, may_have_mutated:false }); return fail(error?.code || 'GITHUB_APP_AUTH_ERROR',String(error?.message || error),{ may_have_mutated:false }); }
export async function closeGithubWorkSurfaceWithGitHubApp(kind,input,options = {}) { const normalized = normalizeGithubWorkSurfaceCloseRequest(kind,input); if (!normalized.ok) return normalized; const withApp = options.withGitHubAppApiClient || withGitHubAppApiClient; const permissionProfile = kind === 'pull_request' ? 'pull_request_close' : 'issue_close'; try { return await withApp(normalized.repo,(apiClient) => closeGithubWorkSurface(kind,input,{ ...options,apiClient }),{ permissionProfile }); } catch (error) { return mapAuthError(error); } }
