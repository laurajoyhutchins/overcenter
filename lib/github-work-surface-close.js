...
async function defaultWithGitHubAppApiClient(){return (await import('./github-app-auth.js')).withGitHubAppApiClient;}
export async function closeGithubPullRequest(input,options={}){const n=normalizeGithubPullRequestCloseRequest(input);if(!n.ok)return n;const withApp=options.withGitHubAppApiClient||await defaultWithGitHubAppApiClient();try{return await withApp(n.repo,api=>closePullRequest(n,{...options,apiClient:api}),{permissionProfile:'pull_request_close'});}catch(error){return authFailure(error,'pull_request');}}
export async function closeGithubIssue(input,options={}){const n=normalizeGithubIssueCloseRequest(input);if(!n.ok)return n;const withApp=options.withGitHubAppApiClient||await defaultWithGitHubAppApiClient();try{return await withApp(n.repo,api=>closeIssue(n,{...options,apiClient:api}),{permissionProfile:'issue_close'});}catch(error){return authFailure(error,'issue');}}
