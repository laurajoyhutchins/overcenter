...
function summaries(response) { return errors(response).map((error) => ({ message:String(error?.message || 'GitHub GraphQL error'), ...(error?.type ? { type:String(error.type) } : {}) })); }

export function normalizeGithubWorkSurfaceCloseRequest(kind, input) {
  if (!['pull_request','issue'].includes(kind)) return fail('INVALID_KIND','unsupported GitHub work-surface kind',{ may_have_mutated:false });
...
async function readSurface(apiClient, normalized, options = {}, phase = 'preflight') {
  ...
  if (!response || Number(response.status) < 200 || Number(response.status) >= 300 || errors(response).length) {
    ...
    const notFound = status === 404 || detail.some((entry) => /not found|could not resolve/i.test(entry.message));
