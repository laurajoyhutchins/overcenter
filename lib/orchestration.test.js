...
export async function runOrchestrationTests() {
  ...
  results.push(await run('diagnosis preserves retryable upstream claim semantics from persisted journal rows', async () => {
    const store = {
      async getRun() { return { run_id:'run-upstream', status:'active', disposition:null, last_work_ref:null, last_gate:null, started_at:'2026-08-24T03:00:00.000Z', deadline_at:'2026-08-24T04:00:00.000Z', finished_at:null }; },
      async lastSuccessfulInvocation() { return null; },
      async recentFailures() { return [{ sequence:1, command:'work.claim', outcome:'failed', error_code:'LINEAR_UPSTREAM_GRAPHQL', retryable:true, rejection:false, may_have_mutated:false, result_projection:{} }]; },
      async latestLease() { return null; },
    };
    const authoritative = { async getIssue() { return null; } };
    const diagnosis = await orchestrationRecovery.createOrchestrationDiagnosisService({ store, authoritative, now:()=>'2026-08-24T04:01:00.000Z' }).diagnose({ run_id:'run-upstream' });
    assert(diagnosis.failure_state === 'TRANSPORT_UNAVAILABLE', `persisted retryable upstream claim degraded to ${diagnosis.failure_state}`);
    assert(diagnosis.automatic_recovery_allowed === true, 'persisted retryable upstream claim lost automatic recovery');
  }));
