...
export async function runOrchestrationTests() {
  ...
  results.push(await run('invocation context persists bounded origin and recovery correlation without changing semantic input', async () => {
    ...
    assert(journal.starts[0]?.recovery_decision_id === 'decision-7' && journal.starts[0]?.recovery_attempt_id === 'attempt-7', 'recovery decision/attempt correlation was not persisted');
    assert(journal.starts[0]?.packet_schema === 'agent-execution-required-v1', 'packet schema attribution was not persisted');
  }));
