...
function canonicalExecution(overrides = {}) {
  return {
...
export async function runOrchestrationTests() {
  ...
  results.push(await run('diagnosis treats a canonical project-transition execution and its compatibility lease row as one authority', async () => {
    const execution = canonicalExecution({ lease_ref:'lease-project-transition' });
    ...
    assert(diagnosis.active_lease?.lease_ref === execution.lease_ref, 'same semantic lease was not diagnosed as the canonical current authority');
  }));
  ...
  results.push(await run('active canonical execution exposes its current checkpoint identity without historical lookup', async () => {
  ...
  results.push(await run('orchestration diagnosis turns current heartbeat exhaustion into exact checkpoint-preserving settlement', async () => {
    ...
    const diagnosis = await orchestrationRecovery.createOrchestrationDiagnosisService({store,authoritative:{ async getIssue(){ throw new Error('canonical execution diagnosis must not consult Linear'); } },now:()=> '2026-08-17T20:30:00.000Z'}).diagnose({run_id:'run-1'});
  ...
  results.push(await run('bounded current failure streak escalates and clearing it restores enabled state', async () => {
    ...
    assert(recovered.failure_state === null && recovered.worker_state === 'enabled' && recovered.automatic_recovery_allowed === false, 'cleared canonical failure state did not restore enabled state');
  }));
  ...
  results.push(await run('maintenance drives registered execution-operation recovery without selecting semantic work', async () => {
    let recoveryCalls = 0;
    ...
    const executionRecoveries = [{ kind:'provider_operation', async reconcilePending({ limit }) { recoveryCalls += 1; assert(limit === 10, 'maintenance did not pass its remaining bounded budget'); return [{ operation_id:'op-provider-1', outcome:'WAITING_EXTERNAL_VERIFICATION' }]; } }];
    const service = orchestrationRuns.createOrchestrationMaintenanceService({ store, leases:{}, executionRecoveries, limit:10 });
    const result = await service.maintain();
    assert(recoveryCalls === 1, 'registered execution-operation recovery was not driven');
    assert(result.actions.length === 1 && result.actions[0].kind === 'execution_operation_reconciliation', 'execution-operation recovery result was not recorded');
    assert(result.actions[0].recovery_kind === 'provider_operation', 'execution-operation recovery kind was not preserved');
    assert(result.actions[0].operation_id === 'op-provider-1', 'execution-operation identity was not preserved');
    assert(result.semantic_work_mutations === 0 && result.work_selection_performed === false, 'execution-operation recovery was misreported as semantic work selection');
  }));
