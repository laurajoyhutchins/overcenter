...
export async function runOrchestrationTests() {
  ...
  results.push(await run('invocation context persists bounded origin and recovery correlation without changing semantic input', async () => {
    const journal = new FakeJournal();
    const response = await executeCorrelatedCommand('github.delete_branch', {
      repo:'owner/repo', branch:'feature/x', expected_head:'a'.repeat(40),
    }, async () => ({ ok:true, repo:'owner/repo', branch:'feature/x', outcome:'deleted' }), {
      journal,
      invocationContext:{
        run_id:'run-recovery', execution_origin:'recovery', reasoning_boundary_id:'reasoning-7',
        recovery_decision_id:'decision-7', recovery_attempt_id:'attempt-7', packet_schema:'agent-execution-required-v1',
      },
    });
    assert(response.body.ok === true && response.body.run_id === 'run-recovery', 'invocation-context run_id was not propagated');
    assert(journal.starts[0]?.execution_origin === 'recovery', 'execution origin was not persisted');
    assert(journal.starts[0]?.reasoning_boundary_id === 'reasoning-7', 'reasoning boundary identity was not persisted');
    assert(journal.starts[0]?.request_projection?.metrics_correlation?.execution_origin === 'recovery', 'execution origin was not persisted');
    assert(journal.starts[0]?.request_projection?.metrics_correlation?.reasoning_boundary_id === 'reasoning-7', 'reasoning boundary identity was not persisted');
    assert(journal.starts[0]?.request_projection?.metrics_correlation?.recovery_decision_id === 'decision-7' && journal.starts[0]?.request_projection?.metrics_correlation?.recovery_attempt_id === 'attempt-7', 'recovery decision/attempt correlation was not persisted');
    assert(journal.starts[0]?.request_projection?.metrics_correlation?.packet_schema === 'agent-execution-required-v1', 'packet schema attribution was not persisted');
    const operatorJournal = new FakeJournal();
    await executeCorrelatedCommand('github.delete_branch', {
      repo:'owner/repo', branch:'feature/x', expected_head:'a'.repeat(40),
    }, async () => ({ ok:true }), { journal:operatorJournal, invocationContext:{ run_id:'run-operator', execution_origin:'operator', reasoning_boundary_id:'reasoning-8' } });
    assert(operatorJournal.starts[0]?.request_projection?.metrics_correlation?.execution_origin === 'operator', 'operator paperwork cannot be distinguished from recovery execution');
    assert(operatorJournal.starts[0]?.request_projection?.metrics_correlation?.reasoning_boundary_id !== journal.starts[0]?.request_projection?.metrics_correlation?.reasoning_boundary_id, 'new reasoning boundary cannot be distinguished from recovery within the existing boundary');
  }));

  results.push(await run('execution origin rejects high-cardinality or unknown classifications', async () => {
    const journal = new FakeJournal();
    const response = await executeCorrelatedCommand('github.delete_branch', {
      repo:'owner/repo', branch:'feature/x', expected_head:'a'.repeat(40),
    }, async () => ({ ok:true }), { journal, invocationContext:{ run_id:'run-bad-origin', execution_origin:'laura-clicked-a-button' } });
    assert(response.body.ok === false && response.body.error === 'REQUEST_INVALID', 'unbounded origin classification was accepted');
    assert(journal.starts.length === 0, 'invalid origin reached the durable journal');
  }));
  ...
  results.push(await run('diagnosis treats a compact project-transition execution and its compatibility lease row as one authority', async () => {
  ...
  results.push(await run('active compact execution exposes its current checkpoint identity without historical lookup', async () => {
  ...
  results.push(await run('orchestration diagnosis turns current heartbeat exhaustion into exact checkpoint-preserving settlement', async () => {
  ...
  results.push(await run('orchestration diagnosis classifies stale current authority and transient current failure without history', async () => {
  ...
  results.push(await run('bounded current failure streak escalates and clearing it restores enabled state', async () => {
    const runRow = {
