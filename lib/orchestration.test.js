...
export async function runOrchestrationTests() {
  ...
  results.push(await run('run finish refuses while an active lease is still owned', async () => {
    ...
    try { await service.finish({ run_id: 'run-active', disposition: 'clean-stop' }); }
      ...
      assert(error.details?.lease_ref === 'lease-1' && error.details?.required_command === 'work.settle', 'finish rejection did not expose the safe settlement transition');
      assert(error.details?.required_transition === 'settle_active_lease_then_retry_finish' && error.details?.retry_command === 'orchestration.finish', 'finish rejection did not define the cleanup state transition');
      return;
    }
