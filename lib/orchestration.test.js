...
export async function runOrchestrationTests() {
  ...
  results.push(await run('semantic claim helper generates stable logical-attempt retry identity', async () => {
    const db = operatorDb();
    const a = await canonicalClaimCommand({ work_ref:'LJH-1', run_id:'run-1', observed_state:'Todo', observed_lane:'lane:repo-implementation' }, db);
    const b = await canonicalClaimCommand({ work_ref:'LJH-1', run_id:'run-1', observed_state:'Todo', observed_lane:'lane:repo-implementation' }, db);
    const changed = await canonicalClaimCommand({ work_ref:'LJH-1', run_id:'run-1', observed_state:'In Progress', observed_lane:'lane:repo-implementation' }, db);
    assert(a.idempotency_key === b.idempotency_key, 'same logical attempt did not get stable retry identity');
    assert(changed.idempotency_key === a.idempotency_key, 'semantic drift escaped the logical attempt identity instead of reaching the request-hash conflict fence');
  ...
  results.push(await run('semantic horizon and finish helpers canonicalize caller-friendly aliases without gaining authority', async () => {
    const horizon = canonicalHorizonCommand({ run_id:'run-h', candidates:[{ work_ref:'LJH-1', observed_state:'Todo', observed_lane:'lane:verification' }] });
  ...
  results.push(await run('status reports overdue active runs as unhealthy with bounded evidence', async () => {
    const status=projectOrchestrationStatus({overdue_active_runs:[{run_id:'z',worker:'Fast Forward',mode:'interactive',deadline_at:'2026-08-18T20:00:00.000Z',updated_at:'2026-08-18T20:01:00.000Z',last_work_ref:'LJH-1',last_gate:'lane:verification',has_live_lease:false,scope:{secret:'large'},lease_token:'wlt_SECRET',observed_at:'2026-08-18T20:00:00.000Z'}]});
    ...
    const encoded=JSON.stringify(status.conditions.overdue_active_runs.oldest_refs[0]);
