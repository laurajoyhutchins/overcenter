...
function resumeHarness(lease, { execution = null, operation = null, slot = null, linearIssue = issue(), run = null } = {}) {
  const currentLegacyLease = lease && ['claiming','active','settling','invalidated'].includes(lease.status) ? lease : null;
  const runRow = run || {
    run_id:'run-1',
    status:lease?.status === 'settled' ? 'finished' : 'active',
    disposition:lease?.status === 'settled' ? 'completed' : null,
    worker:'Repository Implementation',
    last_work_ref:lease?.work_ref || null,
    last_gate:lease?.gate || null,
  };
  const store = {
    async getRun() { return runRow; },
    async currentExecution() { return execution; },
    async unresolvedOperation() { return operation; },
    async currentLegacyLease() { return currentLegacyLease; },
...
function compactExecution(overrides = {}) {
  return {
    subject_key:'github:laurajoyhutchins/overcenter:transition:test',
    subject_kind:'project_transition',
    authority_epoch:7,
    lease_ref:'00000000-0000-4000-8000-000000000007',
    run_id:'run-1',
    authority_repository:'laurajoyhutchins/overcenter',
    authority_revision:'a'.repeat(40),
    expires_at:'2026-08-17T21:00:00.000Z',
    hard_expires_at:'2026-08-17T22:00:00.000Z',
    active_capability_material:'wlt_SECRET_TOKEN',
    checkpoint:null,
...
// Native node:test owns execution and result reporting.
  ...
  await (test('run_id metadata does not alter semantic hash for newly correlated commands', async () => {
  ...
  await (test('semantic claim helper prefers revision fencing and keeps stable logical-attempt identity', async () => {
    const db = operatorDb();
  ...
  await (test('semantic progress helpers derive transport bookkeeping from lease identity', async () => {
  ...
  await (test('semantic horizon and finish helpers canonicalize caller-friendly aliases without gaining authority', async () => {
  ...
  await (test('correlated success records bounded projection and propagates run_id', async () => {
  ...
  await (test('expected rejection and indeterminate mutation are distinct journal outcomes', async () => {
  ...
  await (test('journal projections exclude lease tokens and full source content', async () => {
  ...
  await (test('github.apply_changeset result journal projection preserves object-shaped changed paths', async () => {
  ...
  await (test('github review packet journal projection preserves the real nested exact-head response shape', async () => {
  ...
  await (test('resume packet does not self-shadow the run it is reconstructing', async () => {
    const journal = new FakeJournal();
  ...
  await (test('resume packet includes durable skill state without changing recovery authority', async () => {
  ...
  await (test('resume packet preserves historical recovery when no stored skill snapshot exists', async () => {
  ...
  await (test('diagnosis does not self-shadow the failure evidence it is classifying', async () => {
    const journal = new FakeJournal();
  ...
  await (test('diagnosis treats a compact project-transition execution and its compatibility lease row as one authority', async () => {
    const execution = compactExecution({ lease_ref:'lease-project-transition' });
    const compatibilityLease = activeLease({
      lease_id:'lease-project-transition',
      work_ref:execution.subject_key,
      gate:'project_transition',
      claim_receipt:{ subject:'project_transition' },
    });
    const runRow = { run_id:'run-1', status:'active', disposition:null, worker:'project.advance' };
    const store = {
      async getRun(){ return runRow; },
      async currentExecution(){ return execution; },
      async unresolvedOperation(){ return null; },
      async currentLegacyLease(){ return compatibilityLease; },
      async slot(){ throw new Error('same-authority compatibility projection must not enter legacy slot recovery'); },
    };
    const diagnosis = await orchestrationRecovery.createOrchestrationDiagnosisService({
      store,
      authoritative:{ async getIssue(){ throw new Error('graph-native diagnosis must not consult Linear'); } },
      now:()=> '2026-08-17T20:30:00.000Z',
    }).diagnose({ run_id:'run-1' });
    assert(diagnosis.active_lease?.lease_ref === execution.lease_ref, 'same semantic lease was not diagnosed as the compact current authority');
  }));

  await (test('active lease after session loss returns recover_active_lease with its token', async () => {
    ...
    assert(packet.historical_correlation_missing === false, 'current-state recovery still claimed historical correlation was required');
  }));

  await (test('active compact execution exposes its current checkpoint identity without historical lookup', async () => {
    const execution = compactExecution({
      checkpoint_sha256:'c'.repeat(64),
      checkpoint:{ schema:'work-checkpoint-v1', phase:'diagnostic_complete', next_action_kind:'apply_repository_change' },
    });
    const service = resumeHarness(null, { execution });
    const packet = await service.resume({ run_id:'run-1' });
  ...
  await (test('legacy active lease still recovers against In Progress authority', async () => {
  ...
  await (test('slot-only active lease rejects a legacy In Progress authority mismatch', async () => {
    const lease = activeLease();
    const service = resumeHarness(lease, {
      slot: { lease_id: lease.lease_id, expires_at: lease.expires_at },
      linearIssue: issue('In Progress'),
  ...
  await (test('settling lease returns exact semantic settlement replay material', async () => {
    const lease = activeLease({
  ...
  await (test('settled lease is terminal or quiescent', async () => {
  ...
  await (test('expired active lease does not expose stale ownership token', async () => {
    ...
    assert(packet.active_execution === null, 'expired lease was exposed as active ownership');
  }));

  await (test('materially invalidated lease requires owner action', async () => {
  ...
  await (test('status projection detects each seeded stranded condition and stays bounded', async () => {
  ...
  await (test('run start persists bounded budget and recovers latest compatible predecessor', async () => {
    ...
    assert(first.contract_provenance?.worker_transport_revision === 'worker-transport-v2', 'new run lost its server-owned worker transport revision');
    assert(first.contract_provenance?.orchestration_kernel_revision === 'orchestration-kernel-v1', 'new run did not pin the server-owned orchestration kernel revision');
    assert(first.contract_provenance?.orchestration_recovery_protocol === 'orchestration-recovery-v1', 'new run did not pin the recovery protocol that interprets durable state');
  }));

  await (test('run start accepts team-wide campaign scope without requiring the legacy project container', async () => {
    const rows = new Map();
  ...
  await (test('run start stores declared worker contract provenance immutably without changing execution semantics', async () => {
    const rows = new Map();
    const store = { async getRun(id){return rows.get(id)||null;}, async findPredecessor(){return null;}, async insertRun(row){rows.set(row.run_id,{...row});return rows.get(row.run_id);}, async latestHorizon(){return null;} };
    const service = orchestrationRuns.createOrchestrationRunService({ store, now: () => '2026-08-18T20:00:00.000Z' });
    const provenance = {
    ...
    assert(first.contract_provenance?.orchestration_kernel_revision === 'orchestration-kernel-v1', 'declared worker provenance lost the server-owned orchestration kernel revision');
    assert(first.contract_provenance?.orchestration_recovery_protocol === 'orchestration-recovery-v1', 'declared worker provenance lost the server-owned recovery protocol');
    rows.get('run-provenance').contract_provenance.orchestration_kernel_revision = 'orchestration-kernel-v0';
    rows.get('run-provenance').contract_provenance.orchestration_recovery_protocol = 'orchestration-recovery-v0';
    ...
    assert(replay.contract_provenance?.orchestration_kernel_revision === 'orchestration-kernel-v0' && replay.contract_provenance?.orchestration_recovery_protocol === 'orchestration-recovery-v0', 'run replay silently reinterpreted durable state under the current kernel/protocol');
  }));

  await (test('run start derives an immutable server-owned skill policy for implementation workers', async () => {
    const rows = new Map();
    const store = { async getRun(id){return rows.get(id)||null;}, async findPredecessor(){return null;}, async insertRun(row){rows.set(row.run_id,{...row});return rows.get(row.run_id);}, async latestHorizon(){return null;} };
    const service = orchestrationRuns.createOrchestrationRunService({ store, now: () => '2026-08-18T20:00:00.000Z' });
  ...
  await (test('advisory horizon revalidates semantic fingerprints and never grants ownership', async () => {
    assert(typeof orchestrationRuns.createOrchestrationRunService === 'function', 'createOrchestrationRunService is not implemented');
  ...
  await (test('horizon checkpoint rejects disposed repository before Fast Forward or scheduled reuse', async () => {
  ...
  await (test('stored horizon becomes no_longer_executable when repository is disposed', async () => {
  ...
  await (test('run finish persists machine continuation without changing work authority', async () => {
    assert(typeof orchestrationRuns.createOrchestrationRunService === 'function', 'createOrchestrationRunService is not implemented');
  ...
  await (test('run receipt derives bounded settlement evidence instead of trusting handoff prose', async () => {
  ...
  await (test('run finish returns its derived evidence receipt', async () => {
  ...
  await (test('run receipt derives external effects and authority observations from safe journal projections', async () => {
  ...
  await (test('run finish refuses while an active lease is still owned', async () => {
  ...
  await (test('settlement-aware finish settles the owned lease before terminalizing the run', async () => {
  ...
  await (test('settlement-aware finish never terminalizes the run when lease settlement fails', async () => {
  ...
  await (test('orchestration diagnosis turns current heartbeat exhaustion into exact checkpoint-preserving settlement', async () => {
    assert(typeof orchestrationRecovery.createOrchestrationDiagnosisService === 'function', 'diagnosis service is not implemented');
    const execution = compactExecution({
      lease_ref:'00000000-0000-4000-8000-000000000370',
      checkpoint_sha256:'c'.repeat(64),
      checkpoint:{ schema:'work-checkpoint-v1', phase:'verification', next_action_kind:'continue' },
    });
    const runRow = {
      run_id:'run-1', status:'active', disposition:null, worker:'Source and Data Implementation',
      current_failure_command:'work.heartbeat', current_failure_error_code:'HEARTBEAT_LIMIT_REACHED',
      current_failure_error_class:'validation', current_failure_retryable:false,
      current_failure_rejection:true, current_failure_may_have_mutated:false, current_failure_streak:1,
    };
    const store = {
      async getRun(){ return runRow; },
      async currentExecution(){ return execution; },
    ...
    const diagnosis = await orchestrationRecovery.createOrchestrationDiagnosisService({store,authoritative:{ async getIssue(){ throw new Error('compact execution diagnosis must not consult Linear'); } },now:()=> '2026-08-17T20:30:00.000Z'}).diagnose({run_id:'run-1'});
    assert(diagnosis.failure_state === 'HEARTBEAT_BUDGET_EXHAUSTED', `heartbeat diagnosis was ${diagnosis.failure_state}`);
    assert(diagnosis.active_lease?.lease_id === execution.lease_ref, 'diagnosis lost active lease identity');
    assert(diagnosis.recovery_operation?.command === 'work.settle', 'diagnosis did not prescribe settlement');
    assert(diagnosis.recovery_operation?.input?.lease_ref === execution.lease_ref && diagnosis.recovery_operation?.input?.requeue_class === 'resume_progress', 'diagnosis did not return exact checkpoint-preserving settlement input');
    ...
    assert(diagnosis.recovery_failure_count === 1, 'bounded current failure streak was not reported');
  }));

  await (test('orchestration diagnosis classifies stale current authority and transient current failure without history', async () => {
    const expired = activeLease({ lease_id:'lease-stale', expires_at:'2026-08-17T20:10:00.000Z' });
    const baseRun = { run_id:'run-1', status:'active', disposition:null, worker:'Repository Implementation' };
    const staleStore = {
      async getRun(){ return baseRun; },
      async currentExecution(){ return null; },
      async unresolvedOperation(){ return null; },
      async currentLegacyLease(){ return expired; },
      async slot(){ return { lease_id:expired.lease_id, expires_at:expired.expires_at }; },
    };
    ...
    const transportRun = {
      ...baseRun,
      current_failure_command:'work.claim', current_failure_error_code:'HATCHABLE_MCP_TRANSPORT_ERROR',
      current_failure_error_class:'transport', current_failure_retryable:true,
      current_failure_rejection:false, current_failure_may_have_mutated:false, current_failure_streak:1,
    };
    const transportStore = {
      async getRun(){ return transportRun; },
      async currentExecution(){ return null; },
      async unresolvedOperation(){ return null; },
      async currentLegacyLease(){ return null; },
    };
  ...
  await (test('bounded current failure streak escalates and clearing it restores enabled state', async () => {
    const runRow = {
      run_id:'run-retry', status:'active', worker:'Repository Implementation',
      current_failure_command:'work.claim', current_failure_error_code:'HATCHABLE_MCP_TRANSPORT_ERROR',
      current_failure_error_class:'transport', current_failure_retryable:true,
      current_failure_rejection:false, current_failure_may_have_mutated:false, current_failure_streak:3,
    };
    const store = {
      async getRun(){ return runRow; },
      async currentExecution(){ return null; },
    ...
    assert(diagnosis.recovery_failure_count === 3, 'current failure streak was not preserved');

    Object.assign(runRow, {
      current_failure_command:null, current_failure_error_code:null, current_failure_error_class:null,
      current_failure_retryable:null, current_failure_rejection:null, current_failure_may_have_mutated:null,
    ...
    assert(recovered.failure_state === null && recovered.worker_state === 'enabled' && recovered.automatic_recovery_allowed === false, 'cleared compact failure state did not restore enabled state');
  }));

  await (test('abandoned historical run is classified unobservable without speculative investigation', async () => {
    const store = {
  ...
  await (test('maintenance only performs deterministic bounded reconciliation', async () => {
  ...
  await (test('maintenance drives registered compact recovery without selecting semantic work', async () => {
    let recoveryCalls = 0;
    const store = { async expiredSlots(){return[];}, async stuckLeases(){return[];}, async unresolvedInvocations(){return[];}, async overdueRuns(){return[];} };
    const compactRecoveries = [{ kind:'project_authoring', async reconcilePending({ limit }) { recoveryCalls += 1; assert(limit === 10, 'maintenance did not pass its remaining bounded budget'); return [{ operation_id:'op-authoring-1', outcome:'WAITING_EXTERNAL_VERIFICATION' }]; } }];
    const service = orchestrationRuns.createOrchestrationMaintenanceService({ store, leases:{}, compactRecoveries, limit:10 });
    const result = await service.maintain();
    assert(recoveryCalls === 1, 'registered compact recovery was not driven');
    assert(result.actions.length === 1 && result.actions[0].kind === 'compact_operation_reconciliation', 'compact recovery result was not recorded');
    assert(result.actions[0].recovery_kind === 'project_authoring', 'compact recovery kind was not preserved');
    assert(result.actions[0].operation_id === 'op-authoring-1', 'compact operation identity was not preserved');
    assert(result.semantic_work_mutations === 0 && result.work_selection_performed === false, 'compact recovery was misreported as semantic work selection');
  }));

  await (test('maintenance reconciles interrupted orchestration.start from run-record evidence', async () => {
    const updates = [];
    const store = {
  ...
  await (test('active compatible predecessor is not implicitly transferred to a new run', async () => {
  ...
  await (test('start replay conflicts when budget semantics differ', async () => {
  ...
  await (test('finish replay conflicts when terminal handoff semantics differ', async () => {
  ...
  await (test('horizon checkpoint is fenced by registered run scope', async () => {
  ...
  await (test('maintenance never reconciles a receipt whose semantic request hash differs', async () => {
    const queries=[];
  ...
  await (test('overdue abandoned run is terminalized once without changing continuation evidence', async () => {
  ...
  await (test('run reconciliation ignores future runs and overdue runs with live lease ownership', async () => {
  ...
  await (test('terminalized abandoned run remains eligible as continuation predecessor', async () => {
    const rows=new Map();
    const store={
      async getRun(id){return rows.get(id)||null;},
  ...
  await (test('status reports overdue active runs as unhealthy with bounded evidence', async () => {
  ...
  await (test('Postgres abandoned-run reconciliation locks the run row and revalidates live lease absence atomically', async () => {
    let ops=[];
