...
export async function runOrchestrationTests() {
  ...
  results.push(await run('maintenance drives registered compact recovery without selecting semantic work', async () => {
    ...
    const compactRecoveries = [{
      kind:'project_authoring',
      async reconcilePending({ limit }) {
        recoveryCalls += 1;
        assert(limit === 10, 'maintenance did not pass its remaining bounded budget');
        return [{ operation_id:'op-authoring-1', outcome:'WAITING_EXTERNAL_VERIFICATION' }];
      },
    }];
