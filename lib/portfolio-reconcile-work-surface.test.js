...
class FakeReceiptStore {
  ...
  async claim(key, hash) {
    ...
    if (!row) { this.rows.set(key, { hash, state: 'processing', receipt: null }); return { kind: 'claimed' }; }
    if (row.hash !== hash) return { kind: 'conflict' };
    if (row.state === 'succeeded') return { kind: 'existing', receipt: clone(row.receipt) };
    return { kind: 'in_progress' };
  }
  async succeed(key, hash, receipt) { this.writes += 1; this.rows.set(key, { hash, state: 'succeeded', receipt: clone(receipt) }); }
  async abandon(key, hash) { const row = this.rows.get(key); if (row?.hash === hash && row.state === 'processing') this.rows.delete(key); }
}
...
class FakeLinear {
  ...
  async createIssue(input) {
    ...
    return this.addIssue({ title: input.title, description: input.description, priority: input.priority, state, labels });
  }
...
export async function runPortfolioReconcileWorkSurfaceTests() {
  ...
  results.push(await run('03 pre-Hatchable Repository + Authority markers are discovered', async () => {
    const h = harness(); const issue = h.linear.addIssue({ description: existingDescription() }); const r = await reconcile(h);
  ...
  results.push(await run('16 GitHub permission failure is stable rejection', async () => {
    const h = harness(); h.github.error = Object.assign(new Error('denied'), { code: 'GITHUB_PERMISSION_DENIED' }); const r = await reconcile(h);
    check(r.items[0].reason === 'GITHUB_PERMISSION_DENIED', 'GitHub permission failure was not normalized');
  }));
