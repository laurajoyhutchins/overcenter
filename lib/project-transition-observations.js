...
const RECOVERY_SEED_PATH = '.overcenter/recovery/hatchable-cutover-v1.json';
...
function transitionDefinition(node = {}) {
  ...
  return Object.freeze({
    ...
    executor,
    ...(node.execution_intent == null
      ? {}
      : {
          execution_intent:object(
            node.execution_intent,
            'transition.execution_intent',
          ),
        }),
    phase_bindings:phaseBindings,
    ...(node.execution_intent == null ? {} : { execution_intent:object(node.execution_intent, 'transition.execution_intent') }),
  });
}
...
function normalizeProvenance(raw, transitionId) {
  const provenance = object(raw, 'observation.provenance');
  ...
  if (kind === 'project_transition_settlement') {
    ...
    return Object.freeze({ kind, lease_ref:leaseRef, run_id:runId, settled_at:settledAt });
  }

  if (kind === 'github_recovery_seed') {
    const ref = text(provenance.ref, 'observation.provenance.ref', 512);
    if (ref !== RECOVERY_SEED_PATH) {
      fail('PROJECT_GRAPH_OBSERVATIONS_INVALID', 'project transition recovery provenance must reference the sealed cutover seed', {
        transition_id:transitionId,
        ref,
      });
    }
    return Object.freeze({ kind, ref, settled_at:settledAt });
  }
...
function normalizeObservation(raw, expected = {}) {
  ...
  const provenance = normalizeProvenance(observation.provenance, transitionId);
