...
// Tests execute directly through native node:test discovery.
  ...
  test('checkpoint and heartbeat shared contracts match the lease_ref semantic worker boundary', async () => {
    assertSharedShape(
      'work.checkpoint',
      WORK_CHECKPOINT_INPUT_SCHEMA,
      WORK_CHECKPOINT_REQUIRED_FIELDS,
  ...
  test('checkpoint and heartbeat by-reference canonicalization preserves stable retry identity without capability material', async () => {
    const db = boundaryDb();
  ...
  test('subject-aware checkpoint sends graph-native leases to project transition authority', async () => {
    let legacyCalls = 0;
  ...
  test('subject-aware heartbeat sends graph-native leases to project transition authority', async () => {
    let legacyCalls = 0;
    let transitionInput = null;
  ...
  test('semantic worker checkpoint composes the subject-aware checkpoint runtime', async () => {
    const db = boundaryDb();
    let executed = null;
  ...
  test('semantic worker heartbeat composes the subject-aware heartbeat runtime', async () => {
    const db = boundaryDb();
    let executed = null;
    const response = await executeSemanticWorkerCommand(
      'work.heartbeat',
  ...
  test('malformed and unknown lease references fail before progress mutation', async () => {
