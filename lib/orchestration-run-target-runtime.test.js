...
const EXECUTION_INTENT = Object.freeze({
  ...
  desired_outcome:'Verify the targeted agent execution handoff.',
  acceptance_evidence:Object.freeze([
    Object.freeze({ kind:'verification', requirement:'Evidence must prove the intended targeted handoff behavior.' }),
  ]),
...
function productionGitHubTransport() {
  ...
  const apiClient = {
    async call(service, request) {
      ...
      if (request.path === `/repos/${REPOSITORY}/contents/.overcenter/recovery/hatchable-cutover-v1.json`) {
        assert(request.query?.ref === REVISION, 'recovery seed read was not bound to exact revision');
...
function node(id, priority, executor, requires = []) {
  return {
    ...
    executor,
    ...(executor?.kind === 'agent' ? { execution_intent:EXECUTION_INTENT } : {}),
    phase_bindings:{},
  };
}
...
// Native node:test owns registration, execution, and failure reporting.
  ...
  test('target runtime derives an authoritative graph reader from runtime dependencies', async()=>{
  ...
  test('production target runtime resolves repository-owned graph without caller-built graph dependencies', async()=>{
  ...
  test('production target runtime fails closed when definition facts are stale', async()=>{
    const transport = productionGitHubTransport();
  ...
  test('advance returns a bounded non-secret agent execution packet after exact transition acquisition', async()=>{
  ...
  test('advance skips occupied READY transitions and selects the next deterministic candidate', async()=>{
    const graph = authoritativeGraph([
  ...
  test('advance returns WAITING when every READY transition is already occupied', async()=>{
    const graph = authoritativeGraph([
  ...
  test('advance completes a deterministic operator only after settlement is visible in a fresh graph read', async()=>{
  ...
  test('advance fails closed when deterministic settlement is not observable as DONE', async()=>{
    const graph = authoritativeGraph([
      node('operator-work', 20, { kind:'operator', command:'orchestration.maintain' }),
    ]);
    const service = createOrchestrationAdvanceService({
