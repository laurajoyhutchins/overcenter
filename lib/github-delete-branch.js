...
async function readOpenPullsForBranch(apiClient, normalized, role, options = {}) {
  const [owner, repo] = normalized.repo.split('/');
  const query = role === 'head'
    ? `state=open&head=${encodeURIComponent(`${owner}:${normalized.branch}`)}&per_page=100`
    : `state=open&base=${encodeURIComponent(normalized.branch)}&per_page=100`;
  ...
  try {
    ...
    return {
      ok: false,
      error: 'GITHUB_UPSTREAM_ERROR',
      message: String(error?.message || 'GitHub pull-request lifecycle read failed.'),
      phase: `preflight.open_pr_${role}`,
      github_path: path,
      attempts: Number(error?.githubTransportAttempts || 1),
      may_have_mutated: false,
    };
  ...
  if (!response || response.status < 200 || response.status >= 300) {
    ...
    return {
      ok: false,
      error: status === 401 || status === 403 ? 'GITHUB_PERMISSION_DENIED' : (status === 404 ? 'GITHUB_NOT_FOUND' : 'GITHUB_UPSTREAM_ERROR'),
      message: String(response?.body?.message || `GitHub returned HTTP ${status || 'unknown'}`),
      ...(status ? { upstream_status: status } : {}),
      ...githubTransportEvidence(response, { phase: `preflight.open_pr_${role}`, path, attempts: retried.attempts, mayHaveMutated: false }),
    };
  }
  return {
    ok: true,
    pulls: Array.isArray(response.body) ? response.body : [],
    evidence: githubTransportEvidence(response, { phase: `preflight.open_pr_${role}`, path, attempts: retried.attempts, mayHaveMutated: false }),
  };
}
...
async function deleteNormalized(normalized, apiClient, options = {}) {
  ...
  const openHead = await readOpenPullsForBranch(apiClient, normalized, 'head', options);
  if (!openHead.ok) return openHead;
  if (openHead.pulls.length) {
    return {
      ok: false,
      error: 'GITHUB_REF_REJECTED',
      message: 'The branch is still the head of an open pull request and cannot be deleted.',
      repo: normalized.repo,
      branch: normalized.branch,
      expected_head: normalized.expected_head,
      actual_head: before.actual_head,
      reason: 'open_pull_request_head',
      pull_requests: openHead.pulls.map((pr) => Number(pr?.number || 0)).filter((number) => number > 0),
      phase: 'preflight.open_pr_head',
    };
  }
  const openBase = await readOpenPullsForBranch(apiClient, normalized, 'base', options);
  if (!openBase.ok) return openBase;
  if (openBase.pulls.length) {
    return {
      ok: false,
      error: 'GITHUB_REF_REJECTED',
      message: 'The branch is still the base of an open pull request and cannot be deleted.',
      repo: normalized.repo,
      branch: normalized.branch,
      expected_head: normalized.expected_head,
      actual_head: before.actual_head,
      reason: 'open_pull_request_base',
      pull_requests: openBase.pulls.map((pr) => Number(pr?.number || 0)).filter((number) => number > 0),
      phase: 'preflight.open_pr_base',
    };
  }
