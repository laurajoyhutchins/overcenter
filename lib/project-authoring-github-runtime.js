...
async function applyAtMutationBoundary(applyChangeset, request) {
    try {
...
function confirmedRevision(result) {
    ...
    if (!result?.ok || !/^[0-9a-f]{40}$/.test(revision)) {
        ...
        const providerCode = String(result?.error || '').trim();
        const phase = typeof result?.phase === 'string' ? result.phase.trim().slice(0, 128) : '';
        const error = new Error('GitHub project definition mutation did not return a confirmed exact revision');
        const preserveProviderCode = mayHaveMutated === false
            && phase.startsWith('preflight.')
            && /^[A-Z0-9_]{2,128}$/.test(providerCode);
        error.code = preserveProviderCode
            ? providerCode
            : 'PROJECT_AUTHORING_MUTATION_UNCONFIRMED';
        error.may_have_mutated = mayHaveMutated;
        error.details = Object.freeze({ result, ...(phase ? { phase } : {}), may_have_mutated: mayHaveMutated });
        throw error;
    }
    return revision;
}
function normalizedMutationAuthority(authorityInput, operation, request) {
        ...
        async amend(input = {}) {
            ...
            return amendProjectDefinition(input, {
                ...
                ...(readProjectObservations
                    ? {
                        readProjectObservations: async (authority) => readProjectObservations({
                            project_ref: input.project_ref,
                            repository: authority.repository,
