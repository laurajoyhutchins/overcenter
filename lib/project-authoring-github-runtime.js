...
function normalizedMutationAuthority(authority, operation) {
  const leaseRef = typeof authority?.lease_ref === 'string' ? authority.lease_ref.trim() : '';
  if (!leaseRef) {
    fail('PROJECT_AUTHORING_MUTATION_AUTHORITY_INVALID', 'project authoring mutation authority must resolve a lease_ref', { operation });
  }
...
export function createProjectAuthoringGithubAdapter(dependencies = {}) {
  ...
  const mutationAuthorityFor = async (operation, input, request) => {
    ...
    return legacy.lease_ref ? normalizedMutationAuthority(legacy, operation) : Object.freeze({});
  };
