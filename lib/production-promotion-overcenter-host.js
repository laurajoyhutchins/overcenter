...
export function productionPromotionCommandResult(result) {
  const revision = String(result?.production_revision || '').trim().toLowerCase();
  if (!SHA40.test(revision)) {
    const error = new Error('production promotion returned an invalid production revision');
    error.code = 'PRODUCTION_PROMOTION_RESULT_INVALID';
    throw error;
  }
  return Object.freeze({ ok:true, production_revision:revision });
}
