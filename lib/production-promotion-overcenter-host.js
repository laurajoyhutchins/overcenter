...
async function githubGet(repo, permissionProfile, path, withGitHubAppApiClient) {
  if (typeof withGitHubAppApiClient !== 'function') {
    throw Object.assign(new Error('GitHub App auth provider is required'), { code:'RUNTIME_PROVIDER_MISSING', details:{ provider:'githubAppAuth' }, mayHaveMutated:false });
  }
    ...
    async promote(intent) {
      const result = await promoteProduction(intent, {
        ...
        async readBranchHead(repo, branch) {
          const body = await githubGet(repo, 'changeset', `${repoBase(repo)}/branches/${encodeURIComponent(branch)}`, withGitHubAppApiClient);
        ...
        async verifyExactRevision(repo, revision) {
          ...
          const body = await githubGet(repo, 'actions_storage_read', path, withGitHubAppApiClient);
