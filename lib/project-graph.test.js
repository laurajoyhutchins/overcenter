...
// Register each project-graph contract directly with the canonical Node test runner.

  test('completed prerequisite enables dependent node', async()=>{
  ...
  test('incomplete prerequisites block with exact sorted unmet ids', async()=>{
    const result = evaluateProjectGraph({ nodes:[
  ...
  test('off-nominal node is excluded from ready frontier', async()=>{
  ...
  test('ready frontier is deterministic by priority then id', async()=>{
    const result = evaluateProjectGraph({ nodes:[
  ...
  test('node evaluation preserves lifecycle command resolution', async()=>{
  ...
  test('explicit lifecycle phase bindings are normalized and preserved', async()=>{
    const result = evaluateProjectGraph({ nodes:[operatorNode('bound', {
      phase_bindings:{
        acquire:{ primitive:'work.claim', evidence:['lease_ref'] },
        COMMIT:{ primitive:'github.apply_changeset', evidence:['commit_sha'] },
        confirm:{ primitive:'github.review_packet', evidence:['exact_head','required_checks'] },
      },
    })] });
    const bindings = byId(result,'bound').phase_bindings;
    assert(bindings.ACQUIRE.primitive === 'work.claim', 'acquire primitive was not normalized');
    assert(bindings.COMMIT.evidence.join(',') === 'commit_sha', 'commit evidence was not preserved');
    assert(bindings.CONFIRM.evidence.join(',') === 'exact_head,required_checks', 'confirm evidence was not preserved');
  });

  test('malformed lifecycle phase bindings fail closed', async()=>{
    expectFailure(()=>evaluateProjectGraph({ nodes:[operatorNode('execute-binding',{ phase_bindings:{ EXECUTE:{ primitive:'work.execute', evidence:['result'] } } })] }), 'execute phase binding was accepted');
    expectFailure(()=>evaluateProjectGraph({ nodes:[operatorNode('empty-primitive',{ phase_bindings:{ ACQUIRE:{ primitive:'', evidence:['lease_ref'] } } })] }), 'empty phase primitive was accepted');
    expectFailure(()=>evaluateProjectGraph({ nodes:[operatorNode('missing-evidence',{ phase_bindings:{ CONFIRM:{ primitive:'github.review_packet', evidence:[] } } })] }), 'evidence-free phase binding was accepted');
  });

  test('agent executor without a skill fails closed', async()=>{
  ...
  test('invalid prerequisite topology fails closed', async()=>{
  ...
  // Native node:test owns failure aggregation and reporting.