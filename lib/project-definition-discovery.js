...
export function parseProjectDefinitionDiscovery(content) {
  ...
  if (!source) fail('project definition discovery declaration content must be non-empty');
  let parsed;
  try {
    parsed = JSON.parse(source);
  } catch (error) {
    fail('project definition discovery declaration must contain valid JSON', { cause:String(error?.message || error) });
  }
