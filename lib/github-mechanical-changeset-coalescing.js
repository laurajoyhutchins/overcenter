...
export async function coalesceGithubLeaseScopedMechanicalChangeset(input,options={}){
  ...
  return withGithub({repo:workspace.repository,branch:workspace.branch,changes:request.changes},async github=>{
    ...
    let refError=null;try{await github.replaceBranch(workspace.repository,workspace.branch,head,replacement);}catch(error){refError=error;}
    const after=observedHead(await github.getBranch(workspace.repository,workspace.branch,{phase:'coalesce.readback'}));
    if(after!==replacement){if(refError)throw Object.assign(refError,{may_have_mutated:true});fail('MECHANICAL_COALESCE_REF_INDETERMINATE','authoritative readback did not prove the replacement ref',{expected_head:replacement,actual_head:after,may_have_mutated:true},409,true);}
