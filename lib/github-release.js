import { db as defaultDb } from 'hatchable';
...
export function createGithubReleaseReceiptStore(dbBinding = defaultDb) {
  return {
    async claim(normalized, digest) {
      const token = crypto.randomUUID();
      const inserted = await dbBinding.query(`INSERT INTO github_release_receipts (repo,idempotency_key,request_sha256,request_json,state,attempt_token,target_sha,tag_name) VALUES ($1,$2,$3,$4::jsonb,'processing',$5,$6,$7) ON CONFLICT (repo,idempotency_key) DO NOTHING RETURNING *`, [normalized.repo, normalized.idempotency_key, digest, JSON.stringify(normalized), token, normalized.target_sha, normalized.tag_name]);
      if (inserted.rows?.[0]) return { kind: 'claimed', row: inserted.rows[0], attempt_token: token };
      const selected = await dbBinding.query('SELECT * FROM github_release_receipts WHERE repo=$1 AND idempotency_key=$2', [normalized.repo, normalized.idempotency_key]);
      const row = selected.rows?.[0];
      if (!row) throw Object.assign(new Error('release receipt disappeared during claim'), { code: 'IDEMPOTENCY_UNAVAILABLE' });
      if (row.request_sha256 !== digest) return { kind: 'conflict', row };
      if (row.state === 'succeeded') return { kind: 'existing', row };
      if (row.state === 'partial') {
        const resumed = await dbBinding.query(`UPDATE github_release_receipts SET state='processing',attempt_token=$3,updated_at=now() WHERE repo=$1 AND idempotency_key=$2 AND state='partial' RETURNING *`, [normalized.repo, normalized.idempotency_key, token]);
        if (resumed.rows?.[0]) return { kind: 'claimed', row: resumed.rows[0], attempt_token: token, resumed: true };
      }
      const takeover = await dbBinding.query(`UPDATE github_release_receipts SET attempt_token=$3,updated_at=now() WHERE repo=$1 AND idempotency_key=$2 AND state='processing' AND updated_at < now() - interval '30 seconds' RETURNING *`, [normalized.repo, normalized.idempotency_key, token]);
      if (takeover.rows?.[0]) return { kind: 'claimed', row: takeover.rows[0], attempt_token: token, resumed: true };
      return { kind: 'in_progress', row };
    },
    async markTag(normalized, evidence = {}) {
      await dbBinding.query(`UPDATE github_release_receipts SET tag_created=true,tag_ref_node_id=COALESCE($3,tag_ref_node_id),updated_at=now() WHERE repo=$1 AND idempotency_key=$2`, [normalized.repo, normalized.idempotency_key, evidence.tag_ref_node_id || null]);
    },
    async markPartial(normalized, details = {}) {
      await dbBinding.query(`UPDATE github_release_receipts SET state='partial',tag_created=(tag_created OR $3),last_error=$4,updated_at=now() WHERE repo=$1 AND idempotency_key=$2`, [normalized.repo, normalized.idempotency_key, Boolean(details.tag_may_exist), String(details.error || 'partial release mutation')]);
    },
    async abandon(normalized) {
      await dbBinding.query(`DELETE FROM github_release_receipts WHERE repo=$1 AND idempotency_key=$2 AND state='processing' AND tag_created=false`, [normalized.repo, normalized.idempotency_key]);
    },
    async succeed(normalized, result) {
      await dbBinding.query(`UPDATE github_release_receipts SET state='succeeded',release_id=$3,receipt=$4::jsonb,last_error=NULL,updated_at=now() WHERE repo=$1 AND idempotency_key=$2`, [normalized.repo, normalized.idempotency_key, result.release_id, JSON.stringify(result)]);
    },
  };
}
...
export async function createGithubRelease(input, options = {}) {
  ...
  const digest = await sha256Text(canonicalJson(normalized));
  ...
  const replayTrusted = Boolean(claim && (claim.kind === 'existing' || claim.row?.tag_created || claim.resumed));
  let mutated = false;
  let tagMayExist = false;
  try {
    let pre = await observe(options.apiClient, normalized);
    if (!pre.ok) { if (store && !replayTrusted) await store.abandon?.(normalized); return pre; }
    const preState = classify(pre, normalized);
    if (pre.tag && pre.tag_commit_sha !== normalized.target_sha) return fail('GITHUB_RELEASE_TAG_CONFLICT', 'existing tag resolves to a different commit', { tag_name: normalized.tag_name, requested_sha: normalized.target_sha, observed_sha: pre.tag_commit_sha }, false);
    if (pre.release && !releaseMatches(pre.release, normalized)) return fail('GITHUB_RELEASE_STATE_CONFLICT', 'existing release materially differs from the requested semantic outcome', { tag_name: normalized.tag_name, release_id: pre.release.id || null }, false);
    if (pre.release && !pre.tag) return fail('GITHUB_RELEASE_STATE_CONFLICT', 'release exists but the corresponding Git tag is not authoritatively observable', { tag_name: normalized.tag_name, release_id: pre.release.id || null }, false);
    if (normalized.expected_state.tag === 'absent' && pre.tag && !replayTrusted) return fail('GITHUB_RELEASE_STATE_CHANGED', 'tag expected absent but is present', { tag_name: normalized.tag_name, observed_state: preState }, false);
    if (normalized.expected_state.tag === 'present_same_commit' && !pre.tag) return fail('GITHUB_RELEASE_STATE_CHANGED', 'tag expected present at requested commit but is absent', { tag_name: normalized.tag_name, observed_state: preState }, false);
    if (normalized.expected_state.release === 'absent' && pre.release && !replayTrusted) return fail('GITHUB_RELEASE_STATE_CHANGED', 'release expected absent but is present', { tag_name: normalized.tag_name, observed_state: preState }, false);
    if (normalized.expected_state.release === 'present_matching' && !pre.release) return fail('GITHUB_RELEASE_STATE_CHANGED', 'matching release expected present but is absent', { tag_name: normalized.tag_name, observed_state: preState }, false);

    if (pre.tag && pre.release) {
      const done = receipt(normalized, digest, pre, preState, 'already_satisfied', false, replayTrusted);
      if (store) await store.succeed(normalized, done);
      return done;
    }
    ...
    if (!pre.tag) {
      ...
      try { tagWrite = await api(options.apiClient, 'POST', `${pre.base}/git/refs`, { ref: `refs/tags/${normalized.tag_name}`, sha: normalized.target_sha }); }
        ...
        tagMayExist = true;
        if (store) await store.markPartial(normalized, { tag_may_exist: true, error: error.message });
        return fail('GITHUB_RELEASE_INDETERMINATE', 'tag creation transport failed after dispatch', { tag_name: normalized.tag_name }, true);
      }
      if (!good(tagWrite)) {
        if (store) await store.markPartial(normalized, { tag_may_exist: tagWrite.status >= 500, error: `tag create HTTP ${tagWrite.status}` });
        return fail(tagWrite.status === 422 ? 'GITHUB_RELEASE_TAG_CONFLICT' : 'GITHUB_RELEASE_INDETERMINATE', 'GitHub did not confirm immutable tag creation', { tag_name: normalized.tag_name, upstream_status: tagWrite.status }, tagWrite.status >= 500);
      }
      mutated = true;
      tagMayExist = true;
      if (String(tagWrite.body?.object?.sha || '').toLowerCase() !== normalized.target_sha) {
        if (store) await store.markPartial(normalized, { tag_may_exist: true, error: 'tag write returned unexpected SHA' });
      ...
      if (store) await store.markTag(normalized, { tag_ref_node_id: tagWrite.body?.node_id || null });
    }
    ...
    if (!pre.release) {
      ...
      try { releaseWrite = await api(options.apiClient, 'POST', `${pre.base}/releases`, { tag_name: normalized.tag_name, name: normalized.name, body: normalized.body, draft: normalized.draft, prerelease: normalized.prerelease }); }
        ...
        if (store) await store.markPartial(normalized, { tag_may_exist: tagMayExist || Boolean(pre.tag), error: error.message });
      ...
      if (!good(releaseWrite)) {
        if (store) await store.markPartial(normalized, { tag_may_exist: tagMayExist || Boolean(pre.tag), error: `release create HTTP ${releaseWrite.status}` });
        const permission = releaseWrite.status === 401 || releaseWrite.status === 403;
        return fail(permission ? 'GITHUB_PERMISSION_DENIED' : 'GITHUB_RELEASE_INDETERMINATE', permission ? 'GitHub App lacks permission to create releases' : 'GitHub did not confirm release creation', { tag_name: normalized.tag_name, upstream_status: releaseWrite.status, ...(permission ? { required_permissions: { contents: 'write' } } : {}) }, mutated);
      }
      mutated = true;
    }
    ...
    if (!post.ok || !post.tag || post.tag_commit_sha !== normalized.target_sha || !releaseMatches(post.release, normalized)) {
      if (store) await store.markPartial(normalized, { tag_may_exist: true, error: 'post-write verification failed' });
    ...
    const done = receipt(normalized, digest, post, preState, 'created', true, replayTrusted);
    if (store) await store.succeed(normalized, done);
    return done;
  } catch (error) {
    if (store) {
      if (mutated || tagMayExist) await store.markPartial(normalized, { tag_may_exist: tagMayExist, error: error.message });
      else await store.abandon?.(normalized);
    }
...
export async function createGithubReleaseWithGitHubApp(input, options = {}) {
  ...
  const receiptStore = options.receiptStore === undefined ? createGithubReleaseReceiptStore(options.db || defaultDb) : options.receiptStore;
  return withGitHubAppApiClient(normalized.repo, apiClient => createGithubRelease(normalized, { apiClient, receiptStore }), { permissionProfile: 'release' });
}