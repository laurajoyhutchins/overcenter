...
import { createHighWaterOrchestrationMaintenanceService } from './orchestration-maintenance-high-water.js';
...
export function createPostgresSubjectAwareOrchestrationMaintenanceService(options = {}) {
  ...
  const withGitHubAppApiClient = options.withGitHubAppApiClient || options.githubAppAuth?.withApiClient;
  const compactRecoveries = options.compactRecoveries || (typeof withGitHubAppApiClient === 'function'
    ? [Object.freeze({
        kind:'project_authoring',
        reconcilePending:(request) => createProjectAuthoringRecoveryForRuntime({
          db:dbBinding,
          api:options.api,
          withGitHubAppApiClient,
        }).reconcilePending(request),
      })]
    : []);
  ...
  return createHighWaterOrchestrationMaintenanceService({
    db:dbBinding,
    delegate:maintenance,
    maintenanceIdentity:options.maintenanceIdentity,
    now:options.now,
  });
}
