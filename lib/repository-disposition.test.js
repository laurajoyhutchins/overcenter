...
export async function runRepositoryDispositionTests() {
  ...
  await test('legacy compatibility-bound disposal cannot be resurrected', async () => {
    const store = new MemoryStore();
    const service = createRepositoryLifecycleService({ store, github: new FakeGithub(true), now: () => '2026-08-22T16:00:00.000Z' });
    let failure = null;
  ...
  await test('archived agent-team-context-bridge is terminal with no compatibility work', async () => {
    const store = new MemoryStore();
    const service = createRepositoryLifecycleService({ store, github: new FakeGithub(true), now: () => '2026-08-22T16:00:00.000Z' });
    const row = await service.dispose({ repository:'laurajoyhutchins/agent-team-context-bridge', disposition:'ARCHIVED', reason:'Agent Execution Control Plane retired' });
    assert(row.repository === 'laurajoyhutchins/agent-team-context-bridge', 'wrong repository projected');
    assert(row.ordinary_work_enabled === false && row.issue_discovery_eligible === false && row.fast_forward_eligible === false && row.scheduled_worker_target === false, 'archived compatibility repository remained executable');
    assert(!('compatibility_work_allowed' in row) && !('compatibility_reference' in row), 'retired compatibility semantics remain on current lifecycle projection');
  });
