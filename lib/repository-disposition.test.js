...
export async function runRepositoryDispositionTests() {
  ...
  await test('compatibility-bound archive permits only explicit compatibility operations', async () => {
    const store = new MemoryStore();
    const service = createRepositoryLifecycleService({ store, github: new FakeGithub(true), now: () => '2026-08-22T16:00:00.000Z' });
    const row = await service.dispose({ repository:'owner/bridge', disposition:'ARCHIVED', compatibility_bound:true, reason:'historical compatibility' });
    const release = await service.checkCompatibility({ repository:'owner/bridge', operation_kind:'release' });
    assert(row.ordinary_work_enabled === false && row.compatibility_work_allowed === true && release.allowed === true, 'compatibility exception leaked into ordinary work or explicit release was denied');
  });
