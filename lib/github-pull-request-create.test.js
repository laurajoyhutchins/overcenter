...
function pull(number = 7, extra = {}) { return { number, state: 'open', draft: true, html_url: `https://github.com/owner/repo/pull/${number}`, user: { login: 'portfolio-control-plane[bot]' }, head: { ref: 'feature/test', sha: HEAD }, base: { ref: 'main', sha: BASE }, ...extra }; }
...
export async function runGithubPullRequestCreateRegressionTests() {
  ...
  await test('exact coordinates create a draft and record actor continuity', async () => {
    ...
    check(result.author_login === 'portfolio-control-plane[bot]', 'author evidence missing');
