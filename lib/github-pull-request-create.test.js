...
// Test cases register directly with the native Node test runner.
  ...
  test('request validation is strict and exact-coordinate scoped', async () => {
  ...
  test('existing exact pull request is idempotent and does not create', async () => {
    const calls = [];
  ...
  test('exact coordinates create a draft and record actor continuity', async () => {
    let listCount = 0;
    const apiClient = { call: async (_name, opts) => {
      if (opts.path === '/repos/owner/repo') return response(200, { private: false, visibility: 'public' });
      if (opts.path.includes('/git/ref/heads/main')) return response(200, refBody(BASE));
  ...
  test('lost create acknowledgement reconciles to the exact created pull request', async () => {
    let listCount = 0;
    const apiClient = { call: async (_name, opts) => {
      if (opts.path === '/repos/owner/repo') return response(200, { private: false, visibility: 'public' });
      if (opts.path.includes('/git/ref/heads/main')) return response(200, refBody(BASE));
  ...
  test('unresolved create ambiguity stays indeterminate without retry', async () => {
    const apiClient = { call: async (_name, opts) => {
      if (opts.path === '/repos/owner/repo') return response(200, { private: false, visibility: 'public' });
      if (opts.path.includes('/git/ref/heads/main')) return response(200, refBody(BASE));
      if (opts.path.includes('/git/ref/heads/feature%2Ftest')) return response(200, refBody(HEAD));
  ...
  test('GitHub App wrapper preserves strict public input and uses only the command-owned create profile', async () => {
    let profile = null;
    const apiClient = { call: async (_name, opts) => {
      if (opts.path === '/repos/owner/repo') return response(200, { private: false, visibility: 'public' });
      if (opts.path.includes('/git/ref/heads/main')) return response(200, refBody(BASE));
