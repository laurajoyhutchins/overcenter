...
export async function runGithubPullRequestCreateRegressionTests() {
  ...
  await test('non-public owner repository coordinates are rejected without returning the coordinate', async () => { const apiClient={call:async(_name,opts)=>{if(opts.path==='/repos/owner/repo')return response(200,{private:false,visibility:'public'});if(opts.path==='/repos/laurajoyhutchins/private-example')return response(404,{message:'Not Found'});throw new Error('metadata rejection must precede branch reads and mutation');}}; const result=await createGithubPullRequest({...request,body:'See laurajoyhutchins/private-example'},{apiClient}); check(result.ok===false&&result.error==='PUBLIC_METADATA_POLICY_VIOLATION',`unexpected ${JSON.stringify(result)}`); check(!JSON.stringify(result).includes('private-example'),'policy failure leaked coordinate'); });
