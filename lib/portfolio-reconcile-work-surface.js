...
const ITEM_REJECTION_CODES = new Set([
  ...
  'GITHUB_PERMISSION_DENIED', 'SOURCE_NOT_FOUND', 'SOURCE_NOT_ISSUE',
  ...
  return {
    async inspectIssue(repo, issueNumber) {
      try {
        ...
        if ([401, 403, 422].includes(Number(error?.status))) throw githubApiError('GITHUB_PERMISSION_DENIED', message, { upstream_status: Number(error.status) }, 403);
...
export const portfolioReconcileConfig = Object.freeze({
  ...
  stable_reasons: [
    ...
    'GITHUB_APP_INSTALLATION_NOT_FOUND', 'GITHUB_PERMISSION_DENIED',
  ],
});
