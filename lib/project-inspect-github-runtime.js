...
export function projectInspectForGitHub(options={}){
  ...
  return Object.freeze({
    async inspect(input){
      ...
      const transitions=createPromotionAwareProjectTransitionLeaseService({store:leaseStore,operationStore,readProjectGraph:readSnapshot});
      const inspect=projectInspectFor({
        ...
        async readTransitionOccupancy({project_ref,transition_id,authority_revision,observed_at}){
          ...
          return Object.freeze({occupied:Boolean(lease),expires_at:timestamp(lease?.expires_at),suspended,suspension_reason:suspended?'blocked_settlement_promotion':null,suspension:suspension||null});
        },
      });
