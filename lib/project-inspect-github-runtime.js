  ...
  return Object.freeze({
    async inspect(input) {
      ...
      const inspect = projectInspectFor({
        ...
        async readTransitionOccupancy({ project_ref, transition_id, observed_at, authority_revision }) {
          const active = await leaseStore.getActiveLeasesForTransition(project_ref, transition_id, observed_at, authority_revision);
