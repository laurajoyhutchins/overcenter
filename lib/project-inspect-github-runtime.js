  ...
  return Object.freeze({
    async inspect(input) {
      ...
      const inspect = projectInspectFor({
        ...
        readAuthoringOperations({ project_ref }) {
          return operationStore.listByScope({ command:'project.authoring.recovery', scope:`project:${project_ref}`, limit:8 });
        },
        async readTransitionOccupancy({ project_ref, transition_id, authority_revision, observed_at }) {
          ...
          const [active, suspension] = await Promise.all([
            leaseStore.getActiveLeasesForTransition(project_ref, transition_id, observed_at),
            transitions.suspensionFor({ project_ref, transition_id }),
          ]);
