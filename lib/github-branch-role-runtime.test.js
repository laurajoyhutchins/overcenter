...
function configuredRoles() {
  return {
    async get() {
      return { ok: true, repository: 'laurajoyhutchins/overcenter', development_branch: 'develop', production_branch: 'main', production_source_ref: 'hatchable:test', changed: false };
    },
  };
}
...
export async function runGithubBranchRoleRuntimeTests() {
  ...
  await test('configured PR creation rejects production base before delegate', async () => {
    ...
    assert(result.expected_base === 'develop', 'configured development base missing');
    assert(delegated === 0, 'delegate ran before branch-role rejection');
  });

  await test('configured PR creation accepts configured development base', async () => {
    let delegated = 0;
    const result = await createGithubPullRequestRoleAware(
      { repo: 'laurajoyhutchins/overcenter', base: 'develop' },
      { branchRoleService: configuredRoles(), createPullRequest: async () => { delegated += 1; return { ok: true, outcome: 'delegated' }; } },
    );
    assert(result.ok === true && result.outcome === 'delegated', 'configured development base was not delegated');
  ...
  await test('configured changeset rejects development and production but delegates work branch', async () => {
