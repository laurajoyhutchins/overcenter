...
// Test cases register directly with the native Node test runner.
  ...
  test('configured PR creation rejects production base before delegate', async () => {
    let delegated = 0;
  ...
  test('unconfigured PR creation preserves existing delegate behavior', async () => {
    let delegated = 0;
    const result = await createGithubPullRequestRoleAware(
  ...
  test('configured changeset rejects dev and production but delegates work branch', async () => {
  ...
  test('integration rereads PR base and rejects production before delegate', async () => {
    let delegated = 0;
  ...
  test('malformed requests retain original command validation path', async () => {
    let delegated = 0;
