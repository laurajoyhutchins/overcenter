...
export function createPostgresScheduledExecutionRuntimeStore(dbBinding) {
  return {
...
export function createScheduledExecutionContextService({
  ...
  const runService = runs || createPostgresOrchestrationRunService({ db:dbBinding, api:apiBinding, now });
  const store = runtimeStore || createPostgresScheduledExecutionRuntimeStore(dbBinding);
  const workerTransport = transport || ((command, input) => executeSemanticWorkerCommand(command, input, { db:dbBinding, api:apiBinding, githubAppAuth }));
  const portfolioReconcile = reconcile || ((input) => executeCorrelatedCommand(
    'portfolio.reconcile_work_surface',
    input,
    (request) => reconcilePortfolioWorkSurfaceWithGitHubApp(request, { db:dbBinding, api:apiBinding, githubAppAuth }),
  ...
  const runtimeMaintenance = maintenance || ((input) => executeCorrelatedCommand(
    ...
    input,
    () => createPostgresOrchestrationMaintenanceService({ db:dbBinding, api:apiBinding, now }).maintain(),
...
export function createPostgresScheduledExecutionContextService(options = {}) {
  ...
  if (!githubAppAuth || typeof githubAppAuth.withApiClient !== 'function') throw err('RUNTIME_PROVIDER_MISSING', 'GitHub App auth provider is required', { provider:'githubAppAuth' });
  return createScheduledExecutionContextService({
    runs: options.runs || createPostgresOrchestrationRunService({ ...options, db:dbBinding, api:apiBinding }),
    runtimeStore: options.runtimeStore || createPostgresScheduledExecutionRuntimeStore(dbBinding),
    transport: options.transport || ((command, input) => executeSemanticWorkerCommand(command, input, { db:dbBinding, api:apiBinding, githubAppAuth })),
