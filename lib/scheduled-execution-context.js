...
export function createPostgresScheduledExecutionRuntimeStore(dbBinding = db) {
  return {
    ...
    async claimRecoveryStateForRun(runId, observedAt = new Date().toISOString()) {
      const result = await dbBinding.query(
        `SELECT lease_id AS lease_ref,work_ref,gate,status,expires_at
           FROM work_leases
          WHERE run_id=$1
  ...
  async function execute(input = {}) {
    ...
    if (operation === 'claim') {
      ...
      if (response.status >= 400 || response.body?.ok !== true) {
        let recoveryState = await store.claimRecoveryStateForRun(context.run_id, now());
        if (recoveryState?.status === 'claiming') {
          ...
          recoveryState = await store.claimRecoveryStateForRun(context.run_id, now());
        }
