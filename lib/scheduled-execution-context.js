...
const LANE_BY_PARTICIPANT = Object.freeze({
  'portfolio-dispatcher': 'lane:enable',
...
const OPERATION_FIELDS = Object.freeze({
  ...
  settle:new Set(['disposition','evidence','reason','promotion_condition','requeue_class','operating_condition','continuation','lifecycle_facts']),
  skill_activate:new Set(['skill','reason']),
  skill_complete:new Set(['activation_id','outcome','evidence']),
...
function isRunHasActiveLeaseError(error) {
  return error?.code === 'RUN_HAS_ACTIVE_LEASE' || String(error?.message || '').includes('RUN_HAS_ACTIVE_LEASE');
}
...
export function createPostgresScheduledExecutionRuntimeStore(dbBinding = db) {
  return {
    ...
    async claimRecoveryStateForRun(runId, workRef, observedAt = new Date().toISOString()) {
      const result = await dbBinding.query(
        `SELECT lease_id AS lease_ref,work_ref,gate,status,expires_at
           FROM work_leases
          WHERE run_id=$1
            AND work_ref=$2
            AND status IN ('claiming','active','settling')
            AND expires_at > $3
          ORDER BY created_at DESC
          LIMIT 1`,
        [runId, workRef, observedAt],
      );
      return result.rows?.[0] || null;
    },
    async leaseStateByRef(leaseRef) {
      const result = await dbBinding.query(
        `SELECT lease_id AS lease_ref,work_ref,gate,status,expires_at,settle_receipt
           FROM work_leases
          WHERE lease_id=$1
          LIMIT 1`,
        [leaseRef],
...
export function createScheduledExecutionContextService({
  ...
  if (!store || typeof store.activeLeaseForRun !== 'function' || typeof store.claimRecoveryStateForRun !== 'function') {
    throw new TypeError('runtimeStore.activeLeaseForRun and runtimeStore.claimRecoveryStateForRun are required');
  }
  ...
  async function bootstrap(input = {}) {
    ...
    if (participant.id === 'portfolio-dispatcher') {
      ...
      if (maintenanceResult.status >= 400 || maintenanceResult.body?.ok !== true) {
        const errorCode = String(maintenanceResult.body?.error || 'ORCHESTRATION_MAINTENANCE_ERROR');
        const finished = await runService.finish({
          run_id:runId,
          disposition:'failed',
          last_work_ref:null,
          last_gate:lane,
          stop_reason:`scheduled runtime terminalized after failed orchestration.maintain:${errorCode}`,
        });
        throw err('RUNTIME_MAINTENANCE_FAILED', 'scheduled runtime maintenance failed', {
          failure:maintenanceResult.body || null,
  ...
  async function execute(input = {}) {
    ...
    const publicContext = { participant:participant.id, stage:participant.stage, command:participant.command, cycle_id:context.cycle_id, lane:context.lane };
    ...
    if (operation === 'claim') {
      const existingOwnership = await store.activeLeaseForRun(context.run_id, now());
      if (existingOwnership) {
        return {
          ...
          failure:{
            ok:false,
            error:'ACTIVE_LEASE_PRESENT',
            message:'scheduled run already owns live work',
            work_ref:existingOwnership.work_ref || null,
            gate:existingOwnership.gate || null,
          },
          http_status:409,
        };
      }
      const claimInput = { ...semanticInput, run_id:context.run_id };
      let response = await workerTransport('work.claim', claimInput);
      const safeRetry = response.status >= 400
        && response.body?.ok === false
        && response.body?.retryable === true
        && response.body?.may_have_mutated === false
        && response.body?.recommended_action === 'retry_same_request';
      if (safeRetry) response = await workerTransport('work.claim', claimInput);
      if (response.status >= 400 || response.body?.ok !== true) {
        let recoveryState = await store.claimRecoveryStateForRun(context.run_id, claimInput.work_ref, now());
        if (recoveryState?.status === 'claiming') {
          response = await workerTransport('work.claim', claimInput);
          if (response.status < 400 && response.body?.ok === true) {
            return { ok:true, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:false, result:safeWorkerResult(response.body) };
          }
          recoveryState = await store.claimRecoveryStateForRun(context.run_id, claimInput.work_ref, now());
        }
        if (recoveryState) return { ok:false, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:false, failure:response.body, http_status:response.status };
        const ownedLease = await store.activeLeaseForRun(context.run_id, now());
        if (ownedLease) return { ok:false, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:false, failure:response.body, http_status:response.status };
        const rejected = response.body?.rejection === true;
        const errorCode = String(response.body?.error || 'WORK_CLAIM_ERROR');
        let finished;
        try {
          finished = await runService.finish({
            run_id:context.run_id,
            disposition:rejected ? 'clean-stop' : 'failed',
            last_work_ref:semanticInput.work_ref || null,
            last_gate:context.lane,
            stop_reason:`scheduled runtime terminalized after failed work.claim:${errorCode}`,
          });
        } catch (error) {
          if (isRunHasActiveLeaseError(error)) {
            return { ok:false, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:false, failure:response.body, http_status:response.status };
          }
          throw error;
        }
        return { ok:false, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:true, failure:response.body, http_status:response.status, run_receipt:finished.run_receipt || null };
      }
      return { ok:true, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:false, result:safeWorkerResult(response.body) };
    }

    if (operation === 'skill_activate' || operation === 'skill_complete') {
      const command = operation === 'skill_activate' ? 'skill.activate' : 'skill.complete';
      const response = await workerTransport(command, { ...semanticInput, run_id:context.run_id });
    ...
    const settlementInput = { lease_ref:lease.lease_ref, ...semanticInput };
    let response = await workerTransport('work.settle', settlementInput);
    if (response.status >= 400 || response.body?.ok !== true) {
      let leaseState = typeof store.leaseStateByRef === 'function' ? await store.leaseStateByRef(lease.lease_ref) : null;
      if (leaseState?.status === 'settled' && leaseState.settle_receipt?.ok === true) {
        response = { status:200, body:{ ...leaseState.settle_receipt, idempotent_replay:true } };
      } else if (leaseState?.status === 'active' || leaseState?.status === 'settling') {
        response = await workerTransport('work.settle', settlementInput);
        if (response.status >= 400 || response.body?.ok !== true) {
          leaseState = typeof store.leaseStateByRef === 'function' ? await store.leaseStateByRef(lease.lease_ref) : null;
          if (leaseState?.status === 'settled' && leaseState.settle_receipt?.ok === true) {
            response = { status:200, body:{ ...leaseState.settle_receipt, idempotent_replay:true } };
          }
        }
    ...
    if (response.status >= 400 || response.body?.ok !== true) return { ok:false, schema:OPERATION_SCHEMA, ...publicContext, operation, terminal:false, failure:response.body, http_status:response.status };
