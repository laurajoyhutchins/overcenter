...
export function classifyOrchestrationFailure(input = {}) {
  ...
  const mutationStatusKnown = input.may_have_mutated === true || input.may_have_mutated === false;
  ...
  if (code === 'RECOVERY_FAILED') {
    ...
    && (
      ...
      || code.endsWith('_TRANSPORT_ERROR')
      || (errorClass === 'upstream' && retryable)
    )
  ) {
    result = base(
      'INDETERMINATE_EXTERNAL_EFFECT',
      ...
      recovery('orchestration.diagnose', null, { mode: 'reconcile_authoritative_effect' }),
      true,
      'missing_mutation_status_requires_authoritative_reconciliation',
    );
  } else if (
    mutationStatusKnown
    && !mayHaveMutated
