...
function durableRunId(details) {
  const value = typeof details?.run_id === 'string' ? details.run_id.trim() : '';
  return value || null;
}

function indeterminateRecovery(command, details) {
  if (command === 'project.amend' || command === 'project.define') {
    return recovery('project.inspect', null, {
      mode:'reconcile_authoritative_project_definition',
      use_original_project_ref:true,
      requires:['original_project_ref','fresh_authoritative_revision'],
    });
  }
  const runId = durableRunId(details);
  if (command.startsWith('orchestration.') && runId) {
    return recovery('orchestration.diagnose', { run_id:runId }, {
      mode:'reconcile_authoritative_effect',
    });
  }
  return null;
}
...
export function classifyOrchestrationFailure(input = {}) {
  ...
  const errorClass = String(input.error_class || '');
  ...
  if (code === 'RECOVERY_FAILED') {
  ...
  } else if (code === 'PROJECT_AUTHORING_INTEGRATION_PENDING' || code === 'PRODUCTION_RECONCILIATION_SOURCE_NOT_VERIFIED' || code === 'PRODUCTION_PROMOTION_SOURCE_NOT_VERIFIED') {
    result = base(
      'WAITING_EXTERNAL_VERIFICATION',
      false,
      recovery(command || (code === 'PROJECT_AUTHORING_INTEGRATION_PENDING' ? 'project.amend' : 'production.reconcile'), null, {
        mode: 'retry_same_request_after_external_verification',
    ...
    const operation = indeterminateRecovery(command, details);
    result = base(
      'INDETERMINATE_EXTERNAL_EFFECT',
      false,
      operation,
      ...
      operation
        ...
        : 'potential_external_mutation_has_no_safe_automatic_recovery_identity',
    );
  } else if (command === 'github.apply_text_replacements' && code === 'TEXT_PRECONDITION_FAILED') {
    result = base(
      'REQUEST_PRECONDITION_CHANGED',
      false,
      recovery('github.apply_text_replacements', null, {
  ...
  } else if (command === 'github.apply_changeset' && code === 'CREATE_TARGET_EXISTS') {
    result = base(
      'REQUEST_PRECONDITION_CHANGED',
      false,
      recovery('github.apply_changeset', null, {
        mode: 'reread_exact_workspace_tree_and_recompute_request',
        use_original_request: false,
        requires: ['exact_workspace_tree', 'corrected_changeset_operation'],
      }),
      false,
      null,
    );
  ...
  } else if (errorClass === 'validation' || code === 'REQUEST_INVALID' || code.startsWith('INVALID_')) {
    result = base('REQUEST_INVALID', false, null, false, null);
  } else {
