...
export function createProjectTransitionLeasePostgresStore(dbBinding, options = {}) {
  ...
  async function subjectLeaseWithHistory(sql, params, historySql, historyParams) {
    const current = await subjectLease(sql, params);
    if (current) return current;
    return restoreProjectTransitionLease(await one(historySql, historyParams));
  }
  ...
  return Object.freeze({
    async getLease(leaseId) {
      return subjectLeaseWithHistory(
    ...
    async getLeaseByAcquireIdempotency(idempotencyKey) {
      return subjectLeaseWithHistory(
        ...
        [`${IDEMPOTENCY_PREFIX}${required(idempotencyKey, 'idempotencyKey')}`]
      );
    },
    async getLatestSettledLeaseForTransition(projectRef, transitionId, observedAt = null) {
      ...
      const historyTimeClause = observedAt ? ' AND settled_at <= $3' : '';
      return subjectLeaseWithHistory(
        `SELECT e.lease_ref::text AS lease_id,
                e.subject_key, e.subject_kind, e.execution_id, e.operation_id,
                e.project_ref, e.transition_id, e.run_id, e.lifecycle, e.settled,
        ...
        `SELECT * FROM work_leases
           WHERE claim_receipt->>'subject'='project_transition'
             ...
             AND status='settled'${historyTimeClause}
           ORDER BY settled_at DESC NULLS LAST, created_at DESC, lease_id DESC
           LIMIT 1`,
        params,
      );
    },
        ...
        const tx = await dbBinding.transaction([
          ...
          },
          {
            sql:`INSERT INTO operation_state (
             ...
             SELECT $1,'execution.transaction',$2,$3,$4,'prepared',
                    execution_id,subject_key,run_id,lease_epoch,authority_epoch,
                    authority_repository,authority_revision,0,false,
                    'definitely_not_mutated',$8
               FROM execution_state
              WHERE subject_key=$5 AND lease_ref=$6 AND run_id=$7
             ON CONFLICT (command,idempotency_scope,idempotency_key) DO NOTHING
             RETURNING operation_id
           `,
            params:[
        ...
        if (uniqueViolation(error) || error?.code === '22012') {
          const concurrent = await one(
            `SELECT *
              ...
              LIMIT 1`,
            [STORAGE_SCOPE, persisted.idempotency_key],
          );
          const sameAcquire = concurrent
            && String(concurrent.acquire_request_hash || '') === persisted.acquire_request_hash
            && String(concurrent.run_id || '') === persisted.run_id
            && String(concurrent.project_ref || '') === projectRef
            && String(concurrent.transition_id || '') === transitionId
            && String(concurrent.authority_repository || '') === repository
            && String(concurrent.authority_revision || '') === authorityRevision;
          const replay = sameAcquire ? restoreCanonicalProjectTransitionLease(concurrent) : null;
    ...
    async settleLeaseAtomically(input) {
      ...
      const settlementRequestSha256 = required(input?.settlement_request_sha256, 'settlement_request_sha256');
      ...
      if (updated && Number(atomicityGuard) !== 1) {
        throw Object.assign(new Error('project transition lease and canonical execution could not be atomically settled'), { code:'PROJECT_TRANSITION_LEASE_STALE' });
      }
