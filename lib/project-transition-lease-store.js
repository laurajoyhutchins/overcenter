  ...
  return Object.freeze({
      ...
      if (observedAt) {
        return subjectLease(
          `SELECT *
            ...
            LIMIT 1`,
          [projectRef, transitionId, required(observedAt, 'observedAt')],
        );
      }
      return subjectLease(
        `SELECT *
    ...
    async getActiveLeasesForProject(projectRef, observedAt) {
      const rows = await many(
        ...
        [STORAGE_SCOPE, required(projectRef, 'projectRef'), required(observedAt, 'observedAt')],
      );
    ...
    async getActiveLeasesForTransition(projectRef, transitionId, observedAt, authorityRevision = null) {
      const common = [STORAGE_SCOPE, required(projectRef, 'projectRef'), required(transitionId, 'transitionId'), required(observedAt, 'observedAt')];
      const rows = await many(
        authorityRevision
          ? `SELECT *
               FROM work_leases
              WHERE gate=$1
                ...
                AND status='active'
                AND expires_at > $4
                AND claim_receipt->'project_transition'->>'authority_revision' = $5
              ORDER BY created_at ASC, lease_id ASC
              LIMIT 8`
          : `SELECT *
               FROM work_leases
              ...
              ORDER BY created_at ASC, lease_id ASC
              LIMIT 8`,
        authorityRevision ? [...common, required(authorityRevision, 'authorityRevision')] : common,
      );
      return rows.map(restoreProjectTransitionLease).filter(Boolean);
    },
        ...
        const tx = await dbBinding.transaction([
          {
            sql:`INSERT INTO execution_state (
               ...
               idempotency_key,intent_sha256,settled,settlement_receipt,settled_at,updated_at
             ) VALUES (
               $29,$1,'project_transition',$2,$3,$30,'executing',$35,$5,$6,$35,
               $7,$8,$9,$10,$11,$12,$13,$14,NULL,NULL,'[]'::jsonb,0,NULL,
               0,'definitely_not_mutated',NULL,$32,$33,$34,$31,false,NULL,NULL,$4
             )
          ...
          },
          {
            sql:`INSERT INTO operation_state (
             ...
             SELECT $22,'execution.transaction',$24,$25,$23,'prepared',
                    execution_id,subject_key,run_id,lease_epoch,authority_epoch,
                    authority_repository,authority_revision,0,false,
                    'definitely_not_mutated',$26
               FROM execution_state
    ...
    async settleLeaseAtomically(input) {
      ...
      const currentExecution = await one(
        'SELECT * FROM execution_state WHERE subject_key=$1 LIMIT 1',
        [slotKey],
      );
      if (!currentExecution
          || String(currentExecution.lease_ref || '') !== leaseId
          || String(currentExecution.run_id || '') !== runId
          || epoch(currentExecution.authority_epoch) !== authorityEpoch
          || !currentExecution.execution_id
          || !currentExecution.operation_id) {
        throw Object.assign(new Error('project transition execution identity is stale'), { code:'PROJECT_TRANSITION_LEASE_STALE' });
      }
      const executionId = required(currentExecution.execution_id, 'execution_id');
      const operationId = required(currentExecution.operation_id, 'operation_id');
