...
export function createProjectTransitionLeasePostgresStore(dbBinding, options = {}) {
  ...
  return Object.freeze({
    async getLease(leaseId) {
      return subjectLease(
        `SELECT e.lease_ref::text AS lease_id,
                ...
                e.settlement_receipt, e.settled_at, e.intent_sha256,
                e.idempotency_key, o.request_sha256 AS acquire_request_hash,
    ...
    async reconcileExpired(slotKey, leaseId, observedAtValue = new Date().toISOString()) {
      ...
      if (!canonical) throw Object.assign(new Error('project transition canonical execution could not be expired safely'), { code:'PROJECT_TRANSITION_LEASE_STALE' });
