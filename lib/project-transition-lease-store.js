...
export function createProjectTransitionLeasePostgresStore(dbBinding, options = {}) {
  ...
  return Object.freeze({
    ...
    async getLatestSettledLeaseForTransition(projectRef, transitionId) {
      return subjectLease(
        `SELECT *
           FROM work_leases
    ...
    async getActiveLeasesForTransition(projectRef, transitionId, observedAt, authorityRevision = null) {
      const revision = authorityRevision == null ? null : required(authorityRevision, 'authorityRevision');
      const rows = await many(
        `SELECT *
           FROM work_leases
          WHERE gate=$1
            ...
            AND status='active'
            AND expires_at > $4
            ${revision ? "AND claim_receipt->'project_transition'->>'authority_revision' = $5" : ''}
          ORDER BY created_at ASC, lease_id ASC
          LIMIT 8`,
        revision
          ? [STORAGE_SCOPE, required(projectRef, 'projectRef'), required(transitionId, 'transitionId'), required(observedAt, 'observedAt'), revision]
