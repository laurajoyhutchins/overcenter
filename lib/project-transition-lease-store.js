      ...
      const tx = await dbBinding.transaction([
        {
          sql:`UPDATE work_leases l
                ...
                WHERE l.lease_id=$1 AND l.work_ref=$2 AND l.gate=$3 AND l.run_id=$9
                  AND l.claim_receipt->>'subject'='project_transition'
                  AND l.claim_receipt->'project_transition' @> $8::jsonb
                  AND l.status='active'
        ...
        },
        {
          sql:`DELETE FROM work_lease_slots s
                WHERE s.work_ref=$2 AND s.gate=$3 AND s.lease_id=$1
                  AND EXISTS (SELECT 1 FROM work_leases l WHERE l.lease_id=$1 AND l.status='settled' AND l.settle_idempotency_key=$4)
                RETURNING s.lease_id::text AS lease_id`,
          params,
        },
        {
          sql:`SELECT 1 / CASE WHEN
                  EXISTS (SELECT 1 FROM work_leases l WHERE l.lease_id=$1 AND l.work_ref=$2 AND l.gate=$3 AND l.run_id=$9 AND l.status='settled' AND l.settle_idempotency_key=$4 AND l.settle_plan->>'disposition'=$10)
