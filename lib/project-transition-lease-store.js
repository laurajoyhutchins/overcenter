        ...
        const inserted = await one(
          `WITH advanced AS (
             ...
             SELECT $5,$15,$16,$6,$14,$17,$18,$19,$20,$4,$12,$21,$22,$23,$24,$25,$26,
                    jsonb_set($27::jsonb,'{project_transition,authority_epoch}',to_jsonb(advanced.authority_epoch),true),
                    $28::jsonb,$13
               FROM advanced
             RETURNING *
           ), inserted_slot AS (
             INSERT INTO work_lease_slots (work_ref,gate,lease_id,expires_at)
             SELECT $15,$16,$5,$12 FROM inserted_lease
             RETURNING lease_id
           )
           SELECT inserted_lease.* FROM inserted_lease
            WHERE EXISTS (SELECT 1 FROM inserted_slot)`,
          params,
        );
        if (!inserted) {
