...
export function createProjectAuthoringWorkerBinding(options = {}) {
  const createGraphRuntime = requireFunction(options.createGraphRuntime, 'createGraphRuntime');
  const readRepositoryDisposition = requireFunction(options.readRepositoryDisposition, 'readRepositoryDisposition');
  const applyGithubChangeset = requireFunction(options.applyGithubChangeset, 'applyGithubChangeset');
  const deriveProjectGraph = requireFunction(options.deriveProjectGraph, 'deriveProjectGraph');
  ...
  return function projectAuthoringFor(runtime = {}) {
    ...
    const resolve = () => {
      ...
      composed = createProjectAuthoringHostRuntime({
        graphRuntime,
        readRepositoryDisposition:(repository) => readRepositoryDisposition(repository, runtime),
        applyGithubChangeset:(request, writerOptions) => applyGithubChangeset(request, writerOptions, runtime),
    ...
    return Object.freeze({
      define(input) { return resolve().define(input); },
      amend(input) { return resolve().amend(input); },
    });
  };
}