...
// Assertions stay intentionally explicit while execution is owned by node:test.
...
function client(responses, calls, openPulls = {}) {
  return {
    async call(_service, request) {
      const role = String(request.path).includes('head=') ? 'head' : 'base';
      return { status: 200, headers: {}, body: openPulls[role] || [] };
    },
...
test('normalization requires exact fenced input', async () => {
  ...
  test('default branch is rejected before mutation', async () => {
    const calls = [];
  ...
  test('stale expected head is rejected before mutation', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
  ...
  test('absent branch is idempotent success', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
  ...
  test('open pull request head blocks deletion', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
      { apiClient: client([okPreflight()], calls, { head: [{ number: 12 }] }), sleep: async () => {} },
    );
    assert(result.ok === false && result.error === 'GITHUB_REF_REJECTED', 'open PR head did not block deletion');
    assert(result.reason === 'open_pull_request_head', 'open head reason missing');
    assert(calls.length === 1, 'mutation ran while PR head remained open');
  });

  test('open pull request base blocks deletion', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
      { apiClient: client([okPreflight()], calls, { base: [{ number: 13 }] }), sleep: async () => {} },
    );
    assert(result.ok === false && result.error === 'GITHUB_REF_REJECTED', 'open PR base did not block deletion');
    assert(result.reason === 'open_pull_request_base', 'open base reason missing');
    assert(calls.length === 1, 'mutation ran while PR base remained open');
  });

  test('delete uses atomic beforeOid to zero-OID compare-and-swap', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
  ...
  test('atomic rejection reconciles moved head as HEAD_MISMATCH', async () => {
    const calls = [];
    const rejected = {
      status: 200,
      headers: {},
  ...
  test('policy rejection remains expected ref rejection', async () => {
    const calls = [];
    const rejected = {
      status: 200,
      headers: {},
  ...
  test('mutation transport loss is explicitly indeterminate and retry-safe', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
  ...
  test('delete applies in GitHub, response is lost, and exact retry proves already_absent', async () => {
    let exists = true;
    let mutationCalls = 0;
    const apiClient = {
      async call() { return { status: 200, headers: {}, body: [] }; },
      async graphql(query) {
        if (String(query).includes('updateRefs')) {
          mutationCalls += 1;
          exists = false;
          throw new Error('response lost after GitHub applied deletion');
        }
        return okPreflight({ exists });
      },
    };
    const request = { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED };
    const first = await deleteGithubBranch(request, { apiClient, sleep: async () => {} });
    assert(first.ok === false && first.error === 'BRANCH_DELETE_INDETERMINATE', 'first call did not report indeterminate deletion');
    assert(first.may_have_mutated === true, 'first call omitted mutation ambiguity');
    const second = await deleteGithubBranch(request, { apiClient, sleep: async () => {} });
    assert(second.ok === true && second.outcome === 'already_absent', 'retry did not prove the branch absent');
    assert(mutationCalls === 1, 'retry dispatched a duplicate deletion');
  });

  // Native node:test reports suite disposition directly; no result collector remains.