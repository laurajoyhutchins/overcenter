...
export async function runGithubDeleteBranchTests() {
  ...
  results.push(await run('open pull request head blocks deletion', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
  ...
  results.push(await run('open pull request base blocks deletion', async () => {
    const calls = [];
    const result = await deleteGithubBranch(
      { repo: 'owner/repo', branch: 'feature/test', expected_head: EXPECTED },
  ...
  results.push(await run('delete applies in GitHub, response is lost, and exact retry proves already_absent', async () => {
    let exists = true;
    let mutationCalls = 0;
    const apiClient = {
