...
function project(row, changed = false) {
  ...
  return {
    ...
    github_repository_id:row.github_repository_id == null ? null : normalizeGitHubRepositoryId(row.github_repository_id),
    disposition,
    ...
    controls:repositoryControlsProjection({ disposition }),
    ...
    async put(row) {
      const result = await dbBinding.query(`INSERT INTO portfolio_repository_disposition
        ...
        ON CONFLICT (repository) DO UPDATE SET
          github_repository_id=COALESCE(portfolio_repository_disposition.github_repository_id, EXCLUDED.github_repository_id),
          disposition=EXCLUDED.disposition,
          compatibility_bound=false,
        ...
        WHERE portfolio_repository_disposition.github_repository_id IS NULL OR EXCLUDED.github_repository_id IS NULL OR portfolio_repository_disposition.github_repository_id=EXCLUDED.github_repository_id
        RETURNING *`, [row.repository, row.github_repository_id || null, row.disposition, false, null, row.successor_repository || null, row.github_archived === true, row.github_observed_at || null, row.transition_reason || null, row.transitioned_at, row.updated_at]);
      if (!result.rows?.[0]) throw lifecycleError('REPOSITORY_IDENTITY_COORDINATE_CONFLICT', 'repository coordinate is already bound to a different GitHub repository id', { repository:row.repository, github_repository_id:row.github_repository_id || null }, 409);
...
export function createGitHubRepositoryObserver(options = {}) {
  ...
  return { async getRepository(repositoryInput) {
    const repository = canonicalRepository(repositoryInput); const [owner, repo] = repository.split('/');
...
function semanticEqual(existing, desired) {
  ...
  return Number(existing.github_repository_id || 0) === Number(desired.github_repository_id || 0)
    && String(existing.disposition) === String(desired.disposition)
    && existing.compatibility_bound !== true && !existing.compatibility_reference
...
export function createRepositoryLifecycleService({ store, github, coordinateProjection = null, now = () => new Date().toISOString() } = {}) {
  ...
  async function resolveIdentity(repositoryInput) {
    ...
    const githubRepositoryId = normalizeGitHubRepositoryId(githubRepository.id);
    let byIdentity = typeof store.getByGitHubRepositoryId === 'function' ? await store.getByGitHubRepositoryId(githubRepositoryId) : null;
    let byCoordinate = await store.get(repository);

    if (byIdentity && canonicalRepository(byIdentity.repository).toLowerCase() !== repository.toLowerCase()) {
      if (byCoordinate && Number(byCoordinate.github_repository_id || 0) !== githubRepositoryId) throw lifecycleError('REPOSITORY_IDENTITY_COORDINATE_CONFLICT', 'renamed repository coordinate is already owned by a different GitHub repository id', { repository, github_repository_id:githubRepositoryId, conflicting_github_repository_id:byCoordinate.github_repository_id || null }, 409);
      ...
      byIdentity = await store.rebindRepositoryIdentity({ github_repository_id:githubRepositoryId, from_repository:fromRepository, to_repository:repository, updated_at:now() });
      if (!byIdentity) throw lifecycleError('REPOSITORY_IDENTITY_REBIND_FAILED', 'repository identity rebind did not persist', { from_repository:fromRepository, to_repository:repository }, 500);
      byCoordinate = byIdentity;
    } else if (!byIdentity && byCoordinate?.github_repository_id != null && Number(byCoordinate.github_repository_id) !== githubRepositoryId) {
      throw lifecycleError('REPOSITORY_IDENTITY_COORDINATE_CONFLICT', 'repository coordinate belongs to a different GitHub repository id', { repository, github_repository_id:githubRepositoryId, conflicting_github_repository_id:byCoordinate.github_repository_id }, 409);
    } else if (!byIdentity && byCoordinate && byCoordinate.github_repository_id == null && typeof store.bindGitHubRepositoryId === 'function') {
      byCoordinate = await store.bindGitHubRepositoryId(repository, githubRepositoryId) || byCoordinate;
      byIdentity = byCoordinate;
    }
  ...
  async function observe(repositoryInput) {
    const resolved = await resolveIdentity(repositoryInput);
    const { githubRepository, repository, githubRepositoryId } = resolved;
    ...
    if (archived && !DISPOSED.has(disposition)) { disposition='ARCHIVED'; reason='github_archived_observed'; transitionedAt=observedAt; }
    const desired = { repository, github_repository_id:githubRepositoryId, disposition, successor_repository:existing?.successor_repository || null, github_archived:archived, github_observed_at:observedAt, transition_reason:reason, transitioned_at:transitionedAt, updated_at:observedAt };
    const changed = !semanticEqual(existing, desired);
    return project(await store.put(desired), changed);
  }
  ...
  async function dispose(input = {}) {
    ...
    const resolved = await resolveIdentity(input.repository);
    const canonical = resolved.repository; const githubArchived = resolved.githubRepository?.archived === true; const existing = resolved.existing;
    ...
    if (Object.prototype.hasOwnProperty.call(input, 'compatibility_bound') || Object.prototype.hasOwnProperty.call(input, 'compatibility_reference')) throw lifecycleError('LEGACY_CONTROL_PLANE_RETIRED', 'repository compatibility work was retired with the Agent Execution Control Plane; disposed repositories cannot retain a compatibility execution exception', { repository:canonical, replacement:'busbar' }, 410);
    const timestamp = now();
    const reason = String(input.reason || existing?.transition_reason || 'repository_disposed').slice(0,500);
    const desired = { repository:canonical, github_repository_id:resolved.githubRepositoryId, disposition, successor_repository:successor, github_archived:githubArchived, github_observed_at:timestamp, transition_reason:reason, transitioned_at:existing && semanticEqual(existing,{ ...existing, github_repository_id:resolved.githubRepositoryId, disposition, successor_repository:successor, github_archived:githubArchived, transition_reason:reason }) ? existing.transitioned_at : timestamp, updated_at:timestamp };
    const changed = !semanticEqual(existing, desired);
    return project(await store.put(desired), changed);
  }
  ...
  async function transition(input = {}) {
    ...
    const resolved = await resolveIdentity(input.repository);
    const repository = resolved.repository; const existing = resolved.existing;
...
export function createPostgresRepositoryLifecycleService(options = {}) {
  return createRepositoryLifecycleService({ store:createPostgresRepositoryDispositionStore(options.db || db), github:options.github || createGitHubRepositoryObserver(options), coordinateProjection:options.coordinateProjection || createLinearRepositoryCoordinateProjection({ apiBinding:options.api || api }), now:options.now });
}

export function statusForRepositoryDispositionError(error) {
  if (Number.isInteger(error?.status)) return error.status;
  if (/INVALID_|_CHANGED$|_ARCHIVED$|_DISPOSED$|_CONFLICT$/.test(String(error?.code || ''))) return 409;
  return 500;
}