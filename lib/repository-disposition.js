  ...
  return {
    ...
    async put(row) {
      const result = await dbBinding.query(`INSERT INTO portfolio_repository_disposition
        ...
        RETURNING *`, [row.repository, row.disposition, false, null, row.successor_repository || null, row.github_archived === true, row.github_observed_at || null, row.transition_reason || null, row.transitioned_at, row.updated_at]);
...
function semanticEqual(existing, desired) {
  ...
  return String(existing.disposition) === String(desired.disposition)
    && existing.compatibility_bound !== true
    && !existing.compatibility_reference
...
export function createRepositoryLifecycleService({ store, github, now = () => new Date().toISOString() } = {}) {
  ...
  async function dispose(input = {}) {
    ...
    if (Object.prototype.hasOwnProperty.call(input, 'compatibility_bound') || Object.prototype.hasOwnProperty.call(input, 'compatibility_reference')) {
      throw lifecycleError('LEGACY_CONTROL_PLANE_RETIRED', 'repository compatibility work was retired with the Agent Execution Control Plane; disposed repositories cannot retain a compatibility execution exception', { repository: canonical, replacement: 'portfolio_control_plane' }, 410);
    }
    const timestamp = now();
    const desired = {
