...
import { initialRepositoryDisposition, REPOSITORY_DISPOSITIONS } from './repository-registration-policy.js';

export { REPOSITORY_DISPOSITIONS };
...
export function createPostgresRepositoryDispositionStore(dbBinding) {
...
export function createRepositoryLifecycleService({store,github,coordinateProjection=null,now=()=>new Date().toISOString()}={}) {
  ...
  async function observeResolved(resolved){const {githubRepository,repository,githubRepositoryId}=resolved;const archived=githubRepository.archived===true;const existing=resolved.existing;const observedAt=now();let disposition=existing?.disposition?normalizedDisposition(existing.disposition):initialRepositoryDisposition({archived,existingDisposition:null});let reason=existing?.transition_reason||(existing?'github_observed_existing':(archived?'github_archived_observed':'repository_registered_dormant'));let transitionedAt=existing?.transitioned_at||observedAt;if(archived&&!DISPOSED.has(disposition)){disposition='ARCHIVED';reason='github_archived_observed';transitionedAt=observedAt;}const desired={repository,github_repository_id:githubRepositoryId,disposition,successor_repository:existing?.successor_repository||null,github_archived:archived,github_observed_at:observedAt,transition_reason:reason,transitioned_at:transitionedAt,updated_at:observedAt};const changed=!semanticEqual(existing,desired);return project(await store.put(desired),changed);}
  async function observe(repositoryInput){return observeResolved(await resolveIdentity(repositoryInput));}
  async function register(input={}){const repositoryInput=typeof input==='string'?input:input.repository;const resolved=await resolveIdentity(repositoryInput);const registrationCreated=!resolved.existing;const lifecycle=await observeResolved(resolved);return{ok:true,registration_created:registrationCreated,...lifecycle};}
  ...
  return {register,observe,dispose,transition,verify};
}

export function createPostgresRepositoryLifecycleService(options={}){return createRepositoryLifecycleService({store:createPostgresRepositoryDispositionStore(options.db),github:options.github||createGitHubRepositoryObserver(options),coordinateProjection:options.coordinateProjection||createLinearRepositoryCoordinateProjection({apiBinding:options.api}),now:options.now});}
