...
export function validateRepositoryExecutionResult(result, request) {
  ...
  for (const field of ['authority_revision', 'transition_id', 'lease_ref', 'run_id', 'transition_definition_fingerprint', 'executor_identity', 'executor_fingerprint']) {
  ...
  if (JSON.stringify(result.capabilities) !== JSON.stringify(request.capabilities)) {
    fail('REPOSITORY_EXECUTOR_RESULT_IDENTITY_MISMATCH', 'executor result capabilities do not match request', { field: 'capabilities' });
  }
  ...
  if (!summary || summary.length > 4096) fail('REPOSITORY_EXECUTOR_RESULT_INVALID', 'executor result summary is invalid');
  if (!Array.isArray(result.evidence) || result.evidence.length > 32) fail('REPOSITORY_EXECUTOR_RESULT_INVALID', 'executor result evidence is invalid');
  const evidence = result.evidence.map((entry, index) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry) || JSON.stringify(Object.keys(entry).sort()) !== JSON.stringify(['detail', 'kind'])) {
      fail('REPOSITORY_EXECUTOR_RESULT_INVALID', `executor evidence[${index}] has an invalid shape`);
    }
    const kind = String(entry.kind || '').trim();
    const detail = String(entry.detail || '').trim();
    if (!kind || kind.length > 128 || !detail || detail.length > 2048) fail('REPOSITORY_EXECUTOR_RESULT_INVALID', `executor evidence[${index}] is invalid`);
    return Object.freeze({ kind, detail });
  });
  if (result.status === 'completed' && evidence.length === 0) fail('REPOSITORY_EXECUTOR_RESULT_INVALID', 'completed executor work must provide evidence');
  return Object.freeze({ ...result, summary, capabilities: request.capabilities, evidence: Object.freeze(evidence) });
}
...
export function bindRepositoryExecutionResult(request, boundedResult) {
  const result = {
    ...
    transition_definition_fingerprint: request.transition_definition_fingerprint,
