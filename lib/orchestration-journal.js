...
function normalizeInvocationContext(context) {
  if (context === undefined || context === null) return { run_id:null, metrics_correlation:null };
  const facts = invocationCorrelationFacts(context);
  const { run_id, ...metrics } = facts;
  return { run_id, metrics_correlation:Object.values(metrics).some(Boolean) ? metrics : null };
}
...
export function createPostgresOrchestrationJournal(dbBinding = db) {
  return {
    async start({ run_id, command, request_sha256, idempotency_key, request_projection, target_kind, target_ref, correlation = null }) {
      const result = await dbBinding.query(`INSERT INTO orchestration_command_invocations (
        ...
        correlation?.execution_origin || 'unknown',
        correlation?.reasoning_boundary_id || null,
        correlation?.recovery_decision_id || null,
        correlation?.recovery_attempt_id || null,
        correlation?.packet_schema || null,
      ]);
...
export async function executeCorrelatedCommand(command, input, operation, options = {}) {
  ...
  try {
    ...
    invocationContext = normalizeInvocationContext(options.invocationContext);
    if (split.run_id && invocationContext.run_id && split.run_id !== invocationContext.run_id) {
  ...
  const run_id = invocationContext.run_id || split.run_id;
  ...
  if (run_id && !['orchestration.resume_packet','orchestration.diagnose'].includes(command)) {
    try {
      ...
      invocation = await journal.start({
        ...
        correlation:invocationContext.metrics_correlation,
        ...target,
      });
    } catch {
      invocation = null;
  ...
  if (run_id && !['orchestration.resume_packet','orchestration.diagnose'].includes(command) && (response.body?.ok !== true || !journaled)) {
    try {
      const currentFailure = options.currentFailureStore || createPostgresOrchestrationCurrentFailureStore(options.db || db);
