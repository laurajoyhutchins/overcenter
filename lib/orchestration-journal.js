...
function safeTarget(command, request) {
  ...
  if (command === 'github.repository.create') return { target_kind: 'github_repository', target_ref: request.name ? `laurajoyhutchins/${request.name}` : null };
...
export function splitOrchestrationRequest(command, input) {
  ...
  if (command === 'work.claim' || command === 'work.heartbeat' || command.startsWith('orchestration.')) return { run_id, domain_request: { ...original } };
...
export function safeRequestProjection(command, request) {
  ...
  if (command === 'work.claim') {
    return pick(body, ['work_ref', 'expected_state', 'expected_lane', 'lease_seconds', 'idempotency_key']);
  }
  ...
  if (command === 'github.repository.create') return pick(body, ['name', 'description']);
  ...
  if (command === 'orchestration.start') return { ...pick(body, ['run_id','worker','mode','continuation_key','budget_seconds','settlement_reserve_seconds','minimum_new_gate_seconds']), scope: body.scope ? { project: body.scope.project || null, lanes: boundedStrings(body.scope.lanes, 8), repositories: boundedStrings(body.scope.repositories, 25) } : null };
  ...
  if (command === 'orchestration.finish') return pick(body, ['run_id','disposition','last_work_ref','last_gate','stop_reason']);
...
export function safeResultProjection(command, responseBody) {
  ...
  if (command === 'github.repository.create') return pick(body, ['repo', 'repository_id', 'private', 'outcome', 'created', 'changed', 'verified', 'installation_access', 'approval_id', 'approval_state', 'approval_consumed', 'approval_path', 'approval_expires_at', 'may_have_mutated']);
  ...
  if (command === 'github.review_packet') return pick(body, ['repo', 'pull_number', 'pr_number', 'head_sha', 'base_sha', 'head_moved', 'review_decision']);
...
export function createPostgresOrchestrationJournal(dbBinding = db) {
  return {
    ...
    async finish(invocationId, responseBody) {
...
export async function executeCorrelatedCommand(command, input, operation, options = {}) {
  ...
  if (run_id && command !== 'orchestration.resume_packet') {
    try {
  ...
  if (run_id && journal && invocation?.invocation_id) {
    try { await journal.finish(invocation.invocation_id, response.body); } catch { /* journal is non-authoritative observability */ }
  }
  return response;
}