...
const INVOCATION_CONTEXT_FIELDS = new Set(['run_id','execution_origin','reasoning_boundary_id','recovery_decision_id','recovery_attempt_id','packet_schema']);
...
function normalizeInvocationContext(context) {
  ...
  const executionOrigin = optionalContextString(context.execution_origin, 'execution_origin', 32);
  if (executionOrigin && !EXECUTION_ORIGINS.has(executionOrigin)) {
    throw err('REQUEST_INVALID', 'invocation_context.execution_origin is unsupported', { field:'invocation_context.execution_origin' });
  }
  const metrics_correlation = {
  ...
  return {
    ...
    metrics_correlation:Object.values(metrics_correlation).some(Boolean) ? metrics_correlation : null,
  };
}
  ...
  if (run_id && !['orchestration.resume_packet','orchestration.diagnose'].includes(command)) {
    try {
      ...
      invocation = await journal.start({
        ...
        request_projection: {
          ...safeRequestProjection(command, domain_request),
          ...(invocationContext.metrics_correlation ? { metrics_correlation:invocationContext.metrics_correlation } : {}),
        },
        ...target,
      });
    } catch {
