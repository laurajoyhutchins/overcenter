...
function safeTarget(command, request) {
  ...
  if (command.startsWith('source.')) return { target_kind: 'source_sync', target_ref: 'proj_I6FSm85xrY7T<->laurajoyhutchins/busbar' };
...
export function safeRequestProjection(command, request) {
  ...
  if (command === 'github.archive_repository') return pick(body, ['repo', 'expected_repository_id', 'expected_archived']);
...
export function safeResultProjection(command, responseBody) {
  ...
  if (command === 'github.archive_repository') return pick(body, ['repo', 'requested_repo', 'repository_id', 'expected_repository_id', 'expected_archived', 'actual_repository_id', 'actual_archived', 'archived', 'confirmed', 'outcome', 'phase', 'may_have_mutated']);
  if (command === 'github.required_checks.ensure') {
    return {
