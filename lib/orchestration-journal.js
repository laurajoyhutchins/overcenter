...
function invocationRunId(context) {
  ...
  return optionalRunId(context.run_id);
}
...
function invocationRef(value) {
  ...
  if (!normalized || normalized.length > 128) {
    const error = err('REQUEST_INVALID', 'invocation_id must be a non-empty string of at most 128 characters', { field:'invocation_id' });
...
function invocationState(row) {
  if (!row) return null;
  const completed = row.completed_at != null || row.outcome !== 'running';
  return Object.freeze({
    ...
    outcome: row.outcome,
    request: row.request_projection || {},
    result: completed ? (row.result_projection || {}) : null,
    result_sha256: completed ? (row.result_sha256 || null) : null,
    error_code: completed ? (row.error_code || null) : null,
...
async function readInvocation(dbBinding, invocation_id) {
  const id = invocationRef(invocation_id);
  const result = await dbBinding.query(`SELECT invocation_id, run_id, command, request_sha256, request_projection,
      target_kind, target_ref, outcome, result_sha256, result_projection, error_code,
      may_have_mutated, started_at, completed_at
    FROM orchestration_command_invocations WHERE invocation_id = $1`, [id]);
  return invocationState(result.rows?.[0] || null);
}

export function createPostgresOrchestrationJournal(dbBinding = db) {
  return {
    async peek({ invocation_id } = {}) {
      return readInvocation(dbBinding, invocation_id);
    },
    async attach({ invocation_id, command, request_sha256 } = {}) {
      const observed = await readInvocation(dbBinding, invocation_id);
      if (!observed) {
        const error = err('INVOCATION_NOT_FOUND', 'invocation does not exist in authoritative orchestration state', { invocation_id: invocationRef(invocation_id) });
        error.may_have_mutated = false;
        throw error;
      }
      const normalizedCommand = typeof command === 'string' ? command.trim() : '';
      const normalizedHash = typeof request_sha256 === 'string' ? request_sha256.trim().toLowerCase() : '';
      if (!normalizedCommand || !/^[0-9a-f]{64}$/.test(normalizedHash)) {
        const error = err('REQUEST_INVALID', 'attach requires command and exact request_sha256', { invocation_id: observed.invocation_id });
        error.may_have_mutated = false;
        throw error;
      }
      if (observed.command !== normalizedCommand || observed.request_sha256 !== normalizedHash) {
        const error = err('INVOCATION_AUTHORITY_MISMATCH', 'invocation identity is already bound to different semantic command authority', {
          invocation_id: observed.invocation_id,
          observed_command: observed.command,
          requested_command: normalizedCommand,
        });
        error.may_have_mutated = false;
        throw error;
      }
      return Object.freeze({ ...observed, reconnected: true });
    },
...
export async function executeCorrelatedCommand(command, input, operation, options = {}) {
  ...
  try {
    ...
    contextRunId = invocationRunId(options.invocationContext);
