...
const EXECUTION_ORIGINS = new Set(['system','scheduler','agent','operator','recovery','unknown']);
const INVOCATION_CONTEXT_FIELDS = new Set(['run_id','origin','execution_origin','reasoning_boundary_id','recovery_decision_id','recovery_attempt_id','packet_schema']);

function optionalContextString(value, field, max = 512) {
  if (value === undefined || value === null || value === '') return null;
  if (typeof value !== 'string' || !value.trim() || value.trim().length > max) {
    throw err('REQUEST_INVALID', `${field} must be a non-empty string of at most ${max} characters`, { field:`invocation_context.${field}` });
  }
  return value.trim();
}

export function invocationCorrelationFacts(context = {}) {
  if (!context || typeof context !== 'object' || Array.isArray(context)) {
    throw err('REQUEST_INVALID', 'invocation_context must be an object', { field:'invocation_context' });
  }
  const unknown = Object.keys(context).filter((key) => !INVOCATION_CONTEXT_FIELDS.has(key));
  if (unknown.length) {
    throw err('REQUEST_INVALID', 'invocation_context contains unsupported fields', { field:'invocation_context', unsupported_fields:unknown.sort() });
  }
  const origin = optionalContextString(context.origin ?? context.execution_origin, 'origin', 32);
  if (context.origin && context.execution_origin && context.origin !== context.execution_origin) {
    throw err('REQUEST_INVALID', 'invocation_context origin aliases conflict', { field:'invocation_context.origin' });
  }
  if (origin && !EXECUTION_ORIGINS.has(origin)) {
    throw err('REQUEST_INVALID', 'invocation_context.origin is unsupported', { field:'invocation_context.origin' });
  }
  return {
    run_id:optionalRunId(context.run_id),
    execution_origin:origin || 'unknown',
    reasoning_boundary_id:optionalContextString(context.reasoning_boundary_id, 'reasoning_boundary_id'),
    recovery_decision_id:optionalContextString(context.recovery_decision_id, 'recovery_decision_id'),
    recovery_attempt_id:optionalContextString(context.recovery_attempt_id, 'recovery_attempt_id'),
    packet_schema:optionalContextString(context.packet_schema, 'packet_schema', 128),
  };
}
...
function safeTarget(command, request) {
  if (command === 'project.bootstrap_confirm_graph_deriver') return { target_kind:'project_transition', target_ref:'github:laurajoyhutchins/overcenter:register-project-graph-deriver' };
  if (command === 'project.define' || command === 'project.amend') return { target_kind:'project_definition', target_ref:request.project_ref || null };
  if (command.startsWith('work.')) return { target_kind: 'linear_issue', target_ref: request.work_ref || null };
  if (command === 'production.promote') return { target_kind:'github_repository', target_ref:request.repo || null };
  ...
  if (command.startsWith('github.')) return { target_kind: 'github_repository', target_ref: request.repo || request.repository || request.template_repo || null };
...
export function splitOrchestrationRequest(command, input) {
  ...
  if (command === 'work.claim' || command === 'work.heartbeat' || command === 'skill.activate' || command === 'project.bootstrap_confirm_graph_deriver' || command.startsWith('orchestration.')) return { run_id, domain_request: { ...original } };
...
export async function semanticRequestHash(command, input) {
  ...
  const semantic = { ...domain_request };
  if (command === 'github.apply_changeset') delete semantic.lease_token;
...
export function safeRequestProjection(command, request) {
  ...
  if (command === 'project.bootstrap_confirm_graph_deriver') return pick(body, ['run_id']);
  if (command === 'project.define') {
    return {
      ...pick(body, ['project_ref','expected_revision']),
      transition_count:Array.isArray(body.definition?.transitions) ? body.definition.transitions.length : null,
    };
  }
  if (command === 'project.amend') {
    return {
      ...pick(body, ['project_ref','expected_revision']),
      upsert_transition_count:Array.isArray(body.amendment?.upsert_transitions) ? body.amendment.upsert_transitions.length : 0,
      remove_transition_count:Array.isArray(body.amendment?.remove_transition_ids) ? body.amendment.remove_transition_ids.length : 0,
      confirmed_transition_count:Array.isArray(body.amendment?.confirmed_transition_ids) ? body.amendment.confirmed_transition_ids.length : 0,
    };
  }
  if (command === 'production.promote') return pick(body, ['repo']);
  ...
  if (command === 'github.repository_metadata.ensure') {
    const desired = object(body.desired_state);
    const expected = object(body.expected_state);
    return {
      repo: body.repo || null,
      desired_fields: boundedStrings(Object.keys(desired).sort(), 16),
      expected_fields: boundedStrings(Object.keys(expected).sort(), 16),
    };
  }
  if (command === 'github.repository.rename') {
    return pick(body, ['repo', 'new_name', 'expected_repository_id']);
  }
  if (command === 'github.repository_template.ensure') {
    return {
      repo: body.repo || null,
      desired_is_template: body.desired_state?.is_template ?? null,
      expected_is_template: body.expected_state?.is_template ?? null,
    };
  }
  if (command === 'github.repository_from_template.create') {
    return pick(body, ['template_repo', 'destination_repo', 'private', 'idempotency_key']);
  }
  if (command === 'github.milestone.ensure') {
    const desired = object(body.desired_state);
    const expected = object(body.expected_state);
    return {
      repo: body.repo || null,
  ...
  if (command === 'github.release.create') {
    return {
      ...pick(body, ['repo', 'target_sha', 'tag_name', 'name', 'draft', 'prerelease', 'idempotency_key']),
      expected_tag: body.expected_state?.tag || null,
      expected_release: body.expected_state?.release || null,
    };
  }
  ...
  if (command === 'orchestration.advance') return pick(body, ['run_id']);
...
export function safeResultProjection(command, responseBody) {
  ...
  if (command === 'project.bootstrap_confirm_graph_deriver') {
    return {
      ...pick(body, ['schema','outcome','project_ref','transition_id','transition_definition_fingerprint','lease_ref','settled_at']),
      authority: body.authority ? pick(body.authority, ['kind','repository','revision','derivation']) : null,
      frontier: boundedStrings(body.frontier, 10),
    };
  }
  if (command === 'project.define' || command === 'project.amend') {
    return {
      schema:body.schema || null,
      authority:body.authority ? pick(body.authority, ['kind','repository','revision','derivation']) : null,
      graph_revision:body.graph?.revision || null,
      added_count:Array.isArray(body.diff?.added) ? body.diff.added.length : 0,
      changed_count:Array.isArray(body.diff?.changed) ? body.diff.changed.length : 0,
      removed_count:Array.isArray(body.diff?.removed) ? body.diff.removed.length : 0,
    };
  }
  if (command === 'work.claim') {
  ...
  if (command === 'github.apply_changeset') {
    ...
    if (Array.isArray(paths)) {
      ...
      projection.changed_paths = boundedStrings(paths.map((value) => (
  ...
  if (command === 'github.repository_metadata.ensure') {
    return {
      ...pick(body, ['repo', 'outcome', 'changed', 'verified', 'may_have_mutated']),
      changed_fields: boundedStrings(body.changed_fields, 16),
    };
  }
  if (command === 'github.repository.rename') {
    return pick(body, ['repo', 'new_repo', 'repository_id', 'outcome', 'changed', 'verified', 'may_have_mutated']);
  }
  if (command === 'github.repository_template.ensure') {
    return {
      ...pick(body, ['repo', 'outcome', 'changed', 'verified', 'may_have_mutated']),
      changed_fields: boundedStrings(body.changed_fields, 4),
    };
  }
  if (command === 'github.repository_from_template.create') {
    return pick(body, ['template_repo', 'destination_repo', 'repository_id', 'outcome', 'created', 'verified', 'private', 'may_have_mutated']);
  }
  if (command === 'github.milestone.ensure') {
    return {
      ...pick(body, ['repo', 'title', 'milestone_number', 'outcome', 'changed', 'verified', 'may_have_mutated']),
      changed_fields: boundedStrings(body.changed_fields, 16),
    };
  }
  if (command === 'github.release.create') {
    return pick(body, ['repo', 'requested_commit_sha', 'verified_commit_sha', 'tag_name', 'tag_ref', 'tag_ref_node_id', 'release_id', 'release_url', 'release_name', 'draft', 'prerelease', 'release_created_at', 'release_published_at', 'pre_state', 'post_state', 'outcome', 'created', 'verified', 'idempotency_key', 'request_sha256', 'idempotent_replay', 'may_have_mutated']);
  }
  ...
  if (command === 'orchestration.advance') return {
    ...pick(body, ['schema','outcome','run_id','project_ref','lease_ref','transition_definition_fingerprint','settled_at','expires_at']),
    transition_id:body.transition?.id || null,
    executor_kind:body.transition?.executor?.kind || null,
    executor_role:body.transition?.executor?.role || null,
    executor_skill:body.transition?.executor?.skill || null,
    operator_command:body.transition?.executor?.command || null,
    authority:body.authority ? pick(body.authority, ['kind','repository','revision','derivation']) : null,
    frontier:boundedStrings(body.frontier, 10),
  };
...
export function createPostgresOrchestrationJournal(dbBinding) {
  ...
  return {
    async start({ run_id, command, request_sha256, idempotency_key, request_projection, target_kind, target_ref, correlation = {} }) {
      const result = await dbBinding.query(`INSERT INTO orchestration_command_invocations (
        ...
        execution_origin, reasoning_boundary_id, recovery_decision_id, recovery_attempt_id, packet_schema
      ) VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,'running',$8,$9,$10,$11,$12,$13) RETURNING invocation_id, sequence, started_at`, [
        ...
        correlation.execution_origin || 'unknown', correlation.reasoning_boundary_id || null,
        correlation.recovery_decision_id || null, correlation.recovery_attempt_id || null, correlation.packet_schema || null,
      ]);
    ...
    async finish(invocationId, responseBody, activity = null) {
      ...
      if (activity?.run_id && Number.isFinite(Number(activity.sequence))) {
        try {
          await dbBinding.query(`UPDATE orchestration_runs AS run SET
            last_durable_activity_at = now(),
            last_durable_activity_type = $2,
            last_durable_activity_sequence = $3,
            current_failure_command = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN NULL ELSE run.current_failure_command END,
            current_failure_error_code = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN NULL ELSE run.current_failure_error_code END,
            current_failure_error_class = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN NULL ELSE run.current_failure_error_class END,
            current_failure_retryable = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN NULL ELSE run.current_failure_retryable END,
            current_failure_rejection = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN NULL ELSE run.current_failure_rejection END,
            current_failure_may_have_mutated = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN NULL ELSE run.current_failure_may_have_mutated END,
            current_failure_streak = CASE WHEN finish.succeeded AND (
              run.current_failure_command = finish.command
              OR (run.current_failure_command = 'work.heartbeat' AND finish.command = 'work.settle')
            ) THEN 0 ELSE run.current_failure_streak END,
            updated_at = GREATEST(run.updated_at, now())
            FROM (SELECT $4::boolean AS succeeded, $5::text AS command) AS finish
            WHERE run.run_id = $1 AND (run.last_durable_activity_sequence IS NULL OR run.last_durable_activity_sequence <= $3)`, [
            activity.run_id,
...
export async function executeCorrelatedCommand(command, input, operation, options = {}) {
  ...
  try {
    ...
    const correlation = invocationCorrelationFacts(options.invocationContext || {});
    contextRunId = correlation.run_id;
    options = { ...options, invocationCorrelation: correlation };
    if (split.run_id && contextRunId && split.run_id !== contextRunId) {
      throw err('REQUEST_INVALID', 'semantic run_id conflicts with invocation_context.run_id', { field:'run_id' });
    }
  } catch (error) {
    return executeCommand(command, async () => { throw error; }, options);
  }

  const { domain_request } = split;
  const run_id = contextRunId || split.run_id;
  ...
  if (run_id && !['orchestration.resume_packet','orchestration.diagnose'].includes(command)) {
    try {
      journal = options.journal || createPostgresOrchestrationJournal(options.db);
      ...
      invocation = await journal.start({
        ...
        correlation: options.invocationCorrelation,
        ...target,
      });
    } catch {
      invocation = null;
  ...
  const journaled = Boolean(run_id && journal && invocation?.invocation_id);
  if (run_id && !['orchestration.resume_packet','orchestration.diagnose'].includes(command) && (response.body?.ok !== true || !journaled)) {
    try {
      const currentFailure = options.currentFailureStore || createPostgresOrchestrationCurrentFailureStore(options.db);
      await currentFailure.record(run_id, command, response.body);
    } catch { /* missing recovery metadata can reduce automation but cannot authorize unsafe work */ }
  }
  if (journaled) {
