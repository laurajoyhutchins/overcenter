...
function decodeBase64Bytes(content, path) {
  let binary;
  try { binary = atob(String(content || '').replace(/\s+/g, '')); }
  catch { fail('PROJECT_DEFINITION_FACTS_READ_FAILED', 'GitHub returned invalid base64 repository content', { path }); }
...
async function decodeFile(body, path) {
  ...
  if (!SHA40.test(blobSha)) fail('PROJECT_DEFINITION_FACTS_READ_FAILED', 'GitHub returned incomplete blob identity', { path });
  ...
  const bytes = decodeBase64Bytes(body.content, path);
  ...
  catch { fail('PROJECT_DEFINITION_FACTS_READ_FAILED', 'Project definition content must be UTF-8 text', { path }); }
  return {
    path,
    blob_sha:blobSha,
...
export function createProjectDefinitionFactsReader(apiClient) {
  ...
  return async function readProjectDefinitionFacts(input = {}) {
    ...
    const discoveryBody = await read(apiClient, `${repoPath}/contents/${encodePath(PROJECT_DEFINITION_DISCOVERY_PATH)}`, { ref:revision });
    const discoveryFile = await decodeFile(discoveryBody, PROJECT_DEFINITION_DISCOVERY_PATH);
    ...
    for (const definitionPath of discovery.definitions) {
      ...
      definitions.push(await decodeFile(body, definitionPath));
    }
