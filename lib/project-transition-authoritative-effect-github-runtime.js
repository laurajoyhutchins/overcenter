...
async function readActiveProjectTransitionLease(db, runId, observedAt = new Date().toISOString()) {
  ...
  const result = await db.query(
    `SELECT lease_id::text AS lease_ref, run_id, expires_at
       FROM work_leases
      WHERE run_id=$1
...
function confirmationFacts(result) {
  ...
  if (result?.reason === 'authoritative_effect_not_observed') {
    return {
      status:'absent',
      effect_ref:null,
      predicate:'project-transition-authoritative-effect-absent',
      evidence:{ confirmation:result || null },
    };
  }
  return {
        ...
        ports:{
          ...
          executionContext() {
            return {
              ...
              subject_kind:'project_transition',
              lease_ref:String(activeLease.lease_ref),
              lease_epoch:Number(authority.authority_epoch),
              authority_epoch:Number(authority.authority_epoch),
