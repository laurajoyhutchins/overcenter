...
import { projectTransitionPullRequestReadQuery } from './project-transition-authoritative-effect-github-query.js';
...
export function createPostgresProjectTransitionAuthoritativeEffectConfirmationService(options = {}) {
  ...
  return projectTransitionAuthoritativeEffectConfirmationFor({
    ...
    async readPullRequests({ repository, head, base }) {
      ...
      return githubRead(repository, 'review_packet', `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/pulls`, projectTransitionPullRequestReadQuery({ base }));
    },
