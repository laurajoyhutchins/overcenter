...
export function createPostgresProjectTransitionAuthoritativeEffectConfirmationService(options = {}) {
  ...
  return projectTransitionAuthoritativeEffectConfirmationFor({
    ...
    async readPullRequests({ repository, candidate }) {
      const [owner, name] = repository.split('/');
      return githubRead(repository, 'review_packet', `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/commits/${encodeURIComponent(candidate)}/pulls`, {
        per_page:100,
      });
    },
