...
import { applyGithubLeaseScopedTextReplacements, coalesceGithubLeaseScopedChangeset } from 'lib/github-worker-mutations.js';
...
// Tests register directly with node:test.
...
// Native node:test suite.
  test('lease-scoped request derives every Git execution coordinate from verified authority',async()=>{
  ...
  test('existing workspace head becomes the exact CAS fence and mutation identity input',async()=>{
  ...
  test('lease-scoped mode rejects caller-selected Git coordinates before authority or Git reads',async()=>{
  ...
  test('legacy work leases cannot acquire implicit workspace semantics',async()=>{
    let error=null;
  ...
  test('lease-scoped application revalidates the lease inside the existing changeset authority boundary',async()=>{
    const calls=[];
  ...
  test('authority drift between workspace read and mutation boundary fails closed',async()=>{
  ...
  test('new workspace branch appearing after lease-scope read fails before low-level preflight',async()=>{
    let error=null;
    try{
      await applyGithubLeaseScopedChangeset(input(),{
  ...
  test('worker command discovery exposes only lease-scoped GitHub mutation inputs',async()=>{
    check(CANONICAL_COMMANDS.includes('github.apply_changeset'),'github.apply_changeset is missing from canonical commands');
    check(CANONICAL_COMMANDS.includes('github.coalesce_changeset'),'github.coalesce_changeset is missing from canonical commands');
    ...
    for(const command of ['github.apply_changeset','github.coalesce_changeset','github.apply_text_replacements']){
    ...
    validateSemanticWorkerCommand('github.coalesce_changeset',input({commit_message:'format: coalesce cleanup'}));
  ...
  test('agent execution packet names the lease-authorized mutation commands',async()=>{
    ...
    check(JSON.stringify(result.lease_authorized_mutations)===JSON.stringify(['github.apply_changeset','github.coalesce_changeset','github.apply_text_replacements']),'agent handoff did not name its lease-authorized mutation surface');
  });
  test('lease-scoped text replacements bind source read to the same workspace observation as mutation',async()=>{
  ...
  test('text replacement refuses stale content when workspace advances after exact read',async()=>{
  ...
  test('same-lease final-newline cleanup coalesces the preceding mechanical head into one linear repair commit',async()=>{
    const head='4'.repeat(40), grandparent='3'.repeat(40), replacement='5'.repeat(40), tree='6'.repeat(40);
    let reads=0; const commits=[]; const replacements=[]; const queries=[];
    const parentReceipt={execution_authority:{lease_ref:LEASE_REF}};
    const result=await coalesceGithubLeaseScopedChangeset({lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n',ensure_final_newline:true}],commit_message:'format: finish newline cleanup'},{
      executionAuthority:{async require(){return AUTHORITY;}},
      readBranch:async()=>{reads+=1;return head;},
      db:{async query(sql,params){queries.push({sql,params});if(sql.includes('SELECT idempotency_key'))return{rows:[{idempotency_key:'parent-key',receipt:parentReceipt}]};if(sql.includes('UPDATE github_changeset_receipts'))return{rowCount:1,rows:[{idempotency_key:'parent-key'}]};throw new Error('unexpected query');}},
      withGithub:async(_request,callback)=>callback({
        async getBranch(){return{sha:head};},
        async getCommit(){return{message:'format: first cleanup',parents:[grandparent],tree_sha:'7'.repeat(40)};},
        async getPathEntries(){return new Map([['README.md',{mode:'100644',type:'blob',sha:'8'.repeat(40)}]]);},
        async createTree(){return tree;},
        async createCommit(_repo,input){commits.push(input);return replacement;},
        async replaceBranch(_repo,_branch,before,after){replacements.push({before,after});},
      }),
    });
    check(result.ok===true&&result.commit_sha===replacement,'coalescing did not return the replacement commit');
    check(reads===2,'workspace was not revalidated immediately before replacement');
    check(commits.length===1&&commits[0].parentSha===grandparent,'replacement commit did not bypass the preceding mechanical commit');
    check(replacements.length===1&&replacements[0].before===head&&replacements[0].after===replacement,'branch replacement was not exact-head fenced');
    check(queries.some(entry=>entry.sql.includes('UPDATE github_changeset_receipts')&&entry.params?.includes(head)),'durable receipt was not fenced to the replaced mechanical head');
  });
  test('exact coalescing replay returns the durable replacement receipt without rewriting the ref',async()=>{
    const initial='4'.repeat(40), replacement='5'.repeat(40);let head=initial;let refWrites=0;let stored={commit_sha:initial,idempotency_key:'parent-key',receipt:{execution_authority:{lease_ref:LEASE_REF}}};
    const request={lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'format: cleanup'};
    const db={async query(sql,params){
      if(sql.includes('SELECT idempotency_key'))return params[2]===stored.commit_sha?{rows:[{idempotency_key:stored.idempotency_key,receipt:stored.receipt}]}:{rows:[]};
      if(sql.includes('UPDATE github_changeset_receipts')){stored={commit_sha:params[3],idempotency_key:params[1],receipt:JSON.parse(params[5])};return{rowCount:1,rows:[{idempotency_key:params[1]}]};}
      throw new Error('unexpected query');
    }};
    const options={executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>head,db,withGithub:async(_request,callback)=>callback({
      async getBranch(){return{sha:head};},async getCommit(){return{message:'format: first cleanup',parents:['3'.repeat(40)],tree_sha:'7'.repeat(40)};},
      async getPathEntries(){return new Map([['README.md',{mode:'100644',type:'blob',sha:'8'.repeat(40)}]]);},async createTree(){return'6'.repeat(40);},async createCommit(){return replacement;},
      async replaceBranch(_repo,_branch,before,after){check(before===head,'replay fixture lost exact head');refWrites+=1;head=after;},
    })};
    ...
    check(first.commit_sha===replacement&&replay.commit_sha===replacement,'replay did not converge on the replacement commit');
    check(replay.idempotent_replay===true,'durable replay was not identified');
    check(refWrites===1,'exact replay rewrote the branch ref');
  });
  test('coalescing rejects a different lease before Git mutation',async()=>{
    const head='4'.repeat(40);let gitCalls=0;let error=null;
    try{await coalesceGithubLeaseScopedChangeset({lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'format: cleanup'},{
      executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>head,
      db:{async query(){return{rows:[{idempotency_key:'parent-key',receipt:{execution_authority:{lease_ref:'other-lease'}}}]};}},
      withGithub:async()=>{gitCalls+=1;},
    });}catch(observed){error=observed;}
    check(error?.code==='MECHANICAL_COALESCE_PARENT_AUTHORITY_MISMATCH','different lease did not fail with authority mismatch');
    check(gitCalls===0,'different lease reached Git mutation');
  });
  test('coalescing rejects stale workspace movement before Git mutation',async()=>{
    const head='4'.repeat(40);let reads=0;let gitCalls=0;let error=null;
    try{await coalesceGithubLeaseScopedChangeset({lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'format: cleanup'},{
      executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>{reads+=1;return reads===1?head:'9'.repeat(40);},
      db:{async query(){return{rows:[{idempotency_key:'parent-key',receipt:{execution_authority:{lease_ref:LEASE_REF}}}]};}},
      withGithub:async()=>{gitCalls+=1;},
    });}catch(observed){error=observed;}
    check(error?.code==='EXECUTION_AUTHORITY_STALE','stale workspace did not fail closed');
    check(gitCalls===0,'stale workspace reached Git mutation');
  });
  test('coalescing rejects a non-mechanical predecessor before ref replacement',async()=>{
    const head='4'.repeat(40);let replaced=false;let error=null;
    try{await coalesceGithubLeaseScopedChangeset({lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'format: cleanup'},{
      executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>head,
      db:{async query(){return{rows:[{idempotency_key:'parent-key',receipt:{execution_authority:{lease_ref:LEASE_REF}}}]};}},
      withGithub:async(_request,callback)=>callback({async getBranch(){return{sha:head};},async getCommit(){return{message:'feat: substantive parent',parents:['3'.repeat(40)],tree_sha:'7'.repeat(40)};},async replaceBranch(){replaced=true;}}),
    });}catch(observed){error=observed;}
    check(error?.code==='MECHANICAL_COALESCE_PARENT_NOT_MECHANICAL','non-mechanical predecessor was accepted');
    check(replaced===false,'non-mechanical predecessor reached ref replacement');
  });
  test('coalescing preserves possible mutation certainty when ref replacement cannot be reconciled',async()=>{
    const head='4'.repeat(40);let branchReads=0;let error=null;
    try{await coalesceGithubLeaseScopedChangeset({lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'format: cleanup'},{
      executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>head,
      db:{async query(){return{rows:[{idempotency_key:'parent-key',receipt:{execution_authority:{lease_ref:LEASE_REF}}}]};}},
      withGithub:async(_request,callback)=>callback({
        async getBranch(){branchReads+=1;if(branchReads===1)return{sha:head};throw new Error('readback unavailable');},
        async getCommit(){return{message:'format: first cleanup',parents:['3'.repeat(40)],tree_sha:'7'.repeat(40)};},
        async getPathEntries(){return new Map([['README.md',{mode:'100644',type:'blob',sha:'8'.repeat(40)}]]);},
        async createTree(){return'6'.repeat(40);},async createCommit(){return'5'.repeat(40);},
        async replaceBranch(){const failure=new Error('transport uncertain');failure.code='GITHUB_TRANSPORT_UNCERTAIN';throw failure;},
      }),
    });}catch(observed){error=observed;}
    check(error?.details?.may_have_mutated===true,'uncertain ref replacement lost monotonic mutation certainty');
  });
  // Results are reported by node:test.
