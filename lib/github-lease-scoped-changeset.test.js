...
export async function runGithubLeaseScopedChangesetTests(){
  ...
  results.push(await run('same-lease mechanical cleanup automatically retries through bounded coalescing',async()=>{
    const head='2'.repeat(40);const applied=[];const authorizations=[];
    const result=await applyGithubLeaseScopedChangeset(input({commit_message:'style: repair formatting'}),{
      executionAuthority:{async require(){return AUTHORITY;}},
      readBranch:async()=>head,
      withGithub:withGithub(head),
      applyChangeset:async(request)=>{
        applied.push(request);
        if(request.coalesce_previous_mechanical===true)return{ok:true,branch:request.branch,new_head:'3'.repeat(40)};
        return{ok:false,error:'MECHANICAL_CHANGESET_MUST_COALESCE',parent_head:head};
      },
      authorizeMechanicalCoalesce:async(request)=>{authorizations.push(request);return request.lease_ref===LEASE_REF&&request.parent_head===head;},
    });
    check(result.ok===true,'same-lease mechanical repair did not converge automatically');
    check(applied.length===2&&applied[0].coalesce_previous_mechanical===undefined&&applied[1].coalesce_previous_mechanical===true,'automatic retry did not use the internal coalescing mode exactly once');
    check(authorizations.length===1,'same-lease receipt authorization was not consulted exactly once');
  }));
  results.push(await run('mechanical coalescing fails closed when the prior head is not from the same lease',async()=>{
    const head='2'.repeat(40);let applied=0;
    const result=await applyGithubLeaseScopedChangeset(input({commit_message:'style: repair formatting'}),{
      executionAuthority:{async require(){return AUTHORITY;}},
      readBranch:async()=>head,
      withGithub:withGithub(head),
      applyChangeset:async()=>{applied+=1;return{ok:false,error:'MECHANICAL_CHANGESET_MUST_COALESCE',parent_head:head};},
      authorizeMechanicalCoalesce:async()=>false,
    });
    check(result.error==='MECHANICAL_CHANGESET_MUST_COALESCE','different-lease predecessor did not remain fail-closed');
    check(applied===1,'unauthorized coalescing reached a second mutation attempt');
  }));
