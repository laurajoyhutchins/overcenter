...
import { applyGithubLeaseScopedTextReplacements, coalesceGithubLeaseScopedMechanicalChangeset, coalesceGithubLeaseScopedMechanicalChangesetWithReceipts } from 'lib/github-worker-mutations.js';
...
export async function runGithubLeaseScopedChangesetTests(){
  ...
  results.push(await run('worker command discovery exposes only lease-scoped GitHub mutation inputs',async()=>{
    ...
    for(const command of ['github.apply_changeset','github.apply_text_replacements','github.coalesce_mechanical_changeset']){
  ...
  results.push(await run('agent execution packet names the lease-authorized mutation commands',async()=>{
    ...
    check(JSON.stringify(result.lease_authorized_mutations)===JSON.stringify(['github.apply_changeset','github.apply_text_replacements','github.coalesce_mechanical_changeset']),'agent handoff did not name its lease-authorized mutation surface');
  }));
  ...
  results.push(await run('same-lease mechanical final-newline repair coalesces by replacing the immediately preceding mechanical head',async()=>{
    const head='2'.repeat(40);const parent='1'.repeat(40);const replacement='5'.repeat(40);let branch=head;let refMutations=0;
    const result=await coalesceGithubLeaseScopedMechanicalChangeset({
      lease_ref:LEASE_REF,
      changes:[{path:'README.md',operation:'update',content:'formatted readme\n',ensure_final_newline:true}],
      commit_message:'style: repair final newline',
    },{
      executionAuthority:{async require(){return AUTHORITY;}},
      readBranch:async()=>({sha:branch}),
      readPriorChangeset:async()=>({commit_sha:head,execution_authority:{lease_id:LEASE_REF}}),
      withGithub:async(_scope,callback)=>callback({
        async getBranch(){return{sha:branch};},
        async getCommit(_repo,sha){
          check(sha===head,'coalescing inspected the wrong mechanical head');
          return{sha:head,tree_sha:'3'.repeat(40),message:'style: format first batch',parents:[parent]};
        },
        async getPathEntries(){return new Map([['README.md',{path:'README.md',mode:'100644',type:'blob',sha:'4'.repeat(40)}]]);},
        async createTree(){return'4'.repeat(40);},
        async createCommit(_repo,{parentSha}){check(parentSha===parent,'replacement commit did not preserve the pre-mechanical parent');return replacement;},
        async replaceBranch(_repo,_branch,expectedHead,newHead){check(expectedHead===head&&newHead===replacement,'replacement ref was not exact-head fenced');branch=newHead;refMutations+=1;},
      }),
    });
    check(result.ok===true&&result.coalesced===true,'mechanical recovery did not report coalescing success');
    check(result.old_head===head&&result.new_head===replacement&&branch===replacement,'mechanical recovery did not replace the exact prior head');
    check(refMutations===1,'mechanical recovery did not perform exactly one bounded ref replacement');
  }));
  results.push(await run('mechanical coalescing rejects another lease before Git mutation',async()=>{
    const head='2'.repeat(40);let gitScopes=0;let error=null;
    try{await coalesceGithubLeaseScopedMechanicalChangeset({lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'x\n'}],commit_message:'style: newline'}, {
      executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>({sha:head}),readPriorChangeset:async()=>({commit_sha:head,execution_authority:{lease_id:'other-lease'}}),withGithub:async()=>{gitScopes+=1;throw new Error('must not reach Git');},
    });}catch(observed){error=observed;}
    check(error?.code==='MECHANICAL_COALESCE_AUTHORITY_MISMATCH','another lease was not rejected');check(gitScopes===0,'another lease reached Git mutation scope');
  }));
  results.push(await run('mechanical coalescing exact replay returns the durable receipt without another ref replacement',async()=>{
    const head='2'.repeat(40);const parent='1'.repeat(40);const replacement='5'.repeat(40);let branch=head;let refMutations=0;let operation=null;
    const operations={
      async claim(input){if(operation)return{outcome:operation.state==='succeeded'?'terminal':operation.state==='indeterminate'?'indeterminate':'in_progress',operation};operation={state:'prepared',resolution:null,recovery_payload:{attempt_token:input.attempt_token}};return{outcome:'claimed',operation};},
      async succeed(input){operation={state:'succeeded',resolution:input.resolution,recovery_payload:null};return operation;},
      async markIndeterminate(input){operation={state:'indeterminate',resolution:null,recovery_payload:{...input.recovery_payload,attempt_token:input.attempt_token}};return operation;},
      async abandon(){operation=null;return true;},
    };
    const request={lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'style: repair newline'};
    const options={operations,attemptToken:'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',now:()=> '2026-09-06T00:00:00.000Z',executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>({sha:branch}),readPriorChangeset:async()=>({commit_sha:head,execution_authority:{lease_id:LEASE_REF}}),withGithub:async(_scope,callback)=>callback({async getBranch(){return{sha:branch};},async getCommit(){return{sha:head,tree_sha:'3'.repeat(40),message:'style: first repair',parents:[parent]};},async getPathEntries(){return new Map([['README.md',{mode:'100644',type:'blob',sha:'4'.repeat(40)}]]);},async createTree(){return'4'.repeat(40);},async createCommit(){return replacement;},async replaceBranch(){branch=replacement;refMutations+=1;}})};
    const first=await coalesceGithubLeaseScopedMechanicalChangesetWithReceipts(request,options);const replay=await coalesceGithubLeaseScopedMechanicalChangesetWithReceipts(request,options);
    check(first.ok===true&&replay.ok===true&&replay.idempotent_replay===true,'exact replay did not return durable success');check(refMutations===1,'exact replay repeated the provider ref mutation');
  }));
  results.push(await run('ambiguous coalescing ref transport becomes durable indeterminate state and is not blindly retried',async()=>{
    const head='2'.repeat(40);const parent='1'.repeat(40);let branch=head;let refAttempts=0;let operation=null;
    const operations={async claim(input){if(operation)return{outcome:operation.state==='indeterminate'?'indeterminate':'in_progress',operation};operation={state:'prepared',recovery_payload:{attempt_token:input.attempt_token}};return{outcome:'claimed',operation};},async succeed(){throw new Error('must not succeed');},async markIndeterminate(input){operation={state:'indeterminate',recovery_payload:{...input.recovery_payload,attempt_token:input.attempt_token}};return operation;},async abandon(){operation=null;return true;}};
    const request={lease_ref:LEASE_REF,changes:[{path:'README.md',operation:'update',content:'formatted\n'}],commit_message:'style: repair newline'};
    const options={operations,attemptToken:'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',now:()=> '2026-09-06T00:00:00.000Z',executionAuthority:{async require(){return AUTHORITY;}},readBranch:async()=>({sha:branch}),readPriorChangeset:async()=>({commit_sha:head,execution_authority:{lease_id:LEASE_REF}}),withGithub:async(_scope,callback)=>callback({async getBranch(){return{sha:branch};},async getCommit(){return{sha:head,tree_sha:'3'.repeat(40),message:'style: first repair',parents:[parent]};},async getPathEntries(){return new Map([['README.md',{mode:'100644',type:'blob',sha:'4'.repeat(40)}]]);},async createTree(){return'4'.repeat(40);},async createCommit(){return'5'.repeat(40);},async replaceBranch(){refAttempts+=1;const error=new Error('transport vanished');error.code='GITHUB_TRANSPORT_INDETERMINATE';error.may_have_mutated=true;throw error;}})};
    let first=null;try{await coalesceGithubLeaseScopedMechanicalChangesetWithReceipts(request,options);}catch(error){first=error;}let replay=null;try{await coalesceGithubLeaseScopedMechanicalChangesetWithReceipts(request,options);}catch(error){replay=error;}
    check(first?.may_have_mutated===true,'ambiguous transport lost mutation uncertainty');check(replay?.code==='MECHANICAL_COALESCE_RECOVERY_REQUIRED','indeterminate replay did not fail closed');check(refAttempts===1,'indeterminate replay blindly repeated the ref mutation');
  }));
