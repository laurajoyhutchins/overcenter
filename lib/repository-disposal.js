...
export function createRepositoryDisposalService({ lifecycle, workSurface, leases, now = () => new Date().toISOString() } = {}) {
  ...
  async function verify(repositoryInput) {
    ...
    return {
      ...
      compatibility_work_allowed: lifecyclePacket.compatibility_work_allowed === true,
      remaining_external_dependency: lifecyclePacket.remaining_external_dependency || null,
      ...
      checks: {
        ...
        unresolved_external_dependency: lifecyclePacket.remaining_external_dependency || null,
      },
    };
  }
  ...
  async function dispose(input = {}) {
    ...
    const lifecycleState = await lifecycle.dispose({
      ...
      compatibility_bound: input.compatibility_bound,
      compatibility_reference: input.compatibility_reference,
    ...
    return {
      ...
      compatibility_bound: lifecycleState.compatibility_bound,
      compatibility_reference: lifecycleState.compatibility_reference,
