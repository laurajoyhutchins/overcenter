...
test('all twenty directed productive transitions emerge from one resolver', () => {
  let count = 0;
  for (const from of PRODUCTIVE_STAGES) {
    for (const to of PRODUCTIVE_STAGES) {
      if (from === to) continue;
...
test('not-applicable responsibilities are skipped', () => {
  ...
  const result = resolveWorkLifecycle({ current_stage: 'ENABLE', responsibilities: facts });
...
test('recovery performs fresh resolution rather than returning to prior stage', () => {
  const result = resolveLifecycleAfterRecovery({ current_stage: 'EXECUTE', condition: 'NOMINAL', responsibilities: factsFor('ACQUIRE') });
  assert.equal(result.next_stage, 'ACQUIRE');
});
...
test('completed stage defaults to the next productive responsibility', () => {
  ...
  assert.equal(acquire.next_stage, 'EXECUTE');
  assert.equal(execute.next_stage, 'COMMIT');
  assert.equal(commit.next_stage, 'CONFIRM');
  assert.equal(confirm.next_stage, 'DONE');
...
test('caller reports facts rather than a successor', () => {
  ...
  assert.equal(result.next_stage, 'EXECUTE');
  assert.equal(result.transition_kind, 'feedback');
  assert.throws(() => resolveCompletedStage({
    current_stage: 'EXECUTE',
    lifecycle_facts: { next_stage: 'CONFIRM', responsibilities: factsFor('CONFIRM') },
  }));
});
