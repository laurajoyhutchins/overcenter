...
export async function runGithubProductionPromotionTests() {
  ...
  await test('candidate must be exact current dev head before verification lookup', async () => {
    const api = github({ async getWorkflowRun() { throw new Error('verification lookup must not run for a non-dev candidate'); } });
  ...
  await test('semantic production promotion returns canonical success with exact evidence', async () => {
    const source = B;
    const production = A;
    const verificationRef = 'github-actions-run:12345';
    const promoted = await promoteProduction({ repo:'laurajoyhutchins/overcenter' }, {
      async resolveBranchRoles() { return { development:'dev', production:'main' }; },
      async readBranchHead(_repo, branch) { return branch === 'dev' ? source : production; },
      async verifyExactRevision(_repo, revision) { return { revision, verified:true, verification_ref:verificationRef }; },
      async promoteVerifiedRevision(input) {
        assert(input.source_revision === source && input.production_revision === production, 'semantic promotion lost exact authority coordinates');
        return { production_revision:source };
      },
    });
    assert(promoted.ok === true, 'semantic production promotion must return a canonical success result');
    assert(promoted.repo === 'laurajoyhutchins/overcenter', 'semantic production promotion must retain repository identity');
    assert(promoted.source_revision === source, 'semantic production promotion must expose the exact verified source revision');
    assert(promoted.production_revision === source, 'semantic production promotion must expose authoritative production readback');
    assert(promoted.verification_ref === verificationRef, 'semantic production promotion must retain bounded verification evidence');
  });
