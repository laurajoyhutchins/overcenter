...
// Assertions use node:assert/strict so failures are reported through node:test.
const A = 'a'.repeat(40);
const B = 'b'.repeat(40);
const C = 'c'.repeat(40);
...
// Registered directly with node:test; no legacy result collector or exported runner.

  test('promotion advances production to the existing dev commit only', async () => {
    const api = github();
  ...
  test('stale development head rejects before mutation', async () => {
  ...
  test('stale production head rejects before mutation', async () => {
  ...
  test('candidate must be exact current dev head before verification lookup', async () => {
    const api = github({
  ...
  test('verification must be successful dev push for exact candidate', async () => {
    for (const patch of [
  ...
  test('non fast forward production movement rejects', async () => {
  ...
  test('same idempotency request replays without a second mutation', async () => {
  ...
  test('idempotency key cannot bind a different promotion', async () => {
    const api = github(); const receipts = memoryReceipts();
  ...
  test('already identical production returns verified no-change receipt', async () => {
  ...
  test('readback failure after production update preserves possible mutation', async () => {
    const api = github();
    const baseGetBranch = api.getBranch.bind(api);
    let productionReads = 0;
    api.getBranch = async (repo, branch) => {
      if (branch === 'main') {
        productionReads += 1;
        if (productionReads === 3) {
          const error = new Error('authoritative production readback failed after ref update');
          error.code = 'GITHUB_UPSTREAM_ERROR';
          throw error;
        }
      }
      return baseGetBranch(repo, branch);
    };
    const result = await promoteGithubProductionWithGitHubApp(request(), {
      github: api,
      branchRoles: roles(),
      receipts: memoryReceipts(),
    });
    assert(api.state.main === B, 'production ref was not updated before readback failure');
    assert(result.ok === false, 'readback failure unexpectedly succeeded');
    assert(result.error === 'GITHUB_UPSTREAM_ERROR', `unexpected ${result.error}`);
    assert(result.may_have_mutated === true, 'readback failure downgraded possible mutation');
  });

  test('receipt failure after confirmed production update preserves possible mutation', async () => {
    const api = github();
    const receipts = memoryReceipts();
    receipts.succeed = async () => {
      const error = new Error('receipt persistence failed after confirmed ref update');
      error.code = 'PROMOTION_RECEIPT_WRITE_FAILED';
      throw error;
    };
    const result = await promoteGithubProductionWithGitHubApp(request(), {
      github: api,
    ...
    assert(api.state.main === B, 'production ref was not confirmed before receipt failure');
    assert(result.ok === false, 'receipt failure unexpectedly succeeded');
    assert(result.error === 'PROMOTION_RECEIPT_WRITE_FAILED', `unexpected ${result.error}`);
    assert(result.may_have_mutated === true, 'receipt failure downgraded confirmed mutation');
  });

  // Native node:test owns aggregation and process status.
