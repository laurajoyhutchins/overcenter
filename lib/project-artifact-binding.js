...
function text(value, field, max = 512) {
    ...
    if (!result || result.length > max)
        fail('PROJECT_ARTIFACT_BINDING_INVALID', `${field} is required`, { field });
    return result;
}
function normalize(input) {
    ...
    if (!match?.[1])
        fail('PROJECT_ARTIFACT_BINDING_INVALID', 'project_ref must identify one GitHub repository');
    const expectedRevision = text(input.expected_revision, 'expected_revision', 40).toLowerCase();
    if (!SHA40.test(expectedRevision))
        fail('PROJECT_ARTIFACT_BINDING_INVALID', 'expected_revision must be a full Git SHA');
    ...
    if (!KINDS.has(kind) || !Number.isInteger(number) || number < 1)
        fail('PROJECT_ARTIFACT_BINDING_INVALID', 'provider must name an issue or pull request by positive numeric identity');
    const relationship = text(input.relationship, 'relationship', 64);
    if (relationship !== 'full_coverage_equivalence')
        fail('PROJECT_ARTIFACT_BINDING_INVALID', 'relationship must explicitly assert full_coverage_equivalence');
...
export function classifyProjectArtifactBinding(binding, provider) {
    if (!binding)
        return Object.freeze({ classification: 'ambiguous', reason: 'explicit-binding-required' });
    ...
    const satisfied = binding.satisfaction_condition === 'closed' ? String(provider.state || '').toLowerCase() === 'closed' : Boolean(provider.merged);
