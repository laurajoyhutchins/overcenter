...
const MAX_PROJECT_TRANSITIONS = 1000;
...
function parseDefinition(entry) {
  ...
  if (!Array.isArray(parsed.transitions) || parsed.transitions.length < 1 || parsed.transitions.length > MAX_PROJECT_TRANSITIONS) {
    fail(`definition.transitions must contain between 1 and ${MAX_PROJECT_TRANSITIONS} transitions`);
  }
  return parsed;
}
