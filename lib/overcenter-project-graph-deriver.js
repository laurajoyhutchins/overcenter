...
import { normalizeProjectExecutionIntent } from './project-graph-contracts.js';
...
function transition(input, index) {
  ...
  const supported = ['executor','id','priority','requires'];
  if (input?.version_impact !== undefined) supported.push('version_impact');
  if (input?.execution_intent !== undefined) supported.push('execution_intent');
  exactKeys(input, supported, field);
  ...
  const executionIntent = normalizeProjectExecutionIntent(input.execution_intent, id, (_code, message, details) => fail(message, details));
  return Object.freeze({
    id,
...
function exactDefinitionFacts(facts, authority) {
  ...
  return Array.isArray(definitions.definitions) ? definitions.definitions : [];
}
...
function parseDefinition(entry) {
  ...
  if (!Array.isArray(parsed.transitions) || parsed.transitions.length < 1) {
    fail('definition.transitions must contain at least one transition');
  }
  return parsed;
}
...
export function deriveOvercenterProjectGraph(input = {}) {
  ...
  const definitions = exactDefinitionFacts(input.facts, { repository, revision });
  const matching = [];
  for (const entry of definitions) {
    let candidate;
    try { candidate = JSON.parse(text(entry?.content, 'definition.content')); }
    catch { continue; }
    if (candidate?.schema !== DEFINITION_SCHEMA || candidate?.project_ref !== projectRef) continue;
    matching.push(parseDefinition(entry));
  }
  if (matching.length !== 1) {
    fail('exactly one Overcenter project definition must match requested project_ref', {
