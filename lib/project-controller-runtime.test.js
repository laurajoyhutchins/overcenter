...
export async function runProjectControllerRuntimeTests() {
  ...
  await test('binds ACQUIRE COMMIT and CONFIRM mechanically to declared Busbar primitives', async()=>{
    ...
    const runtime = {
      ...
      primitives:{
        'work.claim':async () => { calls.push(['ACQUIRE','work.claim']); return { ok:true, lease_ref:'lease-1' }; },
        'github.apply_changeset':async () => { calls.push(['COMMIT','github.apply_changeset']); return { ok:true, commit_sha:'abc123' }; },
        'github.review_packet':async () => { calls.push(['CONFIRM','github.review_packet']); return { ok:true, head_sha:'abc123' }; },
      },
      operators:{
