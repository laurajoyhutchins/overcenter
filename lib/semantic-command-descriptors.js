...
const orchestrationDiagnoseSchema = Object.freeze({
    type: 'object',
    required: ['run_id'],
    properties: { run_id: { type: 'string', minLength: 1, maxLength: 512 }, work_ref: { type: 'string', minLength: 1, maxLength: 128 } },
    additionalProperties: false,
});
const projectDefineSchema = Object.freeze({ type: 'object', required: ['project_ref','expected_revision','definition'], properties: { project_ref: { type: 'string', pattern: '^github:[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$' }, expected_revision: { type: 'string', pattern: '^[0-9a-fA-F]{40}$' }, definition: { type: 'object' } }, additionalProperties: false });
const projectAmendSchema = Object.freeze({ type: 'object', required: ['project_ref','expected_revision','amendment'], properties: { project_ref: { type: 'string', pattern: '^github:[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$' }, expected_revision: { type: 'string', pattern: '^[0-9a-fA-F]{40}$' }, amendment: { type: 'object' } }, additionalProperties: false });
function descriptor(command, mcpName, description, inputSchema, surface, exposure = Object.freeze({ worker: true, mcp: true })) {
    return Object.freeze({ command, mcp_name: mcpName, description, input_schema: inputSchema, semantic_fields: Object.freeze(Object.keys(inputSchema.properties)), required_fields: Object.freeze([...(inputSchema.required || [])]), exposure, surface });
}
const DESCRIPTORS = Object.freeze({
    'github.release.create': descriptor('github.release.create', 'github_release_create', 'Create an immutable lightweight Git tag at an exact observed Git commit and a GitHub Release for that tag. Fail closed on expected-state drift or conflicting existing state. Exact replay converges through durable idempotency evidence; no tag retargeting, release editing, deletion, asset upload, note generation, or commit inference is performed. This MCP tool exposes conceptual github.release.create using the underscore-safe transport name.', githubReleaseSchema, 'advanced'),
    'orchestration.diagnose': descriptor('orchestration.diagnose', 'orchestration.diagnose', 'Read current durable orchestration state and return the typed failure class, exact deterministic recovery operation, and escalation boundary. This is state inspection and recovery classification only; it does not plan or select work.', orchestrationDiagnoseSchema, 'operator'),
...
export function semanticCommandDescriptor(command) {
    if (!Object.prototype.hasOwnProperty.call(ALL_DESCRIPTORS, command)) throw new Error(`Semantic command descriptor is not migrated: ${command}`);
    return ALL_DESCRIPTORS[command];
}
export function semanticCommandDescriptorsForSurface(surface) {
    return Object.freeze(MIGRATED_SEMANTIC_COMMANDS.map((command) => semanticCommandDescriptor(command)).filter((descriptor) => descriptor.surface === surface && descriptor.exposure.mcp));
}
export function semanticMcpDiscoveryForSurface(surface) {
    return Object.freeze(semanticCommandDescriptorsForSurface(surface).map((descriptor) => Object.freeze({ command: descriptor.command, name: descriptor.mcp_name, description: descriptor.description, input_schema: descriptor.input_schema })));
}
