...
const githubReleaseSchema = Object.freeze({
    type: 'object',
    required: ['repo', 'target_sha', 'tag_name', 'body', 'draft', 'prerelease', 'expected_state', 'idempotency_key', 'run_id'],
    additionalProperties: false,
    properties: {
...
const DESCRIPTORS = Object.freeze({
    ...
    'production.promote': descriptor('production.promote', 'production.promote', 'Express intent to promote the current verified development revision by repository identity only. Provider-specific branch heads, exact verification evidence, retry identity, and production readback remain mechanically derived by the runtime adapter; MCP exposure stays disabled until that adapter exists.', productionPromoteSchema, 'primary', Object.freeze({ worker: true, mcp: false })),
    'project.advance': descriptor('project.advance', 'project.advance', 'Advance an authoritative project graph by repository-owned project identity only. The runtime adapter owns run creation or resumption, horizon selection, lease acquisition, settlement choreography, and continuation behind this primary semantic boundary.', projectAdvanceSchema, 'primary', Object.freeze({ worker: true, mcp: true })),
