...
const invocationReconnectSchema = Object.freeze({
    type: 'object',
    required: ['invocation_ref'],
    properties: {
        invocation_ref: { type: 'string', minLength: 12, maxLength: 139, pattern: '^invocation:[A-Za-z0-9._:-]{1,128}$', description: 'Opaque durable invocation reference returned by Overcenter.' },
    },
    additionalProperties: false,
});
...
const DESCRIPTORS = Object.freeze({
    ...
    'invocation.attach': descriptor('invocation.attach', 'invocation.attach', 'Attach a fresh caller to one durable command invocation by opaque invocation reference. Returns only a stable reconnect handle and authoritative current invocation state; it does not replay or mutate the command.', invocationReconnectSchema, 'primary', WORKER_AND_MCP_EXPOSURE),
    'invocation.peek': descriptor('invocation.peek', 'invocation.peek', 'Read bounded authoritative command-execution state by opaque invocation reference. This is observation only and never infers, retries, or reconstructs a command from session context.', invocationReconnectSchema, 'primary', WORKER_AND_MCP_EXPOSURE),
