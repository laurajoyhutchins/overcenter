...
const DESCRIPTORS = Object.freeze({
    ...
    'invocation.peek': descriptor('invocation.peek', 'invocation.peek', 'Read one exact durable command invocation as a bounded authoritative snapshot. No work is selected and no execution state is mutated.', invocationObservationSchema, 'primary', WORKER_AND_MCP_EXPOSURE),
    'invocation.attach': descriptor('invocation.attach', 'invocation.attach', 'Reconnect to one exact durable command invocation and its owning run state by opaque invocation reference. Returns a resume reference only while that run remains active; no prior session context is required.', invocationObservationSchema, 'primary', WORKER_AND_MCP_EXPOSURE),
