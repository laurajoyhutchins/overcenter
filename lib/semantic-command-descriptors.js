...
const projectAdvanceSchema = Object.freeze({
    ...
    properties: {
        ...
        transition_id: { type: 'string', minLength: 1, maxLength: 256, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]*$' },
        resume_run_id: { type: 'string', minLength: 1, maxLength: 512 },
    },
    additionalProperties: false,
});
...
const DESCRIPTORS = Object.freeze({
    ...
    'project.advance': descriptor('project.advance', 'project.advance', 'Advance an authoritative project graph in an independent session by default, optionally nominating one exact transition or explicitly resuming one prior run. Overcenter rereads project authority, owns run identity, target fencing, exclusive lease acquisition, settlement choreography, and continuation; exact transition selection never silently falls back to unrelated work.', projectAdvanceSchema, 'primary', Object.freeze({ worker: true, mcp: true })),
    'project.inspect': descriptor('project.inspect', 'project.inspect', 'Inspect authoritative repository-owned project state by project identity only. The runtime adapter derives the exact GitHub authority revision, graph frontier, and bounded frontier occupancy while keeping repository layout and host-specific runtime coordinates outside the primary semantic intent.', projectInspectSchema, 'primary', Object.freeze({ worker: true, mcp: true })),
