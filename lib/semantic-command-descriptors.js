...
const DESCRIPTORS = Object.freeze({
    'github.apply_changeset': descriptor('github.apply_changeset', 'github_apply_changeset', 'Apply an exact repository changeset using only a valid project-transition lease as execution authority. Overcenter derives repository, managed workspace branch, immutable generation base, exact workspace-head fence, retry identity, and GitHub App credentials. Caller-selected Git coordinates are not accepted.', githubApplyChangesetSchema, 'advanced', INTERNAL_EXPOSURE),
    'github.coalesce_changeset': descriptor('github.coalesce_changeset', 'github_coalesce_changeset', 'Replace the immediately preceding mechanical workspace head with one combined mechanical repair under the same project-transition lease. Overcenter derives repository, managed branch, exact head, and credential authority; stale heads, unrelated leases, and non-mechanical parents fail closed before ref mutation.', githubApplyChangesetSchema, 'advanced', INTERNAL_EXPOSURE),
    'github.apply_text_replacements': descriptor('github.apply_text_replacements', 'github_apply_text_replacements', 'Apply bounded exact text replacements under a project-transition lease. Source text is read from an immutable workspace revision and the later changeset is mechanically fenced to that same workspace observation, so stale reads fail closed before mutation. Repository, branch, head, retry identity, and credentials are derived internally.', githubApplyTextReplacementsSchema, 'advanced', INTERNAL_EXPOSURE),
    ...
    'github.pull_request.close': descriptor('github.pull_request.close', 'github_pull_request_close', 'Close one exact-head pull request bound to a semantic artifact identity. Already-closed state is idempotent; uncertain mutation transport is reconciled by authoritative readback and is never blindly retried.', githubPullRequestCloseSchema, 'advanced', INTERNAL_EXPOSURE),
    'github.issue.close': descriptor('github.issue.close', 'github_issue_close', 'Close one exact GitHub issue bound to a semantic artifact identity. Already-closed state is idempotent; uncertain mutation transport is reconciled by authoritative readback and is never blindly retried.', githubIssueCloseSchema, 'advanced', INTERNAL_EXPOSURE),
    ...
    'orchestration.maintain': descriptor('orchestration.maintain', 'orchestration.maintain', 'Run bounded deterministic maintenance over current compact coordination and unresolved recovery state. The caller supplies no work target or recovery recipe; Overcenter discovers only already-authorized maintenance obligations and never selects semantic work.', orchestrationMaintainSchema, 'operator', INTERNAL_EXPOSURE),
...
const PROJECT_AUTHORING_DESCRIPTORS = Object.freeze({
    'project.define': descriptor('project.define', 'project.define', 'Define canonical repository-owned project graph facts at an exact observed Git revision. Overcenter owns repository layout, mutation fencing, retry identity, durable GitHub mutation, and authoritative graph readback.', projectDefineSchema, 'primary', WORKER_AND_MCP_EXPOSURE),
    ...
    'project.add_conversation': descriptor('project.add_conversation', 'project.add_conversation', 'Apply explicit conversation-derived graph judgment through canonical project authoring. The reasoning layer supplies the semantic amendment and bounded conversation provenance; the deterministic graph kernel validates and applies that amendment at an exact Git authority revision and returns authoritative diff/readback evidence.', projectAddConversationSchema, 'primary', Object.freeze({ worker: false, mcp: true })),
});
