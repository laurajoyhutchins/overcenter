...
const DESCRIPTORS = Object.freeze({
    ...
    'github.issue.close': descriptor('github.issue.close', 'github_issue_close', 'Close one exact GitHub issue through the Overcenter GitHub App. The command accepts only repository, numeric issue identity, and an explicit open-state fence; it rejects pull requests exposed through the issues API, converges idempotently when already closed, never retries an uncertain mutation, and confirms closure through fresh authoritative GitHub readback. Project-level code owns semantic authorization to retire the bound artifact.', githubIssueCloseSchema, 'advanced', INTERNAL_EXPOSURE),
    'github.pull_request.close': descriptor('github.pull_request.close', 'github_pull_request_close', 'Close one exact-head GitHub pull request through the Overcenter GitHub App. The command fences repository, numeric pull request identity, open state, and exact head; converges idempotently when already closed at that head; never retries an uncertain mutation; and confirms closure through fresh authoritative GitHub readback. Project-level code owns semantic authorization to retire the bound artifact.', githubPullRequestCloseSchema, 'advanced', INTERNAL_EXPOSURE),
