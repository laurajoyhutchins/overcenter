...
const projectAddObligationSchema = Object.freeze({
    type: 'object',
    required: ['project_ref', 'expected_revision', 'obligation'],
    properties: {
        project_ref: { type: 'string', pattern: '^github:[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$' },
        expected_revision: { type: 'string', pattern: '^[0-9a-fA-F]{40}$' },
        obligation: {
            type: 'object',
            required: ['id', 'desired_outcome', 'acceptance_evidence'],
            properties: {
                id: { type: 'string', minLength: 1, maxLength: 256, pattern: '^\\S+$' },
                desired_outcome: { type: 'string', minLength: 1, maxLength: 4096 },
                priority: { type: 'integer' },
                requires: { type: 'array', items: { type: 'string', minLength: 1, maxLength: 256 } },
                executor: { type: 'object' },
                acceptance_evidence: { type: 'array', minItems: 1, maxItems: 16, items: { type: 'object', required: ['kind', 'requirement'], properties: { kind: { type: 'string' }, requirement: { type: 'string' } }, additionalProperties: false } },
            },
            additionalProperties: false,
        },
    },
...
const PROJECT_AUTHORING_DESCRIPTORS = Object.freeze({
    ...
    'project.add_obligation': descriptor('project.add_obligation', 'project.add_obligation', 'Turn one bounded conversation-derived project judgment into an authoritative project obligation. The caller supplies semantic obligation intent only; Overcenter lowers it to project.amend and retains ownership of repository layout, mutation fencing, retry identity, durable GitHub mutation, and authoritative readback.', projectAddObligationSchema, 'primary', Object.freeze({ worker:false, mcp:true })),
});
