...
const githubPullRequestCloseSchema = Object.freeze({
    type: 'object',
    required: ['repo', 'pull_request', 'expected_head', 'expected_state', 'artifact_ref'],
    additionalProperties: false,
    properties: {
...
const githubIssueCloseSchema = Object.freeze({
    type: 'object',
    required: ['repo', 'issue', 'expected_state', 'artifact_ref'],
    additionalProperties: false,
    properties: {
        repo: { type: 'string', minLength: 3, maxLength: 256, pattern: '^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$' },
...
const DESCRIPTORS = Object.freeze({
    ...
    'github.pull_request.close': descriptor('github.pull_request.close', 'github_pull_request_close', 'Close one exact GitHub pull request by numeric identity and exact head, bound to a semantic artifact reference. Already-closed state is idempotent; uncertain mutation transport is reconciled by fresh authoritative GitHub readback.', githubPullRequestCloseSchema, 'advanced', INTERNAL_EXPOSURE),
    'github.issue.close': descriptor('github.issue.close', 'github_issue_close', 'Close one exact GitHub issue by numeric identity, bound to a semantic artifact reference. Pull-request-shaped issue numbers are rejected, already-closed state is idempotent, and uncertain mutation transport is reconciled by fresh authoritative GitHub readback.', githubIssueCloseSchema, 'advanced', INTERNAL_EXPOSURE),
