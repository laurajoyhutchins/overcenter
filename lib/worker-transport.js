...
import { createGithubIntegrationConvergenceForRuntime } from 'lib/github-integration-convergence.js';
...
function githubIntegrationConvergenceFor(runtime = {}) {
  if (runtime.githubIntegrationConvergence && typeof runtime.githubIntegrationConvergence.converge === 'function') {
    return runtime.githubIntegrationConvergence;
  }
  const withGitHubAppApiClient = runtime.githubAppAuth?.withApiClient || runtime.withGitHubAppApiClient;
  return createGithubIntegrationConvergenceForRuntime({
    ...runtime,
    db:requireRuntimeDb(runtime),
    ...(typeof withGitHubAppApiClient === 'function' ? { withGitHubAppApiClient } : {}),
  });
...
const githubIntegrationReconcileDescriptor = semanticCommandDescriptor('github.integration.reconcile');
...
const specs = {
  ...
  'github.integration.reconcile': {
    allowed: new Set(githubIntegrationReconcileDescriptor.semantic_fields),
    required: new Set(githubIntegrationReconcileDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => githubIntegrationConvergenceFor(runtime).converge(request),
    statusForFailure: () => null,
    defaultError: 'GITHUB_INTEGRATION_CONVERGENCE_ERROR',
    defaultMessage: 'github.integration.reconcile failed',
  },
