// Runtime providers are injected by the composition root.
...
import { closeGithubPullRequestWithGitHubApp, closeGithubIssueWithGitHubApp } from 'lib/github-work-surface-close.js';
import { dispatchGitHubWorkflowWithGitHubApp } from 'lib/github-workflow-dispatch.js';
// GitHub App transport is supplied through runtime.githubAppAuth.
...
function requireRuntimeDb(runtime = {}) {
  if (!runtime.db || typeof runtime.db !== 'object') throw invalid('database provider is unavailable', { provider:'db' });
  return runtime.db;
}

function requireGitHubAppAuth(runtime = {}) {
  const auth = runtime.githubAppAuth;
  if (!auth || typeof auth.withApiClient !== 'function') throw invalid('GitHub App auth provider is unavailable', { provider:'githubAppAuth' });
  return auth;
}
...
function projectBootstrapConfirmationFor(runtime = {}) {
  ...
  const dbBinding = requireRuntimeDb(runtime);
  const graphRuntime = createGitHubProjectGraphRuntime({
    db:dbBinding,
    withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
  });
...
function orchestrationAdvanceFor(runtime = {}) {
  ...
  return createPostgresOrchestrationAdvanceService({ ...runtime, db:requireRuntimeDb(runtime) });
}

function orchestrationMaintenanceFor(runtime = {}) {
  if (runtime.orchestrationMaintenance && typeof runtime.orchestrationMaintenance.maintain === 'function') return runtime.orchestrationMaintenance;
  const withGitHubAppApiClient = runtime.githubAppAuth?.withApiClient || runtime.withGitHubAppApiClient;
  return createPostgresSubjectAwareOrchestrationMaintenanceService({
    db:requireRuntimeDb(runtime),
    api:runtime.api,
...
function projectAdvanceRuntimeFor(runtime = {}) {
  ...
  const runs = runtime.orchestrationRuns || createPostgresTargetAwareOrchestrationRunService(providerOptions);
  const advance = runtime.orchestrationAdvance || createPostgresOrchestrationAdvanceService(providerOptions);
  const finish = runtime.orchestrationFinish || Object.freeze({
  ...
  const confirmAuthoritativeEffect = async (request) => {
    const authoritativeEffect = runtime.projectTransitionAuthoritativeEffect
  ...
  return projectAdvanceFor({
    db:dbBinding,
    runs,
    advance,
    finish,
    confirmAuthoritativeEffect,
  });
}
...
function projectInspectionFor(runtime = {}) {
  ...
  return projectInspectForGitHub({
    db:requireRuntimeDb(runtime),
    withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
...
function releasePublishingRuntimeFor(runtime = {}) {
  ...
  return releasePublishingFor({
    db:requireRuntimeDb(runtime),
    withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
  });
}

function githubMutationsFor(runtime = {}, requiredMethod) {
  if (runtime.githubMutations
      && typeof runtime.githubMutations[requiredMethod] === 'function') return runtime.githubMutations;
  return createGithubWorkerMutationRuntime({
    db:requireRuntimeDb(runtime),
    api:runtime.api,
    withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
  });
...
function workCheckpointFor(runtime = {}) {
  ...
  return createPostgresSubjectAwareLeaseCheckpointService({ db:requireRuntimeDb(runtime), api:runtime.api, withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient });
}
...
function workHeartbeatFor(runtime = {}) {
  ...
  return createPostgresSubjectAwareLeaseHeartbeatService({ db:requireRuntimeDb(runtime), api:runtime.api, withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient });
}
...
function workSettlementFor(runtime = {}) {
  ...
  return createPostgresSubjectAwareLeaseSettlementService({ db:requireRuntimeDb(runtime), api:runtime.api, withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient });
}

const githubApplyChangesetDescriptor = semanticCommandDescriptor('github.apply_changeset');
const githubCoalesceChangesetDescriptor = semanticCommandDescriptor('github.coalesce_changeset');
...
const githubPullRequestCloseDescriptor = semanticCommandDescriptor('github.pull_request.close');
const githubIssueCloseDescriptor = semanticCommandDescriptor('github.issue.close');
...
const projectArtifactBindDescriptor = semanticCommandDescriptor('project.artifact.bind');
...
const specs = {
  'github.apply_changeset': {
    ...
    execute: (request, runtime) => githubMutationsFor(runtime, 'applyChangeset').applyChangeset(request),
  ...
  'github.coalesce_changeset': {
    allowed: new Set(githubCoalesceChangesetDescriptor.semantic_fields),
    required: new Set(githubCoalesceChangesetDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => githubMutationsFor(runtime, 'coalesceChangeset').coalesceChangeset(request),
    statusForFailure: statusForGithubWorkerMutationError,
    defaultError: 'GITHUB_COALESCE_CHANGESET_ERROR',
    defaultMessage: 'github.coalesce_changeset failed',
  },
  'github.apply_text_replacements': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => githubMutationsFor(runtime, 'applyTextReplacements').applyTextReplacements(request),
    statusForFailure: statusForGithubWorkerMutationError,
  ...
  'github.pull_request.mark_ready': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => markGithubPullRequestReadyWithGitHubApp(request, { withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient }),
  ...
  'github.pull_request.close': {
    allowed: new Set(githubPullRequestCloseDescriptor.semantic_fields),
    required: new Set(githubPullRequestCloseDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => closeGithubPullRequestWithGitHubApp(request, { withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient }),
    statusForFailure: () => null,
    defaultError: 'GITHUB_PULL_REQUEST_CLOSE_ERROR',
    defaultMessage: 'github.pull_request.close failed',
  },
  'github.issue.close': {
    allowed: new Set(githubIssueCloseDescriptor.semantic_fields),
    required: new Set(githubIssueCloseDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => closeGithubIssueWithGitHubApp(request, { withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient }),
    statusForFailure: () => null,
    defaultError: 'GITHUB_ISSUE_CLOSE_ERROR',
    defaultMessage: 'github.issue.close failed',
  },
  'github.workflow.dispatch': {
    allowed: new Set(['repo','workflow','ref','expected_head','inputs']),
    required: new Set(['repo','workflow','ref','expected_head']),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => dispatchGitHubWorkflowWithGitHubApp(request, { withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient }),
    statusForFailure: () => null,
    defaultError: 'GITHUB_WORKFLOW_DISPATCH_ERROR',
    defaultMessage: 'github.workflow.dispatch failed',
  },
  'github.release.create': {
    allowed: new Set(githubReleaseDescriptor.semantic_fields),
    required: new Set(githubReleaseDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => createGithubReleaseWithGitHubApp(request, {
      db:requireRuntimeDb(runtime),
      withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
    }),
    statusForFailure: () => null,
  ...
  'orchestration.maintain': {
    ...
    required: new Set(orchestrationMaintainDescriptor.required_fields),
    canonicalize: async () => ({}),
    execute: (_request, runtime) => orchestrationMaintenanceFor(runtime).maintain(),
  ...
  'orchestration.diagnose': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => createPostgresOrchestrationDiagnosisService({ db:requireRuntimeDb(runtime), api:runtime.api }).diagnose(request),
    statusForFailure: statusForOrchestrationRunError,
  ...
  'production.reconcile': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => productionReconciliationFor({
      db:requireRuntimeDb(runtime),
      withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
    }).reconcile(request),
    statusForFailure: () => null,
  ...
  'production.promote': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => productionPromotionFor({
      db:requireRuntimeDb(runtime),
      withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
    }).promote(request),
    statusForFailure: () => null,
  ...
  'project.artifact.bind': {
    allowed: new Set(projectArtifactBindDescriptor.semantic_fields),
    required: new Set(projectArtifactBindDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => projectArtifactBindingFor({
      db:requireRuntimeDb(runtime),
      withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient,
    }).bind(request),
    statusForFailure: () => null,
  ...
  // skill.complete is defined with the canonical activation contract above.
};
