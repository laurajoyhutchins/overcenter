...
const specs = {
  ...
  'work.settle': {
    allowed: new Set(['lease_ref','disposition','evidence','reason','promotion_condition','requeue_class','continuation','next_state','next_lane']),
...
export async function executeSemanticWorkerCommand(command, input) {
  ...
  const service = spec.execute ? null : createPostgresWorkLeaseService();
