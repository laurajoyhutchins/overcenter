...
import { createPostgresSubjectAwareLeaseCheckpointService, createPostgresSubjectAwareLeaseHeartbeatService, createPostgresSubjectAwareLeaseSettlementService } from 'lib/orchestration-finish-runtime.js';
...
const specs = {
  ...
  'project.advance': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => projectAdvanceFor({ db:runtime.db || hatchableDb }).advance(request),
    statusForFailure: statusForOrchestrationAdvanceRuntimeError,
