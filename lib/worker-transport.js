...
function githubMutationsFor(runtime = {}) {
  if (runtime.githubMutations
      ...
      && typeof runtime.githubMutations.applyTextReplacements === 'function'
      && typeof runtime.githubMutations.coalesceMechanicalChangeset === 'function') return runtime.githubMutations;
...
const githubCoalesceMechanicalChangesetDescriptor = semanticCommandDescriptor('github.coalesce_mechanical_changeset');
...
const specs = {
  ...
  'github.coalesce_mechanical_changeset': {
    allowed: new Set(githubCoalesceMechanicalChangesetDescriptor.semantic_fields),
    required: new Set(githubCoalesceMechanicalChangesetDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => githubMutationsFor(runtime).coalesceMechanicalChangeset(request),
    statusForFailure: statusForGithubWorkerMutationError,
    defaultError: 'GITHUB_COALESCE_MECHANICAL_CHANGESET_ERROR',
    defaultMessage: 'github.coalesce_mechanical_changeset failed',
  },
