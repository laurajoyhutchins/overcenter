...
const specs = {
  ...
  'project.inspect': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => projectInspectFor({ readProjectGraph: createAuthoritativeProjectGraphReader(createGitHubProjectGraphRuntime({ db:runtime.db || hatchableDb })) }).inspect(request),
    statusForFailure: () => null,
