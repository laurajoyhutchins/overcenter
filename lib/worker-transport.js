...
const specs = {
  ...
  'github.workflow.dispatch': {
    ...
    canonicalize: async (input) => ({ ...input }),
    execute: (request) => dispatchGitHubWorkflowWithGitHubApp(request, { withGitHubAppApiClient }),
    statusForFailure: () => null,
  ...
  'work.claim': {
    allowed: new Set(['run_id', 'issue_id', 'expected_state', 'expected_lane', 'expected_revision']),
    required: new Set(['run_id', 'issue_id', 'expected_state', 'expected_lane']),
    canonicalize: canonicalClaimCommand,
    execute: (request) => createPostgresWorkLeaseService().claim(request),
  ...
  'skill.complete': {
    allowed: new Set(['run_id','skill','outputs']),
    required: new Set(['run_id','skill']),
    canonicalize: canonicalSkillCompleteCommand,
    execute: (request, runtime) => {
      const service = runtime.skillExecution || createPostgresSkillExecutionService({ db:runtime.db || hatchableDb });
...
export function workerCommandSpec(command) {
  return specs[command] || null;
}

function invalid(message, details = null) {
  const error = new Error(message);
  error.code = 'INVALID_WORKER_COMMAND';
  error.details = details;
  return error;
}

function validate(command, input) {
  const spec = workerCommandSpec(command);
  if (!spec) throw invalid('worker command is not recognized', { command });
  const source = input && typeof input === 'object' && !Array.isArray(input) ? input : {};
  const unknown = Object.keys(source).filter((key) => !spec.allowed.has(key)).sort();
  if (unknown.length) throw invalid('worker command contains unknown fields', { command, unknown });
  const missing = [...spec.required].filter((key) => source[key] === undefined || source[key] === null || source[key] === '');
  if (missing.length) throw invalid('worker command is missing required fields', { command, missing });
  return source;
}

export async function executeWorkerCommand(command, input, runtime = {}) {
  const spec = workerCommandSpec(command);
  let request;
  try {
    request = await spec.canonicalize(validate(command, input));
  } catch (error) {
    return commandFailure(command, sanitizeWorkerBoundaryError(error), {
      ...workerBoundaryFailureOptions(error, { phase:'validation', defaultRetryable:false }),
    });
  }
  const runId = typeof request.run_id === 'string' ? request.run_id : null;
  const execution = await executeCorrelatedCommand(command, request, async () => spec.execute(request, runtime), {
    statusForFailure: spec.statusForFailure,
