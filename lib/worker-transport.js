...
const githubActionsRunDeleteDescriptor = semanticCommandDescriptor('github.actions_run.delete');
...
const specs = {
  ...
  'github.actions_run.delete': {
    allowed: new Set(githubActionsRunDeleteDescriptor.semantic_fields),
    required: new Set(githubActionsRunDeleteDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => deleteGithubActionsRunWithGitHubApp(request, { withGitHubAppApiClient:requireGitHubAppAuth(runtime).withApiClient }),
    statusForFailure: () => null,
    defaultError: 'GITHUB_ACTIONS_RUN_DELETE_ERROR',
    defaultMessage: 'github.actions_run.delete failed',
  },
