...
import { createPostgresOvercenterMetricsSemanticService } from 'lib/overcenter-metrics-semantic-surface.js';
...
function orchestrationMetricsFor(runtime = {}) {
  if (runtime.orchestrationMetrics && typeof runtime.orchestrationMetrics.metrics === 'function') return runtime.orchestrationMetrics;
  return createPostgresOvercenterMetricsSemanticService({ db:requireRuntimeDb(runtime) });
}
...
const orchestrationMetricsDescriptor = semanticCommandDescriptor('orchestration.metrics');
...
const specs = {
  ...
  'orchestration.metrics': {
    allowed: new Set(orchestrationMetricsDescriptor.semantic_fields),
    required: new Set(orchestrationMetricsDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => orchestrationMetricsFor(runtime).metrics(request),
    statusForFailure: statusForOrchestrationRunError,
    defaultError: 'ORCHESTRATION_METRICS_ERROR',
    defaultMessage: 'orchestration.metrics failed',
  },
