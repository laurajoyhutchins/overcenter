...
function projectArtifactBindingRuntimeFor(runtime = {}) {
  if (runtime.projectArtifactBinding && typeof runtime.projectArtifactBinding.mutate === 'function') return runtime.projectArtifactBinding;
  return projectArtifactBindingFor({ db:runtime.db || hatchableDb });
}
...
const projectArtifactBindingDescriptor = semanticCommandDescriptor('project.artifact.bind');
...
const specs = {
  ...
  'project.artifact.bind': {
    allowed: new Set(projectArtifactBindingDescriptor.semantic_fields),
    required: new Set(projectArtifactBindingDescriptor.required_fields),
    canonicalize: async (input) => ({ ...input }),
    execute: (request, runtime) => projectArtifactBindingRuntimeFor(runtime).mutate(request),
    statusForFailure: () => null,
