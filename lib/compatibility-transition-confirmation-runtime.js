...
function completedReceipt(row, workRef) {
  ...
  const valid = row?.status === 'settled'
    ...
    && evidence.length > 0;
  if (!valid) {
    fail('COMPATIBILITY_WORK_NOT_CONFIRMED', 'latest durable work settlement does not prove canonical CONFIRM completion', { work_ref: workRef });
  }
...
export function createCompatibilityWorkSettlementReader(dbBinding = hatchableDb) {
...
export function createCompatibilityTransitionConfirmationRuntime(options = {}) {
  ...
  return createCompatibilityTransitionConfirmationService({ bindings, compatibilityWork, readProjectGraph, projectTransitions });
}