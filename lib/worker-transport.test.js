...
async function testClaimPreMutationUpstreamFailureIsRetryable() {
  const safeReadFailure = workerBoundaryCommandFailure('work.claim', {
    code: 'GITHUB_APP_UPSTREAM_ERROR',
    message: 'temporary GitHub read failed',
    may_have_mutated: false,
  }, { defaultError: 'WORK_CLAIM_ERROR', defaultMessage: 'work.claim failed', logger: { error() {} } });
  check(safeReadFailure.body?.error_class === 'upstream', 'claim read failure should remain upstream');
  check(safeReadFailure.body?.retryable === true, 'pre-mutation claim read failure should be retryable');
  check(safeReadFailure.body?.recommended_action === 'retry_same_request', 'pre-mutation claim read failure should use deterministic same-request retry');

  const ambiguousFailure = workerBoundaryCommandFailure('work.claim', {
    code: 'GITHUB_APP_UPSTREAM_ERROR',
    message: 'ambiguous GitHub failure',
    may_have_mutated: true,
  }, { defaultError: 'WORK_CLAIM_ERROR', defaultMessage: 'work.claim failed', logger: { error() {} } });
  check(ambiguousFailure.body?.retryable === false, 'possibly-mutated claim failure must remain non-retryable');

  const settleFailure = workerBoundaryCommandFailure('work.settle', {
    code: 'LINEAR_UPSTREAM_GRAPHQL',
    message: 'settlement upstream failure',
    may_have_mutated: false,
  }, { defaultError: 'WORK_SETTLE_ERROR', defaultMessage: 'work.settle failed', logger: { error() {} } });
  check(settleFailure.body?.retryable === false, 'retry policy must stay scoped to claim reads');
}
...
export async function runWorkerTransportTests() {
  const tests = [
    ...
    testClaimPreMutationUpstreamFailureIsRetryable,
  ];
  let passed = 0;
  const failures = [];
