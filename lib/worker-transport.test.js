...
async function testMaintenanceIsAvailableThroughSemanticWorkerTransport() {
  const input = validateSemanticWorkerCommand('orchestration.maintain', {});
  check(Object.keys(input).length === 0, 'maintenance semantic transport invented caller-owned state');
  let error = null;
  try { validateSemanticWorkerCommand('orchestration.maintain', { project_ref:'github:owner/repo' }); }
  catch (caught) { error = caught; }
  check(error?.code === 'REQUEST_INVALID', 'maintenance semantic transport accepted caller-selected recovery scope');
}
...
async function testProjectBootstrapConfirmationHasNarrowSemanticBoundary() {
  const command = 'project.bootstrap_confirm_graph_deriver';
  const input = validateSemanticWorkerCommand(command, { run_id:'bootstrap-confirm-run' });
  check(input.run_id === 'bootstrap-confirm-run', 'bootstrap confirmation lost run identity');
  let error = null;
  try { validateSemanticWorkerCommand(command, { run_id:'bootstrap-confirm-run', project_ref:'caller-owned' }); }
  catch (caught) { error = caught; }
  check(error?.code === 'REQUEST_INVALID', 'bootstrap confirmation accepted caller-owned project authority');

  const calls = [];
  const response = await executeSemanticWorkerCommand(command, { run_id:'bootstrap-confirm-run' }, {
    projectBootstrapConfirmation:{
      async confirm(request) {
        ...
        return {
          ok:true,
          outcome:'confirmed',
          transition_id:'register-project-graph-deriver',
          transition_definition_fingerprint:'d'.repeat(64),
          lease_ref:'00000000-0000-4000-8000-00000000beef',
          authority:{ kind:'github', repository:'laurajoyhutchins/overcenter', revision:'a'.repeat(40), derivation:'overcenter-project-graph-v1' },
        };
      },
    },
    logger:{ error() {} },
  });
  check(response.status === 200 && response.body?.ok === true, 'bootstrap confirmation did not execute through semantic worker transport');
  check(calls.length === 1 && calls[0].run_id === 'bootstrap-confirm-run', 'bootstrap confirmation execution changed semantic input');
}
...
test('claim accepts revision semantic shape', testClaimAcceptsRevisionSemanticShape);
test('claim rejects caller reconstructed state strings', testClaimRejectsCallerReconstructedStateStrings);
test('LJH-370 source implementation uses server-issued revision only', testLjh370SourceImplementationUsesServerIssuedRevisionOnly);
test('claim rejects wire bookkeeping', testClaimRejectsWireBookkeeping);
test('lease-bound command uses non-secret lease ref', testLeaseBoundCommandUsesNonSecretLeaseRef);
test('settle accepts lifecycle facts and rejects successor selection', testSettleAcceptsLifecycleFactsAndRejectsSuccessorSelection);
test('maintenance is available through semantic worker transport', testMaintenanceIsAvailableThroughSemanticWorkerTransport);
test('diagnosis is available through semantic worker transport', testDiagnosisIsAvailableThroughSemanticWorkerTransport);
test('skill lifecycle commands use bounded semantic shapes', testSkillLifecycleCommandsUseBoundedSemanticShapes);
test('skill lifecycle executors receive runtime database', testSkillLifecycleExecutorsReceiveRuntimeDatabase);
test('project bootstrap confirmation has narrow semantic boundary', testProjectBootstrapConfirmationHasNarrowSemanticBoundary);
test('unsupported command rejected', testUnsupportedCommandRejected);
test('unsupported command returns command envelope', testUnsupportedCommandReturnsCommandEnvelope);
test('semantic validation returns command envelope', testSemanticValidationReturnsCommandEnvelope);
test('malformed revision fails before lease transport', testMalformedRevisionFailsBeforeLeaseTransport);
test('claim requires revision or legacy pair', testClaimRequiresRevisionOrLegacyPair);
test('malformed lease refs reject before database lookup', testMalformedLeaseRefsRejectBeforeDatabaseLookup);
test('unknown valid lease ref is bounded lease invalid', testUnknownValidLeaseRefIsBoundedLeaseInvalid);
test('database failure is sanitized at worker boundary', testDatabaseFailureIsSanitizedAtWorkerBoundary);
test('upstream provider failure keeps stable code but sanitizes payload', testUpstreamProviderFailureKeepsStableCodeButSanitizesPayload);
