...
import { createProjectDefinitionFactsReader, readProjectDefinitionFactsWithGitHubApp } from './project-definition-facts-reader.js';
...
// Native node:test registrations execute directly at module load.

  test('normalizes exact revision-attributed repository and pull-request facts', async()=>{
  ...
  test('rejects ambiguous or inexact repository authority', async()=>{
  ...
  test('rejects unsupported facts instead of silently extending graph authority', async()=>{
  ...
  test('rejects duplicate pull-request identities', async()=>{
  ...
  test('rejects malformed check state and conclusion attribution', async()=>{
  ...
  test('project definition reads use the narrow contents-read capability', async()=>{
    const profile = githubAppPermissionProfile('project_facts');
    assert(JSON.stringify(profile) === JSON.stringify({ contents:'read' }), 'project facts permission scope is broader than contents read');
  });

  test('project definition discovery is bounded repository-owned structure', async()=>{
  ...
  test('project definition facts preserve exact source identity without derived graph state', async()=>{
  ...
  test('project definition reader binds every content read to the exact revision', async()=>{
    const calls = [];
    ...
    const file = (sha, content) => {
      ...
      return { status:200, body:{ type:'file', sha, encoding:'base64', content:base64, size:bytes.length } };
    };
    const responses = {
    ...
    assert(facts.definitions[0].sha256 === 'aef277fb6a70a89681a85e1b6d23f44ee2a6cc58490f9f5c95fc99db6d2d3542', 'definition content digest mismatch');
  ...
  test('project definition discovery selects one canonical source document', async()=>{
    const result = normalizeProjectDefinitionDiscovery({
      schema:'project-definition-discovery-v1',
      definitions:['.overcenter/definitions/target-architecture.json'],
    });
    assert(JSON.stringify(result.definitions) === JSON.stringify(['.overcenter/definitions/target-architecture.json']), 'canonical definition source changed');
  });

  // Legacy result aggregation removed; node:test owns result reporting.
