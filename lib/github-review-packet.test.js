...
const NO_CLASSIC = async () => { throw new Error('removed policy provider must not be invoked'); };
...
export async function runGithubReviewPacketTests() {
  ...
  results.push(await run('55 known-empty checks differ from unavailable checks', async () => {
    ...
    const unavailable = await reviewGithubPullRequest(request(), { github: gh, protectionProvider: NO_CLASSIC });
  ...
  results.push(await run('57 unavailable check source changes snapshot identity', async () => {
    const opts = { now: () => new Date('2026-08-17T15:00:00Z'), protectionProvider: NO_CLASSIC };
    const known = await reviewGithubPullRequest(request(), { ...opts, github: new FakeGithub() });
    const gh = new FakeGithub();
    gh.listCheckRuns = async () => {
      throw new GitHubReviewPacketError('GITHUB_PERMISSION_DENIED', 'Check runs unavailable.', { status: 403 }, 403);
