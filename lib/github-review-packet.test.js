...
export async function runGithubReviewPacketTests() {
  ...
  results.push(await run('20a first complete-enumeration miss awaits check observation', async () => {
  ...
  results.push(await run('20b repeated aged misses mature to delivery missing', async () => {
    const store = requiredCheckObservationStore();
  ...
  results.push(await run('20c failing check appearing after an empty observation takes normal semantics', async () => {
    const store = requiredCheckObservationStore();
  ...
  results.push(await run('20d a new exact head resets the missing-check observation window', async () => {
    const store = requiredCheckObservationStore();
    const rules = { rules: [rulesetRequiredCheck('test')], complete: true };
  ...
  results.push(await run('20e observed passing and pending checks retain normal semantics', async () => {
  ...
  results.push(await run('22 known-empty ruleset policy is explicit', async () => {
    const r = await reviewGithubPullRequest(request(), { github: new FakeGithub() });
  ...
  results.push(await run('24 ruleset permission limitation is explicit partial knowledge', async () => {
    const gh = new FakeGithub();
    gh.listRulesForBranch = async () => {
  ...
  results.push(await run('39 rulesets are the explicit policy surface', async () => {
    const r = await reviewGithubPullRequest(request(), { github: new FakeGithub() });
  ...
  results.push(await run('40 ruleset upstream limitation is explicit partial knowledge', async () => {
    const gh = new FakeGithub();
    gh.listRulesForBranch = async () => {
  ...
  results.push(await run('42 known-empty rulesets differ from unavailable rulesets', async () => {
  ...
  results.push(await run('53 check-run permission loss is bounded partial knowledge', async () => {
    const gh = new FakeGithub();
  ...
  results.push(await run('54 commit-status permission loss is bounded partial knowledge', async () => {
    const gh = new FakeGithub();
  ...
  results.push(await run('55 known-empty checks differ from unavailable checks', async () => {
    const known = await reviewGithubPullRequest(request(), { github: new FakeGithub() });
    const gh = new FakeGithub();
    gh.listCheckRuns = async () => {
      throw new GitHubReviewPacketError('GITHUB_PERMISSION_DENIED', 'Check runs unavailable.', { status: 403 }, 403);
  ...
  results.push(await run('56 partial check evidence still enforces final head coherence', async () => {
    const gh = new FakeGithub();
    gh.listCheckRuns = async () => {
      throw new GitHubReviewPacketError('GITHUB_PERMISSION_DENIED', 'Check runs unavailable.', { status: 403 }, 403);
    };
  ...
  results.push(await run('57 unavailable check source changes snapshot identity', async () => {
    const opts = { now: () => new Date('2026-08-17T15:00:00Z') };
    const known = await reviewGithubPullRequest(request(), { ...opts, github: new FakeGithub() });
    const gh = new FakeGithub();
    gh.listCheckRuns = async () => {
  ...
  results.push(await run('58 pull-request observation permission loss preserves exact identity', async () => {
    const gh = new FakeGithub();
  ...
  results.push(await run('59 unavailable review evidence cannot imply review satisfaction', async () => {
  ...
  results.push(await run('60 unavailable changed-file evidence participates in snapshot identity', async () => {
    const opts = { now: () => new Date('2026-08-17T15:00:00Z') };
    const known = await reviewGithubPullRequest(request(), { ...opts, github: new FakeGithub() });
  ...
  results.push(await run('61 GitHub App token permission denial is bounded optional evidence', async () => {
    const denied = async () => {
  ...
  results.push(await run('62 rulesets are the sole branch-policy surface', async () => {
    const r = await reviewGithubPullRequest(request(), { github: new FakeGithub() });
