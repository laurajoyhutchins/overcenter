...
export async function runGithubReviewPacketTests() {
  ...
  results.push(await run('61 GitHub App token permission denial is bounded optional evidence', async () => {
    ...
    const r = await reviewGithubPullRequest(request(), { github: new FakeGithub(), protectionProvider: denied });
    check(r.ok, 'GitHub App optional token denial collapsed packet');
    check(r.protection.policy_surfaces.classic_branch_protection.available === false, 'token denial not represented as unavailable');
    check(r.protection.policy_surfaces.classic_branch_protection.unavailable.reason === 'permission_denied', 'token denial reason not classified as permission');
  }));
