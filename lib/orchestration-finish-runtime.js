...
export function createSubjectAwareLeaseSettlementService({ readLease, legacyLeases, projectTransitions } = {}) {
  ...
  async function settleByRef(input = {}) {
    ...
    if (input.continuation != null) {
      if (typeof projectTransitions.checkpoint !== 'function') {
        fail('PROJECT_TRANSITION_CHECKPOINT_UNAVAILABLE', 'project transition continuation requires durable checkpoint support', { lease_ref:leaseRef });
      }
      const settlementKey = typeof input.idempotency_key === 'string' ? input.idempotency_key.trim() : '';
      if (!settlementKey) fail('REQUEST_INVALID', 'idempotency_key is required to persist project transition continuation');
      await projectTransitions.checkpoint({
        lease_ref:leaseRef,
        run_id:runId,
        checkpoint:input.continuation,
        idempotency_key:`${settlementKey}:continuation`,
      });
    }
