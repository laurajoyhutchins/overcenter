...
export function createPostgresSubjectAwareLeaseCheckpointService(options = {}) {
  ...
  const store = options.projectTransitionStore || createProjectTransitionLeasePostgresStore(dbBinding);
  const projectTransitions = projectTransitionsFor({ ...options, projectTransitionStore:store }, dbBinding);
  const transitionCheckpoint = options.projectTransitionCheckpoint || {
    async checkpoint(input = {}) {
      ...
      await projectTransitions.require({ lease_ref:input.lease_ref, run_id:input.run_id });
      if (typeof store.getCheckpointByIdempotency !== 'function' || typeof store.insertCheckpoint !== 'function') {
        fail('PROJECT_TRANSITION_CHECKPOINT_STORAGE_UNAVAILABLE', 'project transition checkpoint persistence is unavailable');
      }
      const request = Object.freeze({ lease_ref:input.lease_ref, run_id:input.run_id, checkpoint:input.checkpoint });
      const requestHash = await sha256Text(canonicalJson(request));
      const prior = await store.getCheckpointByIdempotency(input.lease_ref, input.idempotency_key);
      if (prior) {
        if (prior.request_sha256 !== requestHash) fail('PROJECT_TRANSITION_IDEMPOTENCY_CONFLICT', 'project transition checkpoint idempotency key was already used for different semantics', { lease_ref:input.lease_ref });
        return Object.freeze({ ok:true, schema:'project-transition-lease-checkpoint-v1', subject:'project_transition', lease_ref:input.lease_ref, run_id:input.run_id, checkpoint_sha256:prior.checkpoint_sha256, idempotent_replay:true });
      }
      const checkpointSha = await sha256Text(canonicalJson(input.checkpoint));
      const saved = await store.insertCheckpoint(input.lease_ref, input.idempotency_key, requestHash, input.checkpoint, checkpointSha, options.now ? options.now() : new Date().toISOString());
      return Object.freeze({ ok:true, schema:'project-transition-lease-checkpoint-v1', subject:'project_transition', lease_ref:input.lease_ref, run_id:input.run_id, checkpoint_sha256:saved?.checkpoint_sha256 || checkpointSha, idempotent_replay:false });
    },
  };
  return createSubjectAwareLeaseCheckpointService({
    readLease:options.readLease || readLeaseByRef(dbBinding),
    legacyLeases,
    projectTransitions:transitionCheckpoint,
  });
}
