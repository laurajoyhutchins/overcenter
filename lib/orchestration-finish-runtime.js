         ...
         FROM (
           SELECT e.lease_ref::text AS lease_id,
                  ...
                  jsonb_build_object(
                    'schema','project-transition-lease-claim-v1',
                    'subject','project_transition',
                    'project_transition',jsonb_build_object(
                      'project_ref',e.project_ref,
                      'transition_id',e.transition_id,
                      'repository',e.authority_repository,
                      'authority_revision',e.authority_revision,
                      'authority_derivation',e.authority_derivation,
                      'graph_fingerprint',e.graph_fingerprint,
                      'transition_definition_fingerprint',e.transition_definition_fingerprint,
                      'transition_revision_fingerprint',e.transition_revision_fingerprint,
                      'transition_dependency_fingerprint',e.transition_dependency_fingerprint,
                      'slot_key',e.subject_key,
            ...
            WHERE e.subject_kind='project_transition'
              ...
              AND e.expires_at > $2
           UNION ALL
           SELECT l.lease_id::text,l.work_ref,l.gate,l.run_id,l.status,l.created_at,l.expires_at,l.claim_receipt,
                  e.subject_kind,e.execution_id,e.operation_id,e.lease_epoch,e.authority_epoch,
                  e.lifecycle AS execution_lifecycle,e.settled AS execution_settled,e.settlement_receipt
             FROM work_leases l
             LEFT JOIN execution_state e
            ...
            WHERE l.run_id=$1
              ...
              AND l.expires_at > $2
              AND COALESCE(l.claim_receipt->>'subject','legacy_work') <> 'project_transition'
         ) candidates
