...
export function createPostgresSkillExecutionStore(dbBinding) {
...
export function createPostgresSkillExecutionService(options = {}) {
  return createSkillExecutionService({ store:options.store || createPostgresSkillExecutionStore(options.db) });
}

export async function canonicalSkillCompleteCommand(input = {}, dbBinding) {
  const activationId = text(input.activation_id, 'activation_id', 128);
