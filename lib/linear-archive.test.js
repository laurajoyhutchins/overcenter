...
const cases = [
  ['non-terminal issue is refused before mutation', async () => {
  ...
  ['already archived is idempotent success', async () => {
    const apiBinding = new FakeLinearApi();
  ...
  ['archive applies, response is lost, and retry observes archived state', async () => {
    const apiBinding = new FakeLinearApi();
