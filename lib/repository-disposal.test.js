...
// Native node:test owns execution and failure aggregation.
  ...
  test('dispose blocks lifecycle before retiring stale execution projections', async()=>{
    const h=harness();
  ...
  test('dispose is idempotent and preserves historical projections as archived evidence', async()=>{
    const h=harness();
  ...
  test('retirement packets expose no compatibility maintenance surface', async()=>{
    const h=harness();
    const result=await h.service.dispose({repository:'owner/dead',disposition:'ARCHIVED',reason:'retired'});
  ...
  test('SUPERSEDED disposal records successor without making old repo executable', async()=>{
    const h=harness();
