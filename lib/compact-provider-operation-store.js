...
export function createCompactProviderOperationPostgresStore(dbBinding) {
  ...
  async function claimPending(input = {}) {
    ...
    return one(
      `UPDATE operation_state
          SET recovery_payload=jsonb_set(
                COALESCE(recovery_payload,'{}'::jsonb),
                '{attempt_token}',
                to_jsonb($6::text),
                true
              ),
              updated_at=$7
        WHERE command=$1 AND idempotency_scope=$2 AND idempotency_key=$3
          AND request_sha256=$4 AND state='prepared'
          AND recovery_payload->>'attempt_token'=$5
          AND recovery_payload->>'phase'=$8
        RETURNING *`,
      [command, scope, key, requestSha, priorAttemptToken, attemptToken, updatedAt, phase],
    );
  }
  ...
  async function reject(input = {}) {
    ...
    const attemptToken = required(input.attempt_token, 'attempt_token');
    const updatedAt = required(input.updated_at, 'updated_at');
    const mayHaveMutated = input.may_have_mutated === true;
    return one(
      `UPDATE operation_state
        ...
        WHERE command=$1 AND idempotency_scope=$2 AND idempotency_key=$3
          ...
          AND recovery_payload->>'attempt_token'=$4
        RETURNING *`,
      [command, scope, key, attemptToken, mayHaveMutated, JSON.stringify(jsonObject(input.resolution) || {}), updatedAt],
    );
  }
  ...
  async function listPending(input = {}) {
    ...
    const result = await dbBinding.query(
      `SELECT * FROM operation_state
        WHERE command=$1 AND state='prepared' AND recovery_payload->>'phase'=$2
        ORDER BY updated_at ASC, created_at ASC
        LIMIT ${limit}`,
      [command, phase],
    );
    return Object.freeze((result?.rows || []).map(operation));
  }

  async function listByScope(input = {}) {
    const command = required(input.command, 'command');
    const scope = required(input.scope, 'scope');
    const limit = Math.min(100, Math.max(1, Number(input.limit) || 20));
    const result = await dbBinding.query(
      `SELECT * FROM operation_state
        WHERE command=$1 AND idempotency_scope=$2
          AND (state IN ('prepared','indeterminate') OR (state='succeeded' AND resolution ? 'authoring'))
  ...
  return Object.freeze({ get, claim, claimPending, heartbeat, updateRecovery, resumeIndeterminate, markIndeterminate, succeed, reject, resolveNoEffect, abandon, listPending, listByScope });
}
