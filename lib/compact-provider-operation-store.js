...
export function createCompactProviderOperationPostgresStore(dbBinding) {
  ...
  async function findSucceededEffect(input = {}) {
    const command = required(input.command, 'command');
    const scope = required(input.scope, 'scope');
    const effectKind = required(input.effect_kind, 'effect_kind');
    const effectRef = required(input.effect_ref, 'effect_ref');
    return one(
      `SELECT * FROM operation_state
        WHERE command=$1 AND idempotency_scope=$2 AND state='succeeded'
          AND effect_kind=$3 AND effect_ref=$4
        ORDER BY resolved_at DESC, updated_at DESC
        LIMIT 1`,
      [command, scope, effectKind, effectRef],
    );
  }
  ...
  return Object.freeze({ get, findSucceededEffect, claim, heartbeat, updateRecovery, resumeIndeterminate, markIndeterminate, succeed, resolveNoEffect, abandon, listPending, listByScope });
}
