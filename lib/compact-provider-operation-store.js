...
export function createCompactProviderOperationPostgresStore(dbBinding) {
  ...
  async function pausePrepared(input = {}) {
    const command = required(input.command, 'command');
    const scope = required(input.scope, 'scope');
    const key = required(input.idempotency_key, 'idempotency_key');
    const attemptToken = required(input.attempt_token, 'attempt_token');
    ...
    return one(
      `UPDATE operation_state
          SET may_have_mutated=$5, recovery_payload=$6::jsonb, updated_at=$7
        WHERE command=$1 AND idempotency_scope=$2 AND idempotency_key=$3
          AND state='prepared' AND recovery_payload->>'attempt_token'=$4
        RETURNING *`,
      [command, scope, key, attemptToken, Boolean(input.may_have_mutated), payload(input.recovery_payload, attemptToken), updatedAt],
    );
  }

  async function resumePrepared(input = {}) {
    const command = required(input.command, 'command');
    const scope = required(input.scope, 'scope');
    const key = required(input.idempotency_key, 'idempotency_key');
    const requestSha = required(input.request_sha256, 'request_sha256');
  ...
  return Object.freeze({ get, claim, heartbeat, updateRecovery, pausePrepared, resumePrepared, resumeIndeterminate, markIndeterminate, succeed, resolveNoEffect, abandon });
}
