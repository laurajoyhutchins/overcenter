...
function operation(row) {
  ...
  return Object.freeze({
    ...
    attempt_epoch:Number(row.attempt_epoch || 0),
...
export function createCompactProviderOperationPostgresStore(dbBinding) {
  ...
  async function claim(input = {}) {
    ...
    const inserted = await one(
      `INSERT INTO operation_state (
         ...
         subject_key,run_id,lease_epoch,authority_revision,may_have_mutated,attempt_epoch,
         recovery_payload,created_at,updated_at
       ) VALUES ($1,$2,$3,$4,$5,'prepared',$6,$7,$8,$9,false,1,$10::jsonb,$11,$11)
    ...
    const taken = await one(
      `UPDATE operation_state
          SET attempt_epoch=attempt_epoch+1, recovery_payload=$5::jsonb, updated_at=$6
        WHERE command=$1 AND idempotency_scope=$2 AND idempotency_key=$3
  ...
  async function resumeIndeterminate(input = {}) {
    ...
    return one(
      `UPDATE operation_state
          SET attempt_epoch=attempt_epoch+1, recovery_payload=$7::jsonb, updated_at=$6
        WHERE command=$1 AND idempotency_scope=$2 AND idempotency_key=$3
  ...
  return Object.freeze({ get, claim, heartbeat, updateRecovery, resumeIndeterminate, markIndeterminate, succeed, resolveNoEffect, abandon, listPending, listByScope });
}
