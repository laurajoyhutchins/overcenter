import { mutationCertaintyFromEvidence, mutationRequiresReconciliation } from './mutation-certainty.js';
...
function matchingCandidatePulls(pulls, candidate, workspaceBranch, developmentBranch) {
  ...
  return Object.freeze({
    async confirm(request = {}) {
      ...
      for (const entry of workspaceEntries) {
        ...
        const matching = matchingCandidatePulls(pulls, candidate, entry.workspace.branch, developmentBranch);
      ...
      if ((!pull || !mergedPullRequest(pull, candidate, workspace.branch, developmentBranch)) && typeof integrateCandidate === 'function') {
        ...
        let integration;
        try {
          integration = await integrateCandidate({
        ...
        const certainty = mutationCertaintyFromEvidence(integration, integration?.may_have_mutated === true ? 'possible' : 'none');
        if (!integration?.ok && !mutationRequiresReconciliation(certainty)) {
          fail(integration?.error || integration?.code || 'PROJECT_TRANSITION_AUTHORITATIVE_EFFECT_INTEGRATION_FAILED', integration?.message || 'deterministic authoritative integration failed', {
            ...(integration && typeof integration === 'object' ? integration : {}),
            may_have_mutated:false,
            mutation_certainty:certainty,
          });
        }
        if (integration?.ok && ['merge_submitted', 'merge_pending', 'waiting', 'updated_for_recheck'].includes(String(integration.outcome || ''))) {
          return Object.freeze({
            confirmed:false,
            reason:'authoritative_effect_pending',
            mutation_certainty:'possible',
        ...
        let refreshed;
        try {
          refreshed = await readPullRequests({ repository:workspace.repository, head:workspace.branch, base:developmentBranch });
        } catch (error) {
          if (mutationRequiresReconciliation(certainty)) {
            fail('PROJECT_TRANSITION_AUTHORITATIVE_EFFECT_RECONCILIATION_REQUIRED', 'authoritative integration may have occurred and fresh provider readback failed', {
              may_have_mutated:true,
              mutation_certainty:'possible',
              required_continuation:'github_integration_reconcile',
            });
          }
          throw error;
        }
        const refreshedMatching = matchingCandidatePulls(refreshed, candidate, workspace.branch, developmentBranch);
        ...
        if (mutationRequiresReconciliation(certainty) && !mergedPullRequest(pull, candidate, workspace.branch, developmentBranch)) {
          return Object.freeze({
            confirmed:false,
            reason:'authoritative_effect_pending',
            mutation_certainty:'possible',
            recovery:Object.freeze({ mechanism:'github_integration_reconcile' }),
          });
        }
      }
