...
async function matchingWorkspacePullRequests({ pulls, candidate, authority, currentWorkspace, developmentBranch, deriveWorkspace }) {
  const matching = [];
  for (const pr of Array.isArray(pulls) ? pulls : []) {
    if (String(pr?.head?.sha || '').toLowerCase() !== candidate
        || String(pr?.base?.ref || '') !== developmentBranch) continue;
    const baseRevision = String(pr?.base?.sha || '').toLowerCase();
    const workspace = SHA40.test(baseRevision)
      ? await deriveWorkspace(authorityAtRevision(authority, baseRevision))
      : currentWorkspace;
    if (String(pr?.head?.ref || '') !== workspace.branch) continue;
    matching.push({ pr, workspace });
  }
  return matching;
}
...
export function projectTransitionAuthoritativeEffectConfirmationFor(options = {}) {
  ...
  return Object.freeze({
    async confirm(request = {}) {
      ...
      const currentWorkspace = await deriveWorkspace(authority);
      const roles = await resolveBranchRoles(currentWorkspace.repository);
      ...
      const matching = await matchingWorkspacePullRequests({
        pulls,
        candidate,
        authority,
        currentWorkspace,
      ...
      if (!match || !mergedPullRequest(match.pr, candidate, match.workspace.branch, developmentBranch)) {
      ...
      const developmentHead = String(await readBranchHead({ repository:currentWorkspace.repository, branch:developmentBranch }) || '').toLowerCase();
      ...
      if (developmentHead !== mergeCommit) {
        const comparison = await compareCommits({ repository:currentWorkspace.repository, base:mergeCommit, head:developmentHead });
      ...
      return Object.freeze({
        ...
        evidence:Object.freeze([
          Object.freeze({ kind:'authoritative_effect', ref:`github:${currentWorkspace.repository}#${Number(pull.number)}@${mergeCommit}` }),
          Object.freeze({ kind:'authority_readback', ref:`github:${currentWorkspace.repository}@${developmentHead}` }),
        ]),
      });
    },
  });
