...
export function projectOrchestrationStatus(snapshot) {
  const conditions = {
    overdue_active_runs: condition(snapshot.overdue_active_runs, (row) => ({ run_id: row.run_id, worker: row.worker, mode: row.mode, deadline_at: row.deadline_at, updated_at: row.updated_at, last_work_ref: row.last_work_ref || null, last_gate: row.last_gate || null, has_live_lease: Boolean(row.has_live_lease) })),
  ...
  return {
    async snapshot() {
      const [
        ...
        rows(dbBinding, `SELECT r.run_id,r.worker,r.mode,r.deadline_at,r.updated_at,r.last_work_ref,r.last_gate,r.deadline_at AS observed_at,
            EXISTS (SELECT 1 FROM work_leases l WHERE l.run_id=r.run_id AND l.status IN (${orchestrationRunInternals.liveLeaseStatusSql}) AND l.expires_at > now()) AS has_live_lease
        ...
        rows(dbBinding, `SELECT i.invocation_id, i.run_id, i.command, i.started_at, i.started_at AS observed_at FROM orchestration_command_invocations i
          WHERE i.outcome = 'running' AND i.started_at < now() - interval '${STUCK_MINUTES} minutes' AND NOT EXISTS (SELECT 1 FROM orchestration_invocation_resolutions r WHERE r.invocation_id=i.invocation_id) ORDER BY i.started_at ASC LIMIT 100`),
        rows(dbBinding, `SELECT i.invocation_id, i.run_id, i.command, i.started_at, i.error_code, i.started_at AS observed_at FROM orchestration_command_invocations i
          WHERE i.outcome = 'indeterminate' AND NOT EXISTS (SELECT 1 FROM orchestration_invocation_resolutions r WHERE r.invocation_id=i.invocation_id) ORDER BY i.started_at ASC LIMIT 100`),
