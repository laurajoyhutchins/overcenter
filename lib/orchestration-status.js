  ...
  return `(
    EXISTS (
      SELECT 1
       ...
       WHERE e.run_id=${runAlias}.run_id
    ...
    OR EXISTS (
      SELECT 1
        FROM work_leases l
       WHERE l.run_id=${runAlias}.run_id
         AND l.status IN (${orchestrationRunInternals.liveLeaseStatusSql})
  ...
  return {
    async snapshot() {
      const [
        ...
        rows(dbBinding, `SELECT work_ref, gate, lease_id, expires_at, expires_at AS observed_at
          FROM (
            SELECT e.subject_key AS work_ref, 'project_transition' AS gate,
                   e.lease_ref::text AS lease_id, e.expires_at
              FROM execution_state e
             ...
             WHERE s.expires_at <= now()
               ...
               AND NOT EXISTS (
                 SELECT 1
                   FROM execution_state e
                  WHERE e.subject_kind='project_transition'
                    AND e.subject_key=s.work_ref
                    AND e.lease_ref=s.lease_id
                    AND e.settled=false
               )
          ) expired
         ORDER BY expires_at ASC, work_ref ASC, gate ASC LIMIT 100`),
        rows(dbBinding, `SELECT work_ref, lease_id, run_id, updated_at, updated_at AS observed_at FROM work_leases
          WHERE status = 'claiming' AND COALESCE(claim_receipt->>'subject','') <> 'project_transition' AND updated_at < now() - interval '${STUCK_MINUTES} minutes' ORDER BY updated_at ASC LIMIT 100`),
        rows(dbBinding, `SELECT work_ref, lease_id, run_id, updated_at, updated_at AS observed_at FROM work_leases
          WHERE status = 'settling' AND COALESCE(claim_receipt->>'subject','') <> 'project_transition' AND updated_at < now() - interval '${STUCK_MINUTES} minutes' ORDER BY updated_at ASC LIMIT 100`),
        ...
        rows(dbBinding, `SELECT i.invocation_id, i.run_id, i.command, i.started_at, i.error_code, i.started_at AS observed_at
          ...
          WHERE i.outcome = 'indeterminate'
            ...
            AND (r.run_id IS NULL OR r.status <> 'finished' OR ${liveLeaseExistsSql('r')})
          ORDER BY i.started_at ASC LIMIT 100`),
        rows(dbBinding, `SELECT e.authority_repository AS repo, e.idempotency_key,
                NULL::text AS branch, e.authority_revision AS commit_sha,
                e.updated_at, e.updated_at AS observed_at
           FROM execution_state e
          WHERE e.operation_kind = 'github.changeset.apply'
            AND e.lifecycle IN ('executing', 'effect_uncertain')
            AND e.updated_at < now() - interval '${STUCK_MINUTES} minutes'
        ...
        rows(dbBinding, `SELECT e.authority_repository AS repo, e.idempotency_key,
           ...
           FROM execution_state e
          WHERE e.operation_kind = 'github.changeset.apply'
            AND e.lifecycle = 'prepared'
            AND e.updated_at < now() - interval '${STUCK_MINUTES} minutes'
          ORDER BY e.updated_at ASC LIMIT 100`),
        rows(dbBinding, `SELECT e.idempotency_key, NULL::text AS phase,
                e.updated_at, e.updated_at AS observed_at
           FROM execution_state e
          WHERE e.operation_kind = 'portfolio.reconcile'
            AND e.lifecycle IN ('executing', 'effect_uncertain')
            AND e.updated_at < now() - interval '${STUCK_MINUTES} minutes'
          ORDER BY e.updated_at ASC LIMIT 100`),
        rows(dbBinding, `SELECT e.idempotency_key, NULL::text AS phase,
           ...
           FROM execution_state e
          WHERE e.operation_kind = 'portfolio.reconcile'
            AND e.lifecycle = 'effect_uncertain'
          ORDER BY e.updated_at ASC LIMIT 100`),
        rows(dbBinding, `SELECT i.invocation_id, i.run_id, i.command, i.started_at, i.started_at AS observed_at
