...
} from './github-repository-metadata.js';
...
// Legacy suite wrapper removed during node:test migration.
  ...
  test('request schema is exact and excludes identity or lifecycle mutation', async () => {
  ...
  test('repository metadata permission is command-owned and does not leak into contents changes', async () => {
  ...
  test('already compliant state is a verified no-op', async () => {
  ...
  test('only changed declared fields are patched and topics use their dedicated endpoint', async () => {
  ...
  test('stale expected state fails closed before mutation', async () => {
  ...
  test('ambiguous write reconciles authoritative desired state without replay', async () => {
    const client = queuedClient([
  ...
  test('partial multi-endpoint mutation remains indeterminate when desired state is not verified', async () => {
    const repoPath = '/repos/laurajoyhutchins/example';
    const client = queuedClient([
      { method: 'GET', path: repoPath, result: repository() },
      { method: 'GET', path: `${repoPath}/topics`, result: topics(['old-topic']) },
  ...
  test('command response and journal projections preserve semantics without storing metadata values', async () => {
