...
export async function deleteGithubActionsRun(input, options = {}) {
  ...
  if (!after.absent) {
    if (dispatchUncertain) {
      return failure('GITHUB_ACTIONS_RUN_DELETE_INDETERMINATE', 'Workflow run DELETE transport was lost and authoritative readback did not prove absence.', {
        repo:normalized.repo,
        workflow_run_id:normalized.workflow_run_id,
        expected_head_sha:normalized.expected_head_sha,
        actual_head_sha:after.run.head_sha,
        phase:'reconcile_after_uncertainty',
        github_path:path,
        may_have_mutated:true,
      });
    }
