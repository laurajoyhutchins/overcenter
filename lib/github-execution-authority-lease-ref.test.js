...
// Test cases register directly with the native Node test runner.
  ...
  test('non-secret lease reference resolves the same durable execution authority', async () => {
  ...
  test('unknown lease reference fails closed', async () => {
    const service = await fixture();
  ...
  test('token plus lease reference is rejected as ambiguous', async () => {
    const service = await fixture();
  ...
  test('graph-native project transition authority derives repository from the verified lease subject', async () => {
  ...
  test('expired graph-native authority fails closed', async () => {
  ...
  test('malformed graph-native subject identity fails closed', async () => {
  ...
  test('wrong project-transition repository evidence fails closed', async () => {
    const service = projectTransitionFixture({ verified: { repository: 'laurajoyhutchins/other' } });
    await expectCode(service.require({ lease_ref: PROJECT_LEASE_REF, repository: REPO }), 'EXECUTION_AUTHORITY_INVALID');
  });

  test('production Postgres factory preserves project-transition validator injection', async () => {
