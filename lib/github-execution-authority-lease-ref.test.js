...
export async function runGithubExecutionAuthorityLeaseRefTests() {
  ...
  results.push(await run('graph-native project transition authority requires neither a lane gate nor a Linear reader', async () => {
  ...
  results.push(await run('production Postgres factory preserves project-transition validator injection', async () => {
    const subject = { project_ref:PROJECT_REF, transition_id:TRANSITION_ID, repository:REPO, graph_fingerprint:GRAPH_FINGERPRINT, transition_definition_fingerprint:TRANSITION_FINGERPRINT };
