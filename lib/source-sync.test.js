...
// Tests execute directly through node:test discovery.
...
test('source authority stays fixed while installation coordinates are caller-owned', async () => {
...
test('source materialization rejects missing installation coordinates', async () => {
    let error = null;
    try {
      await planPullSync({
...
test('syncable path filter preserves repository-only surfaces', async () => {
...
test('root developer package remains GitHub-only source metadata', async () => {
    check(!isSyncableSourcePath('package.json'), 'repository developer package must not become Hatchable runtime source');
  });

test('Git blob identity matches canonical empty blob', async () => {
    check(await gitBlobSha('') === 'e69de29bb2d1d6434b8b29ae775ad8c2e48c5391', 'empty Git blob SHA mismatch');
  });

test('source sync exports no reverse GitHub publication planner', async () => {
...
test('GitHub-authoritative plan diagnoses runtime drift', async () => {
...
test('runtime drift never produces GitHub mutations', async () => {
    const plan = await planPullSync({
      ...coordinates(),
...
test('materialization rejects stale Hatchable runtime version', async () => {
    let error = null;
    try {
      await planPullSync({
...
test('materialization rejects stale GitHub head', async () => {
    let error = null;
    try {
      await planPullSync({
...
test('no-op materialization is recognized without writes', async () => {
...
test('materialized plan verifies fetched Git blob identities', async () => {
    const plan = await planPullSync({
      ...coordinates(),
      hatchable_files: [],
...
test('materialized plan produces exact writes deletes and target manifest', async () => {
    const plan = await planPullSync({
      ...coordinates(),
...
test('Hatchable runtime verification detects projection drift', async () => {
...
test('source materialization receipt is generated runtime evidence outside the GitHub source surface', async () => {
...
test('source materialization receipt binds one GitHub head to one immutable Hatchable deployment', async () => {
...
test('source materialization verification rejects stale authority or deployment drift', async () => {
    check(typeof sourceSync.createSourceMaterializationReceipt === 'function', 'receipt creator is missing');
    check(typeof sourceSync.sourceMaterializationReceiptContent === 'function', 'receipt serializer is missing');
    check(typeof sourceSync.verifySourceMaterializationDeployment === 'function', 'deployment verifier is missing');
    const plan = await planPullSync({ ...coordinates(), hatchable_files: [], github_tree: [{ path: 'lib/a.js', type: 'blob', sha: await gitBlobSha('a\n') }] });
    ...
    const wrongVersion = await sourceSync.verifySourceMaterializationDeployment({ receipt, observed_github_head: HEAD, observed_hatchable_version: 200, deployment_manifest: deploymentManifest });
    check(wrongVersion.ok === false && wrongVersion.differences.some(item => item.kind === 'hatchable_version_mismatch'), 'receipt was reusable at a later deployment version');
...
test('materialized plan emits the generated deployment receipt write automatically', async () => {
    const plan = await planPullSync({ ...coordinates(), hatchable_files: [], github_tree: [{ path: 'lib/a.js', type: 'blob', sha: await gitBlobSha('a\n') }] });
    const materialized = await materializePullPlan(plan, [{ path: 'lib/a.js', content: 'a\n' }]);
