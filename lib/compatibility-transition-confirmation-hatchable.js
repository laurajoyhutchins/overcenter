import { hatchableRuntimeProviders } from './hatchable-runtime-providers.js';
...
export function createHatchableCompatibilityTransitionConfirmationRuntime(options = {}) {
  const dbBinding = options.db || hatchableRuntimeProviders.db;
  const withGitHubAppApiClient = options.withGitHubAppApiClient || hatchableRuntimeProviders.githubAppAuth.withApiClient;
  const readProjectGraph = options.readProjectGraph || createAuthoritativeProjectGraphReader(
    createGitHubProjectGraphRuntime({ db:dbBinding, withGitHubAppApiClient }),
  );
