...
const RESULT_REVISION = 'b'.repeat(40);
...
function adapterFor({ applyChangeset, deriveProjectGraph }) {
...
async function postMutationReadbackFailure() {
  ...
  const adapter = adapterFor({
    applyChangeset: async () => {
      ...
      return {
        ok: true,
        new_head: RESULT_REVISION,
        commit_sha: RESULT_REVISION,
        created_branch: true,
      };
    },
    deriveProjectGraph: async () => {
      throw new Error('simulated semantic readback failure after confirmed GitHub mutation');
    },
  });
...
async function testConfirmedProjectAmendmentIsValidCommandSuccess() {
  ...
  check(response.status === 200, 'confirmed project amendment was rejected as an invalid command result');
  check(response.body?.ok === true, 'confirmed project amendment did not return a command success envelope');
  check(response.body?.authority?.revision === RESULT_REVISION, 'confirmed project amendment lost exact resulting authority');
}
