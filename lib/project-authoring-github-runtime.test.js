...
test('potential mutation evidence survives semantic wrapping', testPotentialMutationEvidenceSurvivesSemanticWrapping);
test('known pre-mutation failure remains safe', testKnownPreMutationFailureRemainsSafe);
test('thrown failure after mutation boundary defaults to potential mutation', testThrownFailureAfterMutationBoundaryDefaultsToPotentialMutation);
test('explicit pre-mutation throw remains safe', testExplicitPreMutationThrowRemainsSafe);
test('confirmed mutation then readback failure is potentially mutated', testConfirmedMutationThenReadbackFailureIsPotentiallyMutated);
test('confirmed project amendment is a valid command success', testConfirmedProjectAmendmentIsValidCommandSuccess);
