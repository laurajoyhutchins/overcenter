...
async function testMissingProviderFailureRemainsTyped() {
  const adapter = adapterFor({
    applyChangeset: (request) => applyGithubChangesetWithGitHubApp(request),
    deriveProjectGraph: async () => {
      throw new Error('deriveProjectGraph must not run after a missing provider failure');
    },
  });
  ...
  try {
    ...
    check(error?.code === 'RUNTIME_PROVIDER_MISSING', 'missing provider was hidden behind project authoring mutation uncertainty');
    check(error?.may_have_mutated === false, 'missing provider must remain explicitly pre-mutation');
    check(error?.details?.result?.phase === 'preflight.provider', 'missing provider must retain its preflight phase');
    return;
  }
  throw new Error('project authoring unexpectedly accepted a missing GitHub provider');
}
...
export async function runProjectAuthoringGithubRuntimeTests() {
  ...
  await testMissingProviderFailureRemainsTyped();
