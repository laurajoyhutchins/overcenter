...
export async function leaseIdentity(leaseToken, dbBinding) {
  if (!dbBinding || typeof dbBinding.query !== 'function') throw Object.assign(new Error('database provider is required'), { code:'RUNTIME_PROVIDER_MISSING', details:{ provider:'db' } });
...
export async function leaseIdentityByRef(leaseRef, dbBinding) {
  if (!dbBinding || typeof dbBinding.query !== 'function') throw Object.assign(new Error('database provider is required'), { code:'RUNTIME_PROVIDER_MISSING', details:{ provider:'db' } });
...
async function claimAttemptIdentity(request, dbBinding) {
  if (!dbBinding || typeof dbBinding.query !== 'function') throw Object.assign(new Error('database provider is required'), { code:'RUNTIME_PROVIDER_MISSING', details:{ provider:'db' } });
...
export async function canonicalClaimCommand(args = {}, dbBinding) {
...
export async function canonicalCheckpointCommand(args = {}, dbBinding) {
...
export async function canonicalHeartbeatCommand(args = {}, dbBinding) {
  const lease = await leaseIdentity(args.lease_token, dbBinding);
...
export async function canonicalSettleCommand(args = {}, dbBinding) {
  const lease = await leaseIdentity(args.lease_token, dbBinding);
  return {
    lease_token: args.lease_token,
    run_id: lease.run_id,
...
export async function canonicalCheckpointCommandByRef(args = {}, dbBinding) {
...
export async function canonicalHeartbeatCommandByRef(args = {}, dbBinding) {
  const lease = await leaseIdentityByRef(args.lease_ref, dbBinding);
  const hasCheckpoint = args.phase != null || args.next_action != null || args.next_action_kind != null || args.candidate != null || args.completed != null || args.evidence != null || args.authority_revisions != null;
  const checkpoint = hasCheckpoint ? checkpointFromSemantic(args) : null;
  const extendSeconds = args.extend_seconds == null ? workLeaseConfig.default_heartbeat_seconds : Number(args.extend_seconds);
...
export async function canonicalSettleCommandByRef(args = {}, dbBinding) {
  const lease = await leaseIdentityByRef(args.lease_ref, dbBinding);
  return {
    lease_ref: lease.lease_id,
    run_id: lease.run_id,
