...
import { workLeaseConfig } from 'lib/work-leases.js';
...
export async function leaseIdentityByRef(leaseRef, dbBinding = db) {
  const ref = text(leaseRef, 'lease_ref', 128);
  const result = await dbBinding.query(
    `SELECT lease_id, run_id, work_ref, gate, status, expires_at
       FROM work_leases
      WHERE lease_id = $1
  ...
  const lease = result.rows[0] || null;
  if (!lease) {
    const error = new Error('lease reference is not recognized');
    error.code = 'LEASE_INVALID';
    throw error;
  }
  return lease;
...
export async function canonicalClaimCommand(args = {}, dbBinding = db) {
  ...
  const request = {
    ...
    expected_state: text(args.observed_state ?? args.expected_state, 'observed_state', 128),
    expected_lane: text(args.observed_lane ?? args.expected_lane, 'observed_lane', 128),
...
export async function canonicalSettleCommand(args = {}, dbBinding = db) {
  ...
  const semantic = {
    ...
    next_state: optionalText(args.next_state, 'next_state', 128),
    next_lane: optionalText(args.next_lane, 'next_lane', 128),
  };
  const { lease_id: _identityOnly, ...requestSemantic } = semantic;
  return {
    lease_token: args.lease_token,
    run_id: lease.run_id,
    ...requestSemantic,
