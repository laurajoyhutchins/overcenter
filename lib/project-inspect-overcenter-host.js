...
function occupancyDetail(id, occupancy) {
  ...
  const suspended = !occupied && occupancy.suspended === true;
  return Object.freeze({
    id,
    availability:occupied ? 'occupied' : suspended ? 'suspended' : 'available',
    occupied,
    expires_at:occupied && occupancy.expires_at ? String(occupancy.expires_at) : null,
    ...(suspended ? { suspension_reason:String(occupancy.suspension_reason || 'blocked_settlement_promotion') } : {}),
  });
}
