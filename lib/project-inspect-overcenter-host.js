...
function authoringDetail(operation, projectRef, authorityRevision) {
  ...
  const canonical = operation?.canonical_execution === true;
  ...
  if (!SHA40.test(expectedRevision) || (!canonical && !SHA40.test(stagedRevision))) invalid('project.inspect observed authoring state without exact revision identity');
  ...
  return Object.freeze({
    ...
    staged_revision:SHA40.test(stagedRevision) ? stagedRevision : null,
