...
const FRONTIER_OCCUPANCY_CONCURRENCY = 8;
...
function authoringDetail(operation, projectRef, authorityRevision) {
  ...
  const payload = state === 'succeeded' ? (operation?.resolution?.authoring || {}) : (operation?.recovery_payload || {});
  ...
  const idempotencyKey = String(operation?.idempotency_key || payload.idempotency_key || '').trim();
  const command = String(payload.command || '').trim();
  if (!operationId || !idempotencyKey || !['project.define','project.amend'].includes(command)) {
    invalid('project.inspect observed ambiguous authoring transaction identity');
  }
  ...
  return Object.freeze({
    ...
    command,
    recovery_ref:`project-authoring:${projectRef}:${idempotencyKey}`,
...
function suspensionDetail(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const recoveryRef = String(value.recovery_ref || '').trim();
  const blockedLeaseRef = String(value.blocked_lease_ref || '').trim();
  const reason = String(value.reason || '').trim();
  const conditions = value.conditions && typeof value.conditions === 'object' && !Array.isArray(value.conditions) ? value.conditions : {};
  const promotionCondition = String(conditions.promotion_condition || '').trim();
  const releaseWhen = Array.isArray(conditions.release_when) ? conditions.release_when.map(String) : [];
  if (!recoveryRef || !blockedLeaseRef || !promotionCondition) return null;
  return Object.freeze({
    recovery_ref:recoveryRef,
    blocked_lease_ref:blockedLeaseRef,
    authority_revision:value.authority_revision ? String(value.authority_revision) : null,
    settled_at:value.settled_at ? String(value.settled_at) : null,
    reason:reason || 'blocked project transition',
    promotion_condition:promotionCondition,
    release_when:Object.freeze(releaseWhen),
  });
}
...
function occupancyDetail(id, occupancy) {
  ...
  if (suspensionObserved) {
    ...
    if (suspended) {
      const suspension = suspensionDetail(occupancy.suspension);
      if (suspension) detail.suspension = suspension;
    }
  }
...
async function mapInBatches(values, batchSize, mapper) {
  ...
  for (let start = 0; start < values.length; start += batchSize) {
    const batch = await Promise.all(values.slice(start, start + batchSize).map((value, offset) => mapper(value, start + offset)));
    for (let offset = 0; offset < batch.length; offset += 1) results[start + offset] = batch[offset];
  }
  return results;
}
...
export function projectInspectFor(options = {}) {
  ...
  return Object.freeze({
    async inspect(input = {}) {
      ...
      const frontierOccupancy = Object.freeze(readTransitionOccupancy
        ? await mapInBatches(frontier, FRONTIER_OCCUPANCY_CONCURRENCY, async (id, index) => readTransitionOccupancy({
      ...
      const proofAuthority = Object.freeze({ kind:String(definition.kind || 'github'), repository:String(definition.repository || ''), revision, derivation:String(definition.derivation || '') });
      const proofStates = Object.freeze(await Promise.all(horizon.frontier.map((transition, index) => deriveProjectTransitionProofState({ transition, authority:proofAuthority, occupancy:frontierOccupancy[index], evidence:[] }))));
      ...
      const explanation = Array.isArray(horizon.nodes) ? deriveProjectExplanation({
        project_ref:projectRef,
        authority_revision:revision,
        nodes:horizon.nodes,
