...
function defaultProjectGraphReader(dbBinding) {
  let readProjectGraph = null;
  return async function read(input) {
    if (!readProjectGraph) {
      if (!dbBinding || typeof dbBinding.query !== 'function') {
        invalid('project.inspect runtime database dependency is unavailable');
      }
      const { createGitHubProjectGraphRuntime } = await import('./project-graph-github-runtime.js');
      readProjectGraph = createAuthoritativeProjectGraphReader(createGitHubProjectGraphRuntime({ db:dbBinding }));
    }
    return readProjectGraph(input);
  };
}

export function projectInspectFor(options = {}) {
  const readProjectGraph = options.readProjectGraph || defaultProjectGraphReader(options.db);
