...
async function settlementRequestHash(input) {
  const request = { ...(input || {}) };
...
async function loadCanonicalContinuation(store, subjectKey, executionFingerprint) {
...
export function createProjectTransitionLeaseService({ store, readProjectGraph, now = () => new Date().toISOString(), uuid = () => crypto.randomUUID() } = {}) {
  ...
  async function currentLease(row, input = {}) {
    ...
    if (typeof store.getExecutionState === 'function' && currentAuthorityEpoch > 0) {
      ...
      if (!matches) {
        fail('PROJECT_TRANSITION_LEASE_STALE', 'project transition canonical execution authority changed after lease acquisition', {
          lease_ref:row.lease_id,
          reason:'canonical_authority_fence_changed',
  ...
  async function acquire(input = {}) {
    ...
    if (prior) {
      ...
      const continuation = await loadCanonicalContinuation(store, prior.slot_key, executionFingerprint);
    ...
    if (typeof store.getExecutionState === 'function' && typeof store.reconcileExpired === 'function') {
      ...
      if (execution?.lifecycle !== 'settled'
          && execution?.settled !== true
          && execution?.lease_ref
          && execution.expires_at
          && instant(execution.expires_at, 'execution_state.expires_at') <= observedAt) {
    ...
    if (hasAtomicAcquire) {
      try {
        ...
        const continuation = await loadCanonicalContinuation(store, key, executionFingerprint);
  ...
  async function settle(input = {}) {
    ...
    const settlementRequestSha256 = await settlementRequestHash(input);
    const row = await store.getLease(leaseRef);
    if (!row) fail('PROJECT_TRANSITION_LEASE_INVALID', 'project transition lease reference is unknown');
    if (row.status === 'settled') {
      const priorSettlementHash = row.settlement_receipt?.settlement_request_sha256
        || row.settlement_receipt?.canonical_receipt?.settlement_request_sha256
        || null;
      if (row.settle_idempotency_key === idempotencyKey
          && row.disposition === disposition
          && priorSettlementHash === settlementRequestSha256) {
        return Object.freeze({ ok:true, schema:'project-transition-lease-settlement-v1', lease_ref:row.lease_id, status:'settled', disposition, settled_at:row.settled_at, graph_revision_change:row.graph_revision_change || null, settlement_receipt:row.settlement_receipt || null, idempotent_replay:true });
      }
    ...
    const updated = await store.settleLeaseAtomically({
      ...
      settlement_request_sha256:settlementRequestSha256,
    ...
    return Object.freeze({
      ...
      settled_at:settledAt,
      graph_revision_change:verified.graphRevisionChange,
      settlement_receipt:updated.settlement_receipt || null,
