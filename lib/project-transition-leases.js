...
function hasDurablePromotionCondition(latest) {
  return Array.isArray(latest?.settlement_evidence) && latest.settlement_evidence.some((entry) => {
    return String(entry?.kind || '').trim().toLowerCase() === 'promotion_condition'
      && String(entry?.ref || '').trim().length > 0;
  });
}

export async function projectTransitionBlockedSettlementSuspends(latest, transition) {
  if (!latest || latest.disposition !== 'blocked' || !transition || !hasDurablePromotionCondition(latest)) return false;
  ...
  return latest.transition_revision_fingerprint === revisionFingerprint
    && latest.transition_dependency_fingerprint === dependencyFingerprint;
}
...
export function createProjectTransitionLeaseService({ store, readProjectGraph, now = () => new Date().toISOString(), uuid = () => crypto.randomUUID() } = {}) {
  ...
  async function isSuspended(input = {}) {
    const projectRef = text(input.project_ref, 'project_ref');
    const transitionId = text(input.transition_id, 'transition_id', 256);
    if (typeof store.getLatestSettledLeaseForTransition !== 'function') return false;
    const latest = await store.getLatestSettledLeaseForTransition(projectRef, transitionId);
    if (!latest || latest.disposition !== 'blocked') return false;
    const graph = await readProjectGraph(Object.freeze({ project_ref:projectRef }));
    if (!graph || graph.schema !== 'project-graph-authority-v1' || graph.project_ref !== projectRef) {
      fail('PROJECT_TRANSITION_AUTHORITY_INVALID', 'authoritative project graph does not match requested project', { project_ref:projectRef });
    }
    const transition = evaluateProjectGraph(graph).nodes.find((node) => node.id === transitionId) || null;
    return projectTransitionBlockedSettlementSuspends(latest, transition);
  }
  ...
  return Object.freeze({ acquire, require:requireLease, contextFor, isSuspended, checkpoint, heartbeat, settle });
}
