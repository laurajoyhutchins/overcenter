...
import { reconcileProjectTransitionChange, reconcileProjectTransitionPresence } from './project-graph-reconciliation.js';
...
function publicLease(row, authority, idempotentReplay = false) {
...
export function createProjectTransitionLeaseService({ store, readProjectGraph, now = () => new Date().toISOString(), uuid = () => crypto.randomUUID() } = {}) {
  ...
  async function currentLease(row, input = {}) {
    ...
    if (!exactAuthority) {
      ...
      if (!transition) {
        const presence = reconcileProjectTransitionPresence({
          transition_id:row.transition_id,
          definition_fingerprint:row.transition_definition_fingerprint,
        }, null);
        staleAuthority(row, authority, { reason:presence.kind });
      }
      const currentTransitionFingerprint = await projectTransitionDefinitionFingerprint(transition);
      if (!row.transition_dependency_fingerprint) {
        staleAuthority(row, authority, { reason:'dependency-identity-unavailable' });
      }
      ...
      const reconciliation = reconcileProjectTransitionChange(
        ...
        },
        {
          ...
          definition_fingerprint:currentTransitionFingerprint,
    ...
    return { row, authority, graph, currentGraphFingerprint };
  }
  ...
  async function acquire(input = {}) {
    ...
    if (prior) {
      ...
      return publicLease(prior, verified.authority, true);
    }
