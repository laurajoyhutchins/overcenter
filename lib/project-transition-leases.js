...
import { buildProjectGraphRevisionEvidence, deriveProjectTransitionContinuationEvidence, reconcileProjectTransitionChange, reconcileProjectTransitionPresence, reconcileProjectTransitionRemoval } from './project-graph-reconciliation.js';
...
import { assertProjectTransitionCertificate, projectTransitionCertificateFingerprint } from './project-transition-certificate.js';
...
function settlementEvidence(value) {
  ...
  if (!Array.isArray(value) || value.length > 50) fail('PROJECT_TRANSITION_LEASE_REQUEST_INVALID', 'evidence must be an array of at most 50 items', { field:'evidence' });
  return value.map((item, index) => ({
    kind:text(item?.kind, `evidence[${index}].kind`, 128),
    ref:text(item?.ref, `evidence[${index}].ref`, 1024),
  }));
}
...
async function transitionExecutionFingerprint(revisionFingerprint, dependencyFingerprint) {
  const revision = typeof revisionFingerprint === 'string' ? revisionFingerprint.trim() : '';
  const dependency = typeof dependencyFingerprint === 'string' ? dependencyFingerprint.trim() : '';
  if (!revision || !dependency) return null;
  return sha256Text(canonicalJson({
...
export async function projectTransitionSuspension(latest, transition) {
  if (!latest || latest.disposition !== 'blocked' || !transition || !latest.settlement_promotion_condition) return null;
  const revisionFingerprint = await projectTransitionRevisionFingerprint({
  ...
  if (latest.transition_revision_fingerprint !== revisionFingerprint
      || latest.transition_dependency_fingerprint !== dependencyFingerprint) return null;
  return Object.freeze({
    schema:'project-transition-suspension-v1',
    recovery_ref:`project-transition-suspension:${latest.lease_id}`,
    project_ref:latest.project_ref,
    transition_id:transition.id,
    blocked_lease_ref:latest.lease_id,
    authority_revision:latest.authority_revision || null,
    settled_at:latest.settled_at || null,
    reason:latest.settlement_reason || 'blocked project transition',
    conditions:Object.freeze({
      promotion_condition:latest.settlement_promotion_condition,
      transition_revision_fingerprint:revisionFingerprint,
      transition_dependency_fingerprint:dependencyFingerprint,
      release_when:Object.freeze(['promotion_condition_satisfied','transition_revision_changes','transition_dependencies_change']),
    }),
  });
}

export async function projectTransitionBlockedSettlementSuspends(latest, transition) {
  return Boolean(await projectTransitionSuspension(latest, transition));
}

async function certificateObligationFingerprint(transition = {}) {
  const transitionId = text(transition.id, 'transition.id');
  const revision = await projectTransitionRevisionFingerprint({ transition_id:transitionId, priority:transition.priority, executor:transition.executor, phase_bindings:transition.phase_bindings, execution_intent:transition.execution_intent });
  const dependencies = await projectTransitionDependencyFingerprint({ transition_id:transitionId, requires:transition.requires || [] });
  return sha256Text(canonicalJson({ schema:'project-transition-obligation-fingerprint-v1', transition_revision_fingerprint:revision, transition_dependency_fingerprint:dependencies }));
}

function certificateEvidenceItem(value, index) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('PROJECT_TRANSITION_CERTIFICATE_EVIDENCE_INVALID', 'settlement evidence must contain objects', { index });
  return Object.freeze({ kind:text(value.kind, `evidence[${index}].kind`, 128), ref:text(value.ref, `evidence[${index}].ref`, 1024) });
}

function requiredCertificateEvidenceKinds(transition) {
  const declared = transition?.execution_intent?.acceptance_evidence;
  if (declared == null) return Object.freeze([]);
  if (!Array.isArray(declared)) fail('PROJECT_TRANSITION_CERTIFICATE_SETTLEMENT_INVALID', 'execution_intent.acceptance_evidence must be an array');
  return Object.freeze([...new Set(declared.map((entry, index) => text(entry?.kind, `acceptance_evidence[${index}].kind`, 128)))].sort());
}

function assertCertificateAcceptanceEvidence(transition, evidence) {
  const observedKinds = new Set(evidence.map((entry) => entry.kind));
  const missing = requiredCertificateEvidenceKinds(transition).filter((kind) => !observedKinds.has(kind));
  if (missing.length) fail('PROJECT_TRANSITION_CERTIFICATE_EVIDENCE_INCOMPLETE', 'settlement evidence does not cover every declared acceptance-evidence kind', { missing_kinds:missing });
}

function assertCertificateAuthorityEvidence(authority, evidence) {
  const repository = text(authority?.repository, 'authority.repository', 256);
  const revision = text(authority?.revision, 'authority.revision', 40).toLowerCase();
  if (!/^[0-9a-f]{40}$/.test(revision)) fail('PROJECT_TRANSITION_CERTIFICATE_AUTHORITY_STALE', 'certificate authority must be an exact Git revision');
  const exactRef = `github:${repository}@${revision}`;
  const authorityEvidence = evidence.filter((entry) => entry.kind === 'authority');
  if (authorityEvidence.length && authorityEvidence.some((entry) => entry.ref !== exactRef)) fail('PROJECT_TRANSITION_CERTIFICATE_AUTHORITY_STALE', 'authority evidence does not match the exact current authoritative revision', { expected_ref:exactRef, observed_refs:authorityEvidence.map((entry) => entry.ref) });
  if (evidence.some((entry) => entry.kind === 'candidate_revision')) fail('PROJECT_TRANSITION_CERTIFICATE_AUTHORITY_STALE', 'candidate revision evidence cannot authorize completed settlement');
}

async function certificatePrerequisiteClosure(transition, predecessorCertificates, predecessorTransitions) {
  const required = [...new Set((transition.requires || []).map((value, index) => text(value, `transition.requires[${index}]`, 256)))].sort();
  if (!Array.isArray(predecessorCertificates) || !Array.isArray(predecessorTransitions)) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'predecessor certificate and transition sets must be arrays');
  const byId = new Map();
  for (const raw of predecessorCertificates) {
    const certificate = assertProjectTransitionCertificate(raw);
    const transitionId = certificate.subject.transition_id;
    if (byId.has(transitionId)) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'predecessor certificate set contains duplicate transitions', { transition_id:transitionId });
    byId.set(transitionId, { certificate, certificate_ref:`sha256:${await projectTransitionCertificateFingerprint(certificate)}` });
  }
  const observed = [...byId.keys()].sort();
  if (canonicalJson(observed) !== canonicalJson(required)) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'predecessor certificate closure does not exactly match current requirements', { required, observed });
  const transitionsById = new Map();
  for (const item of predecessorTransitions) {
    const transitionId = text(item?.id, 'predecessor_transition.id', 256);
    if (transitionsById.has(transitionId)) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'predecessor transition set contains duplicate transitions', { transition_id:transitionId });
    transitionsById.set(transitionId, item);
  }
  const observedTransitions = [...transitionsById.keys()].sort();
  if (canonicalJson(observedTransitions) !== canonicalJson(required)) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'predecessor transition semantics do not exactly match current requirements', { required, observed:observedTransitions });
  for (const transitionId of required) {
    const currentFingerprint = await certificateObligationFingerprint(transitionsById.get(transitionId));
    const certifiedFingerprint = byId.get(transitionId).certificate.subject.obligation_fingerprint;
    if (currentFingerprint !== certifiedFingerprint) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'predecessor certificate does not match current predecessor obligation semantics', { predecessor_transition_id:transitionId, expected_obligation_fingerprint:currentFingerprint, observed_obligation_fingerprint:certifiedFingerprint });
  }
  return Object.freeze(required.map((transitionId) => Object.freeze({ transition_id:transitionId, obligation_fingerprint:byId.get(transitionId).certificate.subject.obligation_fingerprint, certificate_ref:byId.get(transitionId).certificate_ref })));
}

async function buildAndReplaySettlementCertificate(store, verified, row, evidence, observedAt) {
  const evaluated = evaluateProjectGraph(verified.graph);
  const transition = evaluated.nodes.find((node) => node.id === row.transition_id) || null;
  if (!transition) fail('PROJECT_TRANSITION_CERTIFICATE_SUBJECT_STALE', 'completed settlement transition is absent from the current authoritative graph', { transition_id:row.transition_id });
  const unresolved = Array.isArray(transition.unmet_requirements) ? transition.unmet_requirements.filter(Boolean) : [];
  if (unresolved.length) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement still has unresolved transition requirements', { transition_id:row.transition_id, unmet_requirements:unresolved });
  if ((transition.requires || []).length && typeof store.getLatestSettledLeaseForTransition !== 'function') fail('PROJECT_TRANSITION_CERTIFICATE_STORAGE_UNAVAILABLE', 'completed settlement cannot load predecessor certificates');
  const predecessorCertificates = [];
  const predecessorTransitions = [];
  for (const predecessorId of transition.requires || []) {
    const predecessorTransition = evaluated.nodes.find((node) => node.id === predecessorId) || null;
    if (!predecessorTransition) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement predecessor is absent from the current authoritative graph', { transition_id:row.transition_id, predecessor_transition_id:predecessorId });
    const predecessor = await store.getLatestSettledLeaseForTransition(row.project_ref, predecessorId, observedAt);
    if (!predecessor?.settlement_certificate) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement predecessor has no durable transition certificate', { transition_id:row.transition_id, predecessor_transition_id:predecessorId });
    predecessorCertificates.push(predecessor.settlement_certificate);
    predecessorTransitions.push(predecessorTransition);
  }
  const normalizedEvidence = Object.freeze(evidence.map(certificateEvidenceItem));
  if (!normalizedEvidence.length) fail('PROJECT_TRANSITION_CERTIFICATE_EVIDENCE_INCOMPLETE', 'completed settlement requires identity-bound acceptance evidence');
  assertCertificateAcceptanceEvidence(transition, normalizedEvidence);
  assertCertificateAuthorityEvidence(verified.authority, normalizedEvidence);
  const subject = Object.freeze({ project_ref:row.project_ref, transition_id:transition.id, obligation_fingerprint:await certificateObligationFingerprint(transition) });
  const prerequisites = await certificatePrerequisiteClosure(transition, predecessorCertificates, predecessorTransitions);
  const certificate = assertProjectTransitionCertificate({ schema:'project-transition-certificate-v1', subject, authority:verified.authority, prerequisites, authoritative_effects:normalizedEvidence.map((entry) => Object.freeze({ ...entry, subject })), claimed_outcome:{ disposition:'completed' } });
  const replaySubjectFingerprint = await certificateObligationFingerprint(transition);
  if (certificate.subject.project_ref !== row.project_ref || certificate.subject.transition_id !== transition.id || certificate.subject.obligation_fingerprint !== replaySubjectFingerprint) fail('PROJECT_TRANSITION_CERTIFICATE_OBLIGATION_STALE', 'certificate obligation fingerprint does not match current transition semantics');
  const replayPrerequisites = await certificatePrerequisiteClosure(transition, predecessorCertificates, predecessorTransitions);
  if (canonicalJson(certificate.prerequisites) !== canonicalJson(replayPrerequisites)) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'certificate predecessor closure is stale or incomplete');
  assertCertificateAcceptanceEvidence(transition, certificate.authoritative_effects);
  assertCertificateAuthorityEvidence(verified.authority, certificate.authoritative_effects);
  return certificate;
}
...
function authorityEpoch(value) {
  const epoch = Number(value);
  return Number.isSafeInteger(epoch) && epoch >= 0 ? epoch : 0;
}

function continuationFromExecution(execution, executionFingerprint) {
  if (!execution || !executionFingerprint) return null;
  if (String(execution.continuation_execution_fingerprint || '') !== executionFingerprint) return null;
  if (!execution.continuation || !execution.continuation_sha256) return null;
  const noProgressStreak = Math.max(0, Number(execution.no_progress_streak) || 0);
  return Object.freeze({
    schema:'project-transition-continuation-v1',
    packet:execution.continuation,
    packet_sha256:String(execution.continuation_sha256),
    execution_fingerprint:executionFingerprint,
    no_progress_streak:noProgressStreak,
    stalled_continuation:noProgressStreak >= 2,
  });
}

async function loadCompactContinuation(store, subjectKey, executionFingerprint) {
  if (typeof store?.getExecutionState !== 'function' || !executionFingerprint) return null;
  return continuationFromExecution(await store.getExecutionState(subjectKey), executionFingerprint);
}

function publicLease(row, authority, idempotentReplay = false, graphRevisionChange = null, continuation = null) {
  return Object.freeze({
    ok:true,
    ...
    authority_epoch:authorityEpoch(row.authority_epoch),
...
export function createProjectTransitionLeaseService({ store, readProjectGraph, now = () => new Date().toISOString(), uuid = () => crypto.randomUUID() } = {}) {
  const hasAtomicAcquire = typeof store?.acquireLeaseAtomically === 'function';
  const hasLegacyAcquire = typeof store?.insertLease === 'function' && typeof store?.insertSlot === 'function';
  if (!store || typeof store.getRun !== 'function' || typeof store.getLease !== 'function' || typeof store.getLeaseByAcquireIdempotency !== 'function'
      || typeof store.getSlot !== 'function' || (!hasAtomicAcquire && !hasLegacyAcquire)
  ...
  async function currentLease(row, input = {}) {
    ...
    const exactDefinitionAuthority = authority.repository === row.repository
      && authority.revision === row.authority_revision
      && authority.derivation === row.authority_derivation;
    const exactAuthority = exactDefinitionAuthority
    ...
    if (!exactAuthority) {
      ...
      const reconciliation = reconcileProjectTransitionChange(
        ...
        deriveProjectTransitionContinuationEvidence(
          { transition_id:row.transition_id, definition_fingerprint:row.transition_revision_fingerprint },
          { transition_id:transition.id, definition_fingerprint:currentRevisionFingerprint },
    ...
    let currentAuthorityEpoch = authorityEpoch(row.authority_epoch);
    if (typeof store.getExecutionState === 'function' && currentAuthorityEpoch > 0) {
      const execution = await store.getExecutionState(row.slot_key);
      const matches = execution
        && String(execution.lease_ref || '') === row.lease_id
        && String(execution.run_id || '') === row.run_id
        && authorityEpoch(execution.authority_epoch) === currentAuthorityEpoch
        && String(execution.authority_repository || '') === row.repository
        && String(execution.authority_revision || '') === row.authority_revision;
      if (!matches) {
        fail('PROJECT_TRANSITION_LEASE_STALE', 'project transition compact execution authority changed after lease acquisition', {
          lease_ref:row.lease_id,
          reason:'compact_authority_fence_changed',
          issued_authority_epoch:currentAuthorityEpoch,
          current_authority_epoch:execution ? authorityEpoch(execution.authority_epoch) : null,
          current_lease_ref:execution?.lease_ref || null,
        });
      }
      currentAuthorityEpoch = authorityEpoch(execution.authority_epoch);
    }
    return { row, authority, graph, currentGraphFingerprint, graphRevisionChange, authorityEpoch:currentAuthorityEpoch };
  }
  ...
  async function acquire(input = {}) {
    ...
    if (prior) {
      ...
      const executionFingerprint = await transitionExecutionFingerprint(prior.transition_revision_fingerprint, prior.transition_dependency_fingerprint);
      const continuation = await loadCompactContinuation(store, prior.slot_key, executionFingerprint);
      return publicLease(prior, verified.authority, true, verified.graphRevisionChange, continuation);
    }
    ...
    const executionFingerprint = await transitionExecutionFingerprint(transitionRevisionFingerprint, transitionDependencyFingerprint);
    const key = slotKey(projectRef, transitionId);

    if (typeof store.getExecutionState === 'function' && typeof store.reconcileExpired === 'function') {
      const execution = await store.getExecutionState(key);
      if (execution?.lease_ref && execution.expires_at && instant(execution.expires_at, 'execution_state.expires_at') <= observedAt) {
        await store.reconcileExpired(key, execution.lease_ref, observedAtText);
      }
    }
    ...
    if (hasAtomicAcquire) {
      try {
        const acquired = await store.acquireLeaseAtomically(row);
        const actual = acquired || row;
        const continuation = await loadCompactContinuation(store, key, executionFingerprint);
        return publicLease(actual, authority, false, null, continuation);
      } catch (error) {
    ...
    const continuation = await loadCompactContinuation(store, key, executionFingerprint);
    return publicLease(row, authority, false, null, continuation);
  }

  async function contextFor(input = {}) {
    const projectRef = text(input.project_ref, 'project_ref');
    const transitionId = text(input.transition_id, 'transition_id', 256);
    if (typeof store.getLatestSettledLeaseForTransition !== 'function') return null;
    const graph = await readProjectGraph(Object.freeze({ project_ref:projectRef }));
    if (!graph || graph.schema !== 'project-graph-authority-v1' || graph.project_ref !== projectRef) {
      fail('PROJECT_TRANSITION_AUTHORITY_INVALID', 'authoritative project graph does not match requested project', { project_ref:projectRef });
    }
    const definition = definitionAuthority(graph);
    if (input.authority?.revision && input.authority.revision !== definition.revision) {
      fail('PROJECT_TRANSITION_AUTHORITY_STALE', 'cross-transition context request is stale against current project authority', {
        project_ref:projectRef,
        transition_id:transitionId,
        expected_revision:input.authority.revision,
        actual_revision:definition.revision,
      });
    }
    const evaluated = evaluateProjectGraph(graph);
    const transition = evaluated.nodes.find((node) => node.id === transitionId) || null;
    if (!transition) {
      fail('PROJECT_TRANSITION_NOT_FOUND', 'cross-transition context target is absent from authoritative project graph', {
        project_ref:projectRef,
        transition_id:transitionId,
      });
    }
    const predecessors = [];
    for (const predecessorId of [...(transition.requires || [])].sort()) {
      const predecessor = evaluated.nodes.find((node) => node.id === predecessorId) || null;
      if (!predecessor) continue;
      const latest = await store.getLatestSettledLeaseForTransition(projectRef, predecessorId);
      if (!latest || latest.disposition !== 'completed') continue;
      const revisionFingerprint = await projectTransitionRevisionFingerprint({
        transition_id:predecessor.id,
        priority:predecessor.priority,
        executor:predecessor.executor,
        phase_bindings:predecessor.phase_bindings,
      });
      const dependencyFingerprint = await projectTransitionDependencyFingerprint({
        transition_id:predecessor.id,
        requires:predecessor.requires || [],
      });
      if (latest.transition_revision_fingerprint !== revisionFingerprint
          || latest.transition_dependency_fingerprint !== dependencyFingerprint) continue;
      predecessors.push(Object.freeze({
        transition_id:predecessorId,
        source_authority_revision:latest.authority_revision,
        transition_revision_fingerprint:revisionFingerprint,
        transition_dependency_fingerprint:dependencyFingerprint,
        settled_at:latest.settled_at || null,
        evidence:Object.freeze(settlementEvidence(latest.settlement_evidence)),
        ...(latest.settlement_reason ? { reason:optionalText(latest.settlement_reason, 'settlement_reason', 4096) } : {}),
      }));
    }
    return Object.freeze({
      schema:'project-transition-context-v1',
      project_ref:projectRef,
      transition_id:transitionId,
      authority:Object.freeze({ ...definition }),
  ...
  async function suspensionFor(input = {}) {
    ...
    const transitionId = text(input.transition_id, 'transition_id', 256);
    if (typeof store.getLatestSettledLeaseForTransition !== 'function') return null;
    const latest = await store.getLatestSettledLeaseForTransition(projectRef, transitionId);
    if (!latest || latest.disposition !== 'blocked') return null;
    const graph = await readProjectGraph(Object.freeze({ project_ref:projectRef }));
    if (!graph || graph.schema !== 'project-graph-authority-v1' || graph.project_ref !== projectRef) {
      fail('PROJECT_TRANSITION_AUTHORITY_INVALID', 'authoritative project graph does not match requested project', { project_ref:projectRef });
    }
    const transition = evaluateProjectGraph(graph).nodes.find((node) => node.id === transitionId) || null;
    return projectTransitionSuspension(latest, transition);
  }

  async function isSuspended(input = {}) {
    return Boolean(await suspensionFor(input));
  }
  ...
  async function requireLease(input = {}) {
    ...
    return Object.freeze({
      ...
      authority_epoch:lease.authorityEpoch,
  ...
  async function checkpoint(input = {}) {
    ...
    if (!input.checkpoint || typeof input.checkpoint !== 'object' || Array.isArray(input.checkpoint)) {
      fail('PROJECT_TRANSITION_LEASE_REQUEST_INVALID', 'checkpoint is required', { field:'checkpoint' });
    }
    for (const method of ['getCheckpointByIdempotency','insertCheckpoint']) {
      if (typeof store[method] !== 'function') fail('PROJECT_TRANSITION_CHECKPOINT_STORAGE_UNAVAILABLE', 'project transition checkpoint persistence is unavailable', { method });
    }
    ...
    const request = Object.freeze({ lease_ref:leaseRef, run_id:runId, checkpoint:input.checkpoint });
    const requestHash = await sha256Text(canonicalJson(request));
    const prior = await store.getCheckpointByIdempotency(leaseRef, idempotencyKey);
    if (prior && prior.request_sha256 !== requestHash) fail('PROJECT_TRANSITION_IDEMPOTENCY_CONFLICT', 'project transition checkpoint idempotency key was already used for different semantics', { lease_ref:leaseRef });
    const verified = await currentLease(row, { run_id:runId });
    if (prior) {
      return Object.freeze({ ok:true, schema:'project-transition-lease-checkpoint-v1', subject:'project_transition', lease_ref:leaseRef, run_id:runId, project_ref:row.project_ref, transition_id:row.transition_id, checkpoint_sha256:prior.checkpoint_sha256, authority:Object.freeze({ ...verified.authority }), graph_revision_change:verified.graphRevisionChange, idempotent_replay:true });
    }
    const checkpointSha = await sha256Text(canonicalJson(input.checkpoint));
    const saved = await store.insertCheckpoint(leaseRef, idempotencyKey, requestHash, input.checkpoint, checkpointSha, now());
    return Object.freeze({ ok:true, schema:'project-transition-lease-checkpoint-v1', subject:'project_transition', lease_ref:leaseRef, run_id:runId, project_ref:row.project_ref, transition_id:row.transition_id, checkpoint_sha256:saved?.checkpoint_sha256 || checkpointSha, authority:Object.freeze({ ...verified.authority }), graph_revision_change:verified.graphRevisionChange, idempotent_replay:false });
  }
  ...
  async function heartbeat(input = {}) {
    ...
    if (prior) {
      ...
      const verified = await currentLease(row, { run_id:runId });
      return Object.freeze({ ok:true, schema:'project-transition-lease-heartbeat-v1', subject:'project_transition', lease_ref:leaseRef, run_id:runId, project_ref:row.project_ref, transition_id:row.transition_id, previous_expires_at:prior.previous_expires_at, expires_at:prior.new_expires_at, hard_expires_at:row.hard_expires_at, checkpoint_sha256:prior.progress_sha256, authority:Object.freeze({ ...verified.authority }), graph_revision_change:verified.graphRevisionChange, idempotent_replay:true });
    }
    const verified = await currentLease(row, { run_id:runId });
    const observedAtText = now();
    ...
    const saved = await store.extendLeaseWithHeartbeat({ lease_id:leaseRef, slot_key:row.slot_key, authority_epoch:verified.authorityEpoch, idempotency_key:idempotencyKey, request_sha256:requestHash, progress_sha256:progressSha, previous_expires_at:row.expires_at, new_expires_at:newExpiresAt, created_at:observedAtText });
  ...
  async function settle(input = {}) {
    ...
    const evidence = settlementEvidence(input.evidence);
    const reason = optionalText(input.reason, 'reason', 2000);
    const promotionCondition = optionalText(input.promotion_condition, 'promotion_condition', 2000);
    const idempotencyKey = text(input.idempotency_key, 'idempotency_key', 256);
    ...
    if (row.status === 'settled') {
      if (row.settle_idempotency_key === idempotencyKey && row.disposition === disposition) {
        return Object.freeze({ ok:true, schema:'project-transition-lease-settlement-v1', lease_ref:row.lease_id, status:'settled', disposition, settled_at:row.settled_at, graph_revision_change:row.graph_revision_change || null, idempotent_replay:true });
      }
    ...
    if (typeof store.settleLeaseAtomically !== 'function') {
      throw new TypeError('project transition settlement requires atomic lease-and-slot storage');
    }
    const verified = await currentLease(row, { run_id:runId });
    const certificateObservedAt = now();
    const settlementCertificate = disposition === 'completed'
      ? await buildAndReplaySettlementCertificate(store, verified, row, evidence, certificateObservedAt)
      : null;
    const continuationExecutionFingerprint = await transitionExecutionFingerprint(row.transition_revision_fingerprint, row.transition_dependency_fingerprint);
    const settledAt = now();
    const updated = await store.settleLeaseAtomically({
      lease_id:row.lease_id,
      slot_key:row.slot_key,
      run_id:runId,
      authority_epoch:verified.authorityEpoch,
      project_ref:row.project_ref,
      transition_id:row.transition_id,
    ...
    return Object.freeze({
      ...
      authority_epoch:authorityEpoch(updated.authority_epoch),
      status:'settled',
      disposition,
      settled_at:settledAt,
      graph_revision_change:verified.graphRevisionChange,
  ...
  return Object.freeze({ acquire, require:requireLease, contextFor, suspensionFor, isSuspended, checkpoint, heartbeat, settle });
}
