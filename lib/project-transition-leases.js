...
async function settlementCertificateFor(store, verified, row, evidence, observedAt) {
  ...
  for (const predecessorId of transition.requires || []) {
    ...
    if (!predecessorTransition) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement predecessor is absent from the current authoritative graph', { predecessor_transition_id:predecessorId });
    const predecessor = await store.getLatestSettledLeaseForTransition(row.project_ref, predecessorId, observedAt);
    if (!predecessor?.settlement_certificate) fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement predecessor has no durable transition certificate', { predecessor_transition_id:predecessorId });
...
export function createProjectTransitionLeaseService({ store, readProjectGraph, now = () => new Date().toISOString(), uuid = () => crypto.randomUUID() } = {}) {
  ...
  async function settle(input = {}) {
    ...
    const settlementCertificate = disposition === 'completed'
      ? await settlementCertificateFor(store, verified, row, evidence, certificateObservedAt)
      : null;
