...
} from './project-transition-certificate-settlement.js';
...
async function completedSettlementCertificate(store, verified, row, evidence) {
  ...
  if (!transition) {
    fail('PROJECT_TRANSITION_CERTIFICATE_SUBJECT_STALE', 'completed settlement transition is absent from the current authoritative graph', { transition_id:row.transition_id });
  }
  const unresolved = Array.isArray(transition.unmet_requirements) ? transition.unmet_requirements.filter(Boolean) : [];
  if (unresolved.length) {
    fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement still has unresolved transition requirements', { transition_id:row.transition_id, unmet_requirements:unresolved });
  }
  if (typeof store.getLatestSettledLeaseForTransition !== 'function') {
    fail('PROJECT_TRANSITION_CERTIFICATE_STORAGE_UNAVAILABLE', 'completed settlement cannot load predecessor certificates');
  }
  ...
  for (const predecessorId of transition.requires || []) {
    ...
    if (!predecessorTransition) {
      fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement predecessor is absent from the current authoritative graph', { transition_id:row.transition_id, predecessor_transition_id:predecessorId });
    }
    const predecessor = await store.getLatestSettledLeaseForTransition(row.project_ref, predecessorId, nowForCertificate(verified));
    if (!predecessor?.settlement_certificate) {
      fail('PROJECT_TRANSITION_CERTIFICATE_PREREQUISITE_CLOSURE_INVALID', 'completed settlement predecessor has no durable transition certificate', { transition_id:row.transition_id, predecessor_transition_id:predecessorId });
    }
...
function nowForCertificate(verified) {
  return verified?.observedAtText || null;
}
...
export function createProjectTransitionLeaseService({ store, readProjectGraph, now = () => new Date().toISOString(), uuid = () => crypto.randomUUID() } = {}) {
  ...
  async function settle(input = {}) {
    ...
    const settlementCertificate = disposition === 'completed'
      ? await completedSettlementCertificate(store, verified, row, evidence)
      : null;
