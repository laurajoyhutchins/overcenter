...
import { createProjectTransitionSettlementCertificate, replayProjectTransitionSettlementCertificate } from './project-transition-certificate-settlement.js';
...
export async function projectTransitionSuspension(latest, transition) {
  ...
  const revisionFingerprint = await projectTransitionRevisionFingerprint({
    transition_id:transition.id, priority:transition.priority, executor:transition.executor, phase_bindings:transition.phase_bindings, execution_intent:transition.execution_intent, execution_intent:transition.execution_intent,
  });
...
async function buildAndReplaySettlementCertificate(store, verified, row, evidence, observedAt) {
  ...
  const input = { project_ref:row.project_ref, transition, authority:verified.authority, evidence, predecessor_certificates:predecessorCertificates, predecessor_transitions:predecessorTransitions };
  const certificate = await createProjectTransitionSettlementCertificate(input);
  await replayProjectTransitionSettlementCertificate({ ...input, certificate });
  return certificate;
}
  ...
  async function contextFor(input = {}) {
    ...
    for (const predecessorId of [...(transition.requires || [])].sort()) {
      ...
      const revisionFingerprint = await projectTransitionRevisionFingerprint({
        ...
        execution_intent:predecessor.execution_intent,
      });
