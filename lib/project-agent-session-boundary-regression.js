...
async function testUnconfirmedAuthoritativeEffectPreservesDeterministicReason() {
  const existing = Object.freeze({
    run_id:'project-advance-unconfirmed-effect',
    status:'active',
    target:Object.freeze({ project_ref:PROJECT_REF, horizon:Object.freeze({ kind:'transition', ref:'target-work' }) }),
  });
  const host = projectAdvanceFor({
    ...
    finish:{ async finish() { throw new Error('unconfirmed effect must not settle'); } },
    confirmAuthoritativeEffect:async()=>Object.freeze({ confirmed:false, reason:'authoritative_effect_not_observed' }),
    readRun:async(runId)=>runId === existing.run_id ? existing : null,
  });
  let error = null;
  try {
    await host.advance({
      project_ref:PROJECT_REF,
      resume_ref:existing.run_id,
      execution_result:{ disposition:'completed', evidence:[{ kind:'candidate_revision', ref:`github:${REPOSITORY}@${REVISION}` }] },
    });
  } catch (caught) { error = caught; }
  assert(error?.code === 'PROJECT_ADVANCE_AUTHORITATIVE_EFFECT_UNCONFIRMED', `unexpected error ${error?.code || 'none'}`);
  assert(error?.details?.confirmation_reason === 'authoritative_effect_not_observed', 'project.advance discarded the deterministic confirmation reason');
}
