...
function agentNode(id, { priority = 10, requires = [], executionIntent = EXECUTION_INTENT } = {}) {
...
async function testUnconfirmedAuthoritativeEffectPreservesDeterministicReason() {
  const existing = Object.freeze({
    run_id:'project-advance-unconfirmed-run',
    status:'active',
    target:Object.freeze({ project_ref:PROJECT_REF, horizon:Object.freeze({ kind:'transition', ref:'cross-epoch-work' }) }),
  });
  const host = projectAdvanceFor({
    db:{ async query() { throw new Error('unconfirmed settlement should use injected semantic dependencies'); } },
    runs:{ async start(request) { return request; } },
    advance:{ async advance() { throw new Error('settlement must not advance'); } },
    finish:{ async finish() { throw new Error('unconfirmed authoritative effect must not settle'); } },
    confirmAuthoritativeEffect:async()=> Object.freeze({
      confirmed:false,
      reason:'authoritative_effect_not_observed',
      observed_workspace:'work/cross-epoch-old',
    }),
    readRun:async(runId) => runId === existing.run_id ? existing : null,
  });
  let error = null;
  try {
    await host.advance({
      ...
      execution_result:Object.freeze({
        ...
        evidence:Object.freeze([{ kind:'candidate_revision', ref:`github:${REPOSITORY}@${REVISION}` }]),
        reason:'candidate is already merged',
      }),
    });
  } catch (caught) { error = caught; }
  assert(error?.code === 'PROJECT_ADVANCE_AUTHORITATIVE_EFFECT_UNCONFIRMED', 'unconfirmed authoritative effect did not fail closed');
  assert(error?.details?.confirmation_reason === 'authoritative_effect_not_observed', 'project.advance discarded the deterministic authoritative confirmation reason');
}
...
async function testResumableExecutionResultPreservesContinuationSemantics() {
  const finishes = [];
  const existing = Object.freeze({
    run_id:'project-advance-resumable-run',
    status:'active',
    target:Object.freeze({ project_ref:PROJECT_REF, horizon:Object.freeze({ kind:'transition', ref:'resumable-work' }) }),
  });
  const host = projectAdvanceFor({
    db:{ async query() { throw new Error('resumable settlement should use injected semantic dependencies'); } },
    runs:{ async start(request) { return request; } },
    advance:{ async advance() { throw new Error('resumable settlement must not advance'); } },
    finish:{ async finish(input) { finishes.push(input); return Object.freeze({ ok:true, run_id:input.run_id, status:'finished', disposition:input.disposition }); } },
    readRun:async(runId) => runId === existing.run_id ? existing : null,
  });
  await host.advance({
    project_ref:PROJECT_REF,
    ...
    execution_result:{
      disposition:'requeue',
      evidence:[{ kind:'candidate_revision', ref:`github:${REPOSITORY}@${REVISION}` }],
      reason:'candidate is waiting for exact-revision verification',
      requeue_class:'stale_candidate',
      continuation:{ candidate_revision:REVISION, next_action:'verify_exact_revision' },
    },
  });
  assert(finishes.length === 1, 'resumable execution_result did not settle exactly once');
  assert(finishes[0].active_lease_settlement?.requeue_class === 'stale_candidate', 'project.advance dropped resumable requeue policy');
  assert(finishes[0].active_lease_settlement?.continuation?.candidate_revision === REVISION, 'project.advance dropped resumable continuation');
}
...
async function testAgentExecutionIntentIsRequiredBeforeLeaseAcquisition() {
  const acquired = [];
  const service = createOrchestrationAdvanceService({
    store:{ async getRun() { return { run_id:'missing-intent-run', status:'active', target:{ project_ref:PROJECT_REF, horizon:{ kind:'transition', ref:'missing-intent-work' } } }; } },
    readProjectGraph:async()=>graph([agentNode('missing-intent-work', { executionIntent:null })]),
    projectTransitions:{
      async acquire(input) { acquired.push(input); throw new Error('missing execution intent must fail before lease acquisition'); },
      async settle() { throw new Error('missing execution intent must not settle'); },
    },
  });
  let error = null;
  try { await service.advance({ run_id:'missing-intent-run' }); }
  catch (caught) { error = caught; }
  assert(error?.code === 'ORCHESTRATION_ADVANCE_EXECUTION_INTENT_REQUIRED', `missing execution intent failed with ${error?.code || 'no error'}`);
  assert(acquired.length === 0, 'missing execution intent minted a transition lease before failing');
}

async function testExecutionIntentChangesTransitionFingerprint() {
  const base = {
    transition_id:'packet-only-work',
    priority:10,
  ...
  const identical = {
    ...base,
    execution_intent:JSON.parse(JSON.stringify(EXECUTION_INTENT)),
  };
  const navigationalOnly = {
    ...base,
    execution_intent:{ ...EXECUTION_INTENT, source_ref:'github:issue:999' },
  };
  const changed = {
    ...base,
    execution_intent:{
      ...EXECUTION_INTENT,
      acceptance_evidence:[{ kind:'artifact', requirement:'Evidence must reference a different verified artifact.' }],
    },
  };
  const baseFingerprint = await projectTransitionRevisionFingerprint(base);
  const identicalFingerprint = await projectTransitionRevisionFingerprint(identical);
  const navigationalFingerprint = await projectTransitionRevisionFingerprint(navigationalOnly);
  const changedFingerprint = await projectTransitionRevisionFingerprint(changed);
  assert(baseFingerprint === identicalFingerprint, 'structurally identical execution intent changed transition fingerprint');
  assert(baseFingerprint === navigationalFingerprint, 'non-authoritative source reference changed transition fingerprint');
  assert(baseFingerprint !== changedFingerprint, 'changed execution intent did not change transition fingerprint');
}
...
async function testPriorLeaseEpochAuthoritativeEffectCanCompleteAfterRequeue() {
  ...
  const currentAuthority = Object.freeze({
    ...
    run_id:'current-run',
    project_ref:PROJECT_REF,
    transition_id:'cross-epoch-work',
    transition_definition_fingerprint:'stable-definition',
    authority:Object.freeze({ revision:'b'.repeat(40) }),
  });
  const priorAuthority = Object.freeze({
    subject:'project_transition',
    run_id:'prior-run',
    project_ref:PROJECT_REF,
    transition_id:'cross-epoch-work',
    transition_definition_fingerprint:'stable-definition',
    authority:Object.freeze({ revision:'a'.repeat(40) }),
  });
  const service = projectTransitionAuthoritativeEffectConfirmationFor({
    readLeaseRef:async()=> 'current-lease',
    executionAuthority:{ async require() { return currentAuthority; } },
    readHistoricalAuthorities:async()=> [priorAuthority],
    deriveWorkspace:async(authority) => Object.freeze({
      repository:REPOSITORY,
      branch:`work/cross-epoch-${authority.authority.revision[0]}`,
    }),
    resolveBranchRoles:async()=> ({ development_branch:'dev' }),
    readPullRequests:async({ head }) => head === 'work/cross-epoch-a' ? [Object.freeze({
      number:615,
      head:Object.freeze({ sha:candidate, ref:head }),
      base:Object.freeze({ ref:'dev' }),
      merged_at:'2026-09-06T02:00:00.000Z',
      merge_commit_sha:mergeCommit,
    })] : [],
    readBranchHead:async()=> mergeCommit,
    compareCommits:async()=> ({ status:'identical', behind_by:0 }),
  });
  const result = await service.confirm({
    run_id:'current-run',
    target:Object.freeze({ project_ref:PROJECT_REF, horizon:Object.freeze({ kind:'transition', ref:'cross-epoch-work' }) }),
    execution_result:Object.freeze({ evidence:Object.freeze([{ kind:'candidate_revision', ref:`github:${REPOSITORY}@${candidate}` }]) }),
  });
  assert(result.confirmed === true, 'merged exact candidate from prior compatible lease epoch was not confirmed after requeue');
  assert(result.evidence?.some((entry) => entry.kind === 'authoritative_effect'), 'cross-epoch confirmation omitted authoritative effect evidence');
}
...
export async function runProjectAgentSessionBoundaryTests() {
  ...
  for (const [name, fn] of [
    ...
    ['unconfirmed authoritative effect preserves deterministic reason', testUnconfirmedAuthoritativeEffectPreservesDeterministicReason],
    ...
    ['resumable execution result preserves continuation semantics', testResumableExecutionResultPreservesContinuationSemantics],
    ...
    ['agent execution intent is required before lease acquisition', testAgentExecutionIntentIsRequiredBeforeLeaseAcquisition],
    ['execution intent changes transition fingerprint', testExecutionIntentChangesTransitionFingerprint],
    ['agent execution packet is self-contained', testAgentExecutionPacketIsSelfContained],
    ['prior lease epoch authoritative effect can complete after requeue', testPriorLeaseEpochAuthoritativeEffectCanCompleteAfterRequeue],
  ]) {
