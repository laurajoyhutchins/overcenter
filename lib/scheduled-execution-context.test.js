...
function fakeMaintenance(calls) {
  return async (input) => {
    calls.push(JSON.parse(JSON.stringify(input)));
    return { status:200, body:{ ok:true, recovered:[], command:'orchestration.maintain' } };
  };
}
...
export async function runScheduledExecutionContextTests() {
  ...
  results.push(await run('dispatcher reconcile owns run correlation and terminalizes a durable non-lease effect', async () => {
    ...
    const result = await service.execute({ participant:'portfolio-dispatcher', operation:'reconcile', input:{ project:'Portfolio Control Plane Reliability', items:[{ source:{kind:'github_issue',repo:'owner/repo',issue_number:7}, projection:{title:'x'} }] } });
