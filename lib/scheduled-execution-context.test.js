...
export async function runScheduledExecutionContextTests() {
  ...
  results.push(await run('scheduled claim retries one safe pre-mutation upstream failure inside runtime', async () => {
    ...
    const transport = async (command, input) => {
      calls.push({ command, input:JSON.parse(JSON.stringify(input)) });
      if (calls.length === 1) return { status:502, body:{ ok:false, error:'GITHUB_APP_UPSTREAM_ERROR', error_class:'upstream', retryable:true, may_have_mutated:false, recommended_action:'retry_same_request' } };
  ...
  results.push(await run('scheduled claim never retries an ambiguous upstream failure', async () => {
    const calls = [];
    const transport = async (command, input) => {
      calls.push({ command, input:JSON.parse(JSON.stringify(input)) });
    ...
    const service = createScheduledExecutionContextService({ runs:new FakeRuns(), runtimeStore:new FakeRuntimeStore(), transport, now:()=>'2026-08-21T15:12:03.000Z' });
    const result = await service.execute({ participant:'repository-implementation', operation:'claim', input:{ work_ref:'LJH-382', observed_revision:'revision-1' } });
    assert(calls.length === 1, 'ambiguous claim failure was retried');
    assert(result.ok === false && result.terminal === false, 'ambiguous claim failure lost fail-closed result');
  }));
