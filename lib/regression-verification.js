...
async function runOrchestrationVerification() {
  const [orchestration, leases, workerTransport, portfolioReconcile, workSurfacePolicy, deterministicWorkSettlement, linearArchive, linearMaintenance, scheduledCycleCompleteness, scheduledExecutionContext, repositoryDisposition, repositoryDisposal, skillExecution] = await Promise.all([
    runOrchestrationTests(),
    runWorkLeaseTests(),
    runWorkerTransportTests(),
    runPortfolioReconcileWorkSurfaceTests(),
    runWorkSurfacePolicyTests(),
    runDeterministicWorkSettlementTests(),
    runLinearArchiveTests(),
    runLinearMaintenanceTests(),
    runScheduledCycleCompletenessTests(),
    runScheduledExecutionContextTests(),
    runRepositoryDispositionTests(),
    runRepositoryDisposalTests(),
    runSkillExecutionTests(),
  ]);
  const suites = {
    orchestration,
    leases,
    ...
    work_surface_policy: workSurfacePolicy,
    deterministic_work_settlement: deterministicWorkSettlement,
    linear_archive: linearArchive,
    linear_maintenance: linearMaintenance,
    scheduled_cycle_completeness: scheduledCycleCompleteness,
    scheduled_execution_context: scheduledExecutionContext,
