...
export async function runCommandResponseTests() {
  ...
  results.push(await run('review packet partial capability remains successful', async () => {
    const body = commandSuccess('github.review_packet', {
      ...
      protection: {
        available: false,
        unavailable: { reason: 'rules evidence unavailable', required_permission: { administration: 'read' } },
      },
    }, { now: () => FIXED_TIME });
    ...
    assert(body.protection.unavailable.required_permission.administration === 'read', 'unavailable evidence changed');
