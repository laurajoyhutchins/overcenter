...
export async function runCommandResponseTests() {
  ...
  const failureCases = [
    ...
    ['repository user authorization is setup', 'GITHUB_USER_AUTH_REQUIRED', 'setup', false, false],
    ['repository creation approval is an expected precondition', 'GITHUB_REPOSITORY_APPROVAL_REQUIRED', 'precondition', false, true],
    ['repository create conflict is a precondition rejection', 'GITHUB_REPOSITORY_CONFLICT', 'precondition', false, true],
    ['repository create indeterminate is retryable upstream', 'GITHUB_REPOSITORY_CREATE_INDETERMINATE', 'upstream', true, false],
