...
export async function runCommandResponseTests() {
  ...
  results.push(await run('deterministic validation failures are request-invalid and non-escalating', async () => {
    const response = commandFailure('github.apply_changeset', {
      ok: false,
      error: 'INVALID_REPO',
  ...
  results.push(await run('public metadata policy rejection is validation, typed, and non-escalating', async () => {
  ...
  results.push(await run('production reconciliation source verification wait is typed and non-escalating', async () => {
  ...
  results.push(await run('production promotion source verification wait is typed and non-escalating', async () => {
