...
export async function runCommandResponseTests() {
  ...
  results.push(await run('cleanup-boundary failures return exact mechanical next transitions', async () => {
    const heartbeat = commandFailure('work.heartbeat', { ok: false, error: 'HEARTBEAT_LIMIT_REACHED', message: 'bounded lease horizon reached' }, { now: () => FIXED_TIME });
    const finish = commandFailure('orchestration.finish', { ok: false, error: 'RUN_HAS_ACTIVE_LEASE', message: 'active lease remains' }, { now: () => FIXED_TIME });
    assert(heartbeat.body.recommended_action === 'settle_active_lease', `heartbeat cleanup action was ${heartbeat.body.recommended_action}`);
    assert(finish.body.recommended_action === 'settle_active_lease_then_retry_finish', `finish cleanup action was ${finish.body.recommended_action}`);
  }));
