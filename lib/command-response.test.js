...
// Success-envelope coverage follows the production canonical registry directly.
...
// Tests execute directly through node:test.

  for (const command of CANONICAL_COMMANDS) {
    test(`success envelope: ${command}`, async () => {
  ...
  test('idempotent replay success stays top-level', async () => {
  ...
  const failureCases = [
    ...
    ['production reconciliation waits for exact source verification', 'PRODUCTION_RECONCILIATION_SOURCE_NOT_VERIFIED', 'precondition', true, true],
    ['production promotion waits for exact source verification', 'PRODUCTION_PROMOTION_SOURCE_NOT_VERIFIED', 'precondition', true, true],
  ...
  test('deterministic validation failures are request-invalid and non-escalating', async () => {
    const response = commandFailure('github.apply_changeset', {
      ok: false,
      error: 'INVALID_REPO',
    ...
    assert(response.body.error_class === 'validation', `validation class was ${response.body.error_class}`);
    assert(response.body.failure_state === 'REQUEST_INVALID', `validation failure state was ${response.body.failure_state}`);
    assert(response.body.escalation_required === false, 'deterministic validation escalated');
    assert(response.body.automatic_recovery_allowed === false, 'deterministic validation became automatic recovery');
    assert(response.body.recommended_action === 'correct_request', `validation action was ${response.body.recommended_action}`);
  });

  test('public metadata policy rejection is validation, typed, and non-escalating', async () => {
    const classification = classifyCommandError('PUBLIC_METADATA_POLICY_VIOLATION');
    assert(classification.error_class === 'validation', `policy rejection class was ${classification.error_class}`);
    assert(classification.retryable === false, 'policy rejection became retryable');
    assert(classification.rejection === true, 'policy rejection lost rejection semantics');
    assert(classification.http_status === 422, `policy rejection status was ${classification.http_status}`);
    const response = commandFailure('github.pull_request.create', { ok: false, error: 'PUBLIC_METADATA_POLICY_VIOLATION', message: 'unsafe public metadata', may_have_mutated: false }, { now: () => FIXED_TIME });
    assert(response.status === 422, `policy failure HTTP status was ${response.status}`);
    assert(response.body.failure_state === 'POLICY_REJECTION', `policy failure state was ${response.body.failure_state}`);
    assert(response.body.escalation_required === false, 'policy rejection escalated');
    assert(response.body.recommended_action === 'correct_request', `policy rejection action was ${response.body.recommended_action}`);
  });

  test('production reconciliation source verification wait is typed and non-escalating', async () => {
    const response = commandFailure('production.reconcile', {
      ok: false,
      error: 'PRODUCTION_RECONCILIATION_SOURCE_NOT_VERIFIED',
      message: 'development verification does not authorize the selected exact revision',
      may_have_mutated: false,
    }, { now: () => FIXED_TIME });
    assert(response.status === 409, `production verification wait status was ${response.status}`);
    assert(response.body.error_class === 'precondition', `production verification wait class was ${response.body.error_class}`);
    assert(response.body.retryable === true, 'production verification wait lost retryable semantics');
    assert(response.body.failure_state === 'WAITING_EXTERNAL_VERIFICATION', `production verification wait state was ${response.body.failure_state}`);
    assert(response.body.escalation_required === false, 'production verification wait escalated');
    assert(response.body.automatic_recovery_allowed === false, 'production verification wait became blind automatic retry');
    assert(response.body.recommended_action === 'retry_same_request', `production verification wait action was ${response.body.recommended_action}`);
    assert(response.body.recovery_operation?.command === 'production.reconcile', 'production verification wait lost semantic retry command');
    assert(response.body.recovery_operation?.mode === 'retry_same_request_after_external_verification', 'production verification wait lost external-evidence gating');
  });

  test('production promotion source verification wait is typed and non-escalating', async () => {
    const response = commandFailure('production.promote', {
      ok: false,
      error: 'PRODUCTION_PROMOTION_SOURCE_NOT_VERIFIED',
      message: 'source revision is not exact-revision verified',
      may_have_mutated: false,
    }, { now: () => FIXED_TIME });
    assert(response.status === 409, `production promotion verification wait status was ${response.status}`);
    assert(response.body.error_class === 'precondition', `production promotion verification wait class was ${response.body.error_class}`);
    assert(response.body.retryable === true, 'production promotion verification wait lost retryable semantics');
    assert(response.body.failure_state === 'WAITING_EXTERNAL_VERIFICATION', `production promotion verification wait state was ${response.body.failure_state}`);
    assert(response.body.escalation_required === false, 'production promotion verification wait escalated');
    assert(response.body.automatic_recovery_allowed === false, 'production promotion verification wait became blind automatic retry');
    assert(response.body.recommended_action === 'retry_same_request', `production promotion verification wait action was ${response.body.recommended_action}`);
    assert(response.body.recovery_operation?.command === 'production.promote', 'production promotion verification wait lost semantic retry command');
    assert(response.body.recovery_operation?.mode === 'retry_same_request_after_external_verification', 'production promotion verification wait lost external-evidence gating');
  });

  test('cleanup-boundary failures return typed deterministic recovery operations', async () => {
  ...
  test('command-specific rejection semantics stay centralized', async () => {
  ...
  test('command-response-v1 preserves top-level detail compatibility', async () => {
    const response = commandFailure('github.apply_changeset', {
      ok: false,
      error: 'HEAD_MISMATCH',
  ...
  test('executeCommand envelopes thrown domain errors', async () => {
  ...
  test('portfolio batch success preserves item-level rejection', async () => {
  ...
  test('review packet partial capability remains successful', async () => {
  ...
  test('optional run_id is additive on success and failure', async () => {
  ...
  // Native node:test reports per-test results.