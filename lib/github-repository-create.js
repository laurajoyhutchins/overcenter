...
export async function createGithubRepository(input, options = {}) {
  ...
  try {
    if (options.apiClient) return createWithClient(normalized, options.apiClient, options);
    return await withGitHubUserApiClient((apiClient) => createWithClient(normalized, apiClient, options), options);
  } catch (error) {
    const code = error?.code || 'GITHUB_USER_AUTH_ERROR';
    return { ok: false, error: code, message: String(error?.message || error), ...(error?.status ? { upstream_status: Number(error.status) } : {}), may_have_mutated: false };
  }
}
...
export async function runGithubRepositoryCreateRegressionTests() {
  ...
  await test('existing matching private repository is idempotent', async () => {
    const api = { call: async () => ({ status: 200, body: repoBody(), headers: {} }) };
    const result = await createGithubRepository({ name: 'fixture', description: 'fixture' }, { apiClient: api, installationProbe: probe, sleep: async () => {} });
  ...
  await test('unresolved transport loss remains indeterminate', async () => {
    const api = { call: async (_name, request) => request.method === 'POST' ? Promise.reject(new Error('connection lost')) : ({ status: 404, body: {}, headers: {} }) };
    const result = await createGithubRepository({ name: 'fixture', description: 'fixture' }, { apiClient: api, installationProbe: probe, sleep: async () => {} });
    check(!result.ok && result.error === 'GITHUB_REPOSITORY_CREATE_INDETERMINATE' && result.may_have_mutated === true, 'unresolved create did not remain indeterminate');
  });
