  ...
  const [transitionResult, branchRoleResult, dispositionResult] = await Promise.all([
    db.query(
      `SELECT e.transition_id,
              e.settlement_receipt->'project_transition'->>'transition_definition_fingerprint' AS transition_definition_fingerprint,
              e.authority_revision AS source_authority_revision,
              e.settled_at
         FROM execution_state e
        WHERE e.subject_kind='project_transition'
          AND e.lifecycle='settled'
          AND e.settlement_receipt->>'disposition'='completed'
          AND e.project_ref=$1
        ORDER BY e.transition_id, e.settled_at`,
      [projectRef],
    ),
    db.query(
 ...
 WHERE singleton=true
   ...
   AND NOT EXISTS (
         SELECT 1
          ...
          WHERE subject_kind='project_transition'
            AND lifecycle IN ('prepared', 'executing', 'effect_uncertain', 'effect_confirmed', 'effect_absent')
   ...
   AND (SELECT count(*)::bigint
         ...
         WHERE subject_kind='project_transition'
           ...
           AND settlement_receipt->>'disposition'='completed'
           AND project_ref=$3) = $4::bigint
   AND (SELECT max(settled_at)
          FROM execution_state
         WHERE subject_kind='project_transition'
           AND lifecycle='settled'
           AND settlement_receipt->>'disposition'='completed'
           AND project_ref=$3) IS NOT DISTINCT FROM $5::timestamptz
...
export async function freezeSourceAtRecoveryCoordinates({ db, project_ref, source_revision, seed_digest, coordinates } = {}) {
  ...
  if (!row) {
    throw failure('CUTOVER_FREEZE_PRECONDITION_MISMATCH', 'source durable coordinates changed or live execution authority exists before freeze; capture a fresh GitHub recovery seed and drain execution before retrying');
  }
  return Object.freeze({
