...
import {
  createGithubProductionPromotionReceiptStore,
...
export async function promoteGithubProductionWithGitHubApp(input, options = {}) {
  ...
  try {
    ...
    const receipts = options.receipts === undefined
      ? createGithubProductionPromotionReceiptStore(options.db || db)
      : options.receipts;
    const github = options.github || createGithubProductionPromotionAdapter(normalized.repo);
    return promoteGithubProduction(normalized, { github, branchRoles, receipts });
  } catch (error) {
