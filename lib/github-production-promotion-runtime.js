...
async function withContents(repo, fn, withGitHubAppApiClient) {
  if (typeof withGitHubAppApiClient !== 'function') throw Object.assign(new Error('GitHub App auth provider is required'), { code:'RUNTIME_PROVIDER_MISSING', details:{ provider:'githubAppAuth' }, mayHaveMutated:false });
  return withGitHubAppApiClient(repo, fn, { permissionProfile: 'changeset' });
}

async function withActionsRead(repo, fn, withGitHubAppApiClient) {
  if (typeof withGitHubAppApiClient !== 'function') throw Object.assign(new Error('GitHub App auth provider is required'), { code:'RUNTIME_PROVIDER_MISSING', details:{ provider:'githubAppAuth' }, mayHaveMutated:false });
  return withGitHubAppApiClient(repo, fn, { permissionProfile: 'actions_storage_read' });
}

export function createGithubProductionPromotionAdapter(repo, options = {}) {
...
export async function promoteGithubProductionWithGitHubApp(input, options = {}) {
  ...
  try {
    const branchRoles = options.branchRoles || await resolveRepositoryBranchRoles(normalized.repo, { db: options.db, service: options.branchRoleService });
    const compactReceipts = options.receipts === undefined
      ? createCompactGithubProductionPromotionReceiptStore(options.db, { now:options.now, runId:options.run_id || input?.run_id || null })
      : null;
    const receipts = compactReceipts || options.receipts;
    const proofs = options.proofs === undefined && options.receipts === undefined
      ? createCompactProofStateStore(options.db)
      : options.proofs || null;
    const baseGithub = options.github || createGithubProductionPromotionAdapter(normalized.repo, { withGitHubAppApiClient:options.withGitHubAppApiClient });
    const github = {
      ...baseGithub,
      ...(proofs ? {
