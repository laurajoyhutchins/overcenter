...
export function createCompactGithubChangesetReceiptStore(dbBinding, options = {}) {
  ...
  return Object.freeze({
    async findSucceededReceipt(input = {}) {
      const repo = String(input.repo || '').trim();
      const branch = String(input.branch || '').trim();
      const commitSha = String(input.commit_sha || '').trim().toLowerCase();
      if (!repo) throw new TypeError('repo must be a non-empty string');
      if (!branch) throw new TypeError('branch must be a non-empty string');
      if (!SHA40.test(commitSha)) throw new TypeError('commit_sha must be an exact Git SHA');
      const operation = await operations.findSucceededEffect({
        command:COMMAND,
        scope:`repository:${repo}`,
        effect_kind:'github_commit_branch',
        effect_ref:`${repo}#${branch}@${commitSha}`,
      });
      if (!operation) return null;
      const receipt = resolution(operation).receipt;
