...
function validSubjectKind(value) {
...
export function assertExecutionSnapshot(value) {
    ...
    if ((snapshot.lifecycle === 'settled') !== snapshot.settled) {
        throw new Error('execution snapshot settled flag does not match lifecycle');
    }
    if (snapshot.effect_ref !== null && typeof snapshot.effect_ref !== 'string') {
        throw new Error('execution snapshot effect_ref is invalid');
    }
    if (snapshot.settled && !snapshot.settlement_receipt) {
        throw new Error('settled snapshot must carry a settlement receipt');
    }
    if (!snapshot.settled && snapshot.settlement_receipt) {
        throw new Error('unsettled snapshot cannot carry a settlement receipt');
    }
    if (snapshot.settlement_receipt) {
        assertSettlementReceipt(snapshot.settlement_receipt);
    }
}
