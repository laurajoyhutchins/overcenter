...
export async function amendProjectDefinition(input, dependencies) {
    ...
    const currentCanonical = canonicalProjectDefinition(currentDefinition);
    const preliminaryAmendment = applyProjectDefinitionAmendment(currentCanonical, input.amendment);
    const noOp = preliminaryAmendment.diff.added.length === 0
        && preliminaryAmendment.diff.changed.length === 0
        && preliminaryAmendment.diff.removed.length === 0;
    if (noOp) {
        if (typeof dependencies.validateAmendment === 'function') {
            await dependencies.validateAmendment(Object.freeze({
                project_ref: projectRef,
                authority,
                current_definition: currentCanonical,
                candidate_definition: preliminaryAmendment.definition,
                diff: preliminaryAmendment.diff,
            }));
        }
        const graph = graphAtRevision(await dependencies.deriveProjectGraph(authority), authority);
        return Object.freeze({
            ok: true,
            schema: 'project-authoring-result-v1',
            authority,
    ...
    const amendmentInput = await amendmentWithAuthoritativeHistory(projectRef, authority, currentCanonical, input.amendment, dependencies);
    const amendment = applyProjectDefinitionAmendment(currentCanonical, amendmentInput);
    if (typeof dependencies.validateAmendment === 'function') {
        await dependencies.validateAmendment(Object.freeze({
            project_ref: projectRef,
            authority,
