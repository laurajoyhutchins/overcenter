...
export async function amendProjectDefinition(input, dependencies) {
    ...
    const candidate = applyProjectDefinitionAmendment(currentDefinition, input.amendment);
    const noOp = candidate.diff.added.length === 0 && candidate.diff.changed.length === 0 && candidate.diff.removed.length === 0;
    if (noOp) {
