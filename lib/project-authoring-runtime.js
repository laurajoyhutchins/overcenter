...
function amendmentTouchesExistingTransition(currentDefinitionInput, amendmentInput) {
  ...
  return removed.some((id) => typeof id === 'string' && existingIds.has(id.trim()))
    || upserted.some((transition) => transition && typeof transition === 'object' && !Array.isArray(transition)
      && typeof transition.id === 'string' && existingIds.has(transition.id.trim()));
}
...
async function resultAfterMutation(authority, mutation, diff, dependencies) {
  ...
  return Object.freeze({ ok:true, schema:'project-authoring-result-v1', authority:resultingAuthority, diff, graph });
}

async function confirmedResultAfterMutation(authority, mutation, diff, dependencies) {
  try { return await resultAfterMutation(authority, mutation, diff, dependencies); }
  catch (error) { throw confirmedMutationFailure(error); }
}
...
export async function defineProjectDefinition(input, dependencies) {
  ...
  if (existingDefinition != null) fail('PROJECT_AUTHORING_ALREADY_DEFINED', 'project.define requires an authority with no existing project definition', { project_ref:projectRef });
  const definition = canonicalProjectDefinition(input.definition);
  if (definition.project_ref !== projectRef) fail('PROJECT_AUTHORING_REQUEST_INVALID', 'definition.project_ref must match project_ref', { project_ref:projectRef, definition_project_ref:definition.project_ref });
  const diff = Object.freeze({ added:Object.freeze(definition.transitions.map((transition) => transition.id).sort()), changed:Object.freeze([]), removed:Object.freeze([]) });
  const mutation = await dependencies.mutateDefinition({ project_ref:projectRef, repository:authority.repository, expected_revision:expectedRevision, derivation:authority.derivation, definition, diff });
...
export async function amendProjectDefinition(input, dependencies) {
  ...
  const mutation = await dependencies.mutateDefinition({ project_ref:projectRef, repository:authority.repository, expected_revision:expectedRevision, derivation:authority.derivation, definition:amendment.definition, diff:amendment.diff });
