...
export async function runRepositoryBranchRoleTests() {
  ...
  await test('development and production cannot alias', async () => {
    ...
    try {
      normalizeRepositoryBranchRoleBinding({
        ...
        production_source_ref: 'hatchable:test',
      });
  ...
  await test('ensure rejects branch-role rewrites', async () => {
    const service = createRepositoryBranchRoleService({ store: memoryStore() });
    await service.ensure({ repository: 'laurajoyhutchins/overcenter', development_branch: 'dev', production_branch: 'main', production_source_ref: 'hatchable:test' });
    let code = null;
    try {
      await service.ensure({ repository: 'laurajoyhutchins/overcenter', development_branch: 'dev', production_branch: 'release', production_source_ref: 'hatchable:test' });
    } catch (error) { code = error.code; }
