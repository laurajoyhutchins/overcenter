...
export async function runRepositoryBranchRoleTests() {
  ...
  await test('development role defaults to dev and preserves configured safe branch', async () => {
    const defaulted = normalizeRepositoryBranchRoleBinding({
      repository: 'laurajoyhutchins/overcenter',
      production_branch: 'main',
      production_source_ref: 'runtime:production-source-materialization',
    });
    assert(defaulted.development_branch === 'dev', 'development branch default drifted');

    const configured = normalizeRepositoryBranchRoleBinding({
      repository: 'laurajoyhutchins/overcenter',
      development_branch: 'develop',
      production_branch: 'main',
      production_source_ref: 'runtime:production-source-materialization',
    });
    assert(configured.development_branch === 'develop', 'configured development branch was not preserved');
    assert(configured.production_branch === 'main', 'production branch drifted');
  });
  ...
  await test('ensure is idempotent for the same binding', async () => {
    ...
    assert(second.development_branch === 'develop', 'configured development branch changed during ensure');
  });
  ...
  await test('ensure rejects branch-role rewrites', async () => {
    ...
    let code = null;
    try {
      await service.ensure({ repository: 'laurajoyhutchins/overcenter', development_branch: 'develop', production_branch: 'main', production_source_ref: 'runtime:test' });
    } catch (error) { code = error.code; }
    assert(code === 'REPOSITORY_BRANCH_ROLE_CHANGED', `unexpected ${code}`);
  });

  await test('ordinary work cannot mutate configured managed role branches', async () => {
    const roles = { development_branch: 'develop', production_branch: 'main' };
  ...
  await test('managed pull requests target configured development branch only', async () => {
    const roles = { development_branch: 'develop', production_branch: 'main' };
    assertDevelopmentBase('develop', roles);
    let details = null;
    try { assertDevelopmentBase('dev', roles); } catch (error) { details = { code: error.code, ...error.details }; }
    assert(details?.code === 'GITHUB_BRANCH_ROLE_VIOLATION', 'non-configured development base was not rejected');
    assert(details?.expected_base === 'develop', 'configured development base missing');
  });
  ...
  await test('production source sync derives main and rejects caller override', async () => {
    ...
    let code = null;
    try { bindProductionSourceCoordinates({ github_branch:'develop' }, roles); } catch (error) { code = error.code; }
