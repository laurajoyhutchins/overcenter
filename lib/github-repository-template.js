import { getGitHubAppInstallationScope, withGitHubAppApiClient } from 'lib/github-app-auth.js';
...
export async function createGithubRepositoryFromTemplateWithGitHubApp(input, options = {}) {
  ...
  try {
    const inspectInstallationScope = options.getGitHubAppInstallationScope || getGitHubAppInstallationScope;
    const installationScope = await inspectInstallationScope(normalized.template_repo);
    if (installationScope.repository_selection !== 'all') {
      return fail(
        'GITHUB_REPOSITORY_TEMPLATE_INSTALLATION_SCOPE_UNSUPPORTED',
        'Repository generation requires a GitHub App installation that automatically includes newly created repositories. Selected-repository installations must fail before mutation.',
        {
          template_repo: normalized.template_repo,
          destination_repo: normalized.destination_repo,
          observed_repository_selection: installationScope.repository_selection,
          required_repository_selection: 'all',
          installation_id: installationScope.installation_id,
          phase: 'precondition',
          may_have_mutated: false,
        },
      );
