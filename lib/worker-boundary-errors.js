...
function providerDiagnosticCode(error, fallback) {
  const nested = error?.details?.result;
  return publicDiagnosticCode(
    error?.details?.diagnostic_error_code
    || nested?.diagnostic_error_code
    || nested?.error_code
    || nested?.error
    || fallback,
  );
...
function boundedFailurePhase(error) {
  const value = error?.details?.phase ?? error?.details?.result?.phase ?? error?.phase;
  if (typeof value !== 'string') return null;
  const phase = value.trim();
  return phase ? phase.slice(0, 128) : null;
}
...
export function sanitizeWorkerBoundaryError(command, error, options = {}) {
  ...
  const diagnosticCode = providerDiagnosticCode(error, code);
  const kind = failureKind(error, diagnosticCode);
  const mutated = mayHaveMutated(error, kind);
  const failurePhase = boundedFailurePhase(error);
