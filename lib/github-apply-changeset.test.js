...
export async function runGithubApplyChangesetTests() {
  ...
  results.push(await run('22 consecutive mechanical cleanup commits must coalesce', async () => {
    ...
    check(second.recovery?.command === 'github.apply_changeset' && second.recovery?.input?.coalesce_mechanical === true, 'rejection must expose executable canonical coalescing recovery');
    ...
    const coalesced = await applyGithubChangeset(request({
      ...
      coalesce_mechanical: true,
      changes: [{ path: 'docs/existing.md', operation: 'update', content: 'formatted docs\n' }],
    }), { github: gh });
    check(coalesced.ok, 'canonical mechanical coalescing recovery must converge');
    check(gh.commitCreates === 2, 'coalescing must create exactly one replacement commit');
    check(gh.branches.get('agent/existing') === coalesced.commit_sha, 'coalescing must replace the exact prior mechanical head');
    const combined = gh.trees.get(coalesced.tree_sha);
    check(combined.get('README.md')?.content === 'formatted readme', 'coalescing lost the prior mechanical change');
    check(combined.get('docs/existing.md')?.content === 'formatted docs\n', 'coalescing lost the follow-up final-newline repair');
    check(gh.commits.get(coalesced.commit_sha)?.parents?.[0] === gh.main, 'coalesced replacement must preserve linear history by replacing the prior mechanical head');
