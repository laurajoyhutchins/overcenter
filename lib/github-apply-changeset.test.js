import { applyGithubChangeset, coalesceGithubMechanicalChangeset, classifyGithubAppChangesetAuthError, createGithubApiAdapter, GitHubChangesetError } from 'lib/github-apply-changeset.js';
...
export async function runGithubApplyChangesetTests() {
  ...
  results.push(await run('22b same-authority final-newline cleanup coalesces the immediately preceding mechanical head', async () => {
    const gh = new FakeGithub();
    const receipts = new MemoryReceipts();
    gh.branches.set('agent/existing', gh.main);
    const authority = { lease_id: '11111111-1111-4111-8111-111111111111', work_ref: 'project-transition:test', run_id: 'run-1', gate: 'project_transition', repository: 'laurajoyhutchins/test', execution_fingerprint: 'same-lease' };
    const executionAuthority = { require: async () => authority };
    const first = await applyGithubChangeset(request({
      branch: 'agent/existing', expected_head: gh.main, idempotency_key: 'mechanical:first', lease_token: 'same-lease-token',
      commit_message: 'style: format fixture',
      changes: [{ path: 'README.md', operation: 'update', content: 'formatted readme' }],
    }), { github: gh, receipts, executionAuthority, idFactory: () => '11111111-1111-4111-8111-111111111111' });
    check(first.ok, 'first mechanical commit failed');
    const repaired = await coalesceGithubMechanicalChangeset(request({
      branch: 'agent/existing', expected_head: first.commit_sha, idempotency_key: 'mechanical:coalesce', lease_token: 'same-lease-token',
      commit_message: 'style: format fixture',
      changes: [{ path: 'docs/existing.md', operation: 'update', content: 'needs final newline', ensure_final_newline: true }],
    }), { github: gh, receipts, executionAuthority, idFactory: () => '22222222-2222-4222-8222-222222222222' });
    check(repaired.ok && repaired.coalesced === true, 'canonical coalescing operation did not converge');
    const visible = gh.commits.get(gh.branches.get('agent/existing'));
    check(visible.parents[0] === gh.main, 'coalesced repair did not replace the prior same-lease mechanical head');
    const finalTree = gh.trees.get(visible.tree_sha);
    check(finalTree.get('README.md')?.content === 'formatted readme', 'coalescing lost the prior mechanical change');
    check(finalTree.get('docs/existing.md')?.content === 'needs final newline\n', 'coalescing lost the follow-up final-newline repair');
  }));
