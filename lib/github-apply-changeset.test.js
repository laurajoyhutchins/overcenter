...
class FakeGithub {
  ...
  async updateBranch(repo, branch, commitSha, updateOptions = {}) {
    if (this.beforeFinal) await this.beforeFinal(this, 'update', branch, commitSha);
    if (this.failFinal) throw this.failFinal;
    this.branches.set(branch, commitSha);
    this.refMutations += 1;
    if (updateOptions.force === true) this.forceUpdates = (this.forceUpdates || 0) + 1;
  }
}
...
export async function runGithubApplyChangesetTests() {
  ...
  results.push(await run('22 consecutive mechanical cleanup commits must coalesce', async () => {
    ...
    const coalesced = await applyGithubChangeset(request({
      branch: 'agent/existing',
      expected_head: first.commit_sha,
      commit_message: 'style: format first and second batch',
      coalesce_previous_mechanical: true,
      changes: [{ path: 'README.md', operation: 'update', content: 'formatted readme', ensure_final_newline: true }],
    }), { github: gh, coalesceAuthorization: { expectedHead: first.commit_sha, parentSha: gh.main } });
    check(coalesced.ok, `canonical coalescing recovery failed: ${JSON.stringify(coalesced)}`);
    check(gh.branches.get('agent/existing') === coalesced.commit_sha, 'coalesced repair did not become branch head');
    check(gh.commits.get(coalesced.commit_sha)?.parents?.[0] === gh.main, 'coalesced repair stacked instead of replacing the prior mechanical head');
    check(gh.trees.get(coalesced.tree_sha)?.get('README.md')?.content === 'formatted readme\n', 'coalesced repair lost final-newline normalization');
    check(gh.forceUpdates === 1, 'coalesced repair did not use the bounded force-update path');
