...
import { applyGithubChangeset, applyGithubChangesetWithGitHubApp, classifyGithubAppChangesetAuthError, createGithubApiAdapter, GitHubChangesetError } from 'lib/github-apply-changeset.js';
import { githubAppChangesetPermissionProfile, githubAppPermissionProfile } from 'lib/github-app-auth.js';
import 'lib/github-text-transport.spec.js';
...
class MemoryReceipts {
  ...
  async heartbeat(n, token, phase) { this.heartbeats.push({ key: this.key(n), token, phase }); return true; }
...
// Changeset cases execute directly through node:test.

  await (test('1 create one file on a new branch', async () => {
  ...
  await (test('1b reject a new legacy execution-identity branch but grandfather an existing one', async () => {
  ...
  await (test('2 update several files in one commit', async () => {
  ...
  await (test('3 create update delete in one changeset', async () => {
    const gh = new FakeGithub();
  ...
  await (test('4 existing branch fast-forward', async () => {
    const gh = new FakeGithub(); gh.branches.set('agent/existing', gh.main);
  ...
  await (test('5 expected_head success', async () => {
    const gh = new FakeGithub(); gh.branches.set('agent/existing', gh.main);
  ...
  await (test('6 stale expected_head rejection with no ref mutation', async () => {
    const gh = new FakeGithub(); gh.branches.set('agent/existing', gh.main);
  ...
  await (test('7 duplicate-path validation', async () => {
    const gh = new FakeGithub();
    const r = await applyGithubChangeset(request({ changes: [
  ...
  await (test('8 missing update/delete target behavior', async () => {
  ...
  await (test('9 invalid path rejection', async () => {
    const gh = new FakeGithub();
  ...
  await (test('10 permission/protected-branch failure propagation', async () => {
  ...
  await (test('11 retry/idempotency behavior', async () => {
  ...
  await (test('11b completion retains the attempt identity that performed the effect', async () => {
    const gh = new FakeGithub();
  ...
  await (test('11c prepared recovery completes with the effect-producing attempt identity', async () => {
    const gh = new FakeGithub();
    const receipts = new MemoryReceipts();
  ...
  await (test('12 target branch creation race', async () => {
    const gh = new FakeGithub();
  ...
  await (test('13 ref update race between preflight and final mutation', async () => {
    const gh = new FakeGithub(); gh.branches.set('agent/existing', gh.main);
  ...
  await (test('14 multi-file operation produces exactly one new commit', async () => {
    const gh = new FakeGithub();
    const r = await applyGithubChangeset(request({ changes: [
  ...
  await (test('15 changeset GitHub App token scope remains contents write only', async () => {
  ...
  await (test('15b workflow path selection does not widen ordinary changesets', async () => {
  ...
  await (test('15c missing workflow permission is typed before mutation', async () => {
  ...
  await (test('15d changeset wrapper consumes the injected GitHub App provider', async () => {
    let providerCalls = 0;
    const result = await applyGithubChangesetWithGitHubApp(request(), {
      executionAuthority: {},
      withGitHubAppApiClient: async (repo, _callback, options) => {
        providerCalls += 1;
        check(repo === 'laurajoyhutchins/test', 'injected provider received the wrong repository');
        check(options?.permissionProfile === 'changeset', 'injected provider received the wrong permission profile');
        throw Object.assign(new Error('injected provider sentinel'), {
          code: 'INJECTED_PROVIDER_SENTINEL',
          mayHaveMutated: false,
        });
      },
    });
    check(providerCalls === 1, 'changeset wrapper ignored the injected GitHub App provider');
    check(result.ok === false && result.error === 'INJECTED_PROVIDER_SENTINEL', 'injected provider failure was not preserved');
  }));

  await (test('16 review-packet permissions cannot leak into changeset calls', async () => {
    const permissions = githubAppPermissionProfile('changeset');
  ...
  await (test('17 ensure_final_newline repairs tool-transport EOF loss', async () => {
    const gh = new FakeGithub();
    const r = await applyGithubChangeset(request({ changes: [
  ...
  await (test('18 multi-file text changes are carried by one tree mutation without per-file blob calls', async () => {
    const gh = new FakeGithub();
    const r = await applyGithubChangeset(request({ changes: [
      { path: 'a.txt', operation: 'create', content: 'a' },
      { path: 'b.txt', operation: 'create', content: 'b' },
  ...
  await (test('19 idempotent processing receipts heartbeat after expensive preflight', async () => {
    const gh = new FakeGithub();
    const receipts = new MemoryReceipts();
  ...
  await (test('19b lost durable attempt fence stops before first provider mutation', async () => {
    const gh = new FakeGithub();
    const receipts = new MemoryReceipts();
  ...
  await (test('20 safe GitHub reads retry bounded transient upstream failures', async () => {
    let calls = 0;
    const apiClient = {
      async call() {
        calls += 1;
  ...
  await (test('21 GitHub mutation transport failures expose phase and request evidence without automatic retry', async () => {
    let calls = 0;
    const apiClient = {
      async call() {
        calls += 1;
  ...
  await (test('22 consecutive mechanical cleanup commits must coalesce', async () => {
    const gh = new FakeGithub();
  ...
  await (test('23 gzip-base64 text content is decoded before semantic normalization', async () => {
    const gh = new FakeGithub();
    const r = await applyGithubChangeset(request({ changes: [
  ...
  // github-text-transport.spec.js registers its native node:test cases on import.