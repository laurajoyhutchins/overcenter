...
export async function runGithubApplyChangesetTests() {
  ...
  results.push(await run('10 permission/protected-branch failure propagation', async () => {
    const gh = new FakeGithub(); gh.branches.set('agent/existing', gh.main);
    gh.failFinal = new GitHubChangesetError('GITHUB_PERMISSION_DENIED', 'branch protection denied update', { status: 403, github_message: 'branch protection denied update' }, 403);
    const r = await applyGithubChangeset(request({ branch: 'agent/existing' }), { github: gh });
    check(!r.ok && r.error === 'GITHUB_PERMISSION_DENIED' && r.status === 403, 'permission failure not propagated');
    check(gh.branches.get('agent/existing') === gh.main, 'permission failure mutated ref');
  }));
