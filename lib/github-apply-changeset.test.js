...
class FakeGithub {
  ...
  async replaceBranch(repo, branch, commitSha) {
    if (this.beforeFinal) await this.beforeFinal(this, 'replace', branch, commitSha);
    if (this.failFinal) throw this.failFinal;
    this.branches.set(branch, commitSha);
    this.refMutations += 1;
  }
...
export async function runGithubApplyChangesetTests() {
  ...
  results.push(await run('15d changeset wrapper consumes the injected GitHub App provider', async () => {
    let providerCalls = 0;
  ...
  results.push(await run('22 consecutive mechanical cleanup commits must coalesce', async () => {
    ...
    check(second.recovery_command === 'github.apply_changeset' && second.recovery_fields?.coalesce_previous_mechanical === true, 'rejection did not return executable canonical recovery');
    ...
    const receipts = new MemoryReceipts();
    const coalescedRequest = request({
      branch: 'agent/existing',
      expected_head: first.commit_sha,
      commit_message: 'style: format second batch',
      coalesce_previous_mechanical: true,
      idempotency_key: 'mechanical-coalesce-22',
      changes: [{ path: 'docs/existing.md', operation: 'update', content: 'formatted docs\n', ensure_final_newline: true }],
    });
    const coalesced = await applyGithubChangeset(coalescedRequest, { github: gh, receipts, idFactory: () => '00000000-0000-4000-8000-000000000022' });
    check(coalesced.ok, 'canonical mechanical coalescing recovery failed');
    const replay = await applyGithubChangeset(coalescedRequest, { github: gh, receipts, idFactory: () => '00000000-0000-4000-8000-000000000023' });
    check(replay.ok && replay.idempotent_replay === true && replay.commit_sha === coalesced.commit_sha, 'coalescing replay was not idempotent');
    const combinedCommit = gh.commits.get(coalesced.commit_sha);
    const combinedTree = gh.trees.get(coalesced.tree_sha);
    check(combinedCommit.parents.length === 1 && combinedCommit.parents[0] === gh.main, 'coalesced commit did not replace the prior mechanical head in linear history');
    check(combinedTree.get('README.md')?.content === 'formatted readme' && combinedTree.get('docs/existing.md')?.content === 'formatted docs\n', 'coalesced tree did not preserve both mechanical repairs');
    check(gh.branches.get('agent/existing') === coalesced.commit_sha, 'coalesced branch did not point at replacement commit');

    const substantive = await applyGithubChangeset(request({
      branch: 'agent/existing',
      expected_head: coalesced.commit_sha,
