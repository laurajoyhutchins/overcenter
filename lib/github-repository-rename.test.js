...
} from './github-repository-rename.js';
...
// Legacy suite wrapper removed during node:test migration.
  ...
  test('request schema is exact and derives the target coordinate', async () => {
  ...
  test('immutable repository identity mismatch fails before mutation', async () => {
  ...
  test('already-renamed authoritative read is a verified no-op', async () => {
  ...
  test('rename patches only the repository name and verifies the immutable id at the new coordinate', async () => {
  ...
  test('definitive GitHub rejection is typed and non-mutating', async () => {
    const path = '/repos/laurajoyhutchins/old-name';
    const client = queuedClient([
  ...
  test('ambiguous write reconciles the target identity without replaying the mutation', async () => {
    const oldPath = '/repos/laurajoyhutchins/old-name';
    const newPath = '/repos/laurajoyhutchins/new-name';
    const client = queuedClient([
      { method: 'GET', path: oldPath, result: repository(OLD_REPO) },
  ...
  test('unverified post-write identity remains indeterminate', async () => {
    const oldPath = '/repos/laurajoyhutchins/old-name';
    const newPath = '/repos/laurajoyhutchins/new-name';
    const client = queuedClient([
      { method: 'GET', path: oldPath, result: repository(OLD_REPO) },
  ...
  test('GitHub App retry can recover against the new coordinate after the old token scope stops resolving', async () => {
    const authRepos = [];
    const client = queuedClient([{ method: 'GET', path: '/repos/laurajoyhutchins/old-name', result: repository(NEW_REPO) }]);
  ...
  test('command response and journal projections preserve rename recovery semantics', async () => {
