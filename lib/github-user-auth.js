...
async function tokenWrapKey(rootSecret) {
  const digest = await crypto.subtle.digest('SHA-256', encoder.encode(`${WRAP_CONTEXT}${String(rootSecret || '')}`));
  return crypto.subtle.importKey('raw', digest, { name: 'AES-GCM' }, false, ['encrypt', 'decrypt']);
}

async function sealSecret(value, rootSecret) {
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const key = await tokenWrapKey(rootSecret);
...
async function openSecret(value, rootSecret) {
  ...
  if (version !== 'v1' || !ivText || !cipherText) throw Object.assign(new Error('Stored GitHub user credential is invalid.'), { code: 'GITHUB_USER_AUTH_CONFIGURATION_ERROR' });
  try {
    const key = await tokenWrapKey(rootSecret);
    ...
    throw Object.assign(new Error('Stored GitHub user credential cannot be decrypted with the current GitHub App key.'), { code: 'GITHUB_USER_AUTH_CONFIGURATION_ERROR' });
  }
}
