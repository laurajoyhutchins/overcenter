...
} from './github-actions-storage.js';
...
// Legacy suite wrapper removed during node:test migration.
  ...
  test('request normalization keeps operations narrow and exact', async () => {
  ...
  test('MCP schema exposes cache inspection and exact-id cache deletion without broadening authority', async () => {
  ...
  test('permission profiles stay split by capability', async () => {
  ...
  test('inspect paginates and totals live artifact bytes', async () => {
    const calls = [];
  ...
  test('inspect includes dependency caches as a distinct Actions storage surface', async () => {
    const apiClient = client(async (_name, options) => {
  ...
  test('coverage selector keeps only newest artifact per coverage name', async () => {
    const apiClient = client(async (_name, options) => {
  ...
  test('delete removes only explicit ids and treats missing ids idempotently', async () => {
    const calls = [];
    const apiClient = client(async (_name, options) => {
      calls.push(options);
  ...
  test('cache delete removes only explicit ids and reports reclaimed bytes from observed cache state', async () => {
    const calls = [];
    const apiClient = client(async (_name, options) => {
      calls.push(options);
  ...
  test('retention update is read-write-read verified', async () => {
    const calls = [];
    const apiClient = client(async (_name, options) => {
      calls.push(options);
