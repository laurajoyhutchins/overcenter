...
test('transition horizon includes prerequisite closure and selects only in-scope frontier', async()=> {
...
test('milestone horizon can complete independently of unrelated project work', async()=> {
  const graph = authoritativeGraph([
    doneNode('schema'),
...
test('project horizon requires every project transition to be done', async()=> {
...
test('off-nominal transition blocks only its horizon and never enters ready frontier', async()=> {
  const graph = authoritativeGraph([
...
test('explicit horizon definition fails closed on missing target nodes', async()=> {
...
test('stale authority coordinates invalidate prior horizon evidence', async()=> {
...
test('release and portfolio horizons use the same verified subset semantics', async()=> {
  const graph = authoritativeGraph([
    doneNode('protocol'),
