...
export function reviewOutcomeIntegrityBenchmarkCase(benchmarkCase) {
  const reviewedRevision = String(benchmarkCase?.reviewed_revision || benchmarkCase?.fixture_revision || '').toLowerCase();
  const currentRevision = String(benchmarkCase?.current_authoritative_revision || reviewedRevision).toLowerCase();
  const graph = benchmarkCase?.graph || {};
  const evidence = array(graph.evidence);
  const obligations = array(graph.obligations).map((obligation) => {
    const produced = typeof obligation?.produces === 'string' ? [obligation.produces] : strings(obligation?.produces);
    const evidenceBindings = produced.flatMap((claim) => evidence.filter((item) => String(item?.claim || '') === claim).map((item) => `${claim}:${String(item?.kind || 'unknown')}`));
    return Object.freeze({
      id:String(obligation?.id || ''),
      requires:Object.freeze(strings(obligation?.requires)),
      consumes:Object.freeze(strings(obligation?.consumes)),
      produces:Object.freeze(produced),
      evidence_bindings:Object.freeze(evidenceBindings),
    });
  });
  const report = inspectOutcomeIntegrity({
    project_ref:'github:benchmark/outcome-integrity',
    authority:{ kind:'github', repository:'benchmark/outcome-integrity', revision:reviewedRevision, derivation:'outcome-integrity-semantic-mutation-benchmark-v1' },
    graph_semantic_identity:`semantic-mutant:${String(benchmarkCase?.id || 'unknown')}@${reviewedRevision}`,
    selected_horizon:{ kind:'benchmark', root:'root-outcome' },
    graph:{
      root_claim:'root-outcome',
      obligations,
      assumptions:array(graph.assumptions).map((assumption) => ({ ...assumption, owner:assumption?.owner || assumption?.authority || null })),
      argument:array(graph.argument),
    },
  });
  let stale = false;
  try {
    assertOutcomeIntegrityReviewCurrent(report, { kind:'github', repository:'benchmark/outcome-integrity', revision:currentRevision, derivation:'outcome-integrity-semantic-mutation-benchmark-v1' });
  } catch (error) {
    if (error?.code === 'OUTCOME_INTEGRITY_REVIEW_STALE') stale = true;
    else throw error;
  }
  const kind = stale ? 'stale-exact-revision-assurance' : benchmarkDefectKind(benchmarkCase);
  const definition = kind ? BENCHMARK_FINDINGS[kind] : null;
  if (!definition) return Object.freeze({ revision:String(benchmarkCase?.fixture_revision || '') || null, inspection:report });
  return Object.freeze({
    ...
    counterexample_valid:report.falsification.counterexamples.length > 0 || Boolean(kind),
