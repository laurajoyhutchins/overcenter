...
import { bindAuthoritativeEffectSettlement, settlementRequiresAuthoritativeEffect } from './work-settle-contract.js';
...
export function projectAdvanceFor(options = {}) {
  ...
  async function finishExecution(run, executionResult) {
    ...
    if (settlementRequiresAuthoritativeEffect(executionResult) && confirmAuthoritativeEffect) {
      confirmation = await confirmAuthoritativeEffect({
        run_id:run.run_id,
        target:run.target,
        execution_result:executionResult,
      });
    }
    const evidence = bindAuthoritativeEffectSettlement(executionResult, confirmation);
