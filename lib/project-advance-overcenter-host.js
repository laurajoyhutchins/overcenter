...
export function projectAdvanceFor(options = {}) {
  ...
  return Object.freeze({
    async advance(input = {}) {
      ...
      if (executionResult) {
        await finishExecution(run, executionResult);
        const resumedTransitionId = transitionId || (run.target.horizon.kind === 'transition' ? run.target.horizon.ref : null);
        run = await startProjectRun(projectRef, repository, resumedTransitionId);
      }
