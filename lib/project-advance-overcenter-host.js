...
function requireAuthoritativeEffectBeforeCompletedSettlement(executionResult) {
  if (executionResult?.disposition !== 'completed') return;
  const kinds = new Set((executionResult.evidence || []).map((entry) => String(entry?.kind || '').trim().toLowerCase()));
  const hasCandidate = kinds.has('candidate_revision') || kinds.has('candidate') || kinds.has('verified_candidate');
  const hasAuthoritativeEffect = kinds.has('authoritative_effect') || kinds.has('authority_readback');
  if (!hasCandidate || hasAuthoritativeEffect) return;
  fail(
    'PROJECT_ADVANCE_AUTHORITATIVE_EFFECT_UNCONFIRMED',
    'verified candidate evidence cannot complete a project transition before the authoritative provider effect is confirmed',
    {
...
export function projectAdvanceFor(options = {}) {
  ...
  async function finishExecution(run, executionResult) {
    ...
    requireAuthoritativeEffectBeforeCompletedSettlement(executionResult);
