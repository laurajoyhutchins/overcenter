import { canonicalJson, sha256Text } from './canonical-json.js';
import { WORK_REQUEUE_CLASSES } from './execution-lifecycle-contracts.js';

function fail(code, message, details = null) {
  const error = new Error(message);
  error.code = code;
  error.details = details;
  throw error;
}

function normalizeProjectRef(value) {
  const projectRef = typeof value === 'string' ? value.trim() : '';
  const match = /^github:([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+)$/.exec(projectRef);
  if (!match) fail('REQUEST_INVALID', 'project_ref must name a GitHub repository project');
  return Object.freeze({ projectRef, repository:match[1] });
}

function optionalText(value, field, max = 512) {
  if (value == null) return null;
  const normalized = typeof value === 'string' ? value.trim() : '';
  if (!normalized || normalized.length > max || /\s/.test(normalized)) {
    fail('REQUEST_INVALID', `${field} is invalid`, { field });
  }
  return normalized;
}

function optionalReason(value) {
  if (value == null) return null;
  const normalized = typeof value === 'string' ? value.trim() : '';
  if (!normalized || normalized.length > 2000) fail('REQUEST_INVALID', 'execution_result.reason is invalid', { field:'execution_result.reason' });
  return normalized;
}

function normalizeExecutionResult(value) {
  if (value == null) return null;
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('REQUEST_INVALID', 'execution_result must be an object');
  const allowed = new Set(['disposition','evidence','reason','promotion_condition','requeue_class','continuation']);
  const unknown = Object.keys(value).filter((key) => !allowed.has(key)).sort();
  if (unknown.length) fail('REQUEST_INVALID', 'execution_result contains unsupported fields', { fields:unknown });
  const disposition = typeof value.disposition === 'string' ? value.disposition.trim() : '';
  if (!['completed','requeue','blocked'].includes(disposition)) fail('REQUEST_INVALID', 'execution_result.disposition is unsupported');
  const evidence = value.evidence == null ? [] : value.evidence;
  if (!Array.isArray(evidence)) fail('REQUEST_INVALID', 'execution_result.evidence must be an array');
  const requeueClass = value.requeue_class == null ? null : String(value.requeue_class).trim();
  if (requeueClass && !WORK_REQUEUE_CLASSES.includes(requeueClass)) fail('REQUEST_INVALID', 'execution_result.requeue_class is unsupported', { field:'execution_result.requeue_class' });
  if (requeueClass && disposition !== 'requeue') fail('REQUEST_INVALID', 'execution_result.requeue_class requires requeue disposition', { field:'execution_result.requeue_class' });
  const continuation = value.continuation == null ? null : value.continuation;
  if (continuation !== null && (!continuation || typeof continuation !== 'object' || Array.isArray(continuation))) fail('REQUEST_INVALID', 'execution_result.continuation must be an object', { field:'execution_result.continuation' });
  return Object.freeze({ disposition, evidence:Object.freeze([...evidence]), reason:optionalReason(value.reason), promotion_condition:optionalReason(value.promotion_condition), requeue_class:requeueClass, continuation });
}

function runFinishDisposition(disposition) {
  if (disposition === 'completed') return 'completed';
  if (disposition === 'blocked') return 'blocked';
  return 'clean-stop';
}

function hasCandidateEvidence(executionResult) {
  const kinds = new Set((executionResult?.evidence || []).map((entry) => String(entry?.kind || '').trim().toLowerCase()));
  return kinds.has('candidate_revision') || kinds.has('candidate') || kinds.has('verified_candidate');
}

function unconfirmedAuthoritativeEffect(confirmation = null) {
  fail(
    'PROJECT_ADVANCE_AUTHORITATIVE_EFFECT_UNCONFIRMED',
    'verified candidate evidence cannot complete a project transition before deterministic authoritative-provider confirmation',
    {
      may_have_mutated:false,
      required_continuation:'deterministic_authoritative_effect_reconciliation',
      ...(confirmation?.reason ? { confirmation_reason:String(confirmation.reason) } : {}),
    },
  );
}

function targetFor(projectRef, transitionId) {
  return Object.freeze({
    project_ref:projectRef,
    horizon:Object.freeze(transitionId
      ? { kind:'transition', ref:transitionId }
      : { kind:'project', ref:projectRef }),
  });
}

function storedTarget(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const projectRef = typeof value.project_ref === 'string' ? value.project_ref.trim() : '';
  const kind = typeof value.horizon?.kind === 'string' ? value.horizon.kind.trim() : '';
  const ref = typeof value.horizon?.ref === 'string' ? value.horizon.ref.trim() : '';
  return projectRef && kind && ref
    ? Object.freeze({ project_ref:projectRef, horizon:Object.freeze({ kind, ref }) })
    : null;
}

function defaultReadRun(dbBinding) {
  return async function readRun(runId) {
    const result = await dbBinding.query(
      'SELECT run_id,status,target FROM orchestration_runs WHERE run_id=$1 LIMIT 1',
      [runId],
    );
    return result.rows?.[0] || null;
  };
}

async function newRunIdentity(projectRef, target, sessionRef) {
  const digest = await sha256Text(canonicalJson({
    command:'project.advance',
    project_ref:projectRef,
    target,
    session_ref:sessionRef,
  }));
  return Object.freeze({
    runId:`project-advance-${digest.slice(0, 32)}`,
    continuationKey:target.horizon.kind === 'transition'
      ? `semantic:project.advance:transition:${target.horizon.ref}:${digest.slice(0, 16)}`
      : `semantic:project.advance:project:${digest}`,
  });
}

export function projectAdvanceFor(options = {}) {
  const dbBinding = options.db;
  const runs = options.runs;
  const advance = options.advance;
  const finish = options.finish || null;
  const confirmAuthoritativeEffect = typeof options.confirmAuthoritativeEffect === 'function' ? options.confirmAuthoritativeEffect : null;
  const readRun = options.readRun || (dbBinding && typeof dbBinding.query === 'function' ? defaultReadRun(dbBinding) : null);
  const newSessionRef = options.newSessionRef || (() => crypto.randomUUID());
  if (!runs || typeof runs.start !== 'function' || !advance || typeof advance.advance !== 'function' || typeof readRun !== 'function') {
    fail('PROJECT_ADVANCE_RUNTIME_INVALID', 'project.advance runtime dependencies are unavailable');
  }

  async function startProjectRun(projectRef, repository, transitionId) {
    const target = targetFor(projectRef, transitionId);
    const sessionRef = optionalText(await newSessionRef(), 'session_ref');
    const identity = await newRunIdentity(projectRef, target, sessionRef);
    const request = {
      run_id:identity.runId,
      worker:'project.advance',
      mode:'interactive',
      continuation_key:identity.continuationKey,
      scope:{
        project:projectRef,
        lanes:[],
        repositories:[repository],
        direction:transitionId
          ? `Advance only authoritative project transition ${transitionId}.`
          : 'Advance the authoritative repository-owned project graph.',
      },
      target,
      budget_seconds:10800,
      settlement_reserve_seconds:300,
      minimum_new_gate_seconds:600,
    };

    try {
      const started = await runs.start(request);
      return Object.freeze({ run_id:started.run_id, target });
    } catch (error) {
      if (String(error?.code || '') !== '23505') throw error;
      const replay = await runs.start(request);
      return Object.freeze({ run_id:replay.run_id, target });
    }
  }

  async function resumeProjectRun(projectRef, resumeRef, transitionId) {
    const existing = await readRun(resumeRef);
    const target = storedTarget(existing?.target);
    if (!existing || existing.run_id !== resumeRef || !target) {
      fail('PROJECT_ADVANCE_RESUME_NOT_FOUND', 'project.advance resume_ref does not identify a durable targeted run', { resume_ref:resumeRef });
    }
    if (existing.status !== 'active') {
      fail('PROJECT_ADVANCE_RESUME_NOT_ACTIVE', 'project.advance resume_ref is not active', { resume_ref:resumeRef, status:existing.status || null });
    }
    if (target.project_ref !== projectRef) {
      fail('PROJECT_ADVANCE_RESUME_MISMATCH', 'project.advance resume_ref belongs to a different project', {
        resume_ref:resumeRef,
        expected_project_ref:projectRef,
        actual_project_ref:target.project_ref,
      });
    }
    if (transitionId && (target.horizon.kind !== 'transition' || target.horizon.ref !== transitionId)) {
      fail('PROJECT_ADVANCE_RESUME_TARGET_MISMATCH', 'project.advance resume_ref has incompatible transition selection', {
        resume_ref:resumeRef,
        expected_transition_id:transitionId,
        actual_target:target.horizon,
      });
    }
    return Object.freeze({ run_id:existing.run_id, target });
  }

  async function finishExecution(run, executionResult) {
    if (!finish || typeof finish.finish !== 'function') {
      fail('PROJECT_ADVANCE_RUNTIME_INVALID', 'project.advance execution completion runtime is unavailable');
    }
    let evidence = executionResult.evidence;
    if (executionResult.disposition === 'completed' && hasCandidateEvidence(executionResult)) {
      if (!confirmAuthoritativeEffect) unconfirmedAuthoritativeEffect();
      const confirmation = await confirmAuthoritativeEffect({
        run_id:run.run_id,
        target:run.target,
        execution_result:executionResult,
      });
      if (confirmation?.confirmed !== true) unconfirmedAuthoritativeEffect(confirmation);
      const confirmedEvidence = Array.isArray(confirmation.evidence) ? confirmation.evidence : [];
      evidence = Object.freeze([...executionResult.evidence, ...confirmedEvidence]);
    }
    return finish.finish({
      run_id:run.run_id,
      disposition:runFinishDisposition(executionResult.disposition),
      stop_reason:`project.advance agent execution ${executionResult.disposition}`,
      active_lease_settlement:{
        disposition:executionResult.disposition,
        evidence,
        reason:executionResult.reason,
        promotion_condition:executionResult.promotion_condition,
        ...(executionResult.requeue_class ? { requeue_class:executionResult.requeue_class } : {}),
        ...(executionResult.continuation ? { continuation:executionResult.continuation } : {}),
      },
    });
  }

  return Object.freeze({
    async advance(input = {}) {
      const { projectRef, repository } = normalizeProjectRef(input.project_ref);
      const transitionId = optionalText(input.transition_id, 'transition_id', 256);
      const resumeRef = optionalText(input.resume_ref, 'resume_ref');
      const executionResult = normalizeExecutionResult(input.execution_result);
      if (executionResult && !resumeRef) fail('REQUEST_INVALID', 'execution_result requires resume_ref');

      const run = resumeRef
        ? await resumeProjectRun(projectRef, resumeRef, transitionId)
        : await startProjectRun(projectRef, repository, transitionId);

      if (executionResult) {
        const result = await finishExecution(run, executionResult);
        if (result?.run_id !== run.run_id) fail('PROJECT_ADVANCE_RUN_MISMATCH', 'project advancement settlement returned evidence for a different run');
        return Object.freeze({ ...result, ok:true, resume_ref:null });
      }

      const result = await advance.advance({ run_id:run.run_id });
      if (result?.run_id !== run.run_id) fail('PROJECT_ADVANCE_RUN_MISMATCH', 'project advancement returned evidence for a different run');
      return Object.freeze({ ...result, resume_ref:run.run_id });
    },
  });
}
