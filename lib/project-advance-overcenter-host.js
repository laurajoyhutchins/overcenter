...
function normalizeExecutionResult(value) {
  ...
  const allowed = new Set(['disposition','evidence','reason']);
  ...
  return Object.freeze({ disposition, evidence:Object.freeze([...evidence]), reason:optionalReason(value.reason) });
}
...
function unconfirmedAuthoritativeEffect(confirmation = null) {
  fail(
    ...
    {
      may_have_mutated:confirmation?.may_have_mutated === true,
      confirmation_reason:String(confirmation?.reason || 'unknown'),
      required_continuation:'deterministic_authoritative_effect_reconciliation',
      ...(confirmation?.recovery ? { recovery:confirmation.recovery } : {}),
    },
  );
}
