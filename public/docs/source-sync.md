...
GitHub repository `laurajoyhutchins/busbar`, branch `main`, is the sole authority for repository source.

Hatchable project `proj_I6FSm85xrY7T` is the Busbar runtime projection. Runtime differences are drift to diagnose and repair from GitHub. They never authorize a write from Hatchable back to GitHub.
...
No source-sync credential exists in the Busbar runtime project.
