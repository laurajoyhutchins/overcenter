...
The MCP contract and semantic worker route accept the same fields:
...
A durable release receipt binds `repo + idempotency_key` to the canonical semantic request hash. If tag creation succeeded but release creation is uncertain or fails, the receipt records partial progress. Re-running the same request may accept the matching tag produced by that earlier attempt and continue with release creation. Reusing the same identity for different semantics fails with `IDEMPOTENCY_CONFLICT`.
