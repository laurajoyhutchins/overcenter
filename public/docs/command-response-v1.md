...
`PROJECT_AUTHORING_INTEGRATION_PENDING` is the important non-blind exception: it means Overcenter knows the exact staged candidate and has durably recorded it as `WAITING_EXTERNAL_VERIFICATION`. The response therefore advertises `recommended_action: "await_automatic_recovery"`, `automatic_recovery_allowed: true`, and `recovery_operation.command: "orchestration.maintain"`. The initiating caller may replay the exact request to observe status, but caller replay is optional. Deterministic maintenance owns progress and re-enters the ordinary exact-authority authoring path; any drift or ambiguous effect fails closed rather than being reinterpreted.
...
For Overcenter commands, existing flattened error-detail fields are temporarily retained alongside the canonical `details` object when current callers may depend on them. For example, a stale-head error can expose both `details.expected_head` and the legacy top-level `expected_head`.
