...
export default {
  ...
  async handler(args,ctx) {
    const response=await executeCorrelatedCommand(
      ...
      args || {},
      (input)=>createCompatibilityTransitionConfirmationRuntime({db:ctx?.db}).confirm(input),
      {
        statusForFailure:statusForCompatibilityTransitionConfirmationError,
        defaultError:'COMPATIBILITY_TRANSITION_CONFIRMATION_ERROR',
        defaultMessage:'orchestration.compatibility_confirm_transition failed',
        flattenDetails:true,
        db:ctx?.db,
      },
    );
