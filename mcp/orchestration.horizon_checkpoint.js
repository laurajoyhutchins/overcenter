...
export default {
  ...
  description:'Persist an agent-selected advisory horizon. Supply only bounded candidate identities plus fresh observed state/lane; Hatchable derives repository identity, position, execution fingerprint, authority revision evidence, and digest from authoritative work state. Horizons never grant ownership or choose work.',
  inputSchema:{type:'object',required:['run_id','candidates'],properties:{run_id:{type:'string'},candidates:{type:'array',minItems:1,maxItems:10,items:{type:'object',required:['work_ref','observed_state','observed_lane'],properties:{work_ref:{type:'string'},observed_state:{type:'string'},observed_lane:{type:'string'},selection_reason:{type:'string'}},additionalProperties:false}}},additionalProperties:false},
