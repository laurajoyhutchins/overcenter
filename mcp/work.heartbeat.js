...
import { createPostgresSubjectAwareLeaseHeartbeatService } from 'lib/orchestration-finish-runtime.js';
...
export default {
  ...
  async handler(args,ctx){const failureOptions={statusForFailure:statusForWorkLeaseError,defaultError:'WORK_HEARTBEAT_ERROR',defaultMessage:'work.heartbeat failed',flattenDetails:true,db:ctx?.db};let input;try{input=await canonicalHeartbeatCommandByRef(args||{},ctx?.db);}catch(error){return workerBoundaryCommandFailure('work.heartbeat',error,failureOptions).body;}const response=await executeCorrelatedCommand('work.heartbeat',input,request=>createPostgresSubjectAwareLeaseHeartbeatService({db:ctx?.db}).heartbeatByRef(request),workerBoundaryFailureOptions('work.heartbeat',failureOptions));return response.body;}
};
