  ...
  inputSchema: {
    ...
    properties: {
      ...
      branch: {
        ...
        minLength: 1,
        maxLength: 255,
        description: 'Unqualified branch name under refs/heads/.',
      },
      expected_head: {
        type: 'string',
        pattern: '^[0-9a-fA-F]{40}$',
        description: 'Exact branch-head SHA used as the optimistic concurrency fence and check-resolution authority.',
      },
      required_checks: {
        type: 'array',
        minItems: 1,
        ...
        description: 'Exact GitHub check-run names to ensure as branch integration requirements. Additive semantics only.',
      },
    },
  },
  async handler(args) {
    const response = await executeCommand(
      ...
      () => ensureGithubRequiredChecksWithGitHubApp(args),
      { flattenDetails: true },
    );
